...
OracleControllerPtr OracleController::m_instancePtr;
OracleControllerPtr OracleController::getInstance(ACE_Thread_Manager* tm)
{
    if( m_instancePtr.get() == NULL )
    {
		m_instancePtr.reset(new  OracleController(tm)) ;
    }
    return m_instancePtr ;
}


int OracleController::open(void *args)
{
    return this->activate(THR_BOUND);
}

int OracleController::close(u_long flags )
{
...
OracleController::OracleController(ACE_Thread_Manager* thrMgr)
...
void OracleController::PreClusterReconstruction()
{

}

bool OracleController::StartAppServices()
{
	DebugPrintf(SV_LOG_DEBUG, "ENTERED %s\n", FUNCTION_NAME) ;
	bool bResult = false;
	return bResult;
...
bool OracleController::IsItActiveNode()
{
	DebugPrintf(SV_LOG_DEBUG, "ENTERED %s\n", FUNCTION_NAME) ;
	bool bResult = false;
	return bResult;
...
SVERROR OracleController::Process()
{
	DebugPrintf(SV_LOG_DEBUG, "ENTERED OracleController::%s\n", FUNCTION_NAME) ;
    ...
    DebugPrintf(SV_LOG_DEBUG, "EXITED OracleController::%s\n", FUNCTION_NAME) ;
	return result;
}
SVERROR OracleController::Consistency()
{
    DebugPrintf(SV_LOG_DEBUG, "ENTERED %s\n", FUNCTION_NAME) ;
    SVERROR result = SVS_FALSE;
    ...
    std::string instanceName = m_policy.m_policyProperties.find("InstanceName")->second;
    std::stringstream instanceNames;
    ...
    try
    {
        ...
        ConsistencyData consistencyData = unmarshal<ConsistencyData>(marshalledConsistencyData) ;
        std::list<std::string>::iterator iter = consistencyData.m_InstancesList.begin();
        while(iter != consistencyData.m_InstancesList.end())
        {
            if (instanceName.compare(*iter) != 0)
            {
                instanceNames << ":";  
                instanceNames << *iter;  
            }
    ...
    if(iter != propsMap.end())
    {
        ...
        {        
            ...
            if(iter != propsMap.end())
            {
                ...
                /*if( strCommandLineOptions.find("-a ORACLE") == std::string::npos )
                {
                    strCommandLineOptions += " -a ORACLE" ;
                }*/
            }
            else
            {
            ...
            if(iter != propsMap.end())
            {
                ...
                if(obj.issueConsistenyTagForOracle(outputFileName, instanceNames.str(), exitCode))
                {
                    if(exitCode == 0)
                    {
...
SVERROR OracleController::prepareTarget()
{
	DebugPrintf(SV_LOG_DEBUG, "ENTERED %s\n", FUNCTION_NAME) ;
    ...
    try
    {
        ...
        stream << "Verifying if the target devices are in use by Oracle database.   " << std::endl ;
            ...
            try
            {
                ...
                if (marshalledDbInfoString.size() > 0)
                {
                    ...
                    while(dbInfoIter != dbInfo.end())
                    {
                        ...
                        DebugPrintf( SV_LOG_DEBUG, "DB Instance Name: '%s' Conf filename : '%s' \n", dbInstanceName.c_str(), dbConfFileName.c_str()) ;
                    ...
                    if (dbConfFileNames.size() > 0 ) 
                    {
                        ...
                        {
                            DebugPrintf(SV_LOG_ERROR,"Failed to spawn oracle prepare target script %s.\n", strCommnadToExecute.c_str());
                            // appConfiguratorPtr->PolicyUpdate(policyId,instanceId,"Failed",output.c_str());
                            // schedulerPtr->UpdateTaskStatus(policyId, TASK_STATE_COMPLETED);
                            // return SVS_FALSE ;
                            result = SVS_FALSE;
                        }
                        ...
                        {
                            DebugPrintf(SV_LOG_INFO,"Successfully spawned the oracle prepare target script.\n");
                            ...
                            if (exitCode != 0)
                            {
                                DebugPrintf(SV_LOG_ERROR,"Failed oracle prepare target script %s.\n", strCommnadToExecute.c_str());
                                // appConfiguratorPtr->PolicyUpdate(policyId,instanceId,"Failed",output.c_str());
                                // schedulerPtr->UpdateTaskStatus(policyId, TASK_STATE_COMPLETED);
                                // return SVS_FALSE ;
                                result = SVS_FALSE;
        ...
        if ( (result != SVS_FALSE) &&  (targetData.find( "tgtVolInfo" ) !=  targetData.end() ) )
        {
            try
            {
                ...
                if (marshalledVolInfoString.size() > 0)
                {
                    ...
                    AppLocalConfigurator localconfigurator ;
                    // XXX add policy id for uniqueness
                    std::string tgtVolInfoFile = localconfigurator.getInstallPath() + "/etc/OraclePrepareTgtVolInfoFile.dat";
                    std::ofstream out;
    ...
    if (result == SVS_FALSE)
    {
        stream << "Oracle Application PrepareTarget failed." << std::endl;
    ...
    {
        stream << "Oracle Application PrepareTarget succeeded." << std::endl;

        WriteStringIntoFile(outputFileName, stream.str());

        appConfiguratorPtr->PolicyUpdate(policyId,instanceId,"Success",std::string("Oracle Application PrepareTarget Success."));
    }
...
void OracleController::MakeAppServicesAutoStart()
{
    //changeServiceType(ORACLE,INM_SVCTYPE_MANUAL);
}

SVERROR OracleController::GetSourceDiscoveryInformation()
{
    DebugPrintf(SV_LOG_DEBUG, "ENTERED %s\n", FUNCTION_NAME) ;
    ...
    try
    {
        if(m_FailoverInfo.m_AppType.compare("ORACLE") == 0)
        {
            m_srcDiscInfo = unmarshal<SrcOracleDiscoveryInformation>(marshalledSrcDiscInfoStr) ;
        }
        bRet = SVS_OK;
    }   
    catch ( ContextualException& ce )
...
void OracleController::UpdateConfigFiles()
{
              ...
              m_FailoverInfo.m_PolicyType.compare("ProductionServerFastFailbackStartApplication") == 0)
    {
         ...
         srcDbGeneratedPFilePath.clear();
         srcDbPFilePath.clear();
        ...
        if(!failoverFiles.empty())
        {
            ...
            {
              DebugPrintf(SV_LOG_DEBUG,"key value in failoverFiles %s \n",((failoverFilesIter)->first).c_str());
              if("PairInfo" != ((failoverFilesIter)->first)) 
              {
                std::list<std::string> fileExtension = tokenizeString((failoverFilesIter)->first, ".");
                if("ora" == fileExtension.back())
                {
                    std::string pFile = installPath + "/failover_data/" + (failoverFilesIter)->first;
                    std::ofstream out;
                    out.open(pFile.c_str(), std::ios::trunc);
                    if (!out) 
                    {
                        DebugPrintf(SV_LOG_ERROR,"Failed to open %s file\n", pFile.c_str());
                        return;
                    ...
                    out.close();
                    srcDbPFilePath = pFile;
                }
                else if("ora_pfile" == fileExtension.back())
                {
                    std::string generatedPfile = installPath +  "/failover_data/" + (failoverFilesIter)->first;
                    std::ofstream out;
                    out.open(generatedPfile.c_str(), std::ios::trunc);
                    if (!out)
                    {
                        DebugPrintf(SV_LOG_ERROR,"Failed to open %s file\n", generatedPfile.c_str());
                        return;
                    }
                    out << (failoverFilesIter)->second;
                    out.close();
                    srcDbGeneratedPFilePath = generatedPfile;
                }
                else if("conf" == fileExtension.back())
                {
                    srcDbInstance=fileExtension.front();
                    DebugPrintf(SV_LOG_DEBUG,"Protected Oracle instance is : %s\n",srcDbInstance.c_str());
                    std::string srcConfigFile = installPath +  "/failover_data/" + (failoverFilesIter)->first;
                    std::ofstream out;
                    ...
                    //srcDbConfigFileMap.insert(std::make_pair(m_srcDiscInfo.m_properties["DBName"], srcConfigFile));
                 }
              }
              else if("PairInfo" == ((failoverFilesIter)->first))
              {
                ...
                try
                {
                   ...
                   std::map<std::string,std::map<std::string,std::string> >::const_iterator pairInfoIter = pairInfo.begin();
                                                                                                                                                                                           //std::string instance = (m_FailoverData.m_InstancesList).front();
                                                                                                                                                                                           DebugPrintf(SV_LOG_DEBUG,"Oracle database protected is %s \n",(srcDbInstance).c_str());
            ...
            if (srcDbConfFilePath.empty() || srcDbGeneratedPFilePath.empty() || srcDbPFilePath.empty())
            {
...
void OracleController::BuildFailoverCommand(std::string& failoverCmd)
{
    DebugPrintf(SV_LOG_DEBUG, "ENTERED %s\n", FUNCTION_NAME) ;
    AppLocalConfigurator localconfigurator ;
            ...
            policyType.compare("DRServerPlannedFailback") == 0)
    {
        ...
        cmdStream << "/scripts/oracle_consistency.sh";
        cmdStream << " failover ";
    ...
    {
        cmdStream << installPath;
        cmdStream << "/scripts/oraclefailover.sh";
        cmdStream << " -a ";
        ...
        if (m_FailoverInfo.m_SolutionType.compare("PRISM") == 0 )
        {
            cmdStream << " -c yes ";
        }
        else
        {
            if ( m_FailoverInfo.m_IsOracleRac.compare("1") == 0 )
                 cmdStream << " -c yes ";
            else
                 cmdStream << " -c no";
        }
            ...
            policyType.compare("ProductionServerPlannedFailbackRollbackVolumes") == 0)
        {
            if (policyType.compare("DRServerUnPlannedFailoverRollbackVolumes") == 0 )
            {
                ...
                else if(m_FailoverInfo.m_RecoveryPointType.compare("LCCP") == 0)
                {
                    std::string tagName;
                    cmdStream <<  "LATEST " ;
                    // GetValueFromMap(m_FailoverData.m_properties,"TagName",tagName) ;
                    cmdStream <<  " -n " ;
                    cmdStream <<  " none " ;
                    // cmdStream << tagName;
                }
                else if(m_FailoverInfo.m_RecoveryPointType.compare("LCT") == 0)
                {
                    ...
                    // GetValueFromMap(m_FailoverData.m_properties,"Timestamp",timeStamp); 
                    cmdStream <<  " -n " ;
                    cmdStream <<  " none " ;
                    // cmdStream << timeStamp;
                }
        ...
        cmdStream << " -p ";
        cmdStream << srcDbPFilePath;
        cmdStream << " -g ";
        cmdStream << srcDbGeneratedPFilePath;
...
void OracleController::PrePareFailoverJobs()
{
    ...
    DebugPrintf(SV_LOG_DEBUG, "OracleController::PrePareFailoverJobs %s\n", m_FailoverInfo.m_PolicyType.c_str()) ;
    ...
    DebugPrintf(SV_LOG_DEBUG, "OracleController::PrePareFailoverJobs Failover command %s\n", failoverCmd.m_Command.c_str()) ;
...
SVERROR OracleController::stopOracleServices(const std::string &appName,bool m_bCluster)
{
	DebugPrintf(SV_LOG_DEBUG, "ENTERED %s\n", FUNCTION_NAME) ;
	SVERROR result = SVS_FALSE;
	DebugPrintf(SV_LOG_DEBUG, "EXITED %s\n", FUNCTION_NAME) ;
...
void OracleController::ProcessMsg(SV_ULONG policyId)
{
	DebugPrintf(SV_LOG_DEBUG, "ENTERED OracleController::%s\n", FUNCTION_NAME) ;
    ...
    if( configuratorPtr->getApplicationPolicies(policyId, "ORACLE",m_policy) )
    {
        Process() ;
    }
    else
