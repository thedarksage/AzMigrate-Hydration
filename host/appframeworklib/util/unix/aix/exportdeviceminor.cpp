...
bool ExportDeviceUsingIscsi(const std::string& deviceName, DeviceExportParams& exportParams, std::stringstream& stream)
{
	...
	if( operation.compare("Export") == 0 )
	{
        ...
        stream << "iSCSI target export not supported from AIX.";
	...
	else if( operation.compare("Login") == 0 )
	{
        ...
        std::string loginScript="grep -w ";
        loginScript += targetName.c_str();
        loginScript += " /etc/iscsi/targets | grep -v ^#; if [ $? -ne 0 ]; then echo ";
        loginScript += targetIp.c_str();
        loginScript += " 3260 " ;
        loginScript += targetName.c_str();
        loginScript += " >> /etc/iscsi/targets; cfgmgr; fi";

        std::string outputFile="/tmp/iscsiexport.log";

        AppCommand appCommand(loginScript, 0, outputFile);

        SV_ULONG exitCode;
        std::string output;

        if (appCommand.Run(exitCode, output, Controller::getInstance()->m_bActive) != SVS_OK)
        {
            DebugPrintf(SV_LOG_ERROR, "Failed to update /etc/iscsi/targets with the target %s - exit code %d\n", targetIp.c_str(), exitCode) ;
        }
        else
        {
            DebugPrintf(SV_LOG_DEBUG, "Updated /etc/iscsi/targets with the target %s - output %s\n", targetIp.c_str(), output.c_str()) ;
        }
	...
	else if(operation.compare("Logout") == 0 )
	{
        ...
        std::string logoutScript="grep -w ";
        logoutScript += targetName.c_str();
        logoutScript += " /etc/iscsi/targets | grep -v ^#; if [ $? -eq 0 ]; then grep -v -w ";
        logoutScript += targetIp.c_str() ;
        logoutScript += " /etc/iscsi/targets > /tmp/targets.$$ ; mv /tmp/targets.$$ /etc/iscsi/targets; fi;";

        std::string outputFile="/tmp/iscsiexport.log";

        AppCommand appCommand(logoutScript, 0, outputFile);
        ...
        if (appCommand.Run(exitCode, output, Controller::getInstance()->m_bActive) != SVS_OK)
        {
            DebugPrintf(SV_LOG_ERROR, "Failed to remove the target %s %s  from /etc/iscsi/targets - exit code %d\n", targetIp.c_str(), targetName.c_str(), exitCode) ;
        }
        else
        {
            DebugPrintf(SV_LOG_DEBUG, "Updated delete the target %s %s - from /etc/iscsi/targets - output %s\n", targetIp.c_str(), targetName.c_str(), output.c_str()) ;
        }
	}
...
bool ExportDeviceUsingCIFS(const std::string& deviceName, const DeviceExportParams& exportParams, std::stringstream& stream)
{
	...
	{
        // XXX - for now
        stream << "CIFS export not supported on AIX.";
	}
	DebugPrintf(SV_LOG_DEBUG, "EXITED %s\n", FUNCTION_NAME) ;
	return bRet ;
}
...
bool ExportDeviceUsingNFS(const std::string& deviceName, const DeviceExportParams& exportParams, std::stringstream& stream)
{
	...
	{
        // XXX - for now
        stream << "NFS export not supported on AIX.";
	}
	DebugPrintf(SV_LOG_DEBUG, "EXITED %s\n", FUNCTION_NAME) ;
	return bRet ;
}
