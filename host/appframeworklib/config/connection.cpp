...
bool SqliteConnection::open()
{
	...
	try
	{
		...
		if( m_con == NULL )
		{
			throw "Failed to create connection object\n" ;
		}
...
bool SqliteConnection::execQuery(const std::string& query, DBResultSet &set)
{
	...
	DebugPrintf( SV_LOG_DEBUG, " Query string is %s \n", query.c_str() );
	bool retVal = false;
	set.clear() ;
	try
	{
...
bool SqliteConnection::beginTransaction()
{
	...
	{
		retVal = false ;
		throw "Unable to create sqlite3_transaction object\n" ;
	}
	try
	{
		m_trans->begin() ;
...
bool SqliteConnection::rollbackTransaction()
{
	...
	{
		retVal = false ;
		throw "Invalid sqlite3_transaction object\n" ;
	}
	try
	{
		m_trans ->rollback() ;
