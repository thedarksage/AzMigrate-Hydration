...
vv_alloc_dev(inm_vdev_t **vvdev, char *dev_path)
{
	...
	retval = inm_alloc_disk_info(vdev);
	if ( retval < 0 ) {
	...
	retval = ddi_soft_state_zalloc(vv_listp,
	...
	vdev_soft_state = (inm_vdev_t **)ddi_get_soft_state(vv_listp,
					INM_SOL_VV_SOFT_STATE_IDX(minor));
	if ( !vdev_soft_state ) {
...
get_soft_state_failed:
	...
	ddi_soft_state_free(vv_listp, INM_SOL_VV_SOFT_STATE_IDX(minor));
	    ...
	    char *dev_path)
{
	...
	/*
	 * Mark the vdev VSNAP_FLAGS_FREEING so as to stop all I/Os
	 * and open/close calls to the vsnap/partitions while they
	 * are being created
	 */
	...
	retval = inm_populate_disk_info(vdev, size);
	if ( retval < 0 ) {
	...
	minor = vdev->vd_index;
	DEV_DBG("adding the disk - %s", INM_VDEV_NAME(vdev));
        sprintf(namebuf, INM_VV_BLKDEV"%d", minor);
	...
	sprintf(namebuf, INM_VV_BLKDEV"%d,raw", minor);
...
#ifndef _SOLARIS_8_
	dev = CALC_DEV_T(inm_vv_major, minor);
...
#ifndef _SOLARIS_8_
	dev = makedevice(inm_vv_major, minor);
	(void) ddi_prop_remove(dev, vdev_dip, SIZE);
