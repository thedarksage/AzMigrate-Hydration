...
//This Function is called with volume spin lock held
VOID
InitializeProfiler(PVOLUME_PROFILE VolumeProfile, PVOLUME_PROFILE_INITIALIZER ProfileInitializer)
{
    ...
    if(ProfileInitializer->Type == READ_BUCKET_INITIALIZER)
    {
        ...
        VolumeProfile->ReadProfile.MostFrequentlyUsedBucket = 1;

        for(ULONG Index = 0; Index < ulMaxBuckets; Index++) {
            VolumeProfile->ReadProfile.ReadIoSizeArray[Index] =  IoReadArraySizeInput->ulIoSizeArray[Index];
            VolumeProfile->ReadProfile.ReadIoSizeCounters[Index] = 0;
        }

        for (ULONG Index = ulMaxBuckets; Index < MAX_VC_IO_SIZE_BUCKETS; Index++) {
            VolumeProfile->ReadProfile.ReadIoSizeArray[Index] = 0;
            VolumeProfile->ReadProfile.ReadIoSizeCounters[Index] = 0;
        }
...
LogReadIOSize(ULONG IoSize, PVOLUME_PROFILE VolumeProfile)
{
    ...
    if(VolumeProfile->ReadProfile.ReadIoSizeArray[VolumeProfile->ReadProfile.MostFrequentlyUsedBucket - 1] < IoSize)
        Index = VolumeProfile->ReadProfile.MostFrequentlyUsedBucket;
    
    for (; Index < MAX_VC_IO_SIZE_BUCKETS; Index++) {
        if (VolumeProfile->ReadProfile.ReadIoSizeArray[Index]) {
            if (IoSize <= VolumeProfile->ReadProfile.ReadIoSizeArray[Index]) {
                VolumeProfile->ReadProfile.ReadIoSizeCounters[Index]++;
                
                if(Index != 0 && VolumeProfile->ReadProfile.ReadIoSizeCounters[Index] > 
                    VolumeProfile->ReadProfile.ReadIoSizeCounters[VolumeProfile->ReadProfile.MostFrequentlyUsedBucket])
                    VolumeProfile->ReadProfile.MostFrequentlyUsedBucket = Index;
                break;
            }
        } else {
            VolumeProfile->ReadProfile.ReadIoSizeCounters[Index]++;
