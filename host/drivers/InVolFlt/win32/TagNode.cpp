...
AllocateTagNode(PTAG_INFO TagInfo, PIRP Irp)
{
    ...
    ULONG       ulSize;
    ULONG       ulNumberOfVolumes = TagInfo->ulNumberOfVolumes;
...
AddTagNodeToList(PTAG_INFO TagInfo, PIRP Irp, etTagState eTagState)
{
    ...
    if (ecTagStateWaitForCommitSnapshot == eTagState) {
        UpdateLockedDataBlockList(DriverContext.ulMaxLockedDataBlocks + TagInfo->ulNumberOfVolumes);
...
AddStatusAndReturnNodeIfComplete(PTAG_INFO TagInfo)
{
    ...
    TagNode = NULL;
    if (NULL != Temp) {
        for (ULONG i = 0; i < TagInfo->ulNumberOfVolumes; i++) {
