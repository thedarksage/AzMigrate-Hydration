...
class ScopeSpinLock;
// This is a tiny class with no parent which uses it's constructors and
// destructors to lock and unlock a spin lock. There is some history of doing
// this in the ksutil.h header and the wdm\bda\europa in the DDK samples. 
// This is also the ONLY class that is allocated in a local stack frame not pool memory, 
// as it's tiny.
...
// address translation. For example a FileStream on a RAW volume may need to be
