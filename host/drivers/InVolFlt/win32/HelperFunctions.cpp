    ...
    )
{
    ...
    if (NULL == pBuffer) {
        LogMemoryAllocationFailure(usMaxLength, PoolType, __WFILE__, __LINE__);
    ...
    )
{
    ...
    if (Dest->MaximumLength < Src->Length + Dest->Length + sizeof(WCHAR)) {
        ...
        PWCHAR  pBuffer = (PWCHAR)ExAllocatePoolWithTag(PoolType, 
                                                ...
                                                (NonPagedPool == PoolType) ? TAG_GENERIC_NON_PAGED : TAG_GENERIC_PAGED);
        if (NULL == pBuffer) {
            LogMemoryAllocationFailure(usLength, PoolType, __WFILE__, __LINE__);
            return STATUS_NO_MEMORY;
        }
    ...
    )
{
    ...
    do {
        ...
        Status = RtlStringCchVPrintfExW(pBuffer, 
                                        ...
                                        arglist);
        if (NT_SUCCESS(Status)) {
            Dst->Length = Dst->MaximumLength - (CchRemaining * sizeof(WCHAR));
        } else {
