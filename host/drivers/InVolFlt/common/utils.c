...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
	...
	if(driver_ctx->last_time_stamp_seqno >= INMAGE_MAX_TS_SEQUENCE_NUMBER) {
    ...
    if(time_stamp->TimeInHundNanoSecondsFromJan1601 <= driver_ctx->last_time_stamp) {
	time_stamp->TimeInHundNanoSecondsFromJan1601 = driver_ctx->last_time_stamp;
    } else {
        driver_ctx->last_time_stamp = time_stamp->TimeInHundNanoSecondsFromJan1601;
    }
...
validate_path_for_file_name(char *filename)
{
    ...
    // TODOs: manage white spaces in PATH
    
    local_fname = (char *) INM_KMALLOC(MAX_LOG_PATHNAME, INM_KM_SLEEP, INM_KERNEL_HEAP);
    if (!local_fname) {
	return -ENOMEM;
    }

    tmp_str = (char *) INM_KMALLOC(MAX_LOG_PATHNAME, INM_KM_SLEEP, INM_KERNEL_HEAP);
    if (!tmp_str) {
    ...
    if (*filename == '/') {
        ...
        if (strcpy_s(local_fname, MAX_LOG_PATHNAME, DEFAULT_LOG_DIRECTORY_VALUE)) {
	        INM_KFREE(local_fname, MAX_LOG_PATHNAME, INM_KERNEL_HEAP);
	        INM_KFREE(tmp_str, MAX_LOG_PATHNAME, INM_KERNEL_HEAP);
		return INM_EFAULT;
	}
...
/* TODOs: the prototype should take taret device and measure the size */
/* This fuction needs to removed from the code */
...
inm_s32_t default_granularity_from_volume_size(inm_u64_t volume_size)
{
    ...
    if (driver_ctx->dc_bmap_info.bitmap_512K_granularity_size &&
        _scale > driver_ctx->dc_bmap_info.bitmap_512K_granularity_size) {
	_rc = SIXTEEN_K_SIZE;
    } else {
	_rc = FOUR_K_SIZE;
    }
    ...
    if (driver_ctx->dc_bmap_info.bitmap_512K_granularity_size &&
        _scale > driver_ctx->dc_bmap_info.bitmap_512K_granularity_size) {
	_rc = FIVE_TWELVE_K_SIZE;
    } else {
        _scale >>= 9;
        _rc = (1 << (18 + inm_find_msb(_scale)));
    }
#endif
...
{
    *path = (char *)INM_KMALLOC(INM_PATH_MAX, INM_KM_SLEEP, INM_KERNEL_HEAP);
    if(!*path)
        return 0;
...
filter_guid_name_val_get(char *pname, char *fname)
{
    ...
    s = INM_KMEM_CACHE_ALLOC_PATH(names_cachep, INM_KM_SLEEP, INM_PATH_MAX, INM_KERNEL_HEAP);
    INM_BUG_ON(!s);
...
inm_get_ts_and_seqno_deltas(change_node_t *cnp, inm_tsdelta_t *tdp)
{
	...
	if (cnp->changes.change_idx) {
	        ...
	        sdelta = ts.ullSequenceNumber - cnp->changes.start_ts.ullSequenceNumber;
...
inm_flush_clean_shutdown(inm_u32_t clean_shutdown)
{
    ...
    snprintf(pathp, INM_PATH_MAX, "%s/%s/CleanShutdown", PERSISTENT_DIR,COMMON_ATTR_NAME);
...
inm_s32_t
write_vol_attr(target_context_t * ctxt, const char *file_name, void *buf, inm_s32_t len)
{
    char *path = NULL;
...
inm_free_host_dev_ctx(struct host_dev_context *hdcp)
{
    ...
    if (hdcp) {
	    inm_list_for_each_safe(ptr, nextptr, &hdcp->hdc_dev_list_head) {
	        inm_list_del(ptr);
...
cnvt_tag_info2stream(tag_info_t * tag_info, inm_s32_t num_tags, inm_u32_t flag)
{
    ...
    stag_info = (tag_info_t *)INM_KMALLOC(sizeof(tag_info_t) * num_tags, INM_KM_SLEEP | flag, INM_KERNEL_HEAP);
    if (!stag_info) {
        dbg("Failed to allocate tag info structure");
        goto out;
    }
    if (memcpy_s(stag_info, sizeof(tag_info_t) * num_tags, tag_info, sizeof(tag_info_t) * num_tags)) {
...
cnvt_stream2tag_info(tag_info_t *stag_info, inm_s32_t num_tags)
{
    ...
    tag_info = (tag_info_t *)INM_KMALLOC(sizeof(tag_info_t) * num_tags, INM_KM_NOSLEEP, INM_KERNEL_HEAP);
    if (!tag_info){
       goto out;
    }
    if (memcpy_s(tag_info, sizeof(tag_info_t) * num_tags, stag_info, sizeof(tag_info_t) * num_tags)) {
...
inm_form_tag_cdb(target_context_t *tcp, tag_info_t *tag_info, inm_s32_t num_tags)
{
   ...
   cmd[14] = 0x0;
   cmd[15] = 0x0;
   error = inm_all_AT_cdb_send(tcp, cmd, VACP_CDB_LEN, 1, (unsigned char *)stag_info, buflen, 0);
    if (error){
...
inm_heartbeat_cdb(target_context_t *tcp)
{
    ...
    error = try_reactive_offline_AT_path(tcp, cmd, HEARTBEAT_CDB_LEN, 1, NULL, 0, 0);
    if(!error){
       goto out;
    }
    					...
    					
{
    ...
    //tag_buf = tag_buf + sizeof(inm_u16_t);
    //tag_buf_len -= sizeof(inm_u16_t);
