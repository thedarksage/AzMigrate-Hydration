...
static void mv_stale_entry_to_dead_list(target_context_t *, struct inm_list_head *,
                                struct inm_list_head *, struct inm_list_head *);
static inm_s32_t inm_deref_all_vol_entry_list(struct inm_list_head *, target_context_t *);
...
init_volume_fully(target_context_t *tgt_ctxt, inm_dev_extinfo_t *dev_info)
{
        ...
        if (strncmp(tgt_ctxt->tc_pname, dev_info->d_pname, INM_GUID_LEN_MAX)) {
            /* Root disk persistent name is different */
            info("Updating %s pname: %s", tgt_ctxt->tc_guid, dev_info->d_pname);
            strcpy_s(tgt_ctxt->tc_pname, INM_GUID_LEN_MAX, dev_info->d_pname);
        }
...
retry:
    ...
    switch(dev_info->d_type) {
        ...
        default: err("Invalid case for filter dev type:%d",dev_info->d_type);
    }
    ...
    if (!tgt_ctx) {
        ...
        switch (ctx->tc_dev_type) {
            case FILTER_DEV_FABRIC_LUN:
            case FILTER_DEV_HOST_VOLUME:
                strcpy_s(ctx->tc_pname, INM_GUID_LEN_MAX, dev_info->d_pname);
                break;
            case FILTER_DEV_MIRROR_SETUP:
                ...
                if (dev_info->d_flags & MIRROR_VOLUME_STACKING_FLAG) {
                    ...
                    list_change_head(&ctx->tc_src_list, dev_info->src_list);
                    list_change_head(&ctx->tc_dst_list, dev_info->dst_list);
                   ...
                   break;
            default:
                err("invalid filter dev type:%d",ctx->tc_dev_type);
        }
    ...
    if (dev_info->d_flags & HOST_VOLUME_STACKING_FLAG) {
        ctx->tc_flags |= VCF_VOLUME_BOOTTIME_STACKED;
            ...
            if ((driver_state & DRV_LOADED_FULLY) &&
                    !(ctx->tc_flags & VCF_VOLUME_INITRD_STACKED)) {
                err("The disk %s is not protected in initrd", ctx->tc_guid);
    ...
    volume_unlock(ctx);
        ...
        INM_DOWN_WRITE(&(driver_ctx->tgt_list_sem));
    if(!(dev_info->d_flags & (MIRROR_VOLUME_STACKING_FLAG | HOST_VOLUME_STACKING_FLAG))){
        ...
        if (tgt_ctx->tc_flags & VCF_VOLUME_STACKED_PARTIALLY) {
            ...
            err("Existing: %s -> %s, Requested %s -> %s",
                ...
                dbg("PNAME: %s == %s",tgt_ctx->tc_pname, dev_info->d_pname);
            }
        } else {
...
/* This interface is used only during uninstallation of the product at which time s2/dp
 * is assumed to be not running.
 */
void
do_unstack_all()
{
...
retry:
    while(!inm_list_empty(&driver_ctx->tgt_list)){
       tgt_ctxt = inm_list_entry(driver_ctx->tgt_list.next, target_context_t, tc_list);
       info("unstack_all : ctx:%p tc_guid:%s", tgt_ctxt, tgt_ctxt->tc_guid);
...
{
    //acquire dc lock
    //TODOs: if process ids match , then set unmap_data to TRUE
    data_mode_cleanup_for_s2_exit();
}

void
involflt_completion(target_context_t *tgt_ctxt,
                    write_metadata_t *wmdp, inm_wdata_t *wdatap, int lock_held)
{
...
do_start_filtering(inm_devhandle_t *idhp, inm_dev_extinfo_t *dev_infop)
{
    ...
    switch(dev_infop->d_type) {
        case FILTER_DEV_HOST_VOLUME:
            ...
            if (r){
                err("volume:%s -> %s stacking failed with %d",dev_infop->d_guid, 
                    dev_infop->d_pname, r);
            }
        break;
        case FILTER_DEV_FABRIC_LUN:
    ...
    if (!ctx->tc_bp->volume_bitmap &&
        ...
        if (dev_infop->d_type == FILTER_DEV_HOST_VOLUME &&
            ...
            set_string_vol_attr(ctx, VolumeMountPoint, dev_infop->d_mnt_pt);
        }
        set_unsignedlonglong_vol_attr(ctx, VolumeIsDeviceMultipath,
         ((dev_infop->d_flags & INM_IS_DEVICE_MULTIPATH)? 1:0));
        set_unsignedlonglong_vol_attr(ctx, VolumeDiskFlags, dev_infop->d_flags);
        ...
        r = INM_EINVAL;
        err("start filtering:%s failing with EINVAL", dev_infop->d_guid);
    }
...
mirror_vol_entry_t *
build_mirror_volume(inm_s32_t vol_length, void *ptr, struct inm_list_head *mirror_list_head,
                    int keep_device_open, int lun_type)
{
    mirror_vol_entry_t *vol_entry = NULL;
    char *vol_ptr = NULL;
    ...
    vol_ptr = (char*)INM_KMALLOC(INM_GUID_LEN_MAX, INM_KM_SLEEP, INM_KERNEL_HEAP);
    if(!vol_ptr){
    ...
    strncpy_s(vol_entry->tc_mirror_guid, INM_GUID_LEN_MAX, vol_ptr, vol_length);
...
print_mirror_list(struct inm_list_head *list_head)
{
    ...
    inm_list_for_each_safe(ptr, nextptr, list_head) {
        vol_entry = inm_list_entry(ptr, mirror_vol_entry_t, next);
        info(":%s: mirror_dev:%p",vol_entry->tc_mirror_guid, vol_entry->mirror_dev);
    }
}
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_MIRROR))){
    ...
    for (i=1; i<=num_src_vols; i++) {
        if (!build_mirror_volume(vol_length, user_buf, src_mirror_list_head, 0, INM_PT_LUN)) {
    ...
    for (i=1; i<=num_dst_vols; i++) {
        if (!build_mirror_volume(vol_length, user_buf, dst_mirror_list_head, 1, INM_AT_LUN)) {
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_MIRROR))){
    ...
    strncpy_s(dev_infop->d_src_scsi_id, INM_GUID_LEN_MAX, mirror_infop->src_scsi_id,
            INM_MAX_SCSI_ID_SIZE); 
    strncpy_s(dev_infop->d_dst_scsi_id, INM_GUID_LEN_MAX, mirror_infop->dst_scsi_id,
            INM_MAX_SCSI_ID_SIZE); 
    ...
    if (ctx) {
        ...
        if (mirror_infop->d_flags & MIRROR_SETUP_PENDING_RESYNC_CLEARED_FLAG) {
        ...
        if (!inm_list_empty(&ctx->tc_src_list)) {
            ...
            inm_list_for_each_safe(ptr1, nextptr1, &src_mirror_list_head) {
                vol_entry1 = inm_list_entry(ptr1, mirror_vol_entry_t, next);
                inm_list_for_each_safe(ptr2, nextptr2, &ctx->tc_src_list) {
                    vol_entry2 = inm_list_entry(ptr2, mirror_vol_entry_t, next);
                    if (!strcmp(vol_entry1->tc_mirror_guid,
                        vol_entry2->tc_mirror_guid)) {
                        inm_list_del(ptr1);
                        inm_list_add_tail(ptr1, &del_src_mirror_list_head);
                        break;
                    }
                }
            }
            ...
            if (!inm_list_empty(&src_mirror_list_head)) {
                ...
                inm_list_for_each_safe(ptr2, nextptr2, &src_mirror_list_head) {
                    vol_entry2 = inm_list_entry(ptr2, mirror_vol_entry_t, next);
...
#endif
                        inm_list_del(ptr2);
                        inm_list_add_tail(ptr2, &ctx->tc_src_list);
                    }
                }
                mark_resync = 1;
            }
            ...
            list_change_head(&ctx->tc_src_list, &src_mirror_list_head);
        }
        ...
        if (!inm_list_empty(&dst_mirror_list_head)) {
             inm_list_splice_at_tail(&dst_mirror_list_head, &ctx->tc_dst_list);
        }
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_MIRROR))){
    ...
    print_mirror_list(&(ctx->tc_dst_list));
        ...
        /* Get the first entry in the list and set it as tcp mirror_dev */
        vol_entry = inm_list_entry(ctx->tc_dst_list.next, mirror_vol_entry_t, next);
        INM_BUG_ON(!vol_entry);
        ctx->tc_vol_entry = vol_entry;
...
err_case:
    ...
    inm_deref_all_vol_entry_list(&deref_mirror_list, ctx);
        ...
        set_int_vol_attr(ctx, VolumeDeviceVendor, mirror_infop->d_vendor);
        set_unsignedlonglong_vol_attr(ctx, VolumeDiskFlags, mirror_infop->d_flags);
        set_unsignedlonglong_vol_attr(ctx, VolumeDevStartOff, mirror_infop->startoff);
        if (mark_resync) {
            queue_worker_routine_for_set_volume_out_of_sync(ctx,
                ERROR_TO_REG_NEW_SOURCE_PATH_ADDED, INM_EINVAL);
        }
...
do_stop_filtering(target_context_t *tgt_ctxt)
{
    ...
    if(!is_target_filtering_disabled(tgt_ctxt)) {
        ...
        * in clear diffs */
       if (tgt_ctxt->tc_pending_confirm && !(tgt_ctxt->tc_pending_confirm->flags & CHANGE_NODE_ORPHANED)) {
        ...
        info(" filtering mode = %d, stop filtering flag = %d, write order state = %d",
         tgt_ctxt->tc_cur_mode, is_target_filtering_disabled(tgt_ctxt), tgt_ctxt->tc_cur_wostate);
        seton = TRUE;
    }
    ...
    set_unsignedlonglong_vol_attr(tgt_ctxt, VolumePrevEndTimeStamp, PrevEndTimeStamp);
    set_unsignedlonglong_vol_attr(tgt_ctxt, VolumePrevEndSequenceNumber, PrevEndSequenceNumber);
    set_unsignedlonglong_vol_attr(tgt_ctxt, VolumePrevSequenceIDforSplitIO, PrevSequenceIDforSplitIO);

    set_int_vol_attr(tgt_ctxt, VolumeDrainBlocked, 0);

    /* TODO: Should queue this work to worker thread in future.
     */
    ...
    telemetry_clear_dbs(&tgt_ctxt->tc_tel.tt_blend, DBS_DRIVER_RESYNC_REQUIRED);
    ...
    set_int_vol_attr(tgt_ctxt, VolumeResyncRequired, tgt_ctxt->tc_resync_required);
    set_int_vol_attr(tgt_ctxt, VolumeOutOfSyncErrorCode, tgt_ctxt->tc_out_of_sync_err_code);
    set_int_vol_attr(tgt_ctxt, VolumeOutOfSyncCount, tgt_ctxt->tc_nr_out_of_sync);
    set_longlong_vol_attr(tgt_ctxt, VolumeOutOfSyncTimestamp, tgt_ctxt->tc_out_of_sync_time_stamp);
    ...
    while (INM_ATOMIC_READ(&tgt_ctxt->tc_nr_in_flight_ios)) {
        ...
        (INM_ATOMIC_READ(&tgt_ctxt->tc_nr_in_flight_ios) == 0), 3 * INM_HZ);
    }
        ...
        tag_guid_t *tag_guid, inm_s32_t index)
{
    ...
    if(!add_tag_in_stream_mode(tag_volinfop, tag_info, num_tags, tag_guid, index))
        add_tag_in_non_stream_mode(tag_volinfop, tag_info, num_tags, tag_guid, 
                                   index, TAG_COMMIT_NOT_PENDING, NULL);
    }
    volume_unlock(ctxt);
...
build_volume_list(inm_s32_t num_vols, void __INM_USER **user_buf, inm_s32_t *error)
{
    ...
    *error = 0;
    ...
    while(i < num_vols) {
        ...
        *user_buf += sizeof(unsigned short);
        ...
        if(ctxt) {
            if(!is_target_filtering_disabled(ctxt) && !is_target_being_frozen(ctxt)) {
                list_temp->ctxt = ctxt;
                vols++;
                // Using this flag, ensure same volume 
                volume_lock(ctxt);
...
get_tag_from_guid(char *guid)
{
    ...
    INM_DOWN_READ(&(driver_ctx->tgt_list_sem));
    for(ptr = driver_ctx->tag_guid_list.next; ptr != &(driver_ctx->tag_guid_list);
...
flt_cleanup_sync_tag(tag_guid_t *tag_guid)
{
    ...
    if(tag_guid->status)
    INM_KFREE(tag_guid->status, tag_guid->num_vols * sizeof(inm_s32_t), INM_KERNEL_HEAP);
...
if(IS_DBG_ENABLED(inm_verbosity, INM_IDEBUG)){
    ...
    *user_buf += sizeof(unsigned short);
    ...
    error = issue_tags(num_vols, vol_list, num_tags, tag_list, flags, tag_guid);
...
/* BOOT-TIME STACKING - CODE : BEGIN */

/* is_flt_disabled fn chks whether vol was filtering at
 * the time of shutdown/reboot */
 ...
 */
static void
mv_stale_entry_to_dead_list(target_context_t *tcp, struct inm_list_head *new_dst_listp,
                            struct inm_list_head *delete_dst_list, struct inm_list_head *deref_list)
{
    struct inm_list_head *ptr, *hd, *nextptr;
    ...
    inm_list_for_each_safe(ptr, nextptr, hd) {
        ...
        inm_list_for_each_safe(ptr_new, nextptr_new, hd_new) {
            vol_entry_new = inm_container_of(ptr_new, mirror_vol_entry_t, next);
            if (!strncmp(vol_entry->tc_mirror_guid, vol_entry_new->tc_mirror_guid, 
                INM_GUID_LEN_MAX)) {
...
static inm_s32_t
inm_deref_all_vol_entry_list(struct inm_list_head *deref_list, target_context_t *tcp)
{
    struct inm_list_head *ptr, *hd, *nextptr;
    mirror_vol_entry_t *vol_entry = NULL;
    inm_s32_t error = 0;
        ...
        inm_u32_t write_len)
{
    ...
    ret = inm_all_AT_cdb_send(tcp, cdbp, WRITE_CANCEL_CDB_LEN, 1, NULL, 0, 0);
    if(ret){
        err("USCSI ioctl failed for cmd %x retval %u",WRITE_CANCEL_CDB_LEN, ret);
        goto out;
    }
...
INM_MIRROR_IODONE(inm_at_mirror_iodone, at_bp, done, error)
{
    ...
    if( INM_BUF_FAILED(at_bp, error) ){
        add_item_to_work_queue(&driver_ctx->wqueue, &(atbuf_wrap->imb_atbuf_wqe));
        goto out;
    }
...
inm_mirror_done(inm_mirror_bufinfo_t *mbufinfo)
{
    ...
    if(mbufinfo->imb_flag & INM_PTIO_CANCEL_PENDING){
        if(mbufinfo->imb_flag & INM_PTIO_FULL_FAILED){
            inm_list_for_each_safe(ptr, nextptr, &mbufinfo->imb_atbuf_list) {
                atbuf_wrap = inm_list_entry(ptr, inm_mirror_atbuf, imb_atbuf_this);
                if(atbuf_wrap->imb_atbuf_flag & INM_ATBUF_FULL_FAILED){
                    failed_atbufs++;
                }
            }
        ...
        add_item_to_work_queue(&driver_ctx->wqueue, &(mbufinfo->ptio_can_wqe));
        goto out;
    }
done:
...
inm_atio_retry(wqentry_t *wqe)
{
    ...
    if( INM_BUF_RESID(at_bp, atbuf_wrap->imb_atbuf_done) == atbuf_wrap->imb_atbuf_iosz ){
    ...
    atbuf_wrap->imb_atbuf_flag &= ~(INM_ATBUF_FULL_FAILED | INM_ATBUF_PARTIAL_FAILED);
    volume_unlock(tcp);
...
atio_done:
    ...
    if(err){
        dbg("AT IO retry Buf: Offset = %lld, count = %u", 
            (long long int)INM_BUF_SECTOR(at_bp),  INM_BUF_COUNT(at_bp));
        queue_worker_routine_for_set_volume_out_of_sync(tcp, ERROR_TO_REG_AT_PATHS_FAILURE, 0);
    }
    goto out;
}
...
issue_ptio_cancel_cdb(wqentry_t *wqe)
{
    ...
    if(ptio_cancel_send(tcp, mbufinfo->imb_io_off, mbufinfo->imb_io_sz)){
        queue_worker_routine_for_set_volume_out_of_sync(tcp, ERROR_TO_REG_PTIO_CANCEL_FAILED, 2);
    }
    /*
    * No need to protect inm_flag as nobody else would be refering it
         ...
         inm_buf_t **org_bpp, mirror_vol_entry_t *vol_entry)
{
    ...
    inm_u32_t no_atbufs = (INM_BUF_COUNT(bp) + max_xfer_size - 1) / max_xfer_size;
    inm_u64_t io_sz = 0;
    ...
    do {
        ...
        dbg("write entry pt : off = %llu, sz = %d", INM_BUF_SECTOR(bp),
            INM_BUF_COUNT(bp));
#endif
        //info("xmemflags = %d, aspace_id = %d", bp->b_xmemd.xmemflags, bp->b_xmemd.aspace_id);
        newbp = NULL;
...
out:
        ...
        do{
            if(!inm_list_empty(&tmp_atbuf_list)){
                atbuf_wrap = inm_list_entry(tmp_atbuf_list.next, inm_mirror_atbuf, imb_atbuf_this);
                if (atbuf_wrap) {
...
populate_atbuf:
            inm_list_add_tail(&(atbuf_wrap->imb_atbuf_this), &(newbp->imb_atbuf_list));
            inm_prepare_atbuf(atbuf_wrap, bp, vol_entry, newbp->imb_atbuf_cnt);
            if(!first_atbuf){
...
free_newbp:
    ...
    INM_KMEM_CACHE_FREE(driver_ctx->dc_host_info.mirror_bioinfo_cache, newbp);
...
error:
    ...
    queue_worker_routine_for_set_volume_out_of_sync(tcp, ERROR_TO_REG_FAILED_TO_ALLOC_BIOINFO, INM_ENOMEM);
exit:
...
inm_freg_atbuf(inm_mirror_atbuf *atbuf_wrap)
{
    ...
    if(max_xfer_size >= atbuf_wrap->imb_atbuf_iosz){
        inm_prepare_atbuf(atbuf_wrap, &atbuf_wrap->imb_atbuf_buf, vol_entry, count);
        ...
        do {
            ...
            dbg("breaking at_pb %p of size %u in %dth buf of size %u",at_bp,  INM_BUF_COUNT(at_bp), count, INM_BUF_COUNT((&atbuf_wrap->imb_atbuf_buf)));
            count++;
        } while(more);
        if(err){
            goto out;
        }
        new_atbuf_wrap = inm_container_of(prev_head.next, inm_mirror_atbuf, imb_atbuf_this);
        volume_lock(tcp);
        inm_list_splice(&prev_head, &(atbuf_wrap->imb_atbuf_this));
        inm_list_del(&(atbuf_wrap->imb_atbuf_this));
        volume_unlock(tcp);
        INM_UNPIN(atbuf_wrap, sizeof(inm_mirror_atbuf));
        INM_KFREE(atbuf_wrap, sizeof(inm_mirror_atbuf), INM_KERNEL_HEAP);
        atbuf_wrap = NULL;
    }
out:
    mbufinfo->imb_atbuf_absoff = org_atbuf_blkno;
    while(err && !inm_list_empty(&prev_head)){
        new_atbuf_wrap = inm_list_entry(prev_head.next, inm_mirror_atbuf, imb_atbuf_this);
        inm_list_del(&(new_atbuf_wrap->imb_atbuf_this));
        INM_UNPIN(new_atbuf_wrap, sizeof(mirror_vol_entry_t));
        INM_KFREE(new_atbuf_wrap, sizeof(mirror_vol_entry_t), INM_KERNEL_HEAP);
        new_atbuf_wrap = NULL;
    }
    dbg("exiting from inm_freg_atbuf with err %d, new_atbuf %p",err, new_atbuf_wrap);
...
inm_alloc_atbuf_wrap(inm_mirror_bufinfo_t *mbufinfo)
{
    ...
    atbuf_wrap = (inm_mirror_atbuf *) INM_KMALLOC(sizeof(inm_mirror_atbuf), INM_KM_SLEEP, INM_KERNEL_HEAP);
    if(!atbuf_wrap){
        err = INM_ENOMEM;
        goto out;
    }
...
inm_free_atbuf_list(inm_list_head_t *atbuf_list)
{
    ...
    while(!inm_list_empty(atbuf_list)){
        atbuf_wrap = NULL;
        atbuf_wrap = inm_list_entry(atbuf_list->next, inm_mirror_atbuf, imb_atbuf_this);
    ...
    *error = 0;
    ...
    for(numtags = 0; numtags < tag_vol->nr_tags; numtags++) {
        ...
        if(memcpy_s(tag_list[numtags].tag_name, INM_GUID_LEN_MAX,
                 tag_vol->tag_names[numtags].tag_name, tmp_len)) {
            *error = -1;
            goto out;
        }
    }
                 ...
                 tag_info_t *tag_list, int commit_pending)
{
    ...
    if(tag_volinfop->ctxt && tag_volinfop->ctxt->tc_dev_type != FILTER_DEV_MIRROR_SETUP &&
        ...
        if (driver_ctx->dc_tag_drain_notify_guid &&
            ...
            info("The disk %s is in non write order state", tag_volinfop->ctxt->tc_guid);
        }
        INM_SPIN_UNLOCK(&driver_ctx->dc_tag_commit_status);

        if (tag_status)
            set_tag_drain_notify_status(tag_volinfop->ctxt, TAG_STATUS_INSERTION_FAILED,
                ...
                tag_info_t *tag_info_listp, int commit_pending)
{
    ...
    if(!add_tag_in_non_stream_mode(tag_volinfop, tag_info_listp, tag_vol->nr_tags, NULL, index, commit_pending, NULL)) {
...
build_volume_node_totag(volume_info_t *vol_info, inm_s32_t *error)
{
    ...
    *error = 0;
    ...
    if(ctxt) {
        ...
        dbg("TAG Input Failed: can't issue tag to %s", vol_info->vol_name);
        *error = -1;
        vol_info->status |= STATUS_TAG_NOT_PROTECTED;
        goto out;
    }
         ...
         int dev_status)
{
    ...
    if (ctxt->tc_tag_commit_status) {
        ...
        if (TAG_STATUS_DROPPED == tag_status) {
            info("The failback tag is dropped for disk %s", ctxt->tc_guid);
        }
