...
typedef struct _bitruns_tag
{
    ...
    /* TODOs : in target context it is a page 
       disk_change_t runs[MAX_KDIRTY_CHANGES];
    */
...
typedef struct _bitmap_api_tag
{
    ...
    inm_u32_t     bitmap_size_in_bytes; //in bytes
...
inm_s32_t bitmap_api_open(bitmap_api_t *bapi, struct _target_context *vcptr,
		    ...
		    char *volume_name, inm_u32_t segment_cache_limit, inm_s32_t *detailed_status);

inm_s32_t bitmap_api_load_bitmap_header_from_filestream(bitmap_api_t *bapi,
						  inm_s32_t *detailed_status, inm_s32_t was_created);

inm_s32_t bitmap_api_is_volume_insync(bitmap_api_t *bapi, inm_u8_t *volume_in_sync,
...
inm_s32_t bitmap_api_open_bitmap_stream(bitmap_api_t *bapi, struct _target_context *vcptr, inm_s32_t *detailed_status);
...
inm_u64_t bitmap_api_get_dat_bytes_in_bitmap(bitmap_api_t *bapi, bmap_bit_stats_t *);
