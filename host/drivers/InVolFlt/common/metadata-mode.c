 ...
 *
 * File       : metadata-mode.c
 *
 * Description: Metadata mode related operations.
 save_data_in_metadata_mode
*/
...
coalesce_metadata_change(target_context_t *vcptr, write_metadata_t *wmd, 
                         inm_s32_t data_source,  change_node_t *chg_node,
                         inm_u32_t *add_length)
{
    inm_u64_t ret = 1;
    inm_u16_t md_idx = 0;
    disk_chg_t *chg = NULL;
    ...
    *add_length = 0;
    ...
    if ((chg->offset <= wmd->offset)) {
        if ((chg->offset + chg->length) >= wmd->offset) {
            if ((chg->offset + chg->length) < (wmd->offset + wmd->length)) {
        ...
        if ((wmd->offset + wmd->length) >= chg->offset) {
            if ((wmd->offset + wmd->length) < (chg->offset + chg->length)) {
                *add_length = chg->offset - wmd->offset;
                sub_offset = chg->offset - wmd->offset;
            }
            else {
                *add_length = (wmd->offset+wmd->length)-(chg->offset+chg->length)
    ...
    if (!ret && ((chg->length + *add_length) <=
        ...
        if (vcptr->tc_optimize_performance & PERF_OPT_DEBUG_COALESCED_CHANGES) {
	 ...
	 inm_s32_t data_source, struct inm_list_head *split_chg_list_hd, inm_wdata_t *wdatap) 
{
    ...
    if (data_source == NODE_SRC_DATA) {
             ...
             * TODO: Create a tunable parameter interface for the max value 
             * in data mode
             */
    ...
    while(remaining_length) {
		...
		wmd_local.length = min(max_data_sz_per_chg_node, remaining_length);
		...
		/* copy the time stamp related info from the first change node as it should
		 * be the same for all split ones.
		 */
		if (!nr_splits) {
             ...
             write_metadata_t *wmd, inm_s32_t data_source, inm_wdata_t *wdatap)
{
	...
	queue_worker_routine_for_set_volume_out_of_sync(vcptr, ERROR_TO_REG_IO_SIZE_64MB_METADATA, 0);
	return 0;
#endif
		    ...
		    if (vcptr->tc_cur_node->type == NODE_SRC_DATA &&
                ...
                if (vcptr->tc_optimize_performance & PERF_OPT_DEBUG_DATA_DRAIN) {
                    info("Appending chg:%p to tgt_ctxt:%p next:%p prev:%p mode:%d",
                        vcptr->tc_cur_node,vcptr, vcptr->tc_cur_node->nwo_dmode_next.next,
                        vcptr->tc_cur_node->nwo_dmode_next.prev, vcptr->tc_cur_node->type);
                }
		    }
        }
    	...
    	if (nr_splits) {
    		inm_list_splice_at_tail(&split_chg_list_hd, &vcptr->tc_node_head);
		    ...
		    add_changes_to_pending_changes(vcptr, vcptr->tc_cur_wostate, nr_splits);
    	}
		return nr_splits;
    }
    ...
    if (chg_node &&
        ...
        avail_space = max_data_sz_per_chg_node - chg_node->changes.bytes_changes;
    ...
    if ((vcptr->tc_optimize_performance & PERF_OPT_METADATA_COALESCE) &&
        !coalesce_metadata_change(vcptr, wmd, data_source, chg_node, &add_length)) {
        ...
        INM_BUG_ON(vcptr->tc_pending_changes < 0 );
        vcptr->tc_bytes_pending_changes += wmd->length;
        add_changes_to_pending_changes(vcptr, chg_node->wostate, nr_splits);
    }

    return nr_splits;
}
/**
 * FUNCTION NAME: save_data_in_metadata_mode
 * 
 * DESCRIPTION : This function saves (disk)change information only
 * 
 * INPUT PARAMETERS :
 *    tgt_ctxt - target context 
 *    bio_info - block io information
 *    bio
 *
 * 
 * OUTPUT PARAMETERS :
 * NOTES : 1. get the target's current change node, and add the disk change info
 *         2. wakeup service thread if it meets hw mark threshold
 *         3. wakeup s2 if it is waiting for dirty block
 * 
 * return value :
 *    0      - for success
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_META))){
    info("entered tcPages:%d dc_cur_res_pages:%d ",
		tgt_ctxt->tc_stats.num_pages_allocated, driver_ctx->dc_cur_res_pages);
}
#define vcptr    tgt_ctxt
    ...
    if (add_metadata(tgt_ctxt, change_node, wmd, NODE_SRC_METADATA, wdatap) < 0) {
        err("Memory pool : out of memory %s\n", tgt_ctxt->tc_guid);
        /** TODOs:
         * set error value to "out of memory for dirty blocks" and maintain
         * this information in target context level using counters.
         * call free change node list() --> queue worker routine
         * if error value enabled then
         *    invoke queue_worker_routine_to_set_target_out_of_sync.
         *
         *
         **/
        ...
        /* previous notes */
        // We can not write to registry from DispatchWrite. We are see the file system
        // flushing the registry holding an exclusive lock. If we write to registry that
        // needs exclusive lock and re-acquiring the lock is causing a dead lock.
        queue_worker_routine_for_set_volume_out_of_sync(vcptr,
							ERROR_TO_REG_OUT_OF_MEMORY_FOR_DIRTY_BLOCKS,
							-ENOMEM);
        /* TODOs: WINDOWS CODE: InVolFltIoCompletion for cluster volumes */
    ...
    if ((driver_ctx->tunable_params.db_high_water_marks[driver_ctx->service_state]) &&
		...
		if (!(vcptr->tc_flags & VCF_VOLUME_STACKED_PARTIALLY) && !vcptr->tc_bp->bmap_busy_wait) {
                                     ...
                                     int commit_pending, tag_history_t *hist)
{
    ...
    if (INM_ATOMIC_READ(&driver_ctx->is_iobarrier_on)) {
        memcpy_s(&chg_node->changes.start_ts, sizeof(TIME_STAMP_TAG_V2),
                        &driver_ctx->dc_crash_tag_timestamps, sizeof(TIME_STAMP_TAG_V2));
    ...
    while(tag_idx < num_tags) {
        ...
        FILL_STREAM_HEADER((pg + idx), STREAM_REC_TYPE_USER_DEFINED_TAG,
			   (hdrlen + padtaglen));
        idx += hdrlen;
