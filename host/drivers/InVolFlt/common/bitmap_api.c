...
bitmap_api_t *bitmap_api_ctr()
{
    ...
    bapi = (bitmap_api_t *)INM_KMALLOC(sizeof(bitmap_api_t), INM_KM_SLEEP, INM_KERNEL_HEAP);
    if (!bapi)
        return NULL;
    ...
    bapi->err_causing_outofsync = 0; // o is success, non-zero is failure
...
/**
 * FUNCTION NAME: bitmap_api_open
 *
 * DESCRIPTION :
 *
 * INPUT PARAMETERS : i
 *        bapi - ptr to bitmap api structure
 *        bitmap_filename -
 *        granularity
 *        offset
 *        volume_size
 ...
 * NOTES
 In Windows:
 // If bitmap file is created for the first time, initial_bitmap_filesize
 // is used to allocate for the file
 *
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
    ...
    if (!bapi || !detailed_status || !bitmap_filename || !granularity || !volume_size)
        return 1; //err
    ...
    *detailed_status = 0;
    ...
    /*64 bit divition issue ===>
     * bapi->nr_bits_in_bitmap = 
     *(inm_u32_t) ((volume_size / (inm_u64_t)granularity) + 1);
     **/
    ...
    /* TODOs: BETA2 FIX. MOVE THIS TO DOWN */
    if ((driver_ctx->dc_bmap_info.current_bitmap_buffer_memory + max_bitmap_buffer_required) >
    ...
    driver_ctx->dc_bmap_info.current_bitmap_buffer_memory += max_bitmap_buffer_required;
    ...
    *detailed_status = _dstatus;
    ...
    if (ret && (bapi->bitmap_filename[0] == '/')) {
        if (is_rootfs_ro()) {
            ...
            info("root file system is full or missing directory hierarchy : "
                 "so can't open/create bitmap files.\n");
            goto cleanup_and_return_failure;
        }
    }
...
void bitmap_api_upgrade_header(bitmap_api_t *bapi)
{
    ...
    while (!error && 
        ...
        case BITMAP_FILE_VERSION1:
            ...
            bapi->bitmap_header.un.header.version = BITMAP_FILE_VERSION2;
            bapi->bitmap_header.un.header.header_size = BITMAP_HDR2_SIZE;
            break;

        default:
            /* This should never happen as header is verified before upgrade */
    ...
    if (!error) {
        ...
        if (bitmap_api_verify_header(bapi, &bapi->bitmap_header)) {
            error = bitmap_api_commit_header(bapi, FALSE, &upgrade_error);
            if (error) {
                err("Cannot persist bitmap header - %x", upgrade_error);
            } else {
                info("Successfully upgraded bitmap to version 0x%x", 
                     bapi->bitmap_header.un.header.version);
            }
        } else {
            err("Bitmap upgraded header verification failed");
        }
    }
}
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
    ...
    *detailed_status = 0;
    ...
    /* if iobuffer_ctr returns null ???? , return appropriate error*/
    if (!iob) {
        info("iob is null");
        return -ENOMEM;
    }
    ...
    if (!was_created)
    {
        ...
        /* TODOs: handle end of file case : for empty files */
        ...
        if (bapi->bitmap_header.un.header.version != BITMAP_FILE_VERSION)
...
cleanup_and_return_failure:
    ...
    if (bapi->io_bitmap_header != NULL) {
        err("err in loading bitmap header");
...
inm_s32_t bitmap_api_is_volume_insync(bitmap_api_t *bapi, inm_u8_t *volume_insync,
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
	...
	ret = bitmap_api_commit_bitmap_internal(bapi, clean_shutdown, close_status);    
	break;
    default:
	ret = EINVAL;
	break;
    ...
    //driver_context->current_bitmap_buffer_memory -= min(bapi->segment_cache_limit * BITMAP_FILE_SEGMENT_SIZE, bapi->bitmap_size_in_bytes);
    driver_ctx->dc_bmap_info.current_bitmap_buffer_memory -= min(bapi->segment_cache_limit * BITMAP_FILE_SEGMENT_SIZE, bapi->bitmap_size_in_bytes);
...
//void bitmap_api_getscaled_offsetandsize_from_diskchange(bitmap_api_t *bapi, disk_change_t *dc, inm_u64_t *scaled_offset, inm_u64_t *scaled_size)
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
    ...
    //*scaled_offset=
    //  (dc->offset/bapi->bitmap_granularity) * bapi->bitmap_granularity; 
    ...
    *scaled_size += ((inm_u64_t)dc->length) + (bapi->bitmap_granularity - 1);

    // *scaled_size /= bapi->bitmap_granularity;
    // 3rd, now scale it to the granularity
    INM_DO_DIV(*scaled_size, bapi->bitmap_granularity);

    // *scaled_offset /= bapi->bitmap_granularity;
...
void bitmap_api_getscaled_offsetandsize_from_writemetadata(bitmap_api_t *bapi,
							   write_metadata_t *wmd, inm_u64_t *scaled_offset, inm_u64_t *scaled_size)
{
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
    ...
    //*scaled_offset=
    // (wmd->offset/bapi->bitmap_granularity) * bapi->bitmap_granularity;
    // round down offset
    INM_DO_DIV(_gran_scaled_offset, bapi->bitmap_granularity);
...
//inm_s32_t bitmap_api_set_writesize_not_to_exceed_volumesize(bitmap_api_t *bapi, disk_change_t *dc)
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
    ...
    if ((dc->length + dc->offset) > bapi->volume_size) {
        if (dc->offset < bapi->volume_size) {
            dc->length = (inm_u32_t)(bapi->volume_size - dc->offset);
            ret = 0;
        } else {
            //ret = E2BIG; //STATUS_ARRAY_BOUNDS_EXCEEDED
            ret = EOF_BMAP; //STATUS_ARRAY_BOUNDS_EXCEEDED
        }
    }
	    ...
	    for(current_run = 0; current_run < nbr_runs; current_run++) {
            ...
            if (!vbmap->bitmap_skip_writes) {
    		    ...
    		    bitmap_api_getscaled_offsetandsize_from_diskchange(bapi,
		    		    			       ...
		    		    			       &scaled_offset, &scaled_size);
		/*info("offset = %llu, length = %d\t scaled_offset = %llu, scaled size = %llu",
...
out_err:
	...
	__inm_list_for_each(ptr, &bruns->meta_page_list){
		...
		while (scaled_size > 0 &&
		   lh->last_chance_changes < (MAX_WRITE_GROUPS_IN_BITMAP_HEADER * MAX_CHANGES_IN_WRITE_GROUP))
		{
        ...
        /*TODOs: ret = 0 */
        bruns->final_status = 0; // success
        break;

    default:
        ret = -EBUSY; //STATUS_DEVICE_NOT_READY
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
	...
	if (ret == 0) {// success
		...
		if (ret == 0) //success
		    ret =
                        bitmap_api_set_writesize_not_to_exceed_volumesize(bapi,
									  &bruns->runs[current_run]);                    
	...
	if (ret == 0 && current_run == MAX_KDIRTY_CHANGES) {
	    ret = EAGAIN; //STATUS_MORE_PROCESSING_REQUIRED
	}
	//else if (ret == E2BIG) //STATUS_ARRAY_BOUNDS_EXCEEDED
	else if (ret == EOF_BMAP) //STATUS_ARRAY_BOUNDS_EXCEEDED
	{
	    ret = 0; //success
	}                
	break;
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
	...
	while(ret == 0 && current_run < MAX_KDIRTY_CHANGES)
	{
	    ret = segmented_bitmap_get_next_bitrun(bapi->sb, (inm_u32_t *)&bruns->runs[current_run].length, &bruns->runs[current_run].offset);
	    bruns->runs[current_run].length *= bapi->bitmap_granularity;
	    bruns->runs[current_run].offset *= bapi->bitmap_granularity;
                
	    if (ret == 0) //success
		ret = bitmap_api_set_writesize_not_to_exceed_volumesize(bapi, &bruns->runs[current_run]);                    
		    ...
		    (bruns->runs[current_run-1].length < 0x1000000))
		{
		    ...
		    bruns->runs[current_run-1].length += bruns->runs[current_run].length;
		}
		else
		{
		    bruns->nbr_runs_processed++;
...
inm_s32_t move_rawio_changes_to_bitmap(bitmap_api_t *bapi, inm_s32_t *inmage_open_status)
{
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
    ...
    //            + bapi->bitmap_granularity - 1) / bapi->bitmap_granularity;
    ...
    for (i = 0; i < max_nr_lcw; i++) {
        ...
        if (write_offset < bapi->volume_size) {
            ...
            if ((write_offset + write_size) > rounded_volume_size) {
                /* TODO: This should never happen. Mark for resync? */
                INM_BUG_ON(1);
            ...
            status = segmented_bitmap_set_bitrun(bapi->sb,
						 (inm_u32_t) scaled_size, scaled_offset);
            if (status) { // not success (status != 0)
...
/*
  inm_s32_t bitmap_api_verify_bitmap_header(bitmap_api_t *bapi, bitmap_header_t *bh) {
  ...
  md5_init(&ctx);
  md5_update(&ctx, (unsigned char *)(&bh->un.header.endian), HEADER_CHECKSUM_DATA_SIZE);
  md5_final(actual_checksum, &ctx);
    

  #define bmap_hdr    bh->un.header
  ...
  (memcmp(bmap_hdr.validation_checksum, actual_checksum, HEADER_CHECKSUM_SIZE));
...
inm_s32_t bitmap_api_init_bitmap_file(bitmap_api_t *bapi, inm_s32_t *inmage_status) {
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
    ...
    default:
        status = 1; //STATUS DEVICE NOT READY
        break;
    }
...
inm_s32_t
bitmap_api_verify_header_blocks(bitmap_api_t *bapi, bitmap_header_t *hdr)
{
    int i = 0;
    inm_u64_t sig = 0;
    char *vname = NULL;
    ...
    vname = bapi->volume_name + 
            (strlen(bapi->volume_name) - BITMAP_LCW_SIGNATURE_PREFIX_SZ);
    ...
    for (i = 0; i < MAX_WRITE_GROUPS_IN_BITMAP_HEADER; i++) {
        if (hdr->change_groups[i].un.length_offset_pair[0] != sig) {
            err("Signature Mismatch. CG[%d] %llx != %llx", i, 
                hdr->change_groups[i].un.length_offset_pair[0], sig);
            matches = FALSE;
            break;
        }
    }
...
void
bitmap_api_clear_signed_header_blocks(bitmap_api_t *bapi)
{
    int i = 0;
...
void
bitmap_api_sign_header_blocks(bitmap_api_t *bapi)
{
    int i = 0;
    inm_u64_t sig = 0;
    char *vname = NULL;
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
    ...
    case BITMAP_FILE_STATE_OPENED:
        bitmap_api_calculate_hdr_integrity_checksums(&bapi->bitmap_header);
        if (memcpy_s(bapi->io_bitmap_header->buffer, sizeof(bitmap_header_t),
				&bapi->bitmap_header, sizeof(bitmap_header_t))) {
		status = 1;
		break;
	}
    ...
    default:
        status = 1; //TODOs: STATUS_DEVICE_NOT_READY
        break;
    }
...
inm_s32_t bitmap_api_verify_header(bitmap_api_t *bapi, bitmap_header_t *bheader) {
    unsigned char actual_checksum[HEADER_CHECKSUM_SIZE] = {0};
    MD5Context ctx;
    inm_s32_t _rc = 0;
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
    ...
    if (bbsp) {
        ...
        bbsp->bbs_max_nr_bits_in_chg = (1024 * 1024)/bapi->bitmap_granularity;
    }
    ...
    if (bapi->sb) {
        data_bytes = segmented_bitmap_get_number_of_bits_set(bapi->sb, bbsp);
...
int
bitmap_api_open_bitmap_stream(bitmap_api_t *bapi, target_context_t *vcptr, inm_s32_t *detailed_status)
{
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
	...
	//recorded changes might have last here, set out of sync here
        *detailed_status = bapi->err_causing_outofsync =
	    LINVOLFLT_ERR_BITMAP_FILE_CREATED;
    ...
    if (ret) //error : this condition never occur
        goto last;
...
/* checks whether bitmap header loaded or not */
inm_s32_t is_bmaphdr_loaded(volume_bitmap_t *vbmap)
{
    if (vbmap && vbmap->bitmap_api && vbmap->bitmap_api->io_bitmap_header) {
	return TRUE;
    }
...
bitmap_api_switch_to_rawio_mode(bitmap_api_t *bapi, inm_u64_t *resync_error)
{
    ...
    error = bitmap_api_commit_bitmap_internal(bapi, !clean_shutdown, &error);
