...
#define MAX_KDIRTY_CHANGES                     (MAX_CHANGE_INFOS_PER_PAGE) /* temporary fix */
...
/**
 * change node data structure
 *   +---------------------------+
 *   | node_type | transaction # |
 *   +---------------------------+
 *   | data_page_head ptr        |
 *   +---------------------------+
 *   | mapped thread tast struct |
 *   +---------------------------+
 *   | disk change head          |
 *   +---------------------------+
 *   | next ptr                  |
...
struct _change_node
{
    ...
    inm_sem_t mutex; // This mutex protects the data pages shared
    ...
    disk_chg_head_t changes; // TODO: Make it as a pointer.
...
change_node_t *get_oldest_change_node(struct _target_context *, inm_s32_t *status);
change_node_t *get_oldest_datamode_change_node(struct _target_context *);
change_node_t *get_change_node_to_update(struct _target_context *, struct inm_writedata *,
							inm_tsdelta_t *);
void cleanup_change_nodes(struct inm_list_head *, etTagStateTriggerReason);
void free_changenode_list(struct _target_context *ctxt, etTagStateTriggerReason);
...
inm_s32_t queue_worker_routine_for_change_node_cleanup(change_node_t *change_node);
...
change_node_t *get_change_node_for_usertag(struct _target_context *, struct inm_writedata *, int commit_pending);
...
inm_s32_t fill_udirty_block(struct _target_context *ctxt, UDIRTY_BLOCK_V2 *udirty,
                      inm_devhandle_t *filp);
inm_s32_t perform_commit(struct _target_context *ctxt, COMMIT_TRANSACTION *commit,
                    inm_devhandle_t *filp);
...
void do_perf_changes(struct _target_context *tgt_ctxt, change_node_t *recent_cnode, int path);
