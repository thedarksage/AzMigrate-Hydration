...
/* Convert system time wrt windows epoch 1601 */
...
__telemetry_rec_alloc(inm_u64_t event_id) 
{
    ...
    INM_SPIN_LOCK(&tel_slock);
    if (tel_nrecs < 
        (driver_ctx->total_prot_volumes * TELEMETRY_REC_NR_MAX_PER_VOL)) {
        tel_nrecs++;
        alloc = 1;
        if (mem_throttled) {
    ...
    if (alloc) {
        tel_buf = (void *)__INM_GET_FREE_PAGE(INM_KM_SLEEP | INM_KM_NOIO, 
                                      INM_KERNEL_HEAP);
        if (!tel_buf) {
...
telemetry_refresh_file(void **phdl)
{
    ...
    tel_file_seq++;
    sprintf_s(tel_fname, INM_PATH_MAX, "%s%llu%s", TELEMETRY_FILE_NAME_PREFIX,
...
telemetry_flush_data(inm_list_head_t *tel_data)
{
    ...
    if (error) {
        ...
        rec = inm_list_entry(inm_list_first(tel_data), tel_rec_t, tr_head);
        start_seqno = rec->tr_seq;

        rec = inm_list_entry(inm_list_last(tel_data), tel_rec_t, tr_head);
...
telemetry_log_start(inm_list_head_t *rec_list)
{
    ...
    TELEMETRY_LOG_ULL(rec_list, "EventRecId", TELEMETRY_REC_CUR_SEQ(rec_list));
                          ...
                          etMessageType msg)
{
    ...
    TELEMETRY_LOG_UINT(recs, "NumOfProtectdDisk", tag_common->tc_ndisks_prot);
    TELEMETRY_LOG_UINT(recs, "NumOfTaggedDisk", tag_common->tc_ndisks_tagged);
    TELEMETRY_LOG_INT(recs, "IoctlCode", tag_common->tc_ioctl_cmd);
    TELEMETRY_LOG_INT(recs, "TagStatus", tag_hist->th_tag_status);
    TELEMETRY_LOG_INT(recs, "GlobalIoctlStatus", tag_common->tc_ioctl_status);
    TELEMETRY_LOG_ULL(recs, "PrevTagEndTS", ctxt->tc_tel.tt_prev_tag_ts);
    TELEMETRY_LOG_ULL(recs, "PrevTagEndSeq", ctxt->tc_tel.tt_prev_tag_seqno);
    TELEMETRY_LOG_ULL(recs, "PrevCommittedTS", ctxt->tc_tel.tt_prev_ts);
    TELEMETRY_LOG_ULL(recs, "PrevCommittedSeq", ctxt->tc_tel.tt_prev_seqno);
    ...
    TELEMETRY_LOG_ULL(recs, "DrainDataPrevInBytes", stats->ts_drained_bytes);
    ...
    TELEMETRY_LOG_WTIME(recs, "LastNPLimitHitTime", nwo->nws_np_limit_time);
    TELEMETRY_LOG_UINT(recs, "NonPagePoolAllocFail", nwo->nws_np_alloc_fail);
	...
	/* Generic info */
    TELEMETRY_LOG_UINT(recs, "LastResyncError", ctxt->tc_hist.ths_osync_err);
    TELEMETRY_LOG_WTIME(recs, "LastResyncTime",ctxt->tc_hist.ths_osync_ts *
                                               HUNDREDS_OF_NANOSEC_IN_SECOND);
    TELEMETRY_LOG_WTIME(recs, "ResyncStartTime", ctxt->tc_tel.tt_resync_start);
    TELEMETRY_LOG_WTIME(recs, "ResyncEndTime", ctxt->tc_tel.tt_resync_end);
    TELEMETRY_LOG_WTIME(recs, "ClearDiffTime", ctxt->tc_hist.ths_clrdiff_ts *
                        HUNDREDS_OF_NANOSEC_IN_SECOND);
    TELEMETRY_LOG_WTIME(recs, "StartFilterKrnlTime", 
                        ctxt->tc_hist.ths_start_flt_ts *
                        HUNDREDS_OF_NANOSEC_IN_SECOND);
    TELEMETRY_LOG_WTIME(recs, "DevCntxtCreateTime", ctxt->tc_tel.tt_create_time);
    TELEMETRY_LOG_WTIME(recs, "LastDrainDbTime", ctxt->tc_tel.tt_getdb_time);
    TELEMETRY_LOG_WTIME(recs, "LastCommitDbTime", ctxt->tc_tel.tt_commitdb_time);
    TELEMETRY_LOG_WTIME(recs, "DrvLoadTime", driver_ctx->dc_tel.dt_drv_load_time);
    ...
    TELEMETRY_LOG_WTIME(recs, "LastS2Stop", driver_ctx->dc_tel.dt_s2_stop_time);
    ...
    TELEMETRY_LOG_UINT(recs, "MessageType", msg | TELEMETRY_LINUX_MSG_TYPE);
                          ...
                          etMessageType msg)
{
	...
	/* Tag Data */
    state = telemetry_get_dbs(ctxt, ecTagStatusInsertFailure, ecNotApplicable);
    ...
    TELEMETRY_LOG_STR(recs, "DiskName", ctxt->tc_guid);
    TELEMETRY_LOG_ULL(recs, "DiskBlendedState", state);
    TELEMETRY_LOG_ULL(recs, "TagState", (inm_u64_t)ecTagStatusInsertFailure);
							...
							inm_s32_t tag_error, etMessageType msg)
{
    ...
    state = telemetry_get_dbs(NULL, ecTagStatusIOCTLFailure, ecNotApplicable);
    ...
    TELEMETRY_LOG_WTIME(recs, "TimeJumpedTS", 
                                driver_ctx->dc_tel.dt_time_jump_cur);
    TELEMETRY_LOG_ULL(recs, "DiskBlendedState", state);
    TELEMETRY_LOG_ULL(recs, "TagState", (inm_u64_t)ecTagStatusIOCTLFailure);
    TELEMETRY_LOG_UINT(recs, "MessageType", msg | TELEMETRY_LINUX_MSG_TYPE);
