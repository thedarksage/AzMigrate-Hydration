...
/* TODOs: This structure is used to collect driver context statistics */
...
/* TODOs: This structure is used to maintain the driver parameters */
...
typedef struct __kernel_thread_t {
    ...
    /* TODOs: Need to understand the scope of these items in current implementation */
...
typedef struct _vm_cx_session {
    inm_u64_t    vcs_flags; /* Flags */
    inm_u64_t    vcs_transaction_id; /* Transaction ID */
    inm_u64_t    vcs_num_disk_cx_sess; /* Number of disk CX sessions */
    inm_u64_t    vcs_start_ts; /* CX session start time */
    inm_u64_t    vcs_end_ts; /* CX session end time */
    inm_u64_t    vcs_base_secs_ts; /* This is the base time to calculate 1s 
                                      intervals */
    inm_u64_t    vcs_tracked_bytes; /* Bytes tracked */
    inm_u64_t    vcs_drained_bytes; /* Bytes drained */
    inm_u64_t    vcs_tracked_bytes_per_second; /* Bytes tracked every second */
    inm_u64_t    vcs_churn_buckets[DEFAULT_NR_CHURN_BUCKETS]; /* Churn
                                                                 buckets */
    inm_u64_t    vcs_default_disk_peak_churn; /* Disk level supported peak
                                                 churn */
    inm_u64_t    vcs_default_vm_peak_churn; /* VM level supported peak churn */
    inm_u64_t    vcs_max_peak_churn; /* Max peak churn */
    inm_u64_t    vcs_first_peak_churn_ts; /* Time of first peak churn */
    inm_u64_t    vcs_last_peak_churn_ts; /* Time of first peak churn */
    inm_u64_t    vcs_excess_churn; /* Excess churn on top of peak churn */
    inm_u64_t    vcs_num_consecutive_tag_failures; /* Number of consecutive tag
                                                      failres observed */
    inm_u64_t    vcs_timejump_ts; /* Time Jump timestamp */
    inm_u64_t    vcs_max_jump_ms; /* Time Jump in msec */
    inm_u64_t    vcs_max_s2_latency; /* S2 latency */
    inm_u32_t    vcs_nth_cx_session; /* CS session number */
} vm_cx_session_t;
...
typedef struct _driver_context {
    ...
    inm_u32_t flags; /* to keep track of allocation of buffers */
    ...
    inm_rwsem_t dc_inmaops_sem;		/* lock to protect a_ops list */
    struct inm_list_head dc_inma_ops_list;	/* list of duplicated address space
					 * operations */
    ...
    inm_u32_t         dc_cp;                /* Consistency Point State      */
    char              dc_cp_guid[GUID_LEN]; /* App/Crash consistency guid   */
    inm_sem_t         dc_cp_mutex;          /* To sync App/Crash consstency */
    workq_t           dc_tqueue;
...
/*void service_shutdown_completion(void);*/
