...
/* TODO: Determine data mode filtering is enabled or not from config file
 * or could be passed as an argument to driver load.
 */
...
inm_s32_t init_driver_context(void)
{
    ...
    if (!driver_ctx->default_data_pool_size_mb) {
        r = -ENOMEM;	
        err("Memory is not enough for driver initialization err = %d", r);
        goto free_dc;
    }
...
#ifndef INM_AIX
    driver_ctx->dc_host_info.bio_info_cache = INM_KMEM_CACHE_CREATE("global_bio_info_cache",
					      INM_BIOSZ, 0, 0, NULL, NULL, INM_MAX_NR_GLOBAL_BUF_INFO_POOL,
	...
	INM_DESTROY_SPIN_LOCK(&driver_ctx->tunables_lock);
        r = INM_ENOMEM;	
        err("INM_KMEM_CACHE_CREATE failed to create bio cache err = %d", r);
        goto free_dc;
    }
#endif
    driver_ctx->dc_host_info.mirror_bioinfo_cache = INM_KMEM_CACHE_CREATE("global_mirror_bioinfo_cache",
					      INM_MIRROR_BIOSZ, 0, 0, NULL, NULL, INM_MAX_NR_GLOBAL_MIRROR_BUF_INFO_POOL,
					      INM_MIN_NR_GLOBAL_MIRROR_BUF_INFO_POOL, INM_PINNED);
...
#endif
        r = INM_ENOMEM;
        err("INM_KMEM_CACHE_CREATE failed to create mirror bio cache err = %d", r);
        goto free_dc;
    }

#ifdef INM_AIX
    driver_ctx->dc_host_info.data_file_node_cache = INM_KMEM_CACHE_CREATE("data_file_node_cache",
					    sizeof(data_file_node_t), 0, 0, NULL, NULL,
					    INM_MAX_NR_DATA_FILE_NODE_POOL, INM_MIN_NR_DATA_FILE_NODE_POOL, INM_PINNED);
    if(!driver_ctx->dc_host_info.data_file_node_cache){
	INM_DESTROY_SPIN_LOCK(&driver_ctx->tunables_lock);
	r = -ENOMEM;
	err("INM_KMEM_CACHE_CREATE failed to create data_file_node_cache err = %d", r);
	goto free_bio_cache;
    }
#endif
    ...
    /* TODO: Read driver config info here
     * if data mode filtering enabled, perform data mode init. 
     */
...
/*   print_driver_context(driver_ctx); */
    ...
    /* Per IO stamp file */
    snprintf(driver_ctx->driver_time_stamp, INM_PATH_MAX , "%s/%s/GlobalTimeStamp",
        PERSISTENT_DIR, COMMON_ATTR_NAME);
    
    /* Per IO stamp seqno file */
    snprintf(driver_ctx->driver_time_stamp_seqno, INM_PATH_MAX , "%s/%s/SequenceNumber",
        PERSISTENT_DIR, COMMON_ATTR_NAME);
    ...
    telemetry_set_dbs(&driver_ctx->dc_tel.dt_blend, DBS_DRIVER_NOREBOOT_MODE);
...
void free_driver_context(void)
{
	...
	tag_guid_t *tag_guid = inm_list_entry(driver_ctx->tag_guid_list.next, tag_guid_t, tag_list);
 ...
 *             EINVAL - for invalid inputs
 <non-zero value> - for failure
 * 
 * The caller should take the driver_ctx->tgt_list_sem write semaphore
 **/
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
   ...
   // tc->tc_hist.ths_start_flt_ts = CURRENT_TIME.tv_sec;
...
inm_s32_t alloc_cache_pools()
{
    ...
    while (i < threshold) {
        pg = (inm_page_t *) INM_KMALLOC(sizeof(inm_page_t), INM_KM_SLEEP, INM_KERNEL_HEAP);
        if (!pg) {
	        err = -ENOMEM;
	        goto next;
        }
	...
	if(INM_PIN(pg, sizeof(inm_page_t))){
        ...
        pg->cur_pg = (unsigned long *)__INM_GET_FREE_PAGE(INM_KM_SLEEP, INM_KERNEL_HEAP);
        if (!pg->cur_pg) {
            err = -ENOMEM;
    ...
    driver_ctx->wq_entry_pool = INM_KMEM_CACHE_CREATE("WQ_ENTRY_POOL",
			 	sizeof(wqentry_t), 0, INM_SLAB_HWCACHE_ALIGN, NULL, NULL,
				INM_MAX_NR_WQ_ENTRY_POOL, INM_MIN_NR_WQ_ENTRY_POOL, INM_PINNED);
...
{
    dbg("svagents exiting pid = %d", driver_ctx->svagent_pid);
    info("svagents exiting pid = %d", driver_ctx->svagent_pid);
...
inm_s2_exit(void)
{
	...
	info("s2 exiting pid = %d", driver_ctx->sentinal_pid);
...
/* cleanup the pending confirm/uncommitted dblk stuff */
...
retry:
 ...
 inm_list_for_each_safe(curp, nxtp, &driver_ctx->tgt_list){ 
     ...
     if (chg_node) {
          ...
          * s2 gets killed with currently mapped matadata change node
          * and in the mean time, data mode change node is available before
          * s2 comes up and again calls getdb, we would serve data mode 
         ...
         if (tgt_ctxt->tc_optimize_performance &
            ...
            (chg_node->flags & CHANGE_NODE_IN_NWO_CLOSED)) {
             ...
             if (tgt_ctxt->tc_optimize_performance & PERF_OPT_DEBUG_DATA_DRAIN) {
                 info("s2_exit: Reset metadata closed state chg_node:%p mode:%d"
     ...
     if (chg_node) {
          if (chg_node->flags & CHANGE_NODE_ORPHANED) { //orphan node

                if (chg_node->type == NODE_SRC_TAGS) 
                    telemetry_log_tag_history(chg_node, tgt_ctxt, 
                                              ecTagStatusDropped, 
