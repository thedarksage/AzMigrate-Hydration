 ...
 * File       : ioctl.c
 *
 * Description: Ioctl implementations of invoflt character mode driver.
 */
...
static inm_u32_t inm_wait_exception_ev(target_context_t *, inm_resync_notify_info_t *);
static inm_s32_t inm_fill_resync_notify_info(target_context_t *tgt_ctxt,  struct _inm_resync_notify_info *resync_info);
static void print_AT_stat_common(target_context_t *tcp, char *page, inm_s32_t *len);

inm_s32_t stop_filtering_volume(char *uuid, inm_devhandle_t *idhp, int dbs_flag); 

static inm_u32_t
inm_wait_exception_ev(target_context_t *tgt_ctxt, inm_resync_notify_info_t *resync_info)
{
    inm_u32_t ret = 0;
...
static inm_s32_t
inm_fill_resync_notify_info(target_context_t *tgt_ctxt, inm_resync_notify_info_t *resync_info)
{
    inm_s32_t ret = 0;
    ...
    dbg("tcp TS %llu resy TS %llu tc_out_of_sync_err_code:%lu",tgt_ctxt->tc_out_of_sync_time_stamp,
        resync_info->rsin_out_of_sync_time_stamp, tgt_ctxt->tc_out_of_sync_err_code);
...
inm_calc_len_required( struct inm_list_head *ptr)
{
	...
	for (; ptr != &(driver_ctx->tgt_list); ptr = ptr->next,
		...
		if(tgt_ctxt->tc_flags & (VCF_VOLUME_CREATING | VCF_VOLUME_DELETING)){
			tgt_ctxt = NULL;
			continue;
		}
        /* return current device name instead of persistent names */
...
/**
 * FUNCTION NAME:process_start_notify_ioctl
 * 
 * DESCRIPTION :
 * 
 * INPUT PARAMETERS : idhp
 *               arg
 *              
 *              
 * 
 ...
 * 
 **/
inm_s32_t process_start_notify_ioctl(inm_devhandle_t *idhp, void __INM_USER *arg)
{
...
if(IS_DBG_ENABLED(inm_verbosity, INM_IDEBUG)){
    ...
    if(!INM_ACCESS_OK(VERIFY_READ, (void __INM_USER *)arg,
        ...
        err("Read access violation for PROCESS_START_NOTIFY_INPUT");
        return -EFAULT;
	}

    /* Fail the ioctl, if start notify ioctl comes from more than one instance
     * of s2 at any time.
     */
    if(driver_ctx->sentinal_idhp || driver_ctx->sentinal_pid) {
        err("Start notify failed, s2 instance with pid : %d already running\n",
            driver_ctx->sentinal_pid);
        return -EINVAL;
	}

    /** TODOs :
     * copy user data into systembuffer
     **/
    driver_ctx->sentinal_pid = INM_CURPROC_PID;
    driver_ctx->sentinal_idhp = idhp;

    info("s2 has started = %d , process name = %s\n",
        INM_CURPROC_PID, INM_CURPROC_COMM);
    get_time_stamp(&(driver_ctx->dc_tel.dt_s2_start_time));
    telemetry_clear_dbs(&driver_ctx->dc_tel.dt_blend, DBS_S2_STOPPED);
    
    /** 
     * TODOs: 
     * check for flag SHUTDOWN_NOTIFY_FLAGS_ENABLE_DATA_FILTERING
     * and enable service_supports_data_filtering = TRUE;
     * check for flag SHUTDOWN_NOTIFY_FLAGS_ENABLE_DATA_FILES
     * and enable service_supports_data_files
     **/
...
/**
 * FUNCTION NAME:process_shutdown_notify_ioctl
 * 
 * DESCRIPTION :
 * 
 * INPUT PARAMETERS : idhp
 ...
 * 
 **/
inm_s32_t process_shutdown_notify_ioctl(inm_devhandle_t  *idhp, void __INM_USER *arg)
{
...
/**
 * FUNCTION NAME:process_volume_stacking_ioctl
 * 
 * DESCRIPTION :
 * 
 * INPUT PARAMETERS : 
 ...
 * 
 **/
inm_s32_t process_volume_stacking_ioctl(inm_devhandle_t *idhp, void __INM_USER *arg)
{
...
if(IS_DBG_ENABLED(inm_verbosity, INM_IDEBUG)){
    ...
    telemetry_clear_dbs(&driver_ctx->dc_tel.dt_blend, DBS_DRIVER_NOREBOOT_MODE);

    if(!INM_ACCESS_OK(VERIFY_READ, (void __INM_USER *)arg, sizeof(inm_dev_info_t))) {
        err("Read access violation for inm_dev_info_t");
		err = INM_EFAULT;
        goto out;
    }

    dev_infop = (inm_dev_extinfo_t *)INM_KMALLOC(sizeof(inm_dev_extinfo_t), INM_KM_SLEEP, INM_KERNEL_HEAP);
    if (!dev_infop) {
    ...
    if(INM_COPYIN((inm_dev_info_t *)dev_infop, arg, sizeof(inm_dev_info_t))) {
    ...
    if (driver_state & DRV_LOADED_FULLY) {
        if (is_flt_disabled(dev_infop->d_pname)) {
            info("Filtering not enabled for %s", dev_infop->d_pname);
            err = -EINVAL;
        }
        ...
        if (dtype != FILTER_DEV_HOST_VOLUME &&
            dtype != dev_infop->d_type) {
                err("Invalid dev type %d for %s", dtype, dev_infop->d_pname);
            err = -EINVAL;
        }
    }
    }
...
/**
 * FUNCTION NAME:process_start_filtering_ioctl
 * 
 * DESCRIPTION :
 * 
 * INPUT PARAMETERS : 
 ...
 * 
 **/
inm_s32_t process_start_filtering_ioctl(inm_devhandle_t *idhp, void __INM_USER *arg)
{
    inm_dev_extinfo_t  *dev_infop = NULL;
    inm_s32_t err = 0;
...
if(IS_DBG_ENABLED(inm_verbosity, INM_IDEBUG)){
    ...
    if (strncpy_s(dev_infop->d_src_scsi_id, INM_GUID_LEN_MAX,
			      dev_infop->d_pname, strlen(dev_infop->d_pname))) {
	INM_KFREE(dev_infop, sizeof(inm_dev_info_t), INM_KERNEL_HEAP);
	return INM_EFAULT;
    }
...
/**
 * FUNCTION NAME:process_start_mirror_ioctl
 * 
 * DESCRIPTION :
 * 
 * INPUT PARAMETERS : 
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_MIRROR))){
    ...
    if(!INM_ACCESS_OK(VERIFY_READ, (void __INM_USER *)arg, sizeof(mirror_conf_info_t))) {
    ...
    if (INM_COPYIN(mirror_infop, arg, sizeof(mirror_conf_info_t))) {
        err("INM_COPYIN failed");
        INM_KFREE(mirror_infop, sizeof(mirror_conf_info_t), INM_KERNEL_HEAP);
        return INM_EFAULT;
    }
    INM_SPIN_LOCK_IRQSAVE(&driver_ctx->clean_shutdown_lock, lock_flag);
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_MIRROR))){
    ...
    info("d_type:%d d_flags:%llu d_nblks:%llu d_bsize:%llu startoff:%llu",
        ...
        mirror_infop->d_nblks, mirror_infop->d_bsize, mirror_infop->startoff);
}
...
/**
 * FUNCTION NAME:process_mirror_volume_stacking
 * 
 * DESCRIPTION :
 * 
 * INPUT PARAMETERS : 
 ...
 **/
inm_s32_t
process_mirror_volume_stacking_ioctl(inm_devhandle_t *idhp, void __INM_USER *arg)
{
    mirror_conf_info_t  *mirror_infop = NULL;
    inm_s32_t err = 0;
    inm_dev_extinfo_t *dev_infop = NULL;
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_MIRROR))){
    ...
    err = populate_volume_lists(&src_mirror_list_head, &dst_mirror_list_head,
                              mirror_infop);
    if (err) {
        goto out;
    }
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_MIRROR))){
    ...
    if (mirror_infop->src_scsi_id[0] == ' ' || mirror_infop->src_scsi_id[0] == '\0') {
    ...
    if (mirror_infop->dst_scsi_id[0] == ' ' || mirror_infop->dst_scsi_id[0] == '\0') {
    ...
    vol_entry = inm_list_entry(src_mirror_list_head.next, mirror_vol_entry_t, next);
    ...
    dev_infop->d_startoff = 0;
    if (strncpy_s(dev_infop->d_guid, INM_GUID_LEN_MAX, vol_entry->tc_mirror_guid,
	strlen(vol_entry->tc_mirror_guid)) ||
	strncpy_s(dev_infop->d_src_scsi_id, INM_MAX_SCSI_ID_SIZE, mirror_infop->src_scsi_id,
	INM_MAX_SCSI_ID_SIZE - 1) ||
	strncpy_s(dev_infop->d_dst_scsi_id, INM_MAX_SCSI_ID_SIZE, mirror_infop->dst_scsi_id,
	INM_MAX_SCSI_ID_SIZE - 1)) {
    ...
    //INM_INIT_LIST_HEAD(&dev_infop->src_list);
    //INM_INIT_LIST_HEAD(&dev_infop->dst_list);
    //list_change_head(&dev_infop->tc_src_list, &src_mirror_list_head);
    //list_change_head(&dev_infop->tc_dst_list, &dst_mirror_list_head);
...
void
process_stop_filtering_common(target_context_t *tgt_ctxt, inm_devhandle_t *idhp)
{
    dbg("entered");
...
stop_filtering_volume(char *uuid, inm_devhandle_t *idhp, int dbs_flag)
{
    ...
    set_tag_drain_notify_status(tgt_ctxt, TAG_STATUS_DROPPED, DEVICE_STATUS_FILTERING_STOPPED);
    volume_unlock(tgt_ctxt);
#ifdef INM_AIX
...
/**
 * FUNCTION NAME:process_stop_filtering_ioctl
 * 
 * DESCRIPTION :
 * 
 * INPUT PARAMETERS : 
 ...
 * 
 **/
inm_s32_t process_stop_filtering_ioctl(inm_devhandle_t *idhp, void __INM_USER *arg)
{
    inm_s32_t error = 0;
    ...
    if(!INM_ACCESS_OK(VERIFY_READ, (void __INM_USER *)arg, sizeof(VOLUME_GUID))) {
        err("Read access violation for VOLUME_GUID");
	return -EFAULT;
    }

    guid = (VOLUME_GUID *)INM_KMALLOC(sizeof(VOLUME_GUID), INM_KM_SLEEP, INM_KERNEL_HEAP);
    if(!guid) {
...
/**
 * FUNCTION NAME:process_stop_mirroring_ioctl
 * 
 * DESCRIPTION :
 * 
 * INPUT PARAMETERS : 
 ...
 * 
 **/
inm_s32_t process_stop_mirroring_ioctl(inm_devhandle_t *idhp, void __INM_USER *arg)
{
    target_context_t *tgt_ctxt = NULL;
    ...
    if (!INM_ACCESS_OK(VERIFY_READ, (void __INM_USER *)arg, sizeof(SCSI_ID))) {
        err("Read access violation for SCSI_ID");
        return -EFAULT;
    }

    scsi_id = (SCSI_ID *)INM_KMALLOC(sizeof(SCSI_ID), INM_KM_SLEEP, INM_KERNEL_HEAP);
    if (!scsi_id) {
...
inm_s32_t process_volume_unstacking_ioctl(inm_devhandle_t *idhp, void __INM_USER *arg)
{
    target_context_t *tgt_ctxt = NULL;
    VOLUME_GUID *guid = NULL;
#ifdef INM_AIX
    ...
    if (idhp->private_data) 
        put_tgt_ctxt(tgt_ctxt); //One extra dereference as we did extra reference in do_start_filtering;
    INM_KFREE(guid, sizeof(VOLUME_GUID), INM_KERNEL_HEAP);
    idhp->private_data = NULL;
...
/**
 * FUNCTION NAME:process_get_db_ioctl
 * 
 * DESCRIPTION :
 * 
 * INPUT PARAMETERS : 
...
if(IS_DBG_ENABLED(inm_verbosity, INM_IDEBUG)){
    ...
    if(!INM_ACCESS_OK(VERIFY_READ, (void __INM_USER *)arg, sizeof(UDIRTY_BLOCK_V2)))
        return -EFAULT;

    //TODO: Can optimize this part. Preallocating user dirty block or
    // by mmaping udirty block to kernel. Also, INM_KMALLOC may not work since
    // udirty block is huge 20K min. Need to revisit.
    ...
    if (ctxt->tc_tel.tt_ds_throttle_stop == TELEMETRY_THROTTLE_IN_PROGRESS) {
        get_time_stamp(&ctxt->tc_tel.tt_ds_throttle_stop);
        telemetry_clear_dbs(&ctxt->tc_tel.tt_blend, DBS_DIFF_SYNC_THROTTLE);
    }
    volume_unlock(ctxt);
...
/**
 * FUNCTION NAME:process_commit_db_ioctl
 * 
 * DESCRIPTION :
 * 
 * INPUT PARAMETERS : 
...
if(IS_DBG_ENABLED(inm_verbosity, INM_IDEBUG)){
    ...
    /* TODOs: If there is no commit dirty block ?? */

    if(!INM_ACCESS_OK(VERIFY_READ, (void __INM_USER *)arg, sizeof(COMMIT_TRANSACTION)))
	return -EFAULT;
...
/**
 * FUNCTION NAME:process_get_time_ioctl
 * 
 * DESCRIPTION :
 * 
 * INPUT PARAMETERS : 
...
if(IS_DBG_ENABLED(inm_verbosity, INM_IDEBUG)){
    ...
    if(!INM_ACCESS_OK(VERIFY_WRITE, (void __INM_USER *)arg, sizeof(long long))) {
...
/**
 * FUNCTION NAME:process_clear_diffs_ioctl
 * 
 * DESCRIPTION :
 * 
 * INPUT PARAMETERS : 
 ...
 * 
 **/
inm_s32_t process_clear_diffs_ioctl(inm_devhandle_t *idhp, void __INM_USER *arg)
{
    char *uuid = NULL;
    target_context_t *tgt_ctxt = NULL;
    ...
    uuid = (char *)INM_KMALLOC(GUID_SIZE_IN_CHARS, INM_KM_SLEEP, INM_KERNEL_HEAP);
    if(!uuid)
        return -ENOMEM;
...
/**
 * FUNCTION NAME:process_set_volume_flags_ioctl
 * 
 * DESCRIPTION :
 * 
 * INPUT PARAMETERS : 
 ...
 * 
 **/
inm_s32_t process_set_volume_flags_ioctl(inm_devhandle_t *idhp, void __INM_USER *arg)
{
...
/**
 * FUNCTION NAME:process_get_volume_flags_ioctl
 * 
 * DESCRIPTION :
 * 
 * INPUT PARAMETERS : 
 ...
 * 
 **/
inm_s32_t process_get_volume_flags_ioctl(inm_devhandle_t *idhp, void __INM_USER *arg)
{
    target_context_t *ctxt;
    VOLUME_FLAGS_INPUT *flagip;
...
/**
 * FUNCTION NAME:process_wait_for_db_ioctl
 * 
 * DESCRIPTION :
 * 
 * INPUT PARAMETERS : 
 ...
 * 
 **/
inm_s32_t process_wait_for_db_ioctl(inm_devhandle_t *idhp, void __INM_USER *arg)
{
    target_context_t *ctxt;
...
inm_s32_t process_wait_for_db_v2_ioctl(inm_devhandle_t *idhp, void __INM_USER *arg)
{
...
inm_s32_t shutdown_volume(target_context_t *vcptr, inm_s32_t freeze_root)
{
    ...
    set_unsignedlonglong_vol_attr(vcptr, VolumePrevEndTimeStamp, vcptr->tc_PrevEndTimeStamp);
    set_unsignedlonglong_vol_attr(vcptr, VolumePrevEndSequenceNumber, vcptr->tc_PrevEndSequenceNumber);
    set_unsignedlonglong_vol_attr(vcptr, VolumePrevSequenceIDforSplitIO, vcptr->tc_PrevSequenceIDforSplitIO);
    set_unsignedlonglong_vol_attr(vcptr, VolumeRpoTimeStamp, vcptr->tc_rpo_timestamp);
...
/**
 * FUNCTION NAME:process_sys_shutdown_notify_ioctl
 * 
 * DESCRIPTION :
 * 
 * INPUT PARAMETERS : 
 ...
 * 
 **/
inm_s32_t process_sys_shutdown_notify_ioctl(inm_devhandle_t *idhp, void __INM_USER *arg)
{
...
retry:
    ...
    inm_list_for_each_safe(ptr, nextptr, &driver_ctx->tgt_list) {
        ...
        if(vcptr->tc_flags & (VCF_VOLUME_CREATING | VCF_VOLUME_DELETING | VCF_VOLUME_FROZEN_SYS_SHUTDOWN)){
            vcptr = NULL;
            continue;
        }
...
#pragma message "Original shutdown" 
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
    ...
    INM_WAKEUP_INTERRUPTIBLE(&driver_ctx->service_thread.wakeup_event);
    INM_COMPLETE(&driver_ctx->service_thread._new_event_completion);
    INM_WAIT_FOR_COMPLETION(&driver_ctx->shutdown_completion);
...
process_lcw_ioctl(inm_devhandle_t *idhp, void __INM_USER *arg)
{
    ...
    if (op->lo_op == LCW_OP_MAP_FILE)
        ...
        error = lcw_perform_bitmap_op(op->lo_name.volume_guid, op->lo_op);
 ...
 */
inm_s32_t process_tag_ioctl(inm_devhandle_t *idhp, void __INM_USER *user_buf, inm_s32_t sync_tag)
{
    inm_s32_t flags = 0;	
...
if(IS_DBG_ENABLED(inm_verbosity, INM_IDEBUG)){
    ...
    if(sync_tag){
	tag_guid = (tag_guid_t *)INM_KMALLOC(sizeof(tag_guid_t), INM_KM_SLEEP, INM_KERNEL_HEAP);
	if(!tag_guid){
	...
	if(!INM_ACCESS_OK(VERIFY_READ , (void __INM_USER *)user_buf, tag_guid->guid_len)){
	    err("TAG Input Failed: Access violation in getting guid");
	    error = INM_EFAULT;
	    goto just_exit;
	}

	tag_guid->guid = (char *)INM_KMALLOC(tag_guid->guid_len + 1, INM_KM_SLEEP, INM_KERNEL_HEAP);
	if(!tag_guid->guid){
	...
	tag_guid->status = INM_KMALLOC(num_vols * sizeof(inm_s32_t), INM_KM_SLEEP, INM_KERNEL_HEAP);
	if(!tag_guid->status){
	...
	if(INM_COPYOUT(user_buf, tag_guid->status, num_vols * sizeof(inm_s32_t))) {
	...
	for(i = 0; i < num_vols; i++){
		...
		inm_list_add_tail(&tag_guid->tag_list, &driver_ctx->tag_guid_list);
...
process_get_tag_status_ioctl(inm_devhandle_t *idhp, void __INM_USER *user_buf)
{
    ...
    if(!INM_ACCESS_OK(VERIFY_READ , (void __INM_USER *)user_buf, guid_len)){
	err("TAG STATUS Input Failed: Access violation in getting guid");
	error = INM_EFAULT;
	goto out_err;
    }

    guid = (char *)INM_KMALLOC(guid_len + 1, INM_KM_SLEEP, INM_KERNEL_HEAP);
    if(!guid){
...
retry:
    ...
    if(INM_COPYOUT(user_buf, tag_guid->status, tag_guid->num_vols * sizeof(inm_s32_t))) {
...
wait:
    if(seconds)
	inm_wait_event_interruptible_timeout(tag_guid->wq, 0, (seconds * INM_HZ));
    else
...
inm_s32_t process_wake_all_threads_ioctl(inm_devhandle_t *idhp, void __INM_USER *user_buf)
{
...
inm_s32_t process_get_db_threshold(inm_devhandle_t *idhp, void __INM_USER *user_buf)
{
    get_db_thres_t *thr;
    target_context_t *ctxt;
	
    if(!INM_ACCESS_OK(VERIFY_READ | VERIFY_WRITE, (void __INM_USER *)user_buf,
...
inm_s32_t process_resync_start_ioctl(inm_devhandle_t *idhp, void __INM_USER *user_buf)
{
...
inm_s32_t process_resync_end_ioctl(inm_devhandle_t *idhp, void __INM_USER *user_buf)
{
...
inm_s32_t process_get_driver_version_ioctl(inm_devhandle_t *idhp, void __INM_USER *user_buf)
{
...
process_get_global_stats_ioctl(inm_devhandle_t *handle, void * arg)
{
	 ...
	 * 	  of driver context since here we the reader only so at most
	 *        we could get little messed info but we cant panic because here 
	 *	  we panic if trying to access some field while driver_ctx is already 
	 *	  gone i.e. involflt is unloaded, so if it is unloaded we cant be here
	 *	  In future if we see really weird behaviour we'll take the proper locks.
	 */
	...
	len += snprintf(page+len, (INM_PAGESZ - len),
       ...
       len += snprintf(page+len, (INM_PAGESZ - len),
               "No. of available objects    : %u\n",
               driver_ctx->dc_host_info.bio_info_cache->pi_reserved);
       len += snprintf(page+len, (INM_PAGESZ - len),
               "No. of used objects         : %u\n",
               driver_ctx->dc_host_info.bio_info_cache->pi_allocd);
       len += snprintf(page+len, (INM_PAGESZ - len),
               "Max / Min limit             : %u/%u\n\n",
               driver_ctx->dc_host_info.bio_info_cache->pi_max_nr,
               driver_ctx->dc_host_info.bio_info_cache->pi_min_nr);
#endif
       ...
       if (vm_cx_sess->vcs_flags & VCS_CX_S2_EXIT)
           len += snprintf(page+len, (INM_PAGESZ - len), ", s2 Exited");
       if (vm_cx_sess->vcs_flags & VCS_CX_SVAGENT_EXIT)
           len += snprintf(page+len, (INM_PAGESZ - len), ", svagent exited");
       ...
       len += snprintf(page+len, (INM_PAGESZ - len),
               ...
               vm_cx_sess->vcs_max_peak_churn, vm_cx_sess->vcs_excess_churn);
       len += snprintf(page+len, (INM_PAGESZ - len),
...
process_get_monitoring_stats_ioctl(inm_devhandle_t *handle, void * arg)
{
    ...
    if (!ctxt) {
        err("Failed to get target context for uuid %s", vol_lw_statsp->VolumeGuid.volume_guid);
        ret = -EINVAL;
        goto ERR_EXIT;
    }
    ...
    switch(vol_lw_statsp->ReqStat) {
        ...
        case GET_CHURN_STATS:
            vol_lw_statsp->ChurnStats.NumCommitedChangesInBytes = ctxt->tc_bytes_commited_changes;
            break;
        default:
...
process_get_volume_stats_ioctl(inm_devhandle_t *handle, void * arg)
{
	...
	/* TODO: Right, we dont check for how much we have written and if
	 * there is enough space or not. Since our output is very less, this
	 * is ok, but when we expand the output, we should deftly check it.
	 */

        vol_stat = (VOLUME_STATS *) INM_KMALLOC(sizeof(VOLUME_STATS), INM_KM_SLEEP, INM_KERNEL_HEAP);
	if(!vol_stat){
	...
	if (ctxt->tc_dev_type != FILTER_DEV_MIRROR_SETUP){
		...
		len += snprintf(page+len, (INM_PAGESZ - len), "%s/%s", strp, strp_2);
		...
		len += snprintf(page+len, (INM_PAGESZ - len),
				...
				ctxt->tc_pending_changes, ctxt->tc_bytes_pending_changes); 
	
		len += snprintf(page+len, (INM_PAGESZ - len),
  			"Pending Changes/bytes in metadata   : %lld/%lld\n", 
			ctxt->tc_pending_md_changes, ctxt->tc_bytes_pending_md_changes);
	
		len += snprintf(page+len, (INM_PAGESZ - len),
			"Commited Changes/bytes              : %lld/%lld\n", 
			ctxt->tc_commited_changes, ctxt->tc_bytes_commited_changes); 
		...
		len += snprintf(page+len, (INM_PAGESZ - len),
			...
			ctxt->tc_data_to_disk_limit, ctxt->tc_stats.dfm_bytes_to_disk);
		...
		len += snprintf(page+len, (INM_PAGESZ - len),
			...
			INM_ATOMIC_READ(&ctxt->tc_async_bufs_pending), INM_ATOMIC_READ(&ctxt->tc_async_bufs_write_pending),
			INM_ATOMIC_READ(&ctxt->tc_async_bufs_processed), INM_ATOMIC_READ(&ctxt->tc_async_bufs_write_processed));
		...
		len += snprintf(page+len, (INM_PAGESZ - len),
			...
			INM_ATOMIC_READ(&ctxt->tc_read_buf_first), INM_ATOMIC_READ(&ctxt->tc_write_buf_first));

		len += snprintf(page+len, (INM_PAGESZ - len),
			"No. of bufs submitted/processed     : %u/%u\n",
			INM_ATOMIC_READ(&ctxt->tc_nr_bufs_pending), INM_ATOMIC_READ(&ctxt->tc_nr_bufs_processed));

		len += snprintf(page+len, (INM_PAGESZ - len),
			"No. of bufs queued/processed        : %lu/%lu\n",
			ctxt->tc_nr_bufs_queued_to_thread, ctxt->tc_nr_bufs_processed_by_thread);
		...
		len += snprintf(page+len, (INM_PAGESZ - len),
			...
			ctxt->tc_more_done_set, ctxt->tc_nr_bufs_submitted_gr_than_one);
...
process_get_volume_stats_v2_ioctl(inm_devhandle_t *handle, void * arg)
{
        ...
        if (INM_COPYIN(telemetry_vol_stats, arg, sizeof(TELEMETRY_VOL_STATS))) {
        ...
        drv_statsp->ulVolumesReturned = 1;  // Linux fills up only one volume stats per call
        drv_statsp->ulNonPagedMemoryLimitInMB = 0; // Not valid for Linux
        drv_statsp->LockedDataBlockCounter = 0; // Not valid for Linux
        ...
        drv_statsp->ulCommonBootCounter = 0; // Not valid for Linux
        // driver_ctx->tunable_params.data_pool_size is in MB, convert to bytes
        ...
        vol_statsp->ullTotalTrackedBytes = tgt_ctxt->tc_bytes_commited_changes; // per volume stat
        ...
        if (INM_COPYOUT(arg, telemetry_vol_stats, sizeof(TELEMETRY_VOL_STATS))) {
...
out:    
        ...
        if (telemetry_vol_stats) {
            INM_KFREE(telemetry_vol_stats, sizeof(TELEMETRY_VOL_STATS),
                      INM_KERNEL_HEAP);
        }
...
process_get_protected_volume_list_ioctl(inm_devhandle_t *handle, void * arg)
{
	...
	if (INM_COPYIN(vol_list, (GET_VOLUME_LIST *) arg, sizeof(GET_VOLUME_LIST))) {
		err("copyin failed\n");
		ret = INM_EFAULT;
		goto out;
	}
	...
	inm_list_for_each_safe(ptr, nextptr, &driver_ctx->tgt_list) {
        ...
        /* return guid names instead of persistent names */
...
out:
	if (ret == INM_EAGAIN){
        ...
        /* for safer side ask user to allocate little extra memory 
         * because next time it'll by that number of protected volumes
         * might increase.
         */
...
process_mirror_exception_notify_ioctl(inm_devhandle_t *handle, void * arg)
{
    ...
    inm_u32_t ret = 0;
        ...
        if (INM_COPYIN(resync_info, (inm_resync_notify_info_t *) arg, sizeof(*resync_info))) {
                err("copyin failed\n");
                ret = INM_EFAULT;
                goto out;
        }
       ...
       if (INM_COPYOUT((inm_resync_notify_info_t *) arg, resync_info, sizeof(*resync_info))) {
           err("copyout failed\n");
           ret = INM_EFAULT;
      }
...
inm_s32_t process_mirror_test_heartbeat(inm_devhandle_t *idhp, void __INM_USER *arg)
{
    target_context_t *tgt_ctxt = NULL;
    SCSI_ID *scsi_id = NULL;
    inm_irqflag_t lock_flag = 0;
...
if(IS_DBG_ENABLED(inm_verbosity, INM_IDEBUG)){
    ...
    if(tcp->tc_dev_type == FILTER_DEV_MIRROR_SETUP){
        ...
        inm_list_for_each_safe(ptr, nextptr, hd){
            ...
            (*len) += snprintf((page+(*len)), (INM_PAGESZ - (*len)), "%s, %llu, %llu, %llu, %s %d\n", vol_entry->tc_mirror_guid,
                  vol_entry->vol_io_issued, vol_entry->vol_io_succeeded, vol_entry->vol_byte_written, vol_entry->vol_error?"offline":"online", INM_ATOMIC_READ(&(vol_entry->vol_ref)));
        }
        volume_unlock(tcp);
   }
if(IS_DBG_ENABLED(inm_verbosity, INM_IDEBUG)){
...
process_get_additional_volume_stats(inm_devhandle_t *handle, void *arg)
{
	...
	if ( !INM_ACCESS_OK(VERIFY_READ, (void __user*)arg, sizeof(VOLUME_STATS_ADDITIONAL_INFO))) {
	...
	if ( INM_COPYIN( vsa_infop, arg, sizeof( VOLUME_STATS_ADDITIONAL_INFO ))) {
		err("INM_COPYIN failed");
		INM_KFREE(vsa_infop, sizeof( VOLUME_STATS_ADDITIONAL_INFO ), INM_KERNEL_HEAP);
		ret = -EFAULT;
		return (ret);
	}
    ...
    /* can't negative RPOs */
    if (vsa_infop->ullDriverCurrentTimeStamp < vsa_infop->ullOldestChangeTimeStamp) {
        vsa_infop->ullDriverCurrentTimeStamp = vsa_infop->ullOldestChangeTimeStamp;
    }

    /* Add outstanding bitmap changes to the total pending changes */
	if (ctxt->tc_bp && ctxt->tc_bp->volume_bitmap && ctxt->tc_bp->volume_bitmap->bitmap_api) {
    ...
    if ( INM_COPYOUT( (void*) arg, vsa_infop, sizeof(VOLUME_STATS_ADDITIONAL_INFO))) {
        err("INM_COPYOUT failed");
        INM_KFREE(vsa_infop, sizeof( VOLUME_STATS_ADDITIONAL_INFO ), INM_KERNEL_HEAP);
        ret =  -EFAULT;
        return (ret);
...
process_get_volume_latency_stats(inm_devhandle_t *handle, void *arg)
{
	...
	target_context_t *ctxt = NULL;
	//change_node_t *chgnodep = NULL;
	//inm_u64_t lock_flag = 0;

	if ( !INM_ACCESS_OK(VERIFY_READ, (void __user*)arg, sizeof(VOLUME_LATENCY_STATS))) {
		err( " Read Access Violation for GET_ADDITIONAL_VOLUME_STATS\n");
		ret = -EFAULT;
		return (ret);
	}
	...
	if ( INM_COPYIN(vol_latstatsp, arg, sizeof(VOLUME_LATENCY_STATS))) {
		err("INM_COPYIN failed");
		INM_KFREE(vol_latstatsp, sizeof(VOLUME_LATENCY_STATS), INM_KERNEL_HEAP);
		ret = -EFAULT;
		return (ret);
	}
	...
	if ( INM_COPYOUT( (void*) arg, vol_latstatsp, sizeof(VOLUME_LATENCY_STATS))) {
		err("INM_COPYOUT failed");
		INM_KFREE(vol_latstatsp, sizeof( VOLUME_LATENCY_STATS ), INM_KERNEL_HEAP);
		ret =  -EFAULT;
		return (ret);
...
process_bitmap_stats_ioctl(inm_devhandle_t *handle, void *arg)
{
	...
	vbstatsp = (VOLUME_BMAP_STATS *) INM_KMALLOC(sizeof(*vbstatsp), INM_KM_SLEEP, INM_KERNEL_HEAP);
	if (!vbstatsp) {
		err("INM_KMALLOC() failed\n");
		return -ENOMEM;
	}
	...
	bbsp = (bmap_bit_stats_t *) INM_KMALLOC(sizeof(*bbsp), INM_KM_SLEEP, INM_KERNEL_HEAP);
	if (!bbsp) {
		err("INM_KMALLOC() failed\n");
		put_tgt_ctxt(tcp);
		INM_KFREE(vbstatsp, sizeof(*vbstatsp), INM_KERNEL_HEAP);
	...
	if ( INM_COPYOUT( (void*) arg, vbstatsp, sizeof(VOLUME_BMAP_STATS))) {
		err("INM_COPYOUT failed");
		INM_KFREE(vbstatsp, sizeof( VOLUME_BMAP_STATS ), INM_KERNEL_HEAP);
		ret =  -EFAULT;
		return (ret);
	}
	INM_KFREE(vbstatsp, sizeof(*vbstatsp), INM_KERNEL_HEAP);
...
/*
 * process ioctl to tag the vol list
 * each volume will have all the tag from tag list
 */
inm_s32_t
process_tag_volume_ioctl(inm_devhandle_t *idhp, void __INM_USER *arg)
{
    ...
    if(INM_COPYIN(tag_vol->tag_names, arg, tag_vol->nr_tags * sizeof(tag_names_t))) {
        err("INM_COPYIN failed");
        ret = -EFAULT;
        goto out_err_vol;
    }
    ...
    if (driver_ctx->dc_tag_drain_notify_guid &&
        ...
        !INM_MEM_CMP(tag_vol->tag_guid,
                     ...
                     GUID_LEN)) {
        set_tag_guid = 1;
        memcpy_s(&driver_ctx->dc_cp_guid, sizeof(driver_ctx->dc_cp_guid),
                     tag_vol->tag_guid, sizeof(tag_vol->tag_guid));
    }
    INM_SPIN_UNLOCK(&driver_ctx->dc_tag_commit_status);

    /* get list of all the protected volumes if vacp sets this flag */
    if ((tag_vol->flags & TAG_ALL_PROTECTED_VOLUME_IOBARRIER) ==
        ...
        if (driver_ctx->dc_cp == INM_CP_CRASH_ACTIVE) {
            dbg("issuing tag");
            ret = iobarrier_issue_tag_all_volume(tag_list, tag_vol->nr_tags, 
                                                 commit_pending, NULL);
            if(ret) {
    ...
    for(numvol = 0; numvol < tag_vol->nr_vols; numvol++) {
        ...
        if(INM_COPYIN(tag_vol->vol_info, arg, sizeof(*tag_vol->vol_info))) {
            err("INM_COPYIN failed");
            ret = -EFAULT;
            break;
        }
        ...
        if(!INM_ACCESS_OK(VERIFY_WRITE, (void __INM_USER *)arg, sizeof(*tag_vol->vol_info))) {
            err("write access verification failed");
            ret = INM_EFAULT;
            break;
        }

        if(INM_COPYOUT(arg, tag_vol->vol_info, sizeof(*tag_vol->vol_info))) {
    ...
    if (no_of_vol_tags_done) {
        ...
        if (driver_ctx->dc_cp == INM_CP_TAG_COMMIT_PENDING) {
            ...
            memcpy_s(&driver_ctx->dc_cp_guid, sizeof(driver_ctx->dc_cp_guid),
                     tag_vol->tag_guid, sizeof(tag_vol->tag_guid));
            start_cp_timer(tag_vol->timeout, inm_fvol_list_thaw_on_timeout);
        }
#endif
    } else {
        INM_BUG_ON(ret >= 0);
    ...
    dbg("no_of_vol_tags_done [%d], no of volumes [%d]", no_of_vol_tags_done,tag_vol->nr_vols);
...
out:
     ...
     if(tag_list) {
         INM_KFREE(tag_list, tag_vol->nr_tags * sizeof(tag_info_t), INM_KERNEL_HEAP);
         tag_list = NULL;
     }

    if(tag_vol) {
        if(tag_vol->vol_info) {
            INM_KFREE(tag_vol->vol_info, sizeof(volume_info_t), INM_KERNEL_HEAP);
            tag_vol->vol_info = NULL;
        }
        if(tag_vol->tag_names) {
            INM_KFREE(tag_vol->tag_names, tag_vol->nr_tags * sizeof(tag_names_t), INM_KERNEL_HEAP);
            tag_vol->tag_names = NULL;
        }
...
ERR_EXIT:
    ...
    if (blk_mq_status != NULL)
        INM_KFREE(blk_mq_status, sizeof(BLK_MQ_STATUS), INM_KERNEL_HEAP);
...
process_replication_state_ioctl(inm_devhandle_t *handle, void *arg)
{
    ...
    if (!ctxt) {
        err("Cannot find %s to set DS throttle", rep->DeviceId.volume_guid);
        error = -EFAULT;
        goto out;
    }
    ...
    if (!ctxt->tc_tel.tt_ds_throttle_start ||
       (ctxt->tc_tel.tt_ds_throttle_stop != TELEMETRY_THROTTLE_IN_PROGRESS)) {
        telemetry_set_dbs(&ctxt->tc_tel.tt_blend, DBS_DIFF_SYNC_THROTTLE);
...
out:
    if (cdf_stats)
        INM_KFREE(cdf_stats, sizeof(COMMIT_DB_FAILURE_STATS), INM_KERNEL_HEAP);
                                      ...
                                      inm_list_head_t *disk_cx_stats_list)
{
    ...
    for (idx = 1; idx <= num_output_disks; idx++) {
        ...
        inm_list_add_tail(&disk_cx_stats_info->dcsi_list, disk_cx_stats_list);
    }
...
inm_s32_t process_get_cxstatus_notify_ioctl(inm_devhandle_t *handle, void *arg)
{
    ...
    if (INM_COPYIN(get_cx_notify, arg,
                    (sizeof(GET_CXFAILURE_NOTIFY) - sizeof(VOLUME_GUID)))) {
    ...
    device_list_arg = arg + sizeof(GET_CXFAILURE_NOTIFY) - sizeof(VOLUME_GUID);
    ...
    if (get_cx_notify->ullFlags & CXSTATUS_COMMIT_PREV_SESSION &&
            ...
            get_cx_notify->ulTransactionID == vm_cx_sess->vcs_transaction_id) {
        ...
        vm_cx_sess->vcs_flags &= ~(VCS_CX_TIME_JUMP_FWD | VCS_CX_TIME_JUMP_BWD);
    }
    ...
    while(1) {
        ...
        ret = inm_wait_event_interruptible_timeout(
                ...
                should_wakeup_monitor_thread(vm_cx_sess, get_cx_notify),
                60 * INM_HZ);

        INM_SPIN_LOCK_IRQSAVE(&driver_ctx->dc_vm_cx_session_lock,
                          driver_ctx->dc_vm_cx_session_lock_flag);
        if (ret || should_wakeup_monitor_thread(vm_cx_sess, get_cx_notify)) {
            driver_ctx->dc_wokeup_monitor_thread = 0;
            break;
        }
    }
    ...
    arg += sizeof(GET_CXFAILURE_NOTIFY) +
         ((get_cx_notify->ulNumberOfProtectedDisks - 1) * sizeof(VOLUME_GUID));
    ...
    for (ptr = driver_ctx->tgt_list.next; ptr != &(driver_ctx->tgt_list);
        ...
        for (disk_cx_stats_ptr = disk_cx_stats_list.next;
            ...
            if (!strncmp(tgt_ctxt->tc_pname,
                  dev_cx_stats->DeviceId.volume_guid, GUID_SIZE_IN_CHARS)) {
                found = 1;
                dev_cx_stats->ullFlags &= ~DISK_CXSTATUS_DISK_NOT_FILTERED;
                break;
            }
...
update_disk_cx_session:
        ...
        dev_cx_stats->ullMaxS2LatencyInMS = (disk_cx_sess->dcs_max_s2_latency /
                                                             10000ULL);
    ...
    memcpy_s(vm_cx_stats->ChurnBucketsMBps,
           sizeof(vm_cx_stats->ChurnBucketsMBps), vm_cx_sess->vcs_churn_buckets,
           sizeof(vm_cx_sess->vcs_churn_buckets));
    vm_cx_stats->ullMaximumPeakChurnInBytes = vm_cx_sess->vcs_max_peak_churn;

    vm_cx_stats->ullMaxS2LatencyInMS = (vm_cx_sess->vcs_max_s2_latency /
                                                                 10000ULL);
    ...
    if (!vm_cx_sess->vcs_transaction_id)
        vm_cx_sess->vcs_transaction_id = ++driver_ctx->dc_transaction_id;
...
inm_s32_t process_tag_drain_notify_ioctl(inm_devhandle_t *handle, void *arg)
{
    ...
    int                     out_size = 0;
    int                     idx;
    int                     nr_protected_disks_found = 0;
    void                    *device_list_arg;
    vm_cx_session_t         *vm_cx_sess = &driver_ctx->dc_vm_cx_session;
    inm_list_head_t         *ptr;
    target_context_t        *tgt_ctxt;
    ...
    if (INM_COPYIN(tag_drain_notify_input, arg,
                    (sizeof(TAG_COMMIT_NOTIFY_INPUT) - sizeof(VOLUME_GUID)))) {
    ...
    tag_drain_notify_output = INM_KMALLOC(out_size, INM_KM_SLEEP, INM_KERNEL_HEAP);
    ...
    for (idx = 0; idx < tag_drain_notify_input->ulNumDisks; idx++) {
        ...
        memcpy_s(&tag_commit_status[idx].DeviceId, sizeof(VOLUME_GUID), guid,
                                                          sizeof(VOLUME_GUID));
        info("input pname = %s", tag_commit_status[idx].DeviceId.volume_guid);
    ...
    info("Number of disks = %llu and protected number of disks = %d",
           tag_drain_notify_input->ulNumDisks, driver_ctx->total_prot_volumes);
    if (tag_drain_notify_input->ulNumDisks > driver_ctx->total_prot_volumes) {
        INM_UP_READ(&(driver_ctx->tgt_list_sem));
    ...
    if (driver_ctx->dc_tag_commit_notify_flag & TAG_COMMIT_NOTIFY_BLOCK_DRAIN_FLAG) {
        int ret = 0;
        for (idx = 0; idx < tag_drain_notify_input->ulNumDisks; idx++) {
...
update_tag_drain_notify_output:
    ...
    if (!INM_ACCESS_OK(VERIFY_WRITE, (void __user*)device_list_arg, out_size)) {
...
out:
    ...
    if (tag_drain_notify_input)
        INM_KFREE(tag_drain_notify_input, (sizeof(TAG_COMMIT_NOTIFY_INPUT) -
                                  sizeof(VOLUME_GUID)), INM_KERNEL_HEAP);
...
inm_s32_t process_modify_persistent_device_name(inm_devhandle_t *handle, void *arg)
{
    ...
    info(
        "Modifying persistent device name source disk : %s, old pname : %s, new pname : %s\n",
    ...
    tgt_ctxt = get_tgt_ctxt_from_uuid_nowait(
                modify_pname_input->DevName.volume_guid);
    if (!tgt_ctxt) {
        err("The disk %s is not protected", modify_pname_input->DevName.volume_guid);
        error = -ENODEV;
        goto out;
    }

    if (strncmp(tgt_ctxt->tc_pname, modify_pname_input->OldPName.volume_guid,
        GUID_SIZE_IN_CHARS)) {
    ...
    old_path = (char *)INM_KMALLOC(INM_PATH_MAX, INM_KM_SLEEP, INM_KERNEL_HEAP);
    if(!old_path){
	    err("Allocation of memory failed for old_path.\n");
	    error = INM_ENOMEM;
	    goto out;
    }
    new_path = (char *)INM_KMALLOC(INM_PATH_MAX, INM_KM_SLEEP, INM_KERNEL_HEAP);
    if(!new_path){
    ...
    snprintf(new_path, INM_PATH_MAX, "%s", modify_pname_input->NewPName.volume_guid);
    ...
    snprintf(new_path, INM_PATH_MAX, "%s/%s%s%s", tgt_ctxt->tc_pname,
        LOG_FILE_NAME_PREFIX, modify_pname_input->NewPName.volume_guid, LOG_FILE_NAME_SUFFIX);
    if (inm_is_upgrade_pname(old_path, new_path)) {
        error = INM_EFAULT;
        goto out;
    }
    error = modify_persistent_device_name(tgt_ctxt, modify_pname_input->NewPName.volume_guid);
...
out:
    ...
    if (modify_pname_input) {
        INM_KFREE(modify_pname_input, sizeof(MODIFY_PERSISTENT_DEVICE_NAME_INPUT),
            INM_KERNEL_HEAP);
    }
...
inm_s32_t process_get_drain_state_ioctl(inm_devhandle_t *handle, void *arg)
{
    ...
    drain_state_output = INM_KMALLOC(out_size, INM_KM_SLEEP, INM_KERNEL_HEAP);
    ...
    device_list_arg = arg + sizeof(GET_DISK_STATE_INPUT) - sizeof(VOLUME_GUID);
    guid = INM_KMALLOC(sizeof(VOLUME_GUID), INM_KM_SLEEP, INM_KERNEL_HEAP);
    if (!guid) {
        err("Failed to allocate VOLUME_GUID");
        error = INM_ENOMEM;
    ...
    for (idx = 0; idx < drain_state_input->ulNumDisks; idx++) {
        ...
        memcpy_s(&drain_state_output->diskState[idx].DeviceId, sizeof(VOLUME_GUID),
        ...
        if (tgt_ctxt->tc_flags & VCF_DRAIN_BLOCKED) {
            ...
            drain_state_output->diskState[idx].ulFlags |= DISK_STATE_DRAIN_BLOCKED;
        }
        else {
            info("Draining is not blocked for uuid : %s\n", uuid);
            drain_state_output->diskState[idx].ulFlags |= DISK_STATE_FILTERED;
        }
        put_tgt_ctxt(tgt_ctxt);
...
inm_s32_t process_set_drain_state_ioctl(inm_devhandle_t *handle, void *arg)
{
    ...
    if (INM_COPYIN(drain_state_input, arg,
                    (sizeof(SET_DRAIN_STATE_INPUT) - sizeof(VOLUME_GUID)))) {
    ...
    drain_state_output->ulNumDisks = drain_state_input->ulNumDisks;
    for (idx = 0; idx < drain_state_input->ulNumDisks; idx++) {
        drain_state_output->diskStatus[idx].Status = SET_DRAIN_STATUS_UNKNOWN;
    }

    device_list_arg = arg + sizeof(SET_DRAIN_STATE_INPUT) - sizeof(VOLUME_GUID);
    guid = INM_KMALLOC(sizeof(VOLUME_GUID), INM_KM_SLEEP, INM_KERNEL_HEAP);
    if (!guid) {
        err("Failed to allocate VOLUME_GUID");
        error = INM_ENOMEM;
    ...
    for (idx = 0; idx < drain_state_input->ulNumDisks; idx++) {
        ...
        memcpy_s(&drain_state_output->diskStatus[idx].DeviceId, sizeof(VOLUME_GUID),
                    guid, sizeof(VOLUME_GUID));
        ...
        if (!tgt_ctxt) {
            err("The disk %s is not protected", uuid);
            error = -ENODEV;
            drain_state_output->diskStatus[idx].Status = SET_DRAIN_STATUS_DEVICE_NOT_FOUND;
            break;
        }
        info("Unblocking drain for disk : %s\n", uuid);
        ...
        if (ret) {
            ...
            error = INM_EFAULT;
            drain_state_output->diskStatus[idx].Status = SET_DRAIN_STATUS_PERSISTENCE_FAILED;
            drain_state_output->diskStatus[idx].ulInternalError = ret;
        }
        else {
            drain_state_output->diskStatus[idx].Status = SET_DRAIN_STATUS_SUCCESS;
        }
        put_tgt_ctxt(tgt_ctxt);
