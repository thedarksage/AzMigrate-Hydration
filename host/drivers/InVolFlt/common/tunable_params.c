...
/**
 * FUNCTION NAME:init_driver_tunable_params
 * 
 * DESCRIPTION :
 * 
 * INPUT PARAMETERS : 
...
void init_driver_tunable_params(void)
{
    ...
    driver_ctx->dc_tel.dt_seqno_in_persistent_store = driver_ctx->last_time_stamp_seqno;

    driver_ctx->tunable_params.max_data_pages_per_target = DEFAULT_MAX_DATA_PAGES_PER_TARGET;
	...
	(((driver_ctx->tunable_params.data_pool_size << (MEGABYTE_BIT_SHIFT-INM_PAGESHIFT)) * 
...
#endif
	/* 
	 * solaris hack - disabled system info part
	 */
...
/* Utility functions for sysfs - Start */
inm_s32_t write_common_attr(const char *file_name, void *buf, inm_s32_t len)
{
    ...
    snprintf(path, INM_PATH_MAX, "%s/%s/%s",PERSISTENT_DIR,COMMON_ATTR_NAME, file_name);
...
inm_s32_t read_common_attr(char *fname, void **buf, inm_s32_t len, inm_s32_t *bytes_read)
{
    inm_s32_t ret = 0;
    char *path = NULL;
    ...
    snprintf(path, INM_PATH_MAX, "%s/%s/%s",PERSISTENT_DIR,COMMON_ATTR_NAME, fname);
...
/* Utility functioned for sysfs - End */

/* START - Sysfs definition for common objects */
...
ssize_t
inm_time_reorg_data_pool_store(const char *file_name, const char *buf, size_t len)
{
    int time_sec = 0;
...
ssize_t
inm_time_reorg_data_pool_factor_store(const char *file_name, const char *buf, size_t len)
{
    int factor = 0;
    inm_irqflag_t lock_flag;
...
ssize_t inm_stable_pages_store(const char *file_name, const char *buf, size_t len)
{
...
ssize_t inm_chained_io_store(const char *file_name, const char *buf, size_t len)
{
    inm_s32_t val = 0;
...
set_default:
    driver_ctx->tunable_params.vacp_iobarrier_timeout = VACP_IOBARRIER_TIMEOUT;
free_buf:
    if(buf)
        INM_KFREE(buf, buf_len, INM_KERNEL_HEAP);
    return 0;
...
ssize_t
inm_vacp_iobarrier_timeout_store(const char *file_name, const char *buf, size_t len)
{
    int timeout = 0;
    inm_irqflag_t lock_flag;
    ...
    // driver hangs if timeout is more than 120 secs
    if (timeout <= 0) {
...
set_default:
    driver_ctx->tunable_params.vacp_app_tag_commit_timeout = VACP_APP_TAG_COMMIT_TIMEOUT;
free_buf:
    if(buf)
        INM_KFREE(buf, buf_len, INM_KERNEL_HEAP);
    return 0;
...
ssize_t
inm_vacp_app_tag_commit_timeout_store(const char *file_name, const char *buf, size_t len)
{
    int timeout = 0;
    inm_irqflag_t lock_flag;
...
ssize_t
inm_percent_change_data_pool_size_store(const char *file_name, const char *buf, size_t len)
{
    int percentage = 0;
    inm_meminfo_t meminfo;
    inm_u32_t nr_pages;
    ...
    INM_SI_MEMINFO(&meminfo);
    nr_pages =
    ((meminfo.totalram * percentage) / 100);
    ...
    INM_SPIN_UNLOCK_IRQRESTORE(&driver_ctx->tunables_lock, lock_flag);
    if (!write_common_attr(file_name, (void *)buf, len)) {
        err("PercentChangeDataPoolSize update failed to write file:%s.",
            file_name);
        return INM_EINVAL;
    }
...
inm_maxdatapool_sz_store(const char *file_name, const char *buf, size_t len)
{
    ...
    if(max_data_pool_limit < driver_ctx->data_flt_ctx.pages_allocated) {
        info(
        "MaxDataPoolSize:%u pages cannot be less than the current alloc'd pages:%u",
                max_data_pool_limit, driver_ctx->data_flt_ctx.pages_allocated);
        return -EINVAL;
    }
...
ssize_t log_dir_store(const char *file_name, const char *buf, size_t len)
{
	...
	INM_SPIN_LOCK_IRQSAVE(&driver_ctx->tunables_lock, lock_flag);
	if (strncpy_s(driver_ctx->tunable_params.data_file_log_dir, INM_PATH_MAX,
                                                                    buf, len)) {
	    INM_SPIN_UNLOCK_IRQRESTORE(&driver_ctx->tunables_lock, lock_flag);
	    return -INM_EFAULT;
	}
...
ssize_t vdf_store(const char *file_name, const char *buf, size_t len)
{
	...
	/* Here, we should ideally walk through all vcptr and dsiable data filtering
	   and switch all of them to metadata mode.
	*/
...
ssize_t vol_dbnotify_store(const char *file_name, const char *buf, size_t len)
{
    info("Reset Global DB Notify limit to %u", 
            driver_ctx->tunable_params.db_notify);
	driver_ctx->tunable_params.db_notify = driver_ctx->tunable_params.max_data_sz_dm_cn;
	return 0;
}
	...
	return -EINVAL;
        ...
        inm_list_for_each_safe(ptr, nextptr, &driver_ctx->tgt_list) {
            ...
            info("Update DB Notify Threshold from %u to %u", 
                 ...
                 driver_ctx->tunable_params.max_data_sz_dm_cn);
            vcptr->tc_db_notify_thres = driver_ctx->tunable_params.max_data_sz_dm_cn;
        }
        INM_UP_READ(&driver_ctx->tgt_list_sem);
    }
...
ssize_t read_pool_size(char *fname)
{
	...
	driver_ctx->tunable_params.data_pool_size = mem;
	info("data pool size set to %dMB", driver_ctx->tunable_params.data_pool_size);
		
	goto free_buf;
    }

set_default:
    driver_ctx->tunable_params.data_pool_size = driver_ctx->default_data_pool_size_mb;
...
ssize_t inm_read_vol_respool_sz(char *fname)
{
    ...
    if(!read_common_attr(fname, &buf, buf_len, &bytes_read)) {
		...
		mem =  inm_atoi(buf);
		if (mem >= 0 && mem <= driver_ctx->default_data_pool_size_mb ) {
    ...
    if(!read_common_attr(fname, &buf, buf_len, &bytes_read)) {
        ...
        if(bytes_read > 2) {
            err("Invalid percentage value. Setting MaxDataPoolSize "
                "based on %d%%", DEFAULT_MAX_DATA_POOL_PERCENTAGE);
            goto set_default;
        }
...
ssize_t read_log_dir(char *fname)
{
	...
	INM_SPIN_LOCK_IRQSAVE(&driver_ctx->tunables_lock, lock_flag);
	if (memcpy_s(driver_ctx->tunable_params.data_file_log_dir, INM_PATH_MAX,
						buf, bytes_read)) {
	    INM_SPIN_UNLOCK_IRQRESTORE(&driver_ctx->tunables_lock, lock_flag);
...
set_default:
    INM_SPIN_LOCK_IRQSAVE(&driver_ctx->tunables_lock, lock_flag);
    strcpy_s(driver_ctx->tunable_params.data_file_log_dir, INM_PATH_MAX, DEFAULT_VOLUME_DATALOG_DIR );
    INM_SPIN_UNLOCK_IRQRESTORE(&driver_ctx->tunables_lock, lock_flag);
...
set_default:
    driver_ctx->tunable_params.db_high_water_marks[SERVICE_NOTSTARTED] = DEFAULT_DB_HIGH_WATERMARK_SERVICE_NOT_STARTED;
free_buf:
    if(buf)
	INM_KFREE(buf, buf_len, INM_KERNEL_HEAP);
    return 0;
...
set_default:
    driver_ctx->tunable_params.db_high_water_marks[SERVICE_RUNNING] = DEFAULT_DB_HIGH_WATERMARK_SERVICE_RUNNING;
free_buf:
    if(buf)
	INM_KFREE(buf, buf_len, INM_KERNEL_HEAP);
    return 0;
...
set_default:
    driver_ctx->tunable_params.db_low_water_mark_while_service_running= DEFAULT_DB_LOW_WATERMARK_SERVICE_RUNNING;
free_buf:
    if(buf)
	INM_KFREE(buf, buf_len, INM_KERNEL_HEAP);
    return 0;
...
set_default:
    driver_ctx->tunable_params.db_high_water_marks[SERVICE_SHUTDOWN]= DEFAULT_DB_HIGH_WATERMARK_SERVICE_SHUTDOWN;
free_buf:
    if(buf)
	INM_KFREE(buf, buf_len, INM_KERNEL_HEAP);
    return 0;
...
set_default:
    driver_ctx->tunable_params.db_topurge_when_high_water_mark_is_reached = DEFAULT_DB_TO_PURGE_HIGH_WATERMARK_REACHED;
free_buf:
    if(buf)
	INM_KFREE(buf, buf_len, INM_KERNEL_HEAP);
    return 0;
...
set_default:
    driver_ctx->tunable_params.data_to_disk_limit = (DEFAULT_VOLUME_DATA_TO_DISK_LIMIT_IN_MB * MEGABYTES);
free_buf:
    if(buf)
	INM_KFREE(buf, buf_len, INM_KERNEL_HEAP);
    return 0;
...
ssize_t read_dbnotify(char *fname)
{
    if (driver_ctx->tunable_params.max_data_sz_dm_cn)
        ...
        driver_ctx->tunable_params.db_notify = DEFAULT_DB_NOTIFY_THRESHOLD;
    return 0;
}
...
ssize_t inm_read_seqno(char *fname)
{
	...
	driver_ctx->last_time_stamp_seqno = inm_atoi64(buf) + RELOAD_TIME_SEQNO_JUMP_COUNT;
	goto free_buf;
    }
...
ssize_t inm_read_ts(void)
{
	...
	if(!is_digit(buf, bytes_read))
	    goto set_default;
	driver_ctx->last_time_stamp = inm_atoi64(buf) + 2 * HUNDREDS_OF_NANOSEC_IN_SECOND;
...
/* read max data size for dm change node from disk */
ssize_t
inm_read_max_data_sz_dm_cn(char *fname)
{
    ...
    if(!read_common_attr(fname, &buf, buf_len, &bytes_read)) {
        ...
        driver_ctx->tunable_params.max_data_sz_dm_cn = inm_atoi64(buf) * (MEGABYTES);
        goto free_buf;
    }

set_default:
    driver_ctx->tunable_params.max_data_sz_dm_cn = DEFAULT_MAX_DATA_SZ_PER_CHANGE_NODE;
free_buf:
    if(buf)
        INM_KFREE(buf, buf_len, INM_KERNEL_HEAP);
...
void load_driver_params(void)
{
    ...
    num_attribs--; //For NULL at the end.
...
/* END - Sysfs definition for common objects */
...
inm_s32_t sysfs_involflt_init(void)
{
    ...
    /* FIXME: Need an API to emulate mkdir -p. Anybody who modified PERSISTENT_DIR need
     * to ensure the directories are created here.
     */
...
inm_read_reserv_dpsize(target_context_t *ctxt, char *fname)
{
    ...
    if(!read_vol_attr(ctxt, fname, &buf, buf_len, &bytes_read)) {
		...
		/* inm_atoi() returns a inm_u32_t value */
		thres = inm_atoi(buf);
		if ((thres >= 0) && 
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_MIRROR))){
    ...
    if (err) {
        ...
        /* add source volumes list to target context */
        inm_list_splice_at_tail(&src_mirror_list_head, &ctxt->tc_src_list);
        //list_change_head(&ctxt->tc_src_list, &src_mirror_list_head);
    }
error_case:
    if (err) {
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_MIRROR))){
    ...
    err = prepare_volume_list(buf, &dst_mirror_list_head, 1);
    if (err) {
        free_mirror_list(&dst_mirror_list_head, 1);
        goto error_case;
    }
    else {
        /* add destination volumes list to target context */
        inm_list_splice_at_tail(&dst_mirror_list_head, &ctxt->tc_dst_list);
    }
    if (!inm_list_empty(&ctxt->tc_dst_list)) {
        /* Get the first entry in the list and set it as tcp mirror_dev */
        vol_entry = inm_list_entry(ctxt->tc_dst_list.next, mirror_vol_entry_t, next);
        INM_BUG_ON(!vol_entry);
        ctxt->mirror_dev = &(vol_entry->mirror_dev);

#ifndef INM_AIX
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_MIRROR))){
        info("Setting up mirror for source:%s mirror dev:%s(%d,%d)",
             ctxt->tc_guid, vol_entry->tc_mirror_guid,
#if (defined(INM_LINUX))
             INM_GET_MAJOR((*ctxt->mirror_dev)->bd_dev),
             INM_GET_MINOR((*ctxt->mirror_dev)->bd_dev)
#endif
#if (defined(INM_SOLARIS))
             INM_GET_MAJOR(*(*ctxt->mirror_dev)),
             INM_GET_MINOR(*(*ctxt->mirror_dev))
#endif
             );
}
#endif
...
error_case:
    if (err) {
        err("Failed to read destination volumes of mirror setup");
    }
    if (buf) {
        INM_KFREE(buf, INM_MAX_VOLUMES_IN_LIST*INM_GUID_LEN_MAX, INM_KERNEL_HEAP);
    }
...
vol_rpo_ts_read(target_context_t *ctxt, char * fname)
{
    ...
    if (!read_vol_attr(ctxt, fname, &buf, buf_len, &bytes_read)) {
        ...
        if (!is_digit(buf, bytes_read)) {
            ...
            /* Read the last value for RPO timestamp */
...
vol_rpo_ts_store(target_context_t *ctxt, char * file_name, const char *buf, inm_s32_t len)
{
     ...
     if (!is_digit(buf, len)) {
         err("Invalid value for RPO timestamp = %s : has non-digit chars",
             buf);
         return -EINVAL;
     }
...
vol_drain_blocked_store(target_context_t *ctxt, char * file_name, const char *buf, inm_s32_t len)
{
     ...
     if (!is_digit(buf, len)) {
         err("Invalid value for volume drain blocked = %s : has non-digit chars",
             buf);
         return -EINVAL;
     }
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
     ...
     * symlinks if any any before creating persistent directory
     */
