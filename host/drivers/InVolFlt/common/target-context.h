...
/* TODOs: This structure is used for target context statistics */
...
typedef struct bitmap_info  {
    ...
    unsigned long   bitmap_granularity;    /* TODOs : doc */
    unsigned long   bitmap_offset;        /* TODOs: doc */
...
typedef struct _target_context {
    ...
    /* TODO: Data Structures for DB notification. */
...
/* change the name of macro */
/* TODOs: is_data_filtering_enabled_for_this_target()
 * check for driver_context->service_supports_data_filtering
 * check for driver_context->enable_data_filtering
 * with '||' operator
 **/
...
#define should_wakeup_s2(ctx) \
	(((((target_context_t *)ctx)->tc_bytes_pending_changes >= \
        ((target_context_t *)ctx)->tc_db_notify_thres) || \
        (((((target_context_t *)ctx)->tc_cur_mode != FLT_MODE_DATA) && \
         ((target_context_t *)ctx)->tc_pending_changes >= MAX_CHANGE_INFOS_PER_PAGE))) ? 1 : 0)
#endif
...
/* abstract functions for inm_device_type */
