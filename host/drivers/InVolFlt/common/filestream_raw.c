...
fstream_raw_print_map(char *file, fstream_raw_hdl_t *hdl)
{
    ...
    info("HDL2: nbsize = %u, bshift = %u, blks = %u, npgs = %u",
        hdl->frh_bsize, hdl->frh_bshift, hdl->frh_nblks, hdl->frh_npages);
    info("HDL3: Blkmap");
        ...
        for (j = 0; j < FSRAW_BLK_PER_PAGE && nblks; nblks--, j++) {
            if (doffset != 0) { /* previous valid block */
                ...
                if (((hdl->frh_blocks[i][j]).fb_disk == disk &&
                    (doffset + len) == (hdl->frh_blocks[i][j]).fb_offset)) {
                    len += hdl->frh_bsize;
                } else {
                    inm_blkdev_name(disk, diskname);
                    info("%16llu %16s %16llu %16u", 
                        foffset, diskname, doffset >> INM_SECTOR_SHIFT, len);
                    foffset += len;
                    doffset = 0;
                }
            }
...
fstream_raw_map_bio(inm_buf_t *bio)
{
    ...
    if (hdl->frh_bsize < PAGE_SIZE) {
        ...
        if ((INM_BUF_COUNT(bio) > hdl->frh_bsize) || INM_BUF_OFFSET(bio)) { 
            err("LCW Learn: bsize = %u, io size = %u, page offset = %lu", 
                hdl->frh_bsize, INM_BUF_COUNT(bio), 
                (long unsigned int)INM_BUF_OFFSET(bio));
            hdl->frh_nblks = 0;
...
fstream_raw_hdl_free(fstream_raw_hdl_t *hdl)
{
    ...
    i = sizeof(fstream_raw_hdl_t) + (sizeof(fr_block_t *) * hdl->frh_npages);
    dbg("Free HDL: %d", i);
                      ...
                      fstream_raw_hdl_t **rhdl)
{
    ...
    /* TODO: Need to align offset to block size if > 0. Not supp for now */
    if (offset)
        return -EINVAL;
    ...
    hdl = INM_KMALLOC(sizeof(fstream_raw_hdl_t) + 
            (sizeof(fr_block_t *) * npages), INM_KM_SLEEP, INM_KERNEL_HEAP);
    if (!hdl) {
        error = -ENOMEM;
        goto out;
    }
    ...
    hdl->frh_len = len;
    hdl->frh_alen = ALIGN(len, (INM_HDL_TO_INODE(filp))->i_sb->s_blocksize);
    ...
    hdl->frh_blocks = (fr_block_t **)((char *)hdl + sizeof(fstream_raw_hdl_t));
    for (hdl->frh_npages = 0; hdl->frh_npages < npages; hdl->frh_npages++) {
                        ...
                        fstream_raw_hdl_t **raw_hdl)
{
    ...
    while (len) {
        ...
        /*
        fr_page = inm_get_page_cache_page(offset);
        err("fr_page = %p", fr_page);
        */     

        if (!flt_write_file(filp, buf, offset, iosize, &iodone) || 
            iodone != iosize) {
            err("Write Failed: %llu:%u:%u", offset, iosize, iodone);
            error = -EIO;
            break;
        }
               ...
               inm_u64_t offset, int write)
{
    ...
    while (len) {
        ...
        error = fstream_raw_perform_block_io(disk, buf, doffset, iosize, write);
        if (error)
            break;
