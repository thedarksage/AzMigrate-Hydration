 ...
 *create_service_thread
 ...
 process_volume_state_change
 service_state_change_thread
...
/**
 * FUNCTION NAME:create_service_thread
 * 
 * DESCRIPTION :
 * 
 * INPUT PARAMETERS : 
...
#ifdef INM_LINUX
    pid = INM_KERNEL_THREAD(service_thread_task, service_state_change_thread, NULL, 0, "inmsvcd");
#else
    pid = INM_KERNEL_THREAD(service_state_change_thread, NULL, 0, "inmsvcd");
#endif
    if (pid >= 0) {
...
/**
 * FUNCTION NAME:destroy_service_thread
 * 
 * DESCRIPTION :
 * 
 * INPUT PARAMETERS : 
...
/**
 * FUNCTION NAME:process_volume_state_change
 * 
 * DESCRIPTION :
 * 
 * INPUT PARAMETERS : 
 ...
 * 
 **/
inm_s32_t process_volume_state_change(target_context_t *tgt_ctxt, struct inm_list_head *lhptr, inm_s32_t statechanged)
{
...
#define vcptr     tgt_ctxt
    ...
    /** TODOs: Need to handle this  */
    if (tgt_ctxt->tc_dev_state == DEVICE_STATE_OFFLINE) {
        dbg("process_volume_state_change():: device marked OFFLINE \n");
        /* We cannot have DEVICE_STATE_OFFLINE so it benign to have BUG_ON here */
        INM_BUG_ON(1);

        //delete volume context from driver context linked list
        //inm_list_del(&tgt_ctxt->list_entry);
        //driver_ctx->num_volumes--;
        success = add_vc_workitem_to_list(WITEM_TYPE_VOLUME_UNLOAD, tgt_ctxt, 0, 0, lhptr);
        dbg("Added target context work item ... \n");        
        remove_tc_from_dc(tgt_ctxt); // TODOs: beta release: revisit
        //put_tgt_ctxt(tgt_ctxt); //dereference driver context linked list entry

        return success;
    }

#define __IS_FLAG_SET(_flag)    (vcptr->tc_flags & (_flag))
    // volume filtering disabled
    if (__IS_FLAG_SET(VCF_FILTERING_STOPPED))
        return success;

    /* TODOs: 
     * if (IS_VOLUME_OFFLINE(vcptr))
     *    return success;
     **/
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
        ...
        if (_HIGH_WATER_MARK(SERVICE_NOTSTARTED) && 
            ...
            (vcptr->tc_pending_changes >= _HIGH_WATER_MARK(SERVICE_NOTSTARTED))) {
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
         ...
         *        change pending < LOW_WATER_MARK
         **/
        if (_LOW_WATER_MARK && !__IS_FLAG_SET(VCF_BITMAP_READ_DISABLED) &&
            (vcptr->tc_pending_changes < _LOW_WATER_MARK)) {
            ...
            if (!vcptr->tc_bp->volume_bitmap) {
                ...
                case ecVBitmapStateReadPaused:
                    work_item_type = WITEM_TYPE_CONTINUE_BITMAP_READ;
                    open_bitmap = FALSE;
                default:
                    break;
                }
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
        ...
        if (statechanged) {
            ...
            if (vcptr->tc_cur_wostate != ecWriteOrderStateRawBitmap)
                set_tgt_ctxt_wostate(vcptr, ecWriteOrderStateBitmap, TRUE,
                                     eCWOSChangeReasonServiceShutdown);
	    // spin_lock_irqrestore(&vcptr->lock, lock_flag);
	    if (!__IS_FLAG_SET(VCF_BITMAP_WRITE_DISABLED)) {
		work_item_type = WITEM_TYPE_BITMAP_WRITE;
		open_bitmap = TRUE;
	    }
	    break; // move out of switch
        }

        if (_HIGH_WATER_MARK(SERVICE_SHUTDOWN) &&
            !__IS_FLAG_SET(VCF_BITMAP_WRITE_DISABLED) &&
            (vcptr->tc_pending_changes >= _HIGH_WATER_MARK(SERVICE_SHUTDOWN))) {
...
/**
 * FUNCTION NAME:service_state_change_thread
 * 
 * DESCRIPTION :
 * 
 * INPUT PARAMETERS : 
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
	...
	// TODOs: dc lock required for modifying flags
