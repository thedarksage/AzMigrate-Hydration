 ...
 *init_work_queue
 cleanup_work_queue
 *alloc_work_queue_entry
 cleanup_work_queue_entry
 get_work_queue_entry
 put_work_queue_entry
 add_item_to_work_queue
 generic_worker_thread_function
...
#if 0
    INM_SPIN_LOCK(&driver_ctx->wq_entry_lock);
    if(list_empty(&driver_ctx->wq_entry_pool)){
	INM_SPIN_UNLOCK(&driver_ctx->wq_entry_lock);
	goto out;
    }

    wqe = inm_list_entry(driver_ctx->wq_entry_pool.next, wqentry_t, list_entry);
    inm_list_del(&wqe->list_entry);
    driver_ctx->nr_wq_entry_used++;
    INM_SPIN_UNLOCK(&driver_ctx->wq_entry_lock);
#endif
...
#if 0
    INM_SPIN_LOCK(&driver_ctx->wq_entry_lock);
    inm_list_add_tail(&wqe->list_entry, &driver_ctx->wq_entry_pool);
    driver_ctx->nr_wq_entry_used--;
    INM_SPIN_UNLOCK(&driver_ctx->wq_entry_lock);
#endif
    wqe = NULL;
...
{
    //workq_t *work_q = (workq_t *)context;
    /* TODO assigned the address directly from driver_ctx */
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
    ...
    timeout_val = INM_MSECS_TO_JIFFIES(INM_MSEC_PER_SEC);
    //initialize list for volume contexts
    while (1) {
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
	/*info("waiting for new event completion in worker thread \n");*/
}
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
	/*info("worker thread wakeup_event %d shutdown_event %d \n", wakeup_event,
             shutdown_event);*/
}
	if (shutdown_event > 0) {
	    INM_ATOMIC_DEC(&work_q->shutdown_event_raised);
	...
	if (!wakeup_event) {
	    /* Temporary hack: to flush the time on every 1 sec */
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
	/*info("received wakeup event in worker thread\n");
	info("worker thread wakeup_event_raised %d ",
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
	    /*info ("processing wqe in workq");
	    info("work queue entry = %p", wqeptr);
	    info("work item type = %d, refcnt = %d",
	    ...
	    /* temporary code for statistics 
	      note : it is possible that context could be anything 
			- vbmap/vcptr/cnode	
		target_context_t *vcptr = NULL;
	    INM_BUG_ON((((bitmap_work_item_t *) wqeptr->context)->eBitmapWorkItem < 0));
	    temporary code for statistics 
	    if (wqeptr->witem_type) 
		   vcptr = ((bitmap_work_item_t *) wqeptr->context)->volume_bitmap->volume_context;
	       if (vcptr) {
	       volume_bitmap_t *vbmap = NULL;

	       down(&vcptr->sem);
	       vbmap = vcptr->volume_bitmap;
	       if ((vcptr->mode != FLT_MODE_RAWBITMAP) && vbmap) {
	       down(&vbmap->sem);
	       INM_BUG_ON(!vbmap->bitmap_api);
	       INM_BUG_ON(!vbmap->bitmap_api->fs);
	       vcptr->nr_bytes_in_bmap = 
	       bitmap_api_get_dat_bytes_in_bitmap(
	       vcptr->volume_bitmap->bitmap_api);
	       up(&vbmap->sem);
	       } else {
	       info("volume = %s in raw mode \n", vcptr->volume_name);
	       }
	       up(&vcptr->sem);
	       } */
...
static inm_s32_t reorg_datapool(inm_u32_t reorg_flag)
{
	...
	if(diff_sec >= thrshld_time || alloc_always){
		...
		if(!prev_prot_vols || alloc_always){
			...
			dbg("reconfig to allocate %u pages, alloc_always is %u, prev_prot volumes %u", nr_pages, alloc_always, prev_prot_vols);
		 ...
		 * The following 3 line of code is doing below, a work around to linux do_div issue.
		 * //expect_free = cur_free_pages - ((cur_prot_vols * rate * 2 * thrshld_time) / prev_prot_vols);//
		 */
