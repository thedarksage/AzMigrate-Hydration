...
get_iobuffer_cache_ptr(fstream_segment_mapper_t *fssm, inm_u32_t buffer_index)
{
    ...
    inm_s32_t page_index = (buffer_index*(sizeof(iobuffer_t*))/BITMAP_FILE_SEGMENT_SIZE);

if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
    info("entered: buffer_index:%d page_index:%d", buffer_index, page_index);
}
    if (fssm->buffer_cache_index) {
        page_ptr = (unsigned char **)fssm->buffer_cache_index[page_index];
...
iobuffer_t*
reset_iobuffer_cache_ptr(fstream_segment_mapper_t *fssm, inm_u32_t buffer_index, iobuffer_t *ptr)
{
    inm_s32_t page_index = (buffer_index*(sizeof(iobuffer_t*))/BITMAP_FILE_SEGMENT_SIZE);
    iobuffer_t *iob = NULL;
    unsigned char **page_ptr = NULL;
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
    ...
    dbg("index:%d out iob:%p page_ptr:%p in ptr:%p", buffer_index, iob, page_ptr, ptr);

    return iob;
}

/**
 * FUNCTION NAME:fstream_segment_mapper_ctr
 * 
 * DESCRIPTION :
 * 
 * INPUT PARAMETERS : 
...
fstream_segment_mapper_t *fstream_segment_mapper_ctr()
{
	...
	fssm = (fstream_segment_mapper_t *)INM_KMALLOC(sizeof(*fssm), INM_KM_SLEEP, INM_KERNEL_HEAP);
...
/**
 * FUNCTION NAME:fstream_segment_mapper_dtr
 * 
 * DESCRIPTION :
 * 
 * INPUT PARAMETERS : 
    ...
    if (fssm->buffer_cache_index)
    {
        ...
        while (index != npages) {
            dbg("Freeing buffer page:%p", fssm->buffer_cache_index[index]);
             INM_KFREE((unsigned char*)fssm->buffer_cache_index[index],BITMAP_FILE_SEGMENT_SIZE,
                       INM_KERNEL_HEAP);
             fssm->buffer_cache_index[index] = NULL;
             index++;
        }
        dbg("Freeing buffer index page:%p", fssm->buffer_cache_index);
        INM_KFREE(fssm->buffer_cache_index,BITMAP_FILE_SEGMENT_SIZE, INM_KERNEL_HEAP);
...
/**
 * FUNCTION NAME:fstream_segment_mapper_attach
 * 
 * DESCRIPTION :
 * 
 * INPUT PARAMETERS : 
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
    ...
    if (fssm->buffer_cache_index) {
        INM_MEM_ZERO(fssm->buffer_cache_index, BITMAP_FILE_SEGMENT_SIZE);
        ...
        while (index != npages) {
            ...
            INM_MEM_ZERO(page_ptr[index], BITMAP_FILE_SEGMENT_SIZE);
            index++;
        }
        ...
        if (index && index != npages) {
            ...
            do {
                ...
                INM_KFREE(page_ptr[index], BITMAP_FILE_SEGMENT_SIZE, INM_KERNEL_HEAP);
                page_ptr[index] = NULL;
            } while (index != 0);

            dbg("Freeing buffer index page:%p", page_ptr);
            INM_KFREE(page_ptr, BITMAP_FILE_SEGMENT_SIZE, INM_KERNEL_HEAP);
            page_ptr = NULL;
            fssm->buffer_cache_index =  NULL;
        }
        else
...
/**
 * FUNCTION NAME:fstream_segment_mapper_read_and_lock
 * 
 * DESCRIPTION :
 * 
 * INPUT PARAMETERS : 
...
inm_s32_t fstream_segment_mapper_read_and_lock(fstream_segment_mapper_t *fssm,
                                         inm_u64_t offset,
                                         unsigned char **return_iobuf_ptr,
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
    ...
    //buffer_index = (inm_u32_t) (offset / fssm->segment_size);
    ...
    if (get_iobuffer_cache_ptr(fssm, buffer_index) == NULL)
    {
        ...
        if (fssm->nr_free_buffers)
        {
            ...
            iob = iobuffer_ctr(fssm->bapi, fssm->segment_size, buffer_index);
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
        ...
        if (iob == NULL)
        {
            ...
            if (ret) {
                ...
                // add to head
                //list_add(&iob->list_entry, &fssm->segment_list); 
                inm_list_add_tail(&iob->list_entry, &fssm->segment_list); 
                iob = NULL;
            }
            ...
            if (iob) {
                ...
                reset_iobuffer_cache_ptr(fssm, iobuffer_get_owner_index(iob), NULL);
        ...
        if (!iob) {
            ...
            iobuffer_set_foffset(iob, 
				 fssm->starting_offset + (buffer_index * fssm->segment_size));
            ...
            if (ret) { //failed
                ...
                reset_iobuffer_cache_ptr(fssm, buffer_index, iob);
                ...
                //inm_list_add_tail(&iob->list_entry, &fssm->segment_list);
                inm_list_add(&iob->list_entry, &fssm->segment_list);
            }
        }
    } else {
        fssm->nr_cache_hits++;
    ...
    if (get_iobuffer_cache_ptr(fssm, buffer_index) != NULL) {
        ...
        /* 64bit division is not directly allowed on linux 32bit kernels$
         * data_size = (inm_u32_t)
         * (fssm->segment_size - (offset % (inm_u64_t) fssm->segment_size));
         **/
...
/**
 * FUNCTION NAME:fstream_segment_mapper_unlock_and_mark_dirty
 * 
 * DESCRIPTION :
 * 
 * INPUT PARAMETERS : 
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
    ...
    // 64 bit division 
    // do_div(_offset, fssm->segment_size);
    // buffer_index = (inm_u32_t)_offset;
    // OR
    // buffer_index = (inm_u32_t)(offset / (inm_u64_t)fssm->segment_size);
    INM_DO_DIV(_offset, fssm->segment_size);
    buffer_index = (inm_u32_t)_offset;
...
/**
 * FUNCTION NAME:fstream_segment_mapper_unlock
 * 
 * DESCRIPTION :
 * 
 * INPUT PARAMETERS : 
...
/**
 * FUNCTION NAME:fstream_segment_mapper_flush
 * 
 * DESCRIPTION :
 * 
 * INPUT PARAMETERS : fssm
 *               offset
 *              
 *              
 * 
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
    ...
    if (iob == NULL)
        return EINVAL;
    // TODOs: checking for dirty iobuffer
    ret = iobuffer_sync_flush(iob);
...
/**
 * FUNCTION NAME:fstream_segment_mapper_sync_flush_all
 * 
 * DESCRIPTION :
 * 
 * INPUT PARAMETERS : fssm
