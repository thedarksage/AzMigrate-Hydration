...
telemetry_put_exception(exception_buf_t *buf)
{
	if (INM_ATOMIC_DEC_AND_TEST(&buf->eb_refcnt)) {
        if (buf == &exception_none || buf == &exception_enomem) {
            err("BUG: Trying to put default exception");
            INM_BUG_ON(buf == &exception_none ||  buf == &exception_enomem);
        } else {
    		INM_KFREE(buf, sizeof(exception_buf_t), INM_KERNEL_HEAP);
        }
    }
}
...
telemetry_get_exception(void)
{
    ...
    if (exception_gen) /* Exceptions */
        buf = INM_KMALLOC(sizeof(exception_buf_t), INM_KM_NOSLEEP | INM_KM_NOIO, 
                          INM_KERNEL_HEAP);
    if (buf) {
        ret = buf;
        ...
        sprintf_s(buf->eb_buf + offset, INM_EXCEPTION_BUFSZ - offset, "]}");
    } else {
        if (exception_gen)
