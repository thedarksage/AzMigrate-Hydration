...
telemetry_set_dbs(inm_u64_t *state, inm_u64_t flag)
{
    ...
    *state |= flag;
    INM_SPIN_UNLOCK_IRQRESTORE(&driver_ctx->dc_tel.dt_dbs_slock, lock_flag);
}
...
void
telemetry_tag_stats_record(target_context_t *tgt_ctxt,
			    					 tgt_stats_t *stats)
{
...
telemetry_tag_common_put(tag_telemetry_common_t *tag_common)
{
	if (INM_ATOMIC_DEC_AND_TEST(&tag_common->tc_refcnt))
		INM_KFREE(tag_common, sizeof(tag_telemetry_common_t), INM_KERNEL_HEAP);
}
...
telemetry_tag_common_alloc(inm_s32_t ioctl_cmd)
{
    ...
    switch (ioctl_cmd) {
        case IOCTL_INMAGE_IOBARRIER_TAG_VOLUME:
            ...
            sprintf_s(tag_common->tc_guid, sizeof(tag_common->tc_guid), "%s",
	...
	if (tag_common) {
	    ...
	    tag_common->tc_req_time = driver_ctx->dc_tel.dt_last_tag_request_time;
							 ...
							 tag_history_t *tag_hist)
{
    ...
    tag_hist->th_prev_succ_tag_time = tgt_ctxt->tc_tel.tt_prev_succ_tag_time;
    ...
    tgt_ctxt->tc_tel.tt_prev_tag_time = tag_hist->th_tag_common->tc_req_time;
                           ...
                           etWOSChangeReason reason)
{
    ...
    nwo->nws_bmap_pending = tgt_ctxt->tc_bp->num_changes_queued_for_writing; 
    ...
    nwo->nws_mem_alloc = tgt_ctxt->tc_stats.num_pages_allocated * PAGE_SIZE; 
...
telemetry_check_time_jump(void)
{
    ...
    if (prev_time) {
       /* Expected time */
       prev_time += TELEMETRY_MSEC_TO_100NSEC(TELEMETRY_FILE_REFRESH_INTERVAL);
