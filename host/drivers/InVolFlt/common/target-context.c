 ...
 *
 * File       : target-context.c
 *
 * Description: 
 
 target_context_ctr
 target_context_dtr
 *get_tgt_ctxt_from_uuid
 set_tgt_ctxt_filtering_mode
 set_malloc_fail_error
 can_switch_to_data_filtering_mode
 process_vcontext_work_items
 *get_volume_override
 fill_bitmap_filename_in_volume_context
 set_bitmap_open_fail_due_to_loss_of_changes
 set_bitmap_open_error
 can_open_bitmap_file
 flush_and_close_bitmap_file
 inmage_flt_save_all_changes
 is_data_filtering_enabled_for_this_volume
 log_bitmap_open_success_event
...
retry:
	...
	if ((data_pool_size - driver_ctx->data_flt_ctx.pages_allocated) < tc_res_pages) {
	    ...
	    len = sprintf(new_data_pool_size,"%u",tc_res_pages + data_pool_size);
	    ret = wrap_common_attr_store(DataPoolSize, new_data_pool_size, len);
	} else {
	    INM_SPIN_UNLOCK_IRQRESTORE(&driver_ctx->data_flt_ctx.data_pages_lock, lock_flag);
	}
...
target_context_ctr(void)
{
    ...
    ctx = (target_context_t *) INM_KMALLOC(sizeof (*ctx), INM_KM_SLEEP, INM_PINNED_HEAP);
    if (ctx == NULL) {
        return NULL;
    }
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
    ...
    ctx->tc_data_log_dir = INM_KMALLOC(INM_PATH_MAX, INM_KM_SLEEP, INM_KERNEL_HEAP);
    if(!ctx->tc_data_log_dir) {	
	    err = -ENOMEM;
	    goto out_err;
    }
    ctx->tc_bp = INM_KMALLOC(sizeof(struct bitmap_info), INM_KM_SLEEP, INM_PINNED_HEAP);
    if (!ctx->tc_bp) {
    ...
    ctx->tc_mnt_pt = INM_KMALLOC(INM_PATH_MAX, INM_KM_SLEEP, INM_KERNEL_HEAP);
    if(!ctx->tc_mnt_pt) {
        INM_KFREE(ctx->tc_data_log_dir, INM_PATH_MAX, INM_KERNEL_HEAP);
        INM_KFREE(ctx->tc_bp, sizeof(struct bitmap_info), INM_PINNED_HEAP);
        err = -ENOMEM;
    ...
    ctx->tc_db_v2 = (UDIRTY_BLOCK_V2 *)INM_KMALLOC(sizeof(UDIRTY_BLOCK_V2), INM_KM_SLEEP, INM_KERNEL_HEAP);
    if(!ctx->tc_db_v2) {
        INM_KFREE(ctx->tc_data_log_dir, INM_PATH_MAX, INM_KERNEL_HEAP);
        INM_KFREE(ctx->tc_bp, sizeof(struct bitmap_info), INM_PINNED_HEAP);
    ...
    ctx->tc_data_to_disk_limit = driver_ctx->tunable_params.data_to_disk_limit;
    ...
    init_latency_stats(&ctx->tc_dbwait_notify_latstat, inm_dbwait_notify_lat_bkts_in_usec);
    init_latency_stats(&ctx->tc_dbcommit_latstat, inm_dbcommit_lat_bkts_in_usec);
    ...
    /* mirror_dst_id_get() can return NULL for dst_scsi_id and compiler
     * complains about using NULL pointer in strncmp(). Disable for now.
     */
    /*
    if ((ctx->tc_dev_type == FILTER_DEV_MIRROR_SETUP) &&
        (mirror_dst_id_get(ctx, dst_scsi_id) ||
        strncmp(dst_scsi_id, dev_info->d_dst_scsi_id, INM_MAX_SCSI_ID_SIZE))) {
        if ((err = mirror_dst_id_set(ctx, dev_info->d_dst_scsi_id))) {
            err("tgt_ctx_eommon_init: failed for %s to set filter_dev_type err = %d",
            ctx->tc_guid, err);
            goto ret3;
        }
...
tgt_ctx_common_deinit(target_context_t *ctx)
{
    ...
    ctx->tc_flags &= VCF_VOLUME_CREATING | VCF_VOLUME_DELETING | VCF_IN_NWO;
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
    ...
    if(ctx->tc_datafile_dir_name) {
        INM_KFREE(ctx->tc_datafile_dir_name, INM_GUID_LEN_MAX, INM_KERNEL_HEAP);
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
    ...
    case FILTER_DEV_FABRIC_LUN:
        if ((error = fabric_volume_init(ctx, (inm_dev_info_t*)dev_info))) {
			goto out;
        }
        error = 0;
	break;
...
check_for_tc_state(target_context_t *tgt_ctx, int write_lock)
{
    ...
    cdw_item = INM_KMALLOC(sizeof(struct create_delete_wait), INM_KM_NOIO, INM_KERNEL_HEAP);
    if(!cdw_item){
	INM_DELAY(3 * INM_HZ);
	return 1;
    }
    ...
    INM_KFREE(cdw_item, sizeof(struct create_delete_wait), INM_KERNEL_HEAP);
...
wake_up_tc_state(target_context_t *tgt_ctx)
{
    ...
    while(!inm_list_empty(&tgt_ctx->cdw_list)){
       cdw_item = inm_list_entry(tgt_ctx->cdw_list.next, struct create_delete_wait, list);
       INM_DOWN(&tgt_ctx->cdw_sem);
...
/* 
 * TODO: Eventually move all tgt ctxt lookups to this function so we
 * can transparently work with current and persistent device name 
 * interchanghably.
 */
target_context_t *
        ...
        if (tgt_ctxt->tc_dev_type == FILTER_DEV_HOST_VOLUME ||
            ...
            if (hdcp) {
                ...
                __inm_list_for_each(hptr, &hdcp->hdc_dev_list_head) {
                    hdc_dev = inm_list_entry(hptr, host_dev_t, hdc_dev_list);
                    if (hdc_dev->hdc_dev == dev) {
                        dbg("uuid %s uuid_dev %u dev %u", uuid, dev, hdc_dev->hdc_dev);
                        break;
                    }
                    hdc_dev = NULL;
                }
...
target_context_t *get_tgt_ctxt_from_uuid_nowait(char *uuid)
{
    ...
    if(tgt_ctxt && (tgt_ctxt->tc_flags & (VCF_VOLUME_CREATING | VCF_VOLUME_DELETING)))
        tgt_ctxt = NULL;
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
    info("entered cur mode:%d req mode:%d", tgt_ctxt->tc_cur_mode, new_flt_mode);
}

    /* validate the inputs and store the volume */
    /* TODOs : write assert macro*/
    ...
    /* Bug #7718: The time (last_time) of switching to one of the mode is
	...
	/* TODOs: update counter (nr_times_changed_data_flt_mode) */
	if (service_initiated) {
	    /* TODOs: SERVICE initiated from bitmap mode */
	    /* TODOs: ADDING TAG FUNCTION */
	} else {
	    /* TODOs: SERVICE initiated from metadata mode */
	    /* TODOs: ADDING TAG FUNCTION */
        }
        break;
    ...
    /* TODOs: declaring previous filtering mode */
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
    info("leaving cur mode:%d req mode:%d", tgt_ctxt->tc_cur_mode, new_flt_mode);
}
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
    info("entered cur write order state:%d req write order state:%d", tgt_ctxt->tc_cur_wostate, new_wostate);
}
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
    info("leaving cur write order state:%d req write order state:%d", tgt_ctxt->tc_cur_wostate, new_wostate);
}
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
	...
	/* TODOs: Logging no memory event : this may not required in linux*/
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
    ...
    if (!(tgt_ctxt->tc_optimize_performance && PERF_OPT_DATA_MODE_CAPTURE_WITH_BITMAP))
    {
	 ...
	 */
    if (tgt_ctxt->tc_stats.num_pages_allocated > tgt_ctxt->tc_reserved_pages) {
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
    ...
    if(!tgt_ctxt->tc_bp || !tgt_ctxt->tc_bp->volume_bitmap || (ecVBitmapStateReadCompleted != tgt_ctxt->tc_bp->volume_bitmap->eVBitmapState))
	return FALSE;
...
void
add_changes_to_pending_changes(target_context_t *ctx, etWriteOrderState wostate, inm_u32_t num_changes)
{
    switch(wostate){
	case ecWriteOrderStateData:
...
void
subtract_changes_from_pending_changes(target_context_t *ctx, etWriteOrderState wostate, inm_u32_t num_changes)
{
    switch(wostate){
	case ecWriteOrderStateData:
	    ctx->tc_pending_wostate_data_changes -= num_changes;
...
fs_freeze_volume(target_context_t *ctxt, struct inm_list_head *head)
{
...
#ifdef INM_LINUX
        vinfo->bdev = inm_open_by_devnum(hdc_dev->hdc_dev, FMODE_READ | FMODE_WRITE);
#endif
	...
	while(!inm_list_empty(head)){
        ...
        if (success) {
            err("Freeze Failed for one or more paths of device %s", ctxt->tc_guid);
        }
        else {
...
thaw_volume(target_context_t *ctxt, struct inm_list_head *head)
{
    ...
    while(!inm_list_empty(head)){
        ...
        inm_thaw_bdev(vinfo->bdev, vinfo->sb);
        if (vinfo->sb) {
            dbg("Thaw successful for device %s for filesystem", ctxt->tc_guid);
        } else {
...
target_context_t *get_tc_using_dev(inm_dev_t rdev)
{
...
retry:
    inm_list_for_each_safe(ptr, nextptr, &driver_ctx->tgt_list) {
        ...
        if(check_for_tc_state(vcptr, 0)){
            vcptr = NULL;
            goto retry;
        }

        if (vcptr->tc_dev_type == rdev)
            break;
        /* if there is no match, last tc should not be the victim */
        vcptr = NULL;
    }
    INM_UP_READ(&driver_ctx->tgt_list_sem);
...
{
    info("Target Context destroyed %d:%d device:%s",INM_GET_MAJOR(inm_dev_id_get(ctxt)),\
...
inm_validate_tc_devattr(target_context_t *tcp, inm_dev_info_t *dip)
{
	...
	case FILTER_DEV_FABRIC_VSNAP:
		/*
		 * TODOs: Yet to implement
		 * inm_validate_fabric_vsnap(tcp, dip);
		 */
...
void do_clear_diffs(target_context_t *tgt_ctxt)
{
    ...
    /* CRIT FIX: bug # 6775: data corruption issue,
     * caused due to releasing the data pages of 
    ...
    //TODOs: verify with window's code
...
/* bitmap related data */
    ...
    //TODOs: clear volume stats
    tgt_ctxt->tc_hist.ths_clrdiff_ts = INM_GET_CURR_TIME_IN_SEC;
    tgt_ctxt->tc_hist.ths_nr_clrdiffs++;

    volume_unlock(tgt_ctxt);

    /* The following code frees change node list... where free_changenode_list also does the same thing.... Let's keep one code path
     */
...
void
retrieve_volume_latency_stats(target_context_t *tcp, VOLUME_LATENCY_STATS *vlsp)
{
	inm_u32_t	idx, o_idx;
	...
	memcpy_s(vlsp->s2dbret_bkts, sizeof(tcp->tc_dbret_latstat.ls_bkts),
		tcp->tc_dbret_latstat.ls_bkts, sizeof(tcp->tc_dbret_latstat.ls_bkts));
	memcpy_s(vlsp->s2dbret_freq, sizeof(tcp->tc_dbret_latstat.ls_freq),
		tcp->tc_dbret_latstat.ls_freq, sizeof(tcp->tc_dbret_latstat.ls_freq));
	...
	memcpy_s(vlsp->s2dbwait_notify_bkts, sizeof(tcp->tc_dbwait_notify_latstat.ls_bkts),
		tcp->tc_dbwait_notify_latstat.ls_bkts, sizeof(tcp->tc_dbwait_notify_latstat.ls_bkts));
	memcpy_s(vlsp->s2dbwait_notify_freq, sizeof(tcp->tc_dbwait_notify_latstat.ls_freq),
		tcp->tc_dbwait_notify_latstat.ls_freq, sizeof(tcp->tc_dbwait_notify_latstat.ls_freq));
	vlsp->s2dbwait_notify_nr_avail_bkts = tcp->tc_dbwait_notify_latstat.ls_nr_avail_bkts;
	vlsp->s2dbwait_notify_log_min = tcp->tc_dbwait_notify_latstat.ls_log_min;
	vlsp->s2dbwait_notify_log_max = tcp->tc_dbwait_notify_latstat.ls_log_max;
	
	memcpy_s(vlsp->s2dbcommit_bkts, sizeof(tcp->tc_dbcommit_latstat.ls_bkts),
		tcp->tc_dbcommit_latstat.ls_bkts, sizeof(tcp->tc_dbcommit_latstat.ls_bkts));
	memcpy_s(vlsp->s2dbcommit_freq, sizeof(tcp->tc_dbcommit_latstat.ls_freq),
		tcp->tc_dbcommit_latstat.ls_freq, sizeof(tcp->tc_dbcommit_latstat.ls_freq));
	vlsp->s2dbcommit_nr_avail_bkts = tcp->tc_dbcommit_latstat.ls_nr_avail_bkts;
	...
	for (o_idx = 0; o_idx < INM_LATENCY_LOG_CAPACITY; o_idx++) {
		idx = (tcp->tc_dbret_latstat.ls_log_idx + INM_LATENCY_LOG_CAPACITY - 1) %
				INM_LATENCY_LOG_CAPACITY;
		vlsp->s2dbret_log_buf[o_idx] = tcp->tc_dbret_latstat.ls_log_buf[idx];
		tcp->tc_dbret_latstat.ls_log_idx++;

		idx = (tcp->tc_dbwait_notify_latstat.ls_log_idx + INM_LATENCY_LOG_CAPACITY - 1) %
				INM_LATENCY_LOG_CAPACITY;
		vlsp->s2dbwait_notify_log_buf[o_idx] = tcp->tc_dbwait_notify_latstat.ls_log_buf[idx];
		tcp->tc_dbwait_notify_latstat.ls_log_idx++;

		idx = (tcp->tc_dbcommit_latstat.ls_log_idx + INM_LATENCY_LOG_CAPACITY - 1) %
				INM_LATENCY_LOG_CAPACITY;
		vlsp->s2dbcommit_log_buf[o_idx] = tcp->tc_dbcommit_latstat.ls_log_buf[idx];
                  ...
                  change_node_t *pending_confirm)
{
    ...
    //INM_BUG_ON(!ctxt->tc_rpo_timestamp);
    ...
    if (inm_list_empty(&ctxt->tc_node_head)) {
        if (ctxt->tc_cur_wostate != ecWriteOrderStateBitmap &&
            (ctxt->tc_cur_wostate != ecWriteOrderStateUnInitialized)) {
    ...
    oldest_chg_node = (change_node_t *)inm_list_entry(ptr, change_node_t, next);
...
void end_cx_session()
{
    ...
    for (ptr = driver_ctx->dc_disk_cx_sess_list.next;
            ptr != &(driver_ctx->dc_disk_cx_sess_list); ptr = ptr->next) {
        disk_cx_sess = inm_list_entry(ptr, disk_cx_session_t, dcs_list);
...
{
    vm_cx_session_t *vm_cx_sess = &driver_ctx->dc_vm_cx_session;
    inm_u32_t disk_churn_in_MB = (disk_cx_sess->dcs_tracked_bytes_per_second >>
                                   MEGABYTE_BIT_SHIFT);
                       ...
                       inm_u32_t nr_bytes)
{
    ...
    if (!vm_cx_sess->vcs_num_disk_cx_sess) {
        vm_cx_sess->vcs_tracked_bytes = ctxt->tc_bytes_pending_changes +
                                                                  nr_bytes;
    ...
    disk_cx_sess->dcs_tracked_bytes = ctxt->tc_bytes_pending_changes + nr_bytes;
                                                         ...
                                                         inm_u32_t nr_bytes)
{
    ...
    if (driver_ctx->dc_disk_level_supported_churn) {
        ...
        vm_cx_sess->vcs_default_disk_peak_churn = DISK_LEVEL_SUPPORTED_CHURN;
        vm_cx_sess->vcs_default_vm_peak_churn = VM_LEVEL_SUPPORTED_CHURN;
    }
...
disk_cx_stats_info_t *find_disk_stat_info(target_context_t *ctxt)
{
    ...
    for (disk_cx_stats_ptr = &driver_ctx->dc_disk_cx_stats_list;
        ...
        if (!strcmp(ctxt->tc_pname, dev_cx_stats->DeviceId.volume_guid))
            return disk_cx_stats_info;
    }
...
void close_disk_cx_session(target_context_t *ctxt, int reason_code)
{
    ...
    if (disk_cx_sess->dcs_flags & DCS_CX_SESSION_STARTED) {
        if (reason_code == CX_CLOSE_STOP_FILTERING_ISSUED ||
            ...
            disk_cx_stats_info = disk_cx_sess->dcs_disk_cx_stats_info;
            ...
            if (reason_code & CX_CLOSE_STOP_FILTERING_ISSUED)
                disk_cx_stats->ullFlags |= DISK_CXSTATUS_DISK_NOT_FILTERED;

            if (reason_code & CX_CLOSE_DISK_REMOVAL)
                disk_cx_stats->ullFlags |= DISK_CXSTATUS_DISK_REMOVED;
        }
                            ...
                            vm_cx_session_t *vm_cx_sess, inm_u32_t nr_bytes)
{
    ...
    if (cuur_ts - vm_cx_sess->vcs_base_secs_ts >= HUNDREDS_OF_NANOSEC_IN_SECOND)
        update_disk_churn_buckets(disk_cx_sess);
...
void update_cx_session(target_context_t *ctxt, inm_u32_t nr_bytes)
{
    ...
    if (!(vm_cx_sess->vcs_flags & VCS_CX_SESSION_STARTED)) {
        ...
        if (!(disk_cx_sess->dcs_flags & DCS_CX_SESSION_STARTED)) {
            ...
            update_disk_cx_session(disk_cx_sess, vm_cx_sess, nr_bytes);
            update_vm_cx_session(vm_cx_sess, nr_bytes);
        }
    }
...
void update_cx_product_issue(int flag)
{
    ...
    switch (flag) {
        case VCS_CX_S2_EXIT:
        info("s2 exited while CX sesion is in progress");
        break;
...
void update_cx_with_s2_latency(target_context_t *ctxt)
{
    ...
    if (disk_cx_sess->dcs_flags & DCS_CX_SESSION_STARTED) {
        ...
        if (disk_cx_sess->dcs_max_s2_latency > vm_cx_sess->vcs_max_s2_latency)
            vm_cx_sess->vcs_max_s2_latency = disk_cx_sess->dcs_max_s2_latency;
    }
...
void reset_s2_latency_time()
{
    ...
    for (ptr = driver_ctx->tgt_list.next; ptr != &(driver_ctx->tgt_list);
        ...
        if (ctxt->tc_flags & (VCF_VOLUME_CREATING | VCF_VOLUME_DELETING))
            continue;
...
void volume_lock_all_close_cur_chg_node(void)
{
    ...
    for (ptr = driver_ctx->tgt_list.next; ptr != &(driver_ctx->tgt_list);
        ...
        if ((ctxt->tc_optimize_performance &
            ...
            ctxt->tc_cur_node) {
            do_perf_changes(ctxt, ctxt->tc_cur_node, IN_IOCTL_PATH);
        }
