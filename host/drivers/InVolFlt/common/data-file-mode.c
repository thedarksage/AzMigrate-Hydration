...
#ifndef INM_AIX
    file_node = INM_KMALLOC(sizeof(data_file_node_t), flags, INM_KERNEL_HEAP);
#else
    file_node = INM_KMEM_CACHE_ALLOC(driver_ctx->dc_host_info.data_file_node_cache, flags);
#endif

/* TODO Remove the code */
#if 0
    INM_SPIN_LOCK(&driver_ctx->data_file_node_lock);
    if(inm_list_empty(&driver_ctx->data_file_node_pool)){
	INM_SPIN_UNLOCK(&driver_ctx->data_file_node_lock);
	goto out;
    }

    file_node = inm_list_entry(driver_ctx->data_file_node_pool.next, data_file_node_t, next);
    inm_list_del(&file_node->next);
    driver_ctx->nr_data_file_node_used++;
    INM_SPIN_UNLOCK(&driver_ctx->data_file_node_lock);
...
#else
    INM_KMEM_CACHE_FREE(driver_ctx->dc_host_info.data_file_node_cache, file_node);
#endif

/* TODO Remove the code */
#if 0
    INM_SPIN_LOCK(&driver_ctx->data_file_node_lock);
    inm_list_add_tail(&file_node->next, &driver_ctx->data_file_node_pool);
    driver_ctx->nr_data_file_node_used--;
    INM_SPIN_UNLOCK(&driver_ctx->data_file_node_lock);
#endif
}
...
inm_unlink_datafile(target_context_t *tgt_ctxt, char *data_file_name)
{
    ...
    if (!get_path_memory(&parent)) {
        ...
        sprintf_s(parent, INM_PATH_MAX, "%s/%s", 
                  tgt_ctxt->tc_data_log_dir, tgt_ctxt->tc_datafile_dir_name);
        dbg("Parent: %s", parent);
					    ...
					    target_context_t *tgt_ctxt)
{
    ...
    filename = (char*)INM_KMALLOC(INM_PATH_MAX, INM_KM_SLEEP, INM_KERNEL_HEAP);
    if(!filename)
	return NULL;

    log_dir = (char *)INM_KMALLOC(INM_PATH_MAX, INM_KM_SLEEP, INM_KERNEL_HEAP);
    if (!log_dir) {
        INM_KFREE(filename, INM_PATH_MAX, INM_KERNEL_HEAP);
        return NULL;
    }

    // TODO: Need to revisit on the lock used. This path could be changed through proc.
    // Hence locking is required.
	...
	snprintf(filename, 2048, "%s/%s", log_dir, tgt_ctxt->tc_datafile_dir_name);
	inm_mkdir(filename, 0700);
	volume_lock(tgt_ctxt);
	tgt_ctxt->tc_flags |= VCF_DATAFILE_DIR_CREATED;
	volume_unlock(tgt_ctxt);
    }
    if (node->flags & (KDIRTY_BLOCK_FLAG_START_OF_SPLIT_CHANGE | KDIRTY_BLOCK_FLAG_PART_OF_SPLIT_CHANGE)) {
					...
					change_node_t *node, inm_u64_t *file_offset)
{
    ...
    *file_offset = 0;
    ...
    buffer = INM_KMALLOC_ALIGN(INM_PAGESZ, INM_KM_SLEEP, INM_PAGESHIFT, INM_KERNEL_HEAP);
    if(!buffer)
	return success;

    INM_BUG_ON(inm_list_empty(&node->data_pg_head));

    rec_write_meta = INM_KMALLOC(sizeof(inm_rec_write_meta_t), INM_KM_SLEEP, INM_KERNEL_HEAP);
    if(!rec_write_meta){
	...
	INM_KFREE(buffer, INM_PAGESZ, INM_KERNEL_HEAP);
	INM_KFREE(rec_write_meta, sizeof(inm_rec_write_meta_t), INM_KERNEL_HEAP);
	return success;
    }
    ...
    inm_list_add_tail(&rec_write_meta->list, &driver_ctx->recursive_writes_meta_list);
...
#ifdef INM_AIX
	if(!flt_open_data_file(fname, (INM_RDWR | INM_CREAT | INM_DIRECT), &hdl)) {
#else
...
data_file_writer_thread_func(void *args)
{
    ...
    INM_DAEMONIZE("fw%d", inm_dev_id_get((target_context_t*)thr->ctxt), thr->id);
    ...
    INM_COMPLETE(&thr->exit); //signal the process waiting for completion.
    return 0;
}
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
    ...
    /* TODO: Perform validation on the number of threads to be created.
     */
	...
	if(INM_KERNEL_THREAD(data_file_writer_thread_func, &thr, sizeof(thr), pname) < 0) {
#endif
...
void flush_work_items(target_context_t *tgt_ctxt, data_file_thread_t *thr)
{
	...
	// FIXME: Purposefully made the tgt ctxt lock with irq version as
	// we are holding irq version before this.
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_META))){
    ...
    total_unres_pages = driver_ctx->data_flt_ctx.pages_allocated - driver_ctx->dc_cur_unres_pages;
...
inm_s32_t create_datafile_dir_name(target_context_t *ctxt, inm_dev_info_t *dev_info)
{
    char *temp = NULL;
    char *ptr, *path;
    inm_s32_t len,i;
    ...
    temp = (char *)INM_KMALLOC(INM_GUID_LEN_MAX, INM_KM_SLEEP, INM_KERNEL_HEAP);
    if(!temp) {
        err("proc entry creation for volume %s failed: No Memory", path);
        return 1;
    }
