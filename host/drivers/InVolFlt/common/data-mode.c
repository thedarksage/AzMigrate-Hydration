...
const inm_s32_t sv_ts_sz      = (sizeof(SVD_PREFIX) + sizeof(SVD_TIME_STAMP_V2));
const inm_s32_t sv_drtd_sz    = (sizeof(SVD_PREFIX) + sizeof(inm_u64_t));
const inm_s32_t sv_chg_sz     = (sizeof(SVD_PREFIX) + sizeof(SVD_DIRTY_BLOCK_V2));
 ...
 * 
 * TODO: This variable could be passed as a parameter to the driver while
 *       loading.
 */
...
#define CORRUPTION_OFF_BY       1
        ...
        if (node->type == NODE_SRC_DATA) {
            ...
            if ((now - driver_ctx->dc_tel.dt_drv_load_time) >
                ...
                if (CHANGE_NODE_IS_FIRST_DATA_PAGE(node, pg)) {
                    ...
                    err("Corrupting Page 0: offset = %d", node->stream_len);
                } else if (CHANGE_NODE_IS_LAST_DATA_PAGE(node, pg)) {
                    /* Last page - Underrun */
                    pg_offset -= CORRUPTION_OFF_BY;
                    err("Corrupting Page N: offset = %d", node->stream_len);
                } else {
                    /* Random corruption */
                    if (now & 1) 
                        pg_offset += CORRUPTION_OFF_BY;
                        ...
                        pg_offset -= CORRUPTION_OFF_BY ;
                    err("Corrupting DB: offset = %d", node->stream_len);
                } 
            }
        } 
#endif
    ...
    while(1) {
        ...
        memcpy_s((char *)(dst + pg_offset), to_copy, (char *)(buf + buf_offset), to_copy);
        ...
        if(pg_free == 0) {
            ...
            if(((void *)pg == (void *)&node->data_pg_head)) {
                ...
                if (((org_pg_offset + len) & (INM_PAGESZ - 1)) != pg_offset) {
                    err("Data copy error: Org: %d Len: %d New: %d Last: 1", 
    ...
    pg = get_cur_data_pg(node, &pg_offset);
    if (((org_pg_offset + len) & (INM_PAGESZ - 1)) != pg_offset) {
        err("Data copy error: Org: %d Len: %d New: %d First: %d Last: %d", 
            org_pg_offset, len, pg_offset,
    ...
    *overflow_pages = 0;
    ...
    if (tc_allocated_pages < tc_res_pages) {
        if ((tc_allocated_pages + num_pages) <= tc_res_pages) {
            ...
            /* case 2 */
            *overflow_pages = (tc_allocated_pages + num_pages) - tc_res_pages;
        }
    }
    else {
        /* case 3 */
...
inm_tc_resv_fill(void)
{
    ...
    inm_list_for_each_safe(ptr, nextptr, &driver_ctx->tgt_list) {
         ...
         */
         if (tgt_ctxt->tc_flags & (VCF_VOLUME_DELETING | VCF_VOLUME_CREATING))
             continue;
         ...
         /* Not enough pages? */
         if (inm_tc_resv_add(tgt_ctxt, driver_ctx->dc_vol_data_pool_size)) {
    	     volume_unlock(tgt_ctxt);
    	     break;
         }
...
inm_tc_resv_add(target_context_t *tgt_ctxt, inm_u32_t num_pages)
{
    ...
    if (tc_allocated_pages <= tgt_ctxt->tc_reserved_pages) {
        ...
        if (tc_allocated_pages < updated_tc_reserved_pages) {
            /* case 2 */
            num_unres_pages = updated_tc_reserved_pages - tc_allocated_pages;
        }
        else {
            /* case 3 */
            num_unres_pages = 0;
...
inm_tc_resv_del(target_context_t *tgt_ctxt, inm_u32_t num_pages)
{
    ...
    if (tc_allocated_pages <= updated_tc_reserve_pages) {
        ...
        if (tc_allocated_pages < tgt_ctxt->tc_reserved_pages) {
            /* case 2 */
            num_unres_pages = tgt_ctxt->tc_reserved_pages - tc_allocated_pages;
        }
        else {
            /* case 3 */
            num_unres_pages = 0;
...
inm_s32_t init_data_flt_ctxt(data_flt_t *data_ctxt)
{
	...
	if(!alloc_data_pages(&data_ctxt->data_pages_head,
	    ...
	    /* TODO: Check what we had got is sufficient for filtering.
	     * otherwise do approriate mode change or notify user about
	     * the same.
	     */
	...
	data_ctxt->dp_nrpgs_slab = (total_ram_pgs * driver_ctx->tunable_params.percent_change_data_pool_size) / 100;
...
add_data_pages(inm_u32_t num_pages)
{
    ...
    INM_SPIN_LOCK_IRQSAVE(&driver_ctx->data_flt_ctx.data_pages_lock, lock_flag); 
    inm_list_splice_at_tail(&pg_head, &(driver_ctx->data_flt_ctx.data_pages_head));
...
should_wakeup_worker_alloc(inm_u32_t overflow_pages)
{
	...
	alloc_limit = MIN(driver_ctx->data_flt_ctx.dp_nrpgs_slab, driver_ctx->data_flt_ctx.pages_allocated);	
...
out:
     if(wakeup){
         ...
         INM_SPIN_UNLOCK_IRQRESTORE(&(driver_ctx->wqueue.lock), lock_flag);
     }
...
if(IS_DBG_ENABLED(inm_verbosity, INM_IDEBUG)){
    ...
    /*  Bug Fix: 7736: This problem is only visible on Uniprocessor kernels.
     *  This is because the function spin_is_locked will always return false
     *  even if the lock is held. But this is not the case with SMP kernels.
     *  Again if the kernel is configured with the option CONFIG_DEBUG_SPINLOCK
     *  being set to "YES", the problem will not persist.
     */
    /*INM_BUG_ON(!INM_IS_SPINLOCK_HELD(&tcp->tc_lock)); */
    ...
    inm_list_for_each_safe(ptr, nextptr, lhp) {
        inm_list_del(ptr);
        inm_list_add_tail(ptr, &driver_ctx->data_flt_ctx.data_pages_head);
        driver_ctx->data_flt_ctx.pages_free++;
    }
 ...
 */
int
inm_get_pages_for_node(change_node_t *chg_node, struct inm_list_head *pg_head, inm_s32_t len)
{
    ...
    if (bytes_req) {
        inm_list_splice_at_tail(&headp_local, &(chg_node->data_pg_head));
...
inm_u32_t
inm_split_change_in_data_mode(target_context_t *tgt_ctxt, write_metadata_t *wmd,
					    inm_wdata_t *wdatap)
{
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_META))){
    ...
    inm_list_for_each_safe(headp, next_headp, &split_chg_node_list) {
        chg_node = inm_list_entry(headp, change_node_t, next);
   		...
   		/* change nodes are new hence we need to account sv_chg_sz + sv_const_sz */
	...
	inm_list_for_each_safe(headp, next_headp, &split_chg_node_list) {
		chg_node = inm_list_entry(headp, change_node_t, next);
   	    ...
   	    num_pages = inm_get_pages_for_node(chg_node, &pg_head, disk_chg_ptr->length);
    	if (!num_pages) {
            /* should not be here */
            INM_BUG_ON(1);
            goto error;
	...
	(*wdatap->wd_copy_wd_to_datapgs)(tgt_ctxt,wdatap, split_chg_node_list.next);
...
add_change_node_to_list:
    ...
    if (is_barrier_on) {
        ...
        inm_list_splice_at_tail(&split_chg_node_list, &chg_node->vcptr->tc_node_head);
    }
#else
    inm_list_splice_at_tail(&split_chg_node_list, &chg_node->vcptr->tc_node_head);
#endif
	...
	add_changes_to_pending_changes(tgt_ctxt, tgt_ctxt->tc_cur_wostate, nr_splits);
...
void
save_data_in_data_mode_normal(target_context_t *tgt_ctxt, write_metadata_t *wmd,
			     inm_wdata_t *wdatap)
{
    inm_u32_t num_pages = 0;
    struct inm_list_head pg_head;
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_META))){
	...
	if (save_data_in_metadata_mode(tgt_ctxt, wmd, wdatap)) {
            /* TODOs: save_data_in_metadata_mode failed here 
            tgt_ctxt->resync_required = 1; */
            err("save_data_in_metadata_mode failed");		
        }
        return;
    }
    ...
    copy_data_to_data_pages((void *)&db_prefix, sizeof(SVD_PREFIX), chg_node);
    copy_data_to_data_pages((void *)&dblock, sizeof(SVD_DIRTY_BLOCK_V2), chg_node);
...
// This gets called when driver detects that s2 is going down.
...
void finalize_data_stream(change_node_t *node)
{
    ...
    prefix.Flags = 0;
    prefix.count = 1;
    memcpy_s(&svd_ts, sizeof(STREAM_REC_HDR_4B), (void *)&node->changes.start_ts, sizeof(STREAM_REC_HDR_4B));
    ...
    svd_ts.TimeInHundNanoSecondsFromJan1601 = node->changes.end_ts.TimeInHundNanoSecondsFromJan1601;
    ...
    svd_ts.TimeInHundNanoSecondsFromJan1601 = node->changes.start_ts.TimeInHundNanoSecondsFromJan1601;
    copy_data_to_data_pages(&svd_ts, sizeof(SVD_TIME_STAMP_V2), node);
...
inm_s32_t add_tag_in_stream_mode(tag_volinfo_t *tag_volinfop, tag_info_t *tag_buf,
	inm_s32_t num_tags, tag_guid_t *tag_guid, inm_s32_t index)
{
    ...
    chg_node = get_change_node_for_usertag(ctxt, &wdata, TAG_COMMIT_NOT_PENDING);
    tag_volinfop->chg_node = wdata.wd_chg_node;
    tag_volinfop->meta_page = wdata.wd_meta_page;
#else
    chg_node = get_change_node_for_usertag(ctxt, NULL, TAG_COMMIT_NOT_PENDING);
#endif
    if(!chg_node) {
    ...
    while(idx < num_tags) {
        ...
        copy_data_to_data_pages(&prefix, sizeof(SVD_PREFIX), chg_node);
        copy_data_to_data_pages(&tag_buf->tag_name[0], tag_buf->tag_len, chg_node);
