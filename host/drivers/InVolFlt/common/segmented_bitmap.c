 ...
 *
 * File       : segmented_bitmap.c
 *
 * Description: This file contains data mode implementation of the
 *              filter driver.
 *
 * Functions defined are
 *segmented_bitmap_dtr
 *segmented_bitmap_get
 segmented_bitmap_put
 segmented_bitmap_clear_all_bits
 segmented_bitmap_sync_flush_all
...
/**
 * FUNCTION NAME:segmented_bitmap_ctr
 * 
 * DESCRIPTION :
 * 
 * INPUT PARAMETERS : 
 ...
 * NOTES
 * 
 * return value :    segmented_bitmap_ctr      - for success
...
/**
 * FUNCTION NAME:segmented_bitmap_dtr
 * 
 * DESCRIPTION :
 * 
 * INPUT PARAMETERS : 
...
void segmented_bitmap_dtr(segmented_bitmap_t *sb)
{
    ...
    //BUG(INM_ATOMIC_READ(sb->refcnt) != 0);
...
/**
 * FUNCTION NAME:segmented_bitmap_process_bitrun
 * 
 * DESCRIPTION :
 * 
 * INPUT PARAMETERS : 
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
    ...
    while(bits_to_process > 0)
    {
        ...
        ret = fstream_segment_mapper_read_and_lock(sb->fssm, byte_offset,
						   &bit_buffer, &bit_buffer_byte_size);
...
/**
 * FUNCTION NAME:segmented_bitmap_get_next_bitrun
 * 
 * DESCRIPTION :
 * 
 * INPUT PARAMETERS : 
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
    ...
    *bitoffset = 0;
    ...
    while(sb->next_search_offset < sb->bits_in_bitmap)
    {
        ...
        //ret = fstream_segment_mapper_read_and_lock(sb->fssm, byte_offset, &bit_buffer, &bit_buffer_byte_size);
        ret = fstream_segment_mapper_read_and_lock(sb->fssm, byte_offset,
						   &bit_buffer, &bit_buffer_byte_size);
        ...
        //ret = get_next_bitmap_bitrun(bit_buffer, adjusted_buffer_size, &search_bit_offset, &run_length, &run_offset);
        ret = GetNextBitmapBitRun(bit_buffer, adjusted_buffer_size,
				  &search_bit_offset, &run_length, &run_offset);
...
inm_s32_t find_bmap_io_pat(char *buf, long long bits_in_bmap, bmap_bit_stats_t *bbsp, int eobmap);
/**
 * FUNCTION NAME:segmented_bitmap_get_number_of_bits_set
 * 
 * DESCRIPTION :
 * 
 * INPUT PARAMETERS : 
 ...
 * 
 **/
inm_u64_t segmented_bitmap_get_number_of_bits_set(segmented_bitmap_t *sb, bmap_bit_stats_t *bbsp)
{
    inm_s32_t ret = 0;
    unsigned char *bit_buffer = NULL;
    inm_u32_t bit_buffer_byte_size = 0;
