...
commit_usertag(target_context_t *ctxt)
{
    ...
    /*
     * no need for safe. There is no equivalent unsafe macro for use
     */ 
...
revoke_usertag(target_context_t *ctxt, int timedout)
{
    ...
    if (revoke) {
        ...
        telemetry_log_tag_history(revoke, ctxt, ecTagStatusRevoked, 
                              timedout ? ecRevokeTimeOut : ecRevokeCommitIOCTL, 
                              ecMsgTagRevoked);
...
**/
inm_s32_t init_change_node(change_node_t *node, inm_s32_t from_pool, inm_s32_t flag, inm_wdata_t *wdatap)
{
    inm_page_t *pgp = NULL;
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_META))){
    ...
    /* TODOs: node->type is required to initialized in get code
     * Account for different node types.
     */
...
/**
 * FUNCTION NAME: get_change_node_to_update
 * 
 * DESCRIPTION : This function gets the change_node ptr
 *        to store change information
 * 
 * INPUT PARAMETERS : tgt_ctxt - target context 
 *              
 * 
 * OUTPUT PARAMETERS :
 * NOTES
 * 
 * return value :    change_node_t ptr      - for success
 *             EINVAL - for invalid inputs
 *            NULL - for failure
 * 
 **/

change_node_t *get_change_node_to_update(target_context_t *tgt_ctxt, inm_wdata_t *wdatap,
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_META))){
    ...
    /*
     * Check if the current change node is matching with the filter mode. If
     * not create a new change node and set its type corresponding to the
     * the filter mode. However if the change_idx of the node is 0 it means we
     * have a freshly allocated node and it can used, no need for allocation.
     * At present we have no limit on the number of changes in data-mode
     * change node while the meta-data-mode node is limited by
	...
	if(node){
		...
		node = (!node->changes.change_idx) ? (node->type = NODE_SRC_DATA, node) : NULL;
    ...
    default:
        ...
        if (node->type == NODE_SRC_DATA)
            node = (!node->changes.change_idx) ? (node->type = NODE_SRC_METADATA, node) : NULL;
        else if(node->wostate != tgt_ctxt->tc_cur_wostate)
	    node = (!node->changes.change_idx) ? node : NULL;
	...
	node->wostate = tgt_ctxt->tc_cur_wostate;
        ...
        /* TODOs: tunable to restrict the meta data mode db size */
    ...
    if (node && node->changes.change_idx > 0 &&
        (node->changes.change_idx % (MAX_CHANGE_INFOS_PER_PAGE)) == 0) {
...
alloc_chg_node:
    ...
    if (!node) {
        ...
        if ((tgt_ctxt->tc_optimize_performance &
            ...
            recent_cnode = (change_node_t *)inm_list_entry(ptr, change_node_t, next);
            do_perf_changes(tgt_ctxt, recent_cnode, IN_IO_PATH);
        }
        //TODO: Alloc from cache pool.
...
void do_perf_changes(target_context_t *tgt_ctxt, change_node_t *recent_cnode, int path)
{
    if (recent_cnode && recent_cnode->type == NODE_SRC_DATA &&
        ...
        if (tgt_ctxt->tc_optimize_performance & PERF_OPT_DEBUG_DATA_DRAIN) {
            info("Appending chg:%p to tgt_ctxt:%p next:%p prev:%p mdoe:%d",
                recent_cnode,tgt_ctxt, recent_cnode->nwo_dmode_next.next,
                recent_cnode->nwo_dmode_next.prev, recent_cnode->type);
        }
    }
}
...
void do_perf_changes_all(target_context_t *tgt_ctxt, int path)
{
    ...
    inm_list_for_each_safe(ptr, nextptr, &tgt_ctxt->tc_non_drainable_node_head) {
...
close_change_node(change_node_t *chg_node, inm_u32_t path)
{
     ...
     * PS - Change node with write order is always closed, so this fn should
    ...
    if (chg_node->wostate == ecWriteOrderStateData)
        return;
    if (tgt_ctxt->tc_optimize_performance & PERF_OPT_DEBUG_DATA_DRAIN) {
        info("Closing chg:%p to tgt_ctxt:%p next:%p prev:%p mode:%d path:%d",
    ...
    switch (chg_node->type) {
        case NODE_SRC_DATA:
			...
			&chg_node->changes.start_ts, sizeof(TIME_STAMP_TAG_V2));
            break;
        case NODE_SRC_METADATA:
            ...
            /* split IOs in metadata mode no meaning and atomicity could be
             * lost if partial split IOs make it to bitmap
             */
            ...
            memcpy_s(&chg_node->changes.end_ts, sizeof(TIME_STAMP_TAG_V2),
			&chg_node->changes.start_ts, sizeof(TIME_STAMP_TAG_V2));
            /* Tweak the time stamp and seq deltas in all changes */
            __inm_list_for_each(ptr, &chg_node->changes.md_pg_list) {
                ...
                while (i < MAX_CHANGE_INFOS_PER_PAGE) {
                    ...
                    i++;
                    count++;
                    if (count == chg_node->changes.change_idx) {
                        break;
                    }
                }
            }
            break;
        case NODE_SRC_TAGS:
            /* control may come here for tag tracked with page alloc failure */
            break;
        default:
            err("Invalid change node type while closing a change node:%p",
                 chg_node);
            INM_BUG_ON(1);
            break;
    }
 ...
 *               and sequence number
 * 
 * INPUT PARAMETERS :  Target context
 *               
 *              
 *              
 * 
 * OUTPUT PARAMETERS : Change node pointer
 * NOTES
 * 
 * return value :    0      - for success
 *             EINVAL - for invalid inputs
                                     ...
                                     inm_s32_t *status)
{
     ...
     * if change node list is empty then tc_pending_confirm would be NULL i.e.
     * tso file case
     */
    if (INM_UNLIKELY(tgt_ctxt->tc_pending_confirm || 
		inm_list_empty(&tgt_ctxt->tc_node_head))) {
    ...
    /* Walk though real change node list to drain write order changes as it is*/
    if (!inm_list_empty(&tgt_ctxt->tc_node_head)) {
        oldest_ptr = tgt_ctxt->tc_node_head.next;
        chg_node = (change_node_t *)inm_list_entry(oldest_ptr, change_node_t,
                                                   next);
        if (chg_node) {
            if (chg_node->wostate == ecWriteOrderStateData) {
                if (tgt_ctxt->tc_optimize_performance & PERF_OPT_DEBUG_DATA_DRAIN) {
                info("Drain wo chgnode  from tc_node_head chg_node:%p tc_cur_node:%p",
                ...
                if (chg_node->type == NODE_SRC_DATA ||
                    ...
                    chg_node->type == NODE_SRC_TAGS) { /*Tag page alloc failure*/
                    if (tgt_ctxt->tc_optimize_performance & PERF_OPT_DEBUG_DATA_DRAIN) {
                        info("Drain chg node from tc_node_head chg_node:%p mode:%d"
    ...
    if (chg_node) {
         ...
         */
        /* Walk though the non write order mode data mode change node list */
        if (!inm_list_empty(&tgt_ctxt->tc_nwo_dmode_list)) {
            ptr = tgt_ctxt->tc_nwo_dmode_list.next;
            chg_node = (change_node_t *)inm_list_entry(ptr, change_node_t,
                                                       nwo_dmode_next);
        	INM_BUG_ON(chg_node->type != NODE_SRC_DATA &&
                       chg_node->type != NODE_SRC_DATAFILE);
            if (tgt_ctxt->tc_optimize_performance & PERF_OPT_DEBUG_DATA_DRAIN) {
                info("Draining chgnode from nwo list chg_node:%p next:%p prev:%p mode:%d",
    	...
    	if (!chg_node) {
            ...
            chg_node = (change_node_t *)inm_list_entry(oldest_ptr, change_node_t,
                                                       next);
            if (!chg_node) {
                err("Empty change node list tc_mode:%d", tgt_ctxt->tc_cur_wostate);
                goto ret;
            }
            INM_BUG_ON(chg_node->type != NODE_SRC_METADATA);
            if (chg_node != tgt_ctxt->tc_cur_node &&
            ...
            if (tgt_ctxt->tc_optimize_performance & PERF_OPT_DEBUG_DATA_DRAIN) {
                dbg("Draining change node in chg_node:%p mode:%d delta ts:%llu seq:%llu"
                    "tc_cur_node:%p", chg_node,chg_node->type,
        ...
        if (tgt_ctxt->tc_optimize_performance &
            ...
            if (chg_node->type == NODE_SRC_DATA &&
                ...
                inm_list_add_tail(&chg_node->nwo_dmode_next,   
                                  &tgt_ctxt->tc_nwo_dmode_list);
                if (tgt_ctxt->tc_optimize_performance & PERF_OPT_DEBUG_DATA_DRAIN) {
                    info("Appending tc_cur_node chg:%p to tgt_ctxt:%p next:%p prev:%p mode:%d",
                         chg_node,tgt_ctxt, chg_node->nwo_dmode_next.next,
                         chg_node->nwo_dmode_next.prev, chg_node->type);
                }
            }
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_META))){
    ...
    if (tgt_ctxt->tc_cur_node && (tgt_ctxt->tc_optimize_performance &
        ...
        if (tgt_ctxt->tc_cur_node->type == NODE_SRC_DATA &&
            tgt_ctxt->tc_cur_node->wostate != ecWriteOrderStateData) {
            close_change_node(tgt_ctxt->tc_cur_node, IN_IOCTL_PATH);
    ...
    for( ptr = hd->next; ptr != hd; ) {
        ...
        if (chg_node->type == NODE_SRC_TAGS)
            telemetry_log_tag_history(chg_node, chg_node->vcptr,
                                      ecTagStatusDropped, reason, 
                                      ecMsgTagDropped);
        
        if (chg_node->tag_guid) {
            chg_node->tag_guid->status[chg_node->tag_status_idx] = STATUS_DELETED;
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
	...
	{           
		...
		INM_BUG_ON(ctxt->tc_pending_changes < (cnode->changes.change_idx));
		ctxt->tc_pending_changes -= (cnode->changes.change_idx);
		if (cnode->type == NODE_SRC_METADATA) {
		    ctxt->tc_pending_md_changes -= (cnode->changes.change_idx);
		    ctxt->tc_bytes_pending_md_changes -= (cnode->changes.bytes_changes);
		}
		ctxt->tc_bytes_pending_changes -= cnode->changes.bytes_changes;
		subtract_changes_from_pending_changes(ctxt, cnode->wostate, cnode->changes.change_idx);
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
    ...
    if (!wqe)
        return 1;
    /* CODE CHANGE - FIXME important - temporary code */
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_META))){
    ...
    while(ptr != &ctxt->tc_node_head) {
        ...
        if ((node->type == NODE_SRC_DATA) &&
           (0 == (node->flags & (CHANGE_NODE_FLAGS_QUEUED_FOR_DATA_WRITE | 
...
inm_page_t *
get_page_from_page_pool(inm_s32_t alloc_from_page_pool, inm_s32_t flag, inm_wdata_t *wdatap)
{
    inm_page_t *pg = NULL;
#ifndef INM_AIX
    unsigned long lock_flag = 0;
...
alloc_fresh:
    pg = (inm_page_t *) INM_KMALLOC(sizeof(inm_page_t), flag, INM_KERNEL_HEAP);
    if (pg) {
        pg->cur_pg = (unsigned long *) __INM_GET_FREE_PAGE(flag, INM_KERNEL_HEAP);
        if (!pg->cur_pg) {
...
void update_change_node(change_node_t *chg_node,
			...
			inm_tsdelta_t *tdp)
{
    inm_s32_t md_idx = chg_node->changes.change_idx % (MAX_CHANGE_INFOS_PER_PAGE);
...
if(IS_DBG_ENABLED(inm_verbosity, INM_IDEBUG)){
    ...
    udirty->uHdr.Hdr.ulSequenceIDforSplitIO = chg_node->seq_id_for_split_io;
    ...
    udirty->uHdr.Hdr.ulicbTotalChangesPending =
        (ctxt->tc_bytes_pending_changes - udirty->uHdr.Hdr.ulicbChanges);
    ...
    //TODO: Check if we can get rid of this INM_MEM_ZERO operation.
    ...
    memcpy_s(&udirty->uTagList.TagList.TagTimeStampOfFirstChange, sizeof(TIME_STAMP_TAG_V2),
					&chg_node->changes.start_ts, sizeof(TIME_STAMP_TAG_V2));
    memcpy_s(&udirty->uTagList.TagList.TagTimeStampOfLastChange, sizeof(TIME_STAMP_TAG_V2),
					&chg_node->changes.end_ts, sizeof(TIME_STAMP_TAG_V2));

    if (chg_node->flags & KDIRTY_BLOCK_FLAG_SPLIT_CHANGE_MASK) {
        if (chg_node->flags & KDIRTY_BLOCK_FLAG_START_OF_SPLIT_CHANGE) {
            udirty->uHdr.Hdr.ulFlags |= UDIRTY_BLOCK_FLAG_START_OF_SPLIT_CHANGE;
        } else if (chg_node->flags & KDIRTY_BLOCK_FLAG_PART_OF_SPLIT_CHANGE) {
            udirty->uHdr.Hdr.ulFlags |= UDIRTY_BLOCK_FLAG_PART_OF_SPLIT_CHANGE;
        } else if (chg_node->flags & KDIRTY_BLOCK_FLAG_END_OF_SPLIT_CHANGE) {
            udirty->uHdr.Hdr.ulFlags |= UDIRTY_BLOCK_FLAG_END_OF_SPLIT_CHANGE;
        }
    }

    ctxt->tc_CurrEndTimeStamp = chg_node->changes.end_ts.TimeInHundNanoSecondsFromJan1601;
    ctxt->tc_CurrEndSequenceNumber = chg_node->changes.end_ts.ullSequenceNumber;
    ...
    udirty->uHdr.Hdr.ullPrevEndSequenceNumber = ctxt->tc_PrevEndSequenceNumber;
    udirty->uHdr.Hdr.ulPrevSequenceIDforSplitIO = ctxt->tc_PrevSequenceIDforSplitIO;
    ...
    if ((!(udirty->uHdr.Hdr.ullPrevEndTimeStamp <=
        ...
        if (!ctxt->tc_resync_required) {
            queue_worker_routine_for_set_volume_out_of_sync(ctxt, ERROR_TO_REG_OOD_ISSUE, 1);
        }
    }
    ...
    case NODE_SRC_TAGS:
        if(0 == (chg_node->flags & CHANGE_NODE_TAG_IN_STREAM)) {
            ...
            memcpy_s(&udirty->uTagList.TagList.TagEndOfList, (UDIRTY_BLOCK_TAGS_SIZE - bytes),
			chg_node->changes.cur_md_pgp, (UDIRTY_BLOCK_TAGS_SIZE - bytes));
            break;
        }
        // Fall through to map tag in stream mode
        /* GCC 7 requires the following marker comment to not print a warning */
        /* fall through */

    case NODE_SRC_DATA:
        if(0 == (chg_node->flags & CHANGE_NODE_DATA_STREAM_FINALIZED)) {
            finalize_data_stream(chg_node);
        }

        udirty->uTagList.TagList.TagDataSource.ulDataSource = INVOLFLT_DATA_SOURCE_DATA;
...
#ifndef INM_AIX
            udirty->uHdr.Hdr.ppBufferArray = (void **)chg_node->mapped_address;
#endif
        }
    ...
    case NODE_SRC_METADATA:
        ...
        udirty->uTagList.TagList.TagDataSource.ulDataSource = INVOLFLT_DATA_SOURCE_META_DATA;
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_META))){
    ...
    case NODE_SRC_DATAFILE:
        ...
        strcpy_s((char *)udirty->uTagList.DataFile.FileName, UDIRTY_BLOCK_MAX_FILE_NAME,
    ...
    if (chg_node->type) {
        ...
        collect_latency_stats(&ctxt->tc_dbret_latstat, (ts_in_usec - last_ts));
    }
...
/**
 * FUNCTION NAME:copy_ts_to_udirty
 * 
 * DESCRIPTION :
 * 
 * INPUT PARAMETERS : 
				     ...
				     UDIRTY_BLOCK_V2 *udirty, inm_devhandle_t *filp)
{
    ...
    if (!(udirty->uHdr.Hdr.ullPrevEndTimeStamp <=
        ...
        err("TSO File Previous TS:%llu Seq:%llu Start TS:%llu Seq:%llu End TS:%llu Seq:%llu",
            ...
            udirty->uTagList.TagList.TagTimeStampOfLastChange.ullSequenceNumber);
        if (!ctxt->tc_resync_required) {
            queue_worker_routine_for_set_volume_out_of_sync(ctxt, ERROR_TO_REG_OOD_ISSUE, 2);
        }
    }

    /* Set the write order state. In case of tso file, the write order state
     * can be DATA if bitmap read is complete. Otherwise use the current
     * write order state.
     */
    if(ctxt->tc_bp && ctxt->tc_bp->volume_bitmap && (ecVBitmapStateReadCompleted == ctxt->tc_bp->volume_bitmap->eVBitmapState))
	udirty->uHdr.Hdr.eWOState = ecWriteOrderStateData;
    else
	udirty->uHdr.Hdr.eWOState = ctxt->tc_cur_wostate;

    /* TODOs: set the data source based on target context's filtering mode */
    //udirty->uTagList.TagList.TagDataSource.ulDataSource = INVOLFLT_DATA_SOURCE_DATA;
...
/**
 * FUNCTION NAME:fill_udirty_block
 * 
 * DESCRIPTION :
 * 
 * INPUT PARAMETERS : 
...
if(IS_DBG_ENABLED(inm_verbosity, INM_IDEBUG)){
    ...
    if (ctxt->tc_optimize_performance & PERF_OPT_DRAIN_PREF_DATA_MODE_CHANGES_IN_NWO) {
        /* Preferably get non write order data mode change or
         * get metadata change node by tweaking the time stamps
         * and sequence number
         */
    ...
    if (INM_LIKELY(chg_node)) {
        ...
        if (status)
            ...
            if (ctxt->tc_flags & VCF_IO_BARRIER_ON) {
                dbg("Drain Barrier due the pending list of change nodes is non-empty, returning EAGAIN")
                status = INM_EAGAIN;
                goto out;
            }
	...
	if (ctxt->tc_bp->volume_bitmap &&
		...
		break;
            ...
            if (witem_type)
                add_vc_workitem_to_list(witem_type, ctxt, 0, FALSE, &lh);
	}
	break;

    case ecWriteOrderStateRawBitmap:
	if (!(ctxt->tc_flags & VCF_VOLUME_IN_BMAP_WRITE))
            add_vc_workitem_to_list(WITEM_TYPE_START_BITMAP_READ, ctxt, 0, FALSE, &lh);
	break;
...
if(IS_DBG_ENABLED(inm_verbosity, INM_IDEBUG)){
	...
	/* Check if this node is being cleaned up while the data pages are mapped
	 */
        volume_lock(tgt_ctxt);
 ...
 */  
	...
	if(INM_UNLIKELY(tgt_ctxt->tc_pending_confirm == chg_node)) {
	    ...
	    volume_unlock(tgt_ctxt);
//        if (INM_CURPROC ==  chg_node->mapped_thread)
        /* current->mm could be NULL if this func. gets called from flt_release()
         * when s2 exits. As s2 is exiting, so unmap is not applicable. Otherwise if
         * current->mm is valid then we must unmap the change node
         */
#ifndef INM_AIX
	...
	if(0 != inm_unlink_datafile(tgt_ctxt, chg_node->data_file_name)) {
		...
		chg_node->data_file_name);
	    volume_lock(tgt_ctxt);
	    tgt_ctxt->tc_stats.dfm_bytes_to_disk -= chg_node->data_file_size;
    ...
    default:
//	err("Unknown node type encountered in commit %d", chg_node->type);
	break;
    }
    ...
    if (tgt_ctxt->tc_optimize_performance & PERF_OPT_DEBUG_DATA_DRAIN) {
        info("commit - chg_node:%p next:%p prev:%p mode:%d closed:%d",
            chg_node, chg_node->nwo_dmode_next.next, chg_node->nwo_dmode_next.prev,
            chg_node->type, (chg_node->flags & CHANGE_NODE_IN_NWO_CLOSED));
    }
    deref_chg_node(chg_node);
#if defined(SLES15SP3) || LINUX_VERSION_CODE >= KERNEL_VERSION(5, 8, 0)
    inm_alloc_pools();
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
    ...
    if (ctxt->tc_pending_confirm) {
        ...
        if (ctxt->tc_optimize_performance & PERF_OPT_DEBUG_DBLK_CHANGES) {
            print_change_node_off_length(ctxt->tc_pending_confirm);
        }
    }

    if (commit->ulFlags & COMMIT_TRANSACTION_FLAG_RESET_RESYNC_REQUIRED_FLAG) {
    ...
    if (chg_node && (commit->ulTransactionID == chg_node->transaction_id)) {
        ctxt->tc_pending_confirm = NULL;
        /* If change node on non write order data mode list, then
         * remove it from that list
         */
        ...
        if (is_tag && ((chg_node->flags & CHANGE_NODE_FAILBACK_TAG) ||
            ...
            if (ctxt->tc_tag_commit_status) {
                ...
                ctxt->tc_tag_commit_status->TagSequenceNumber = ctxt->tc_CurrEndSequenceNumber;

                info("The tag is drained for disk %s, dirty block = %p, "
                     "TagInsertionTime = %llu, TagSequenceNumber = %llu",
                     ctxt->tc_guid, chg_node, ctxt->tc_tag_commit_status->TagInsertionTime,
        ...
        ctxt->tc_PrevEndSequenceNumber = ctxt->tc_CurrEndSequenceNumber;
        ctxt->tc_PrevSequenceIDforSplitIO = ctxt->tc_CurrSequenceIDforSplitIO;
        get_rpo_timestamp(ctxt, IOCTL_INMAGE_COMMIT_DIRTY_BLOCKS_TRANS, chg_node);

    	if (chg_node->tag_guid) {
    	    chg_node->tag_guid->status[chg_node->tag_status_idx] = STATUS_COMMITED;
    	    INM_WAKEUP_INTERRUPTIBLE(&chg_node->tag_guid->wq);
    	    chg_node->tag_guid = NULL;
    	}
        ...
        /* not a orphan node */
        if ((chg_node->flags & CHANGE_NODE_ORPHANED) == 0) {
            INM_BUG_ON(ctxt->tc_pending_changes < (chg_node->changes.change_idx));
            ctxt->tc_pending_changes -= (chg_node->changes.change_idx);
            if (chg_node->type == NODE_SRC_METADATA) {
                ctxt->tc_pending_md_changes -= (chg_node->changes.change_idx);
                ctxt->tc_bytes_pending_md_changes -= 
                    (chg_node->changes.bytes_changes);
            }
            ctxt->tc_bytes_pending_changes -= chg_node->changes.bytes_changes;
            ctxt->tc_commited_changes += (chg_node->changes.change_idx);
            ctxt->tc_bytes_commited_changes += chg_node->changes.bytes_changes;
            subtract_changes_from_pending_changes(ctxt, chg_node->wostate,
                                                  chg_node->changes.change_idx);
            ...
            if (ecWriteOrderStateData != ctxt->tc_cur_wostate) {
                if (is_data_filtering_enabled_for_this_volume(ctxt) &&
                    ...
                    if (ctxt->tc_bp && ctxt->tc_bp->volume_bitmap &&
                       (ecVBitmapStateReadCompleted==ctxt->tc_bp->volume_bitmap->eVBitmapState)) {
                        if ((driver_ctx->service_state == SERVICE_RUNNING) &&
                            !ctxt->tc_pending_wostate_bm_changes &&
                            !ctxt->tc_pending_wostate_rbm_changes) {
                       	    set_tgt_ctxt_wostate(ctxt, ecWriteOrderStateMetadata, 
                                                 FALSE, 
        ...
        if (is_tag) {
            telemetry_log_tag_history(chg_node, ctxt, 
                                      ...
                                      ecNotApplicable, ecMsgTagCommitDBSuccess);

			ctxt->tc_tel.tt_prev_tag_ts =  ctxt->tc_PrevEndTimeStamp;
    ...
    if (ctxt->tc_tso_file) {
        if (commit->ulTransactionID == ctxt->tc_tso_trans_id) {
            ...
            ctxt->tc_tel.tt_prev_ts = ctxt->tc_PrevEndTimeStamp;
            ctxt->tc_tel.tt_prev_seqno = ctxt->tc_PrevEndSequenceNumber;
            get_rpo_timestamp(ctxt, IOCTL_INMAGE_COMMIT_DIRTY_BLOCKS_TRANS, NULL);
        }
...
if(IS_DBG_ENABLED(inm_verbosity, INM_IDEBUG)){
    ...
    while (driver_ctx->dc_res_cnode_pgs < threshold) {
        pg = (inm_page_t *) INM_KMALLOC(sizeof(inm_page_t), alloc_flag, INM_KERNEL_HEAP);
        INM_BUG_ON(!pg);
	if(INM_PIN(pg, sizeof(inm_page_t))){
        ...
        INM_MEM_ZERO(pg, sizeof(inm_page_t));
        pg->cur_pg = (unsigned long *)__INM_GET_FREE_PAGE(alloc_flag, INM_KERNEL_HEAP);
        if (!pg->cur_pg) {
	    INM_UNPIN(pg, sizeof(inm_page_t));
            INM_KFREE(pg, sizeof(*pg), INM_KERNEL_HEAP);
            return;
...
/**
 * FUNCTION NAME:map_change_node_to_user
 *
 * DESCRIPTION :
 *
 * INPUT PARAMETERS :
                        ...
                        inm_devhandle_t *idhp)
{
     ...
     * can be returned to S2.
     */
    if(chg_node->mapped_thread) {
        dbg("Returning already mapped address: 0x%ld", chg_node->mapped_address);
        goto rel_mutex;
    }
...
print_change_node_off_length(change_node_t *chg_node)
{
    ...
    info("Printing change info trans_id:%lld start seq:%llu change node:%d splcnt:%u",
        chg_node->transaction_id, chg_node->changes.start_ts.ullSequenceNumber,
    ...
    info("Printing change info trans_id:%lld end seq:%llu\n",
        chg_node->transaction_id, chg_node->changes.end_ts.ullSequenceNumber);
...
verify_change_node(change_node_t *chg_node)
{
    ...
    if (chg_node->wostate != ecWriteOrderStateData) {
    */
    /*     Add these checks when bitmap mode ensures that deltas are zero..
           At the same time, deltas set to zero in close change node may not be 
           required..
        __inm_list_for_each(ptr, &chg_node->changes.md_pg_list) {
            pgp = inm_list_entry(ptr, inm_page_t, entry);
            i = 0;
            while (i < MAX_CHANGE_INFOS_PER_PAGE) {
                ...
                chg = (disk_chg_t *) ((char *)pgp->cur_pg +
                      (sizeof(disk_chg_t) * md_idx));
                if (chg->seqno_delta || chg->time_delta) {
                    return 1;
                }
                i++;
                count++;
    ...
    */
    if (chg_node->changes.change_idx) {
        ...
        md_idx = ((chg_node->changes.change_idx-1) % (MAX_CHANGE_INFOS_PER_PAGE));
        chg = (disk_chg_t *)((char *)pgp->cur_pg +
                                     (sizeof(disk_chg_t) * md_idx));
    ...
    switch(cnode->type) {
        case NODE_SRC_DATA: 
            ...
            if (IS_ERR(buf)) {
                ...
                error = inm_verify_change_node_data(buf, cnode->stream_len, 
                                                    0);
