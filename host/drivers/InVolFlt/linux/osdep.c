...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,30) && LINUX_VERSION_CODE <= KERNEL_VERSION(2,6,35))
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,9,0) || defined SLES12 || defined SLES15
#if LINUX_VERSION_CODE < KERNEL_VERSION(4,10,0) || defined SLES12 || defined SLES15
...
static inm_s32_t inm_one_AT_cdb_send(inm_block_device_t *, unsigned char *, inm_u32_t, 
                 inm_s32_t, unsigned char *, inm_u32_t);
...
#if LINUX_VERSION_CODE <= KERNEL_VERSION(2,6,13)
static inm_s32_t queue_request_scsi(inm_block_device_t *, unsigned char *, inm_u32_t , inm_s32_t ,
                unsigned char *, inm_u32_t );
#endif

//static target_context_t * get_tgt_ctxt_from_req(struct request_queue *);
//static inm_s32_t inm_get_page(inm_block_device_t *bdev, unsigned char page, unsigned char *buf);
...
/*
 * TODO: Move the timeout func above freeze_ioctl and remove this
 */
...
inm_s32_t iobarrier_add_volume_tags(tag_volinfo_t *tag_volinfop,
                                    tag_info_t *tag_info_listp, int nr_tags,
                                    int commit_pending,
                                    tag_telemetry_common_t *);
...
void lock_volumes(inm_s32_t vols, tag_volinfo_t *vol_list)
{
    ...
    while(num_vols < vols) {
        if(vol_list->ctxt) {
            /* Bug Fix: 4868: We explicitly use hard interrupt spin lock
             * version instead of volume_lock. See bug details for more information.
             */
            INM_SPIN_LOCK_IRQSAVE(&vol_list->ctxt->tc_lock, vol_list->ctxt->tc_lock_flag);
        }
        num_vols++;
        vol_list++;
    }
...
void unlock_volumes(inm_s32_t vols, tag_volinfo_t *vol_list)
{
    while(vols > 0) {
        ...
        if(vol_list[vols].ctxt) {
            /* Bug Fix: 4868: We explicitly use hard interrupt spin unlock
             * version instead of volume_unlock. See bug details for more information.
             */
            INM_SPIN_UNLOCK_IRQRESTORE(&vol_list[vols].ctxt->tc_lock, vol_list[vols].ctxt->tc_lock_flag);
        }
    }
}
inm_s32_t
...
get_bmfile_granularity(target_context_t *vcptr)
{
    ...
    if (!buffer) {
        dbg("Unable to allocate memory while getting bitmap file granularity from file");
        goto close_return;
    }

    if (flt_read_file(hdl, buffer, 0, INM_SECTOR_SIZE, (inm_s32_t *) &read)) {
...
/* Utility routines for ioctl processing
 */
...
if(IS_DBG_ENABLED(inm_verbosity, INM_IDEBUG)){
    ...
    *hdcp = (host_dev_ctx_t *) INM_KMALLOC(sizeof(host_dev_ctx_t), INM_KM_SLEEP, INM_KERNEL_HEAP);
    if (!(*hdcp))
        return 1;
    ...
    switch (dev_info->d_type) {
	    ...
	    if (bdev) {
            ...
            (*hdcp)->hdc_actual_end_sect = (*hdcp)->hdc_start_sect + get_capacity(bdev->bd_disk) - 1;
#endif
...
if(IS_DBG_ENABLED(inm_verbosity, INM_IDEBUG_MIRROR_IO)){
               info("get_tgt %s (start:%llu end:%llu) hdc_start:%llu hdc_end:%llu rw:%d bi_rw:%d",
                    tgt_ctxt->tc_guid, 
               ...
               /*  Bug Id - 7778: If any of the below conditions gets true,
                *  raise the resync flag as our driver doesn't have support
                *  to track the changes in case of a split I/O accross
                *  multiple volumes in a single bio.
                */
		     ...
		     inm_u32_t *actual_nr_pages, inm_s32_t flags)
{
    ...
    *actual_nr_pages = 0;
    while (*actual_nr_pages < nr_pages ) {
        ...
        page = (data_page_t *)INM_KMALLOC(sizeof(*page), INM_KM_SLEEP, INM_KERNEL_HEAP);
        if(!page)
            break;
...
inm_get_mirror_dev(mirror_vol_entry_t *vol_entry)
{
	...
	if((vol_entry->vol_flags & INM_AT_LUN) && !find_dc_at_lun_entry(vol_entry->tc_mirror_guid)){
		info("%s AT lun is not masked, failing the mirroring IOCTL", vol_entry->tc_mirror_guid);
		ret = -ENXIO;
		goto out;
	}
...
#endif
{
	...
	disk = bdev->bd_disk;
        ...
        if (disk && inm_get_parent_dev(disk)) {
            ...
            INM_BUG_ON(strncmp(sdp->vendor, "InMage  ", strlen("InMage  ")));
        }
     }
...
inm_get_scsi_id(char *path)
{
    ...
    buf = (char *)INM_KMALLOC(SCSI_INQ_BUFF_LEN, INM_KM_SLEEP, INM_KERNEL_HEAP);
    if(!buf) {
        err("INM_KMALLOC failed to allocate memory for buf");
        error = -ENOMEM;
        goto out;
    }
    INM_MEM_ZERO(buf, SCSI_INQ_BUFF_LEN);

    snprintf(guid->volume_guid, GUID_SIZE_IN_CHARS-1, path);
    ...
    if ((error = inm_get_page(bdev, PAGE_0, buf))) {
        goto out;
    }
    dbg("buf len is %d %u %x %x",buf[3], buf[4], buf[5],(unsigned char) buf[5]);
    for( i = 4; i < buf[3] + 4; i++){
           dbg("%c %x \n",buf[i], buf[i]);
    }
    for ( i = 4; i <= (buf[3]+3); i++){
        if( ((unsigned char)buf[i])  == PAGE_83 ){
            INM_MEM_ZERO(buf, SCSI_INQ_BUFF_LEN);
            if(!inm_get_page(bdev, PAGE_83, buf)){
                page = PAGE_83;
                error = 0;
                goto done;
            } else {
               dbg("PAGE_83 is not supported for %s", path);
               goto out;
            }
        }
    }
    for ( i = 4; i <= (buf[3]+10); i++){
        if(((unsigned char)buf[i])  == (unsigned char)PAGE_80){
            INM_MEM_ZERO(buf, SCSI_INQ_BUFF_LEN);
            if(!inm_get_page(bdev, PAGE_80, buf)){
                error = 0;
                page = PAGE_80;
            } else {
               dbg("PAGE_83 and PAGE_80 not supported for %s", path);
               error = 1;
               goto out;
            }
        }
...
out:
   dbg("leaving with error %d",error);
   if (guid){
       INM_KFREE(guid, sizeof(VOLUME_GUID), INM_KERNEL_HEAP);
   }
   if (buf){
      INM_KFREE(buf, SCSI_INQ_BUFF_LEN, INM_KERNEL_HEAP);
   }
   if (bdev){
      close_bdev(bdev, FMODE_READ);
   }
...
static inm_s32_t
inm_get_page(inm_block_device_t *bdev, unsigned char page, unsigned char *buf)
{
    struct gendisk *bd_disk = NULL;
    struct scsi_device *sdp = NULL;
    unsigned char cmd[INQUIRY_CMDLEN] = {0x12, 1, page, 0, SCSI_INQ_BUFF_LEN, 0};
    ...
    error = blk_rq_map_kern(q, rq, buf, SCSI_INQ_BUFF_LEN, __GFP_WAIT);
    if (error){
       err("err in blk_rq_map_kern errno is %d",error);
       goto out;
    }
    memcpy_s(rq->cmd, INQUIRY_CMDLEN, cmd, INQUIRY_CMDLEN);

    rq->cmd_len = INQUIRY_CMDLEN;
    memset(sense, 0, SCSI_SENSE_BUFFERSIZE);
    rq->sense = sense;
    rq->sense_len = 0;
    rq->timeout = 400;
    rq->flags |= (REQ_BLOCK_PC | REQ_FAILFAST | REQ_NOMERGE | REQ_SPECIAL);
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,11,0)
static inm_s32_t
inm_one_AT_cdb_send(inm_block_device_t *bdev, unsigned char *cmd, inm_u32_t cmdlen,
 ...
 */
    ...
    if (!scsi_normalize_sense(sense, SCSI_SENSE_BUFFERSIZE, &sshdr)) {
        ...
        info("response_code 0x%x, sense_key 0x%x, asc 0x%x, \
            ascq 0x%x, byte4 0x%x, byte5 0x%x, byte6 0x%x, additional_length 0x%x",
            sshdr.response_code, sshdr.sense_key, sshdr.asc, sshdr.ascq, sshdr.byte4,
...
inm_s32_t
try_reactive_offline_AT_path(target_context_t *tcp, unsigned char *cmd, inm_u32_t cmdlen,
                             inm_s32_t rw, unsigned char *buf, inm_u32_t buflen,
                             inm_u32_t flag)
{
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
    ...
    if(tcp->tc_dev_type == FILTER_DEV_MIRROR_SETUP){
       (*len) += sprintf((page+(*len)), "AT name, #IO issued, #successful IOs, #No of Byte written, Status, no of ref \n");
       volume_lock(tcp);
       hd = &(tcp->tc_dst_list);
       inm_list_for_each_safe(ptr, nextptr, hd){
           vol_entry = inm_container_of(ptr, mirror_vol_entry_t, next);
           (*len) += sprintf((page+(*len)), "%s, %llu, %llu, %llu, %s, %u\n", vol_entry->tc_mirror_guid,
...
void free_tc_global_at_lun(struct inm_list_head *dst_list)
{
	...
	while (!inm_list_empty(&local_at_list)) {
		dc_vol_entry =  inm_container_of(local_at_list.next, dc_at_vol_entry_t, dc_at_this_entry);
		inm_list_del(&(dc_vol_entry->dc_at_this_entry));
...
process_block_at_lun(inm_devhandle_t *handle, void * arg)
{
    ...
    at_lun_reconf = (inm_at_lun_reconfig_t *) INM_KMALLOC(sizeof(inm_at_lun_reconfig_t),
                                                          INM_KM_SLEEP, INM_KERNEL_HEAP);
    if (!at_lun_reconf) {
        err = 1;
        goto out;
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
    ...
    (at_lun_reconf->flag & ADD_AT_LUN_GLOBAL_LIST)? "add":"del", at_lun_reconf->atdev_name);
}
    if (at_lun_reconf->flag & ADD_AT_LUN_GLOBAL_LIST) {
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
        info("Add dc_at_blocking_entry ioctl for for %s", at_lun_reconf->atdev_name);
        ...
        if (!(dc_vol_entry->dc_at_dev=open_by_dev_path(at_lun_reconf->atdev_name, FMODE_WRITE))) {
           err("Fail to open AT LUN device %s",at_lun_reconf->atdev_name);
           err = INM_EINVAL;
           goto out;
        }
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
            info("Entry without driverfs_dev is not an InMage AT device %s",
                at_lun_reconf->atdev_name);
}
            err = INM_EINVAL;
            goto out;
	...
	if (dc_vol_entry->dc_at_dev->bd_disk->fops ==
            ...
            dbg("Device %s already masked ", at_lun_reconf->atdev_name);
            close_bdev(dc_vol_entry->dc_at_dev, FMODE_WRITE);
            dc_vol_entry->dc_at_dev = NULL;
            INM_KFREE(dc_vol_entry, sizeof(dc_at_vol_entry_t), INM_KERNEL_HEAP);
            err = INM_EEXIST;
            goto out1;
	}
        strcpy_s(dc_vol_entry->dc_at_name, INM_GUID_LEN_MAX, at_lun_reconf->atdev_name);
		dc_vol_entry->dc_at_dev->bd_disk->fops =
            &(driver_ctx->dc_at_lun.dc_at_drv_info.mod_dev_ops);
        INM_SPIN_LOCK_WRAPPER(&(driver_ctx->dc_at_lun.dc_at_lun_list_spn), flag);
        chk_dc_vol_entry = find_dc_at_lun_entry(at_lun_reconf->atdev_name);
        if (chk_dc_vol_entry) {
            INM_BUG_ON(strcmp(chk_dc_vol_entry->dc_at_name, at_lun_reconf->atdev_name));
            INM_SPIN_UNLOCK_WRAPPER(&(driver_ctx->dc_at_lun.dc_at_lun_list_spn), flag);
            if (IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
                dbg("Entry already exist for device %s", at_lun_reconf->atdev_name);
            }
            close_bdev(dc_vol_entry->dc_at_dev, FMODE_WRITE);
            dc_vol_entry->dc_at_dev = NULL;
            INM_KFREE(dc_vol_entry, sizeof(dc_at_vol_entry_t), INM_KERNEL_HEAP);
        ...
        INM_BUG_ON(!(at_lun_reconf->flag & DEL_AT_LUN_GLOBAL_LIST));
        dbg("deleting dc_at_blocking_entry ioctl for for %s", at_lun_reconf->atdev_name);
        INM_BUG_ON(at_lun_reconf->flag & ADD_AT_LUN_GLOBAL_LIST);
        INM_SPIN_LOCK_WRAPPER(&(driver_ctx->dc_at_lun.dc_at_lun_list_spn), flag);
        dc_vol_entry = find_dc_at_lun_entry(at_lun_reconf->atdev_name);
        if (!dc_vol_entry) {
            INM_SPIN_UNLOCK_WRAPPER(&(driver_ctx->dc_at_lun.dc_at_lun_list_spn), flag);
            info("lun is not blocked but still got unblocked called, device name %s", 
                 at_lun_reconf->atdev_name);
            goto out;
        }
        inm_list_del(&dc_vol_entry->dc_at_this_entry);
...
out1:
    if (at_lun_reconf) {
        INM_KFREE(at_lun_reconf, sizeof(inm_at_lun_reconfig_t), INM_KERNEL_HEAP);
    }
...
free_all_at_lun_entries()
{
	...
	dbg("entered free_all_at_lun_entries");
        ...
        INM_SPIN_LOCK_WRAPPER(&(driver_ctx->dc_at_lun.dc_at_lun_list_spn), lock_flag);
        inm_list_replace_init(&(driver_ctx->dc_at_lun.dc_at_lun_list), &llist);
        INM_SPIN_UNLOCK_WRAPPER(&(driver_ctx->dc_at_lun.dc_at_lun_list_spn), lock_flag);
        inm_list_for_each_safe(ptr, nextptr, &llist) {
                at_vol_entry =  inm_container_of(ptr, dc_at_vol_entry_t, dc_at_this_entry);
...
free_dc_vol_entry(dc_at_vol_entry_t *at_vol_entry)
{
    if (at_vol_entry->dc_at_dev) {
        if (at_vol_entry->dc_at_dev->bd_disk && (at_vol_entry->dc_at_dev->bd_disk->fops ==
            &(driver_ctx->dc_at_lun.dc_at_drv_info.mod_dev_ops))) {
...
{
        dc_at_vol_entry_t *at_vol_entry = NULL;
        at_vol_entry = (dc_at_vol_entry_t *)INM_KMALLOC(sizeof(dc_at_vol_entry_t), INM_KM_SLEEP, INM_KERNEL_HEAP);
        if(!at_vol_entry){
                goto out;
        }
...
find_dc_at_lun_entry(char *devname)
{
        ...
        inm_list_for_each_safe(ptr, nextptr, &(driver_ctx->dc_at_lun.dc_at_lun_list)) {
                at_vol_entry =  inm_container_of(ptr, dc_at_vol_entry_t, dc_at_this_entry);
...
/*
 * TIMER
 * TODO: There is a race between post_timeout_task_to_wthread and end_timer.
 */
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4,14,0))
void
post_timeout_task_to_wthread(inm_timer_t *timer)
{
    flt_timer_t *flt_timer = inm_container_of(timer, flt_timer_t, ft_timer);
    ...
    inm_list_for_each_safe(cur, next, &driver_ctx->tgt_list) {
        ...
        if (is_target_tag_commit_pending(tgt_ctxt)) {
            ...
            if (tag_committed ||                                 /* Committed */
                should_wakeup_s2_ignore_drain_barrier(tgt_ctxt)) /* Changes   */
...
process_freeze_volume(freeze_info_t *freeze_vol)
{
    ...
    inm_list_for_each_safe(ptr, nextptr, &driver_ctx->freeze_vol_list) {
        freeze_ele = inm_list_entry(ptr, freeze_vol_info_t, freeze_list_entry);
        if(freeze_ele) {
            if(!strcmp(freeze_ele->vol_name,
                       freeze_vol->vol_info->vol_name)) {
                dbg ("the volume [%s] is alreday frozen", freeze_ele->vol_name);
                ret = -1;
                /* release the lock */
                //INM_UP (&(driver_ctx->dc_cp_mutex));
                goto out;
    ...
    strcpy_s (freeze_vinfo->vol_name, TAG_VOLUME_MAX_LENGTH, freeze_vol->vol_info->vol_name);

    /* open by device path */
    freeze_vinfo->bdev = open_by_dev_path_v2 (freeze_vinfo->vol_name, FMODE_READ | FMODE_WRITE);
...
process_freeze_volume_ioctl(inm_devhandle_t *idhp, void __INM_USER *arg)
{
    ...
    if(freeze_vol->nr_vols <= 0) {
        err("Freeze Input Failed: Number of volumes can't be zero or negative");
        ret = -EINVAL;
        goto out_err;
    }
    ...
    for ( numvol = 0; numvol < freeze_vol->nr_vols; numvol++) {
        ...
        if(INM_COPYIN(freeze_vol->vol_info, arg, sizeof(*freeze_vol->vol_info))) {
            err("INM_COPYIN failed");
            ret = -EFAULT;
            break;
        }
        ...
        if(INM_COPYOUT(arg, freeze_vol->vol_info, sizeof(*freeze_vol->vol_info))) {
    ...
    if (no_of_vol_freeze_done) {
        ...
        if (driver_ctx->dc_cp == INM_CP_NONE) {
            ...
            memcpy_s(&driver_ctx->dc_cp_guid, sizeof(driver_ctx->dc_cp_guid),
                     freeze_vol->tag_guid, sizeof(freeze_vol->tag_guid));
...
out:
    if(freeze_vol) {
        if (freeze_vol->vol_info) {
            INM_KFREE(freeze_vol->vol_info, sizeof(volume_info_t), INM_KERNEL_HEAP);
            freeze_vol->vol_info = NULL;
        }
...
process_thaw_volume(thaw_info_t *thaw_vol)
{
    ...
    inm_list_for_each_safe(ptr, nextptr, &driver_ctx->freeze_vol_list) {
        freeze_ele = inm_list_entry(ptr, freeze_vol_info_t, freeze_list_entry);
        if(freeze_ele &&
           (!strcmp(freeze_ele->vol_name, thaw_vol->vol_info->vol_name))) {
           ...
           inm_list_del(&freeze_ele->freeze_list_entry);
            ...
            freeze_ele->sb = NULL;
            INM_KFREE(freeze_ele, sizeof(freeze_vol_info_t), INM_KERNEL_HEAP);
            freeze_ele = NULL;
            ptr = NULL;
            nextptr = NULL;
            ret = 0;
...
process_thaw_volume_ioctl(inm_devhandle_t *idhp, void __INM_USER *arg)
{
    ...
    if(thaw_vol->nr_vols <= 0) {
        err("Thaw Input Failed: Number of volumes can't be zero or negative");
        ret = -EINVAL;
        goto out_err;
    }
    ...
    /* allocate a buffer and reuse to store volume info for a set of volumes */
    ...
    for (numvol = 0; numvol < thaw_vol->nr_vols; numvol++) {
        ...
        if(INM_COPYIN(thaw_vol->vol_info, arg, sizeof(*thaw_vol->vol_info))) {
            err("INM_COPYIN failed");
            ret = -EFAULT;
            goto out_unlock;
        }
        ...
        if(INM_COPYOUT(arg, thaw_vol->vol_info, sizeof(*thaw_vol->vol_info))) {
...
out:
    if(thaw_vol) {
        if (thaw_vol->vol_info) {
            INM_KFREE(thaw_vol->vol_info, sizeof(volume_info_t), INM_KERNEL_HEAP);
            thaw_vol->vol_info = NULL;
        }
...
inm_fvol_list_thaw_on_timeout(wqentry_t *not_used)
{
    ...
    inm_list_for_each_safe(ptr, nextptr, &driver_ctx->freeze_vol_list){
        ...
        if(freeze_ele){
            ...
            /* thaw the volume using bdev and sb present in link list */
            inm_thaw_bdev(freeze_ele->bdev, freeze_ele->sb);
            close_bdev(freeze_ele->bdev, FMODE_READ | FMODE_WRITE);
            freeze_ele->bdev = NULL;
            freeze_ele->sb = NULL;
            /* delete entry from local list */
            //inm_list_del(ptr);
            inm_list_del(&freeze_ele->freeze_list_entry);
            INM_KFREE(freeze_ele, sizeof(freeze_vol_info_t), INM_KERNEL_HEAP);
            freeze_ele = NULL;
                               ...
                               tag_telemetry_common_t *tag_common)
{
    ...
    for (ptr = driver_ctx->tgt_list.next; ptr != &(driver_ctx->tgt_list);
        ...
        if(tgt_ctxt) {
            ...
            if(tgt_ctxt->tc_dev_type != FILTER_DEV_MIRROR_SETUP &&
               tgt_ctxt->tc_cur_wostate != ecWriteOrderStateData) {
                ...
                if (driver_ctx->dc_tag_drain_notify_guid &&
                    ...
                    info("The disk %s is in non write order state", tgt_ctxt->tc_guid);
                }
            ...
            ret = iobarrier_add_volume_tags(tag_volinfop, tag_list, nr_tags, 
                                            commit_pending, tag_common);
            if (ret) {
...
tag_fail:
			if (ret) {
                if (tag_common) {
				    ...
				    telemetry_log_tag_failure(tgt_ctxt, tag_common, ret, 
 ...
 * process ioctl to:
 *  --Create IO barrier.
 *  --Issue TAG for all or vacp specified protected volumes those are in data mode.
...
process_iobarrier_tag_volume_ioctl(inm_devhandle_t *idhp, void __INM_USER *arg)
{
    ...
    if (tag_common) {
        inm_get_tag_marker_guid(tag_vol->tag_names[0].tag_name, 
                              ...
                              tag_common->tc_guid, sizeof(tag_common->tc_guid));
    ...
    for(numvol = 0; numvol < tag_vol->nr_vols; numvol++) {
        ...
        ret = process_tag_volume(tag_vol, tag_list, TAG_COMMIT_NOT_PENDING);
        if(ret) {
            if (ret == INM_EAGAIN)
                tag_failed = 1;
...
process_create_iobarrier_ioctl(inm_devhandle_t *idhp, void __INM_USER *arg)
{
    ...
    bcreate = (flt_barrier_create_t *)INM_KMALLOC(sizeof(flt_barrier_create_t),
                                              INM_KM_SLEEP, INM_KERNEL_HEAP);
    if( !bcreate ) {
        err("INM_KMALLOC failed to allocate memory for flt_barrier_create_t");
        error = -ENOMEM;
        goto out;
    }
    ...
    error = create_io_barrier_all(bcreate->fbc_guid, sizeof(bcreate->fbc_guid),
...
out:
    if( bcreate )
        INM_KFREE(bcreate, sizeof(flt_barrier_create_t), INM_KERNEL_HEAP);
...
process_remove_iobarrier_ioctl(inm_devhandle_t *idhp, void __INM_USER *arg)
{
    ...
    bremove = (flt_barrier_remove_t *)INM_KMALLOC(sizeof(flt_barrier_remove_t),
                                              INM_KM_SLEEP, INM_KERNEL_HEAP);
    if( !bremove ) {
        err("INM_KMALLOC failed to allocate memory for flt_barrier_remove_t");
        error = -ENOMEM;
        goto out;
    }
    ...
    error = remove_io_barrier_all(bremove->fbr_guid, sizeof(bremove->fbr_guid));
    if( error )
        err("Remove barrier failed - %d", error);

out:
    if( bremove )
        INM_KFREE(bremove, sizeof(flt_barrier_remove_t), INM_KERNEL_HEAP);
...
inm_blkdev_get(inm_bio_dev_t *bdev)
{
    ...
    return (IS_ERR(blkdev_get_by_dev(bdev->bd_dev, FMODE_READ | FMODE_WRITE, NULL)) ? 1 : 0);
