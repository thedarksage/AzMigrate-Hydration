...
typedef struct target_volume_ctx
{
    ...
    inm_u32_t flags;
    struct inm_list_head init_list; /* list of "initiator_node_t" */
} target_volume_ctx_t;
...
update_initiator(target_volume_ctx_t* tvcptr,
                 char *iname)
{
    strncpy_s(tvcptr->initiator_name, MAX_INITIATOR_NAME_LEN, iname, MAX_INITIATOR_NAME_LEN - 1); /* iSCSI iqn ??? */
...
inm_s32_t get_at_lun_last_host_io_timestamp(AT_LUN_LAST_HOST_IO_TIMESTAMP *last_host_io_timestamp);
...
inm_s32_t fabric_read_persistent_attr(char* fname, char* fabric_attr, void **buf, inm_s32_t len, int* bytes_read);
...
inm_s32_t inm_validate_fabric_vol(target_context_t *tcp, inm_dev_info_t const *dip);
