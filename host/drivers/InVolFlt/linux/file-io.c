...
inm_s32_t flt_read_file (void *hdl, void *buffer, inm_u64_t offset, inm_u32_t length,
 ...
 * FinalOffset = vfs_llseek(fp, (loff_t)Offset, WhereToSeekFrom);
 **/
...
inm_s32_t read_full_file(char *filename, char *buffer, inm_u32_t length, inm_u32_t *bytes_read)
{
    inm_u32_t read = 0;
    void *hdl = NULL;
    loff_t size = 0;
    ...
    do
    {
	if (!flt_read_file(hdl, buffer, 0, length, (inm_s32_t *) &read))
	{
                  ...
                  inm_u32_t * bytes_written, int oflag)
{
    ...
    if (1 != flt_open_file(filename, O_RDWR | O_CREAT | O_SYNC | oflag, &hdl))
    {
	dbg("involflt: Opening file %s failed.", filename);
	return 0;
    }
    do
    {
	if (!flt_write_file(hdl, buffer, 0, length, (inm_s32_t *) bytes_written))
        {
...
{
    return __write_to_file(filename, buffer, length, bytes_written, O_TRUNC);
}
...
inm_cached_lookup(struct dentry *base, struct qstr *qstr, struct nameidata *nameidata)
{
	...
	if (!dir->d_op->d_revalidate(dir, nameidata) && !d_invalidate(dir)) {
	    dput(dir);
	    dir = NULL;
	}
    }
...
static struct dentry *
inm__lookup_hash(struct qstr *qstr, struct dentry *base_dir, struct nameidata *nameidata)
{
    struct dentry *dir;
...
#else
	error = base_dir->d_op->d_hash(base_dir, base_dir->d_inode, qstr);
#endif
	dir = ERR_PTR(error);
	if (error < 0)
	    goto out;
...
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,27)
    return inm__lookup_hash(&nameidata->last, nameidata->path.dentry, nameidata);
#else
    return inm__lookup_hash(&nameidata->last, nameidata->dentry, nameidata);
#endif
}
#endif
...
inm_lookup_create(struct nameidata *nameidata, inm_s32_t is_dentry)
{
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,27)
    mutex_lock_nested(&nameidata->path.dentry->d_inode->i_mutex, I_MUTEX_PARENT);
#else
    ...
    if (!is_dentry && nameidata->last.name[nameidata->last.len] && !dir->d_inode)
	goto enoent;
...
#else
	dir = kern_path_create(AT_FDCWD, tmp, &nameidata.path, LOOKUP_DIRECTORY);
#endif
	err = PTR_ERR(dir);
	if (!IS_ERR(dir)) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,27)
        ...
        err = vfs_mkdir(mnt_user_ns(nameidata.path.mnt), nameidata.path.dentry->d_inode, dir, mode);
#else
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,30)
	    err = vfs_mkdir(nameidata.path.dentry->d_inode, dir, mode);
#else
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,27)
            err = vfs_mkdir(nameidata.path.dentry->d_inode, dir, nameidata.path.mnt, mode);
#else
#if (defined suse && DISTRO_VER==10 && PATCH_LEVEL>=2) || LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,24)
	    err = vfs_mkdir(nameidata.dentry->d_inode, dir, nameidata.mnt, mode);
#else
    ...
    if (!error) {
        ...
        do {
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5,12,0)
                error = vfs_unlink(mnt_user_ns(path.mnt), parent_inode, dentry, &deleg);
#else
                error = vfs_unlink(parent_inode, dentry, &deleg);
#endif
                if (error && !deleg)
            ...
            if (deleg) {
                ...
                if (error)
                    err("Cannot break delegation %p, error %d", deleg, error);
                else
                    retry = 1;
            }
        } while(retry); /* cant use deleg as deleg=NULL in break_deleg_wait()*/
...
__inm_unlink(const char * pathname, char *unused)
{
    ...
    error = kern_path(name, LOOKUP_DIRECTORY | LOOKUP_PARENT, &parent_path);
    if (!error) {
        do {
            deleg = NULL;
            retry = 0;
            ...
            if (!error) {
                ...
                error = vfs_unlink(parent_path.dentry->d_inode, dentry, &deleg);
                if (error && !deleg)
            	    err("vfs_unlink failed with error %d\n", error);
...
#else
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,27)
       err = vfs_unlink(nameidata.path.dentry->d_inode, dir, nameidata.path.mnt);
#else
#if (defined suse && DISTRO_VER==10 && PATCH_LEVEL>=2) || LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,24)
	err = vfs_unlink(nameidata.dentry->d_inode, dir, nameidata.mnt);
#else
...
inm_unlink_symlink(const char * pathname, char *parent_path)
{
    ...
    if (IS_ERR(filp)) {
        if (PTR_ERR(filp) == -ELOOP) { /* O_NOFOLLOW gives ELOOP for slink */
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(3,10,0)
/*
 * TODO: Delete this function. Nobody is using it.
 */
long
...
#else
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,27)
        err = vfs_rmdir(nameidata.path.dentry->d_inode, dir, nameidata.path.mnt);
#else
#if (defined suse && DISTRO_VER==10 && PATCH_LEVEL>=2) || LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,24)
...
inm_prepare_tohandle_recursive_writes(struct inode *inodep)
{
	...
	t_inma_opsp = inm_get_inmaops_from_aops(mapping, INM_ORG_ADDR_SPACE_OPS);
	if (t_inma_opsp) {
    ...
    dbg("Add recursive object: Lookup = %p, Mapping = %p", t_inma_opsp, 
         mapping);
	inm_list_add_tail(&t_inma_opsp->ia_list, &driver_ctx->dc_inma_ops_list);
    ret = 0;
...
*inm_alloc_inma_ops(void)
{
	...
	a_opsp = (inm_address_space_operations_t *)
		  INM_KMALLOC(sizeof(inm_address_space_operations_t), INM_KM_SLEEP, INM_KERNEL_HEAP);
	if (!a_opsp) {
		goto exit;
	}

	inma_opsp = (inma_ops_t *) INM_KMALLOC(sizeof(inma_ops_t), INM_KM_SLEEP, INM_KERNEL_HEAP);
	if (!inma_opsp) {
		kfree(a_opsp);
		goto exit;
	}
			   ...
			   inm_u32_t lookup_flag)
{
	...
	inm_list_for_each_safe(lp, np, &driver_ctx->dc_inma_ops_list) {
		t_inma_opsp = (inma_ops_t *) inm_list_entry(lp, inma_ops_t, ia_list);
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_META))){
	...
	t_inma_opsp = inm_get_inmaops_from_aops(a_opsp, INM_ORG_ADDR_SPACE_OPS);
	...
	memcpy_s(t_inma_opsp->ia_dup_aopsp, sizeof(*a_opsp), a_opsp, sizeof(*a_opsp));
	inm_list_add_tail(&t_inma_opsp->ia_list, &driver_ctx->dc_inma_ops_list);
	INM_UP_WRITE(&driver_ctx->dc_inmaops_sem);
