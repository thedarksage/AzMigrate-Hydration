 ...
 * File       : filter_host.c
 *
 * Description: Device Mapper interface to perform filtering.
 */
...
extern void
involflt_completion(target_context_t *tgt_ctxt, write_metadata_t *wmd, void *bio, int lock_held);
...
req_queue_info_t *get_qinfo_from_kobj(struct kobject *kobj)
{
    ...
    if(dev_ktype->release == flt_queue_obj_rel)
        req_q = container_of(dev_ktype, req_queue_info_t, mod_kobj_type);
    else
        req_q = NULL;
...
reset_stable_pages_for_all_devs(void)
{
    ...
    INM_SPIN_UNLOCK_IRQRESTORE(&driver_ctx->dc_host_info.rq_list_lock, flag);
#else
    return;
#endif
}
...
inm_exchange_strategy(host_dev_ctx_t *hdcp)
{
    ...
    __inm_list_for_each(ptr, &hdcp->hdc_dev_list_head) {
        ...
        INM_SPIN_LOCK_IRQSAVE(&driver_ctx->dc_host_info.rq_list_lock, lock_flag);
        if(INM_ATOMIC_DEC_AND_TEST(&q_info->vol_users)){
            remove_qinfo_from_dc(q_info);
#if defined(RHEL9) && !defined(RHEL9_0)
            (void)xchg(&hdc_dev->hdc_disk_kobj_ptr->ktype, q_info->orig_bdev_ktype);
#endif
#if defined(SLES15SP3) || LINUX_VERSION_CODE >= KERNEL_VERSION(5, 8, 0)
            (void)xchg(&q_info->q->mq_ops, q_info->orig_mq_ops);
#else
            (void)xchg(&q_info->q->make_request_fn, q_info->orig_make_req_fn);
#endif
        ...
        INM_SPIN_UNLOCK_IRQRESTORE(&driver_ctx->dc_host_info.rq_list_lock, lock_flag);
    }
}
    ...
    struct kobject **hdc_disk_kobj_ptr)
{
    ...
    if (!bdev_is_partition(bdev)) {
...
#if defined(RHEL9) && !defined(RHEL9_0)
        q_info->orig_bdev_ktype = bdev_kobj(bdev)->ktype;
        memcpy_s(&q_info->mod_bdev_ktype, sizeof(struct kobj_type),
            q_info->orig_bdev_ktype, sizeof(struct kobj_type));
        q_info->mod_bdev_ktype.release = &flt_disk_obj_rel;
        (void)xchg(&bdev_kobj(bdev)->ktype, &q_info->mod_bdev_ktype);
#else
        ...
        *hdc_disk_kobj_ptr = bdev_kobj(bdev);
        ...
        memcpy_s(&q_info->mod_bdev_ktype, sizeof(struct kobj_type),
            q_info->orig_bdev_ktype, sizeof(struct kobj_type));
        q_info->mod_bdev_ktype.release = &flt_part_obj_rel;
        (void)xchg(&bdev_kobj(bdev)->ktype, &q_info->mod_bdev_ktype);
#else
    ...
    if(bdev == bdev->bd_contains) {
        if(flt_disk_release_fn == NULL) {
            INM_BUG_ON(bdev->bd_disk->part0.__dev.kobj.ktype->release == NULL);
            flt_disk_release_fn = bdev->bd_disk->part0.__dev.kobj.ktype->release;
            disk_ktype_ptr = bdev->bd_disk->part0.__dev.kobj.ktype;
        }
        (void)xchg(&bdev->bd_disk->part0.__dev.kobj.ktype->release, &flt_disk_obj_rel);
        *hdc_disk_kobj_ptr = &bdev->bd_disk->part0.__dev.kobj;
    } else {
        if(flt_part_release_fn == NULL) {
            INM_BUG_ON(bdev->bd_part->__dev.kobj.ktype == NULL);
            flt_part_release_fn = bdev->bd_part->__dev.kobj.ktype->release;
            part_ktype_ptr = bdev->bd_part->__dev.kobj.ktype;
        }
        (void)xchg(&bdev->bd_part->__dev.kobj.ktype->release, &flt_part_obj_rel);
    ...
    if(!bdev->bd_part) {
        if(flt_disk_release_fn == NULL) {
            INM_BUG_ON(bdev->bd_disk->dev.kobj.ktype->release == NULL);
            flt_disk_release_fn = bdev->bd_disk->dev.kobj.ktype->release;
            disk_ktype_ptr = bdev->bd_disk->dev.kobj.ktype;
        }
        (void)xchg(&bdev->bd_disk->dev.kobj.ktype->release, &flt_disk_obj_rel);
        *hdc_disk_kobj_ptr = &bdev->bd_disk->dev.kobj;
    } else {
        if(flt_part_release_fn == NULL) {
            INM_BUG_ON(bdev->bd_part->dev.kobj.ktype == NULL);
            flt_part_release_fn = bdev->bd_part->dev.kobj.ktype->release;
            part_ktype_ptr = bdev->bd_part->dev.kobj.ktype;
        }
        (void)xchg(&bdev->bd_part->dev.kobj.ktype->release, &flt_part_obj_rel);
        *hdc_disk_kobj_ptr = &bdev->bd_part->dev.kobj;
    }
#else
    if(!bdev->bd_part) {
        if(flt_disk_release_fn == NULL) {
            INM_BUG_ON(bdev->bd_disk->kobj.kset->ktype->release == NULL);
            flt_disk_release_fn = bdev->bd_disk->kobj.kset->ktype->release;
            disk_ktype_ptr = bdev->bd_disk->kobj.kset->ktype;
        } 
        (void)xchg(&bdev->bd_disk->kobj.kset->ktype->release, &flt_disk_obj_rel);
        *hdc_disk_kobj_ptr = &bdev->bd_disk->kobj;    
    } else {
        if(flt_part_release_fn == NULL) {
        INM_BUG_ON(bdev->bd_part->kobj.ktype == NULL);
        flt_part_release_fn = bdev->bd_part->kobj.ktype->release;
        part_ktype_ptr = bdev->bd_part->kobj.ktype;
        }
        (void)xchg(&bdev->bd_part->kobj.ktype->release, &flt_part_obj_rel);
...
alloc_and_init_qinfo(inm_block_device_t *bdev, target_context_t *ctx)
{
    ...
    if (q_info) {
        INM_KFREE(new_q_info, sizeof(req_queue_info_t), INM_KERNEL_HEAP);
        goto out;
    }
    ...
    //(void)xchg(&(q_info->orig_kobj_type), q->kobj.ktype);
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,9,0)
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_META))){
    ...
    while (seg_rem) {
        if (*bytes_res_node) {
            ...
            dbg("SPage = %p, SOffset = %d, DPage = %p, DOffset = %d copy = %d", 
                src, seg_offset, dst, pg_offset, to_copy); 
    ...
    if (!(node->flags & KDIRTY_BLOCK_FLAG_SPLIT_CHANGE_MASK) &&
        ((((org_poffset + wdatap->wd_cplen) & (INM_PAGESZ - 1)) != pg_offset) || 
        bytes_to_copy)) {
        ...
        if (!print_once) {
            ...
            err("bytes_res_node: %d to_copy = %d seg_rem = %d "
                ...
                *bytes_res_node, to_copy, seg_rem, seg_offset, pg_rem); 
                                     ...
                                     struct inm_list_head *change_node_list)
{
    ...
    while (iolen) {
        ...
        change_node_list = copy_vec_to_data_pages(tgt_ctxt, &vec, wdatap,
                                                  change_node_list, 
                                                  &bytes_res_node);
                                   ...
                                   struct inm_list_head *change_node_list)
{
    ...
    inm_buf_t *bio = wdatap->wd_privp;
    dm_bio_info_t *info = bio->bi_private;
    inm_bvec_iter_t idx = INM_BVEC_ITER_INIT();
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,16,0)
    inm_bvec_iter_t iter = INM_BVEC_ITER_INIT();
    struct bio_vec vec = {0};
#endif
    ...
    __bio_for_each_segment(bvec, bio, idx, info->bi_idx) {
#endif
        change_node_list = copy_vec_to_data_pages(tgt_ctxt, bvec, wdatap,
                                                      change_node_list, 
                                                      &bytes_res_node);
        copied += bvec->bv_len;
        if ((wdatap->wd_cplen - copied) == 0)
            break;
    }
}
*/
...
void
copy_bio_data_to_data_pages(target_context_t *tgt_ctxt,
                            inm_wdata_t *wdatap, 
                            struct inm_list_head *change_node_list)
{
    inm_buf_t *bio = wdatap->wd_privp;
    ...
    if (INM_IS_OFFLOAD_REQUEST_OP(bio)) {
        if (INM_IS_BIO_WOP(bio, INM_REQ_WRITE_SAME))
            copy_single_vector_bio_data_to_data_pages(tgt_ctxt, wdatap, 
                                                 change_node_list);
        else
            copy_no_vector_bio_data_to_data_pages(tgt_ctxt, wdatap, 
                                             change_node_list);
    } else {
        copy_normal_bio_data_to_data_pages(tgt_ctxt, wdatap, change_node_list);
    }
}
...
inm_bio_supported_in_data_mode(struct bio *bio)
{
    if (!INM_IS_SUPPORTED_REQUEST_OP(bio)) {
        err("Unsupported IO Op: 0x%lx", (unsigned long)inm_bio_op(bio));
        return 0;
    }
    ...
    if (INM_IS_OFFLOAD_REQUEST_OP(bio) &&
        ...
        err("Large offload IO: 0x%lx:%u", 
            (unsigned long)inm_bio_op(bio), INM_BUF_COUNT(bio));
        return 0;
    }
...
free_bio_info:
    ...
    if (INM_BUF_COUNT(bio) == 0) {
        if (bio_info->orig_bio_copy) {
            INM_KFREE(bio_info->orig_bio_copy, sizeof(struct bio), INM_KERNEL_HEAP);
        }
...
flt_end_io_chain(struct bio *bio, inm_s32_t error)
{
    ...
    do {
        ...
        if (bio != bio_to_complete[cpuid]) {
            dbg("CHILD: Bio from parent(%d): %p -> %p", cpuid, bio, 
...
static_inline void
flt_save_bio_info(target_context_t *ctx, dm_bio_info_t **bio_info, struct bio *bio)
{
...
#if defined(SLES15SP3) || LINUX_VERSION_CODE >= KERNEL_VERSION(5, 8, 0)
    *bio_info = INM_KMALLOC(sizeof(dm_bio_info_t), GFP_ATOMIC | __GFP_NOWARN, INM_KERNEL_HEAP);
    if (!(*bio_info)) {
    ...
    *bio_info = INM_MEMPOOL_ALLOC(hdcp->hdc_bio_info_pool, INM_KM_SLEEP);
    ...
    if (full_disk) {
        ...
        (*bio_info)->bi_sector  = (INM_BUF_SECTOR(bio) - hdcp->hdc_start_sect);
    }
    ...
    if (ctx->tc_dev_type == FILTER_DEV_HOST_VOLUME) {
        ...
        while (remaining_length) {
            alloced_from_pool = 0;
#if defined(SLES15SP3) || LINUX_VERSION_CODE >= KERNEL_VERSION(5, 8, 0)
            chg_node = INM_KMALLOC(sizeof(change_node_t), GFP_ATOMIC | __GFP_NOWARN, INM_KERNEL_HEAP);
            if (!chg_node) {
                chg_node = inm_alloc_chgnode();
                alloced_from_pool = 1;
            }
#else
            chg_node = INM_KMALLOC(sizeof(change_node_t), INM_KM_NOIO, INM_KERNEL_HEAP);
#endif
            if (!chg_node) {
                queue_worker_routine_for_set_volume_out_of_sync(ctx,
...
get_root_disk(struct bio *bio)
{
    ...
    if (ctx) {
        ...
        if (!(ctx->tc_flags & (VCF_VOLUME_DELETING | VCF_VOLUME_CREATING)) &&
            !(ctx->tc_flags & VCF_ROOT_DEV)) {
            info("Root Disk - %s (%s)", ctx->tc_guid, ctx->tc_pname);
...
is_our_io(struct bio *biop) 
{
...
#if defined (RHEL8)
        if (unlikely(lcwModeOn && strcmp(current->comm, "inmshutnotify") == 0)) {
    ...
    if (!virt_addr_valid(bvecp) || !INM_VIRT_ADDR_VALID(INM_PAGE_TO_VIRT(bvecp->bv_page)))
        return FALSE;
    ...
    t_inma_opsp = inm_get_inmaops_from_aops(a_opsp, INM_DUP_ADDR_SPACE_OPS);
    INM_UP_READ(&driver_ctx->dc_inmaops_sem);
    if (t_inma_opsp) {
#ifdef INM_RECUSIVE_ADSPC
        INM_BUG_ON(t_inma_opsp->ia_mapping != bvecp->bv_page->mapping);
#endif
        dbg("Recursive write: Lookup = %p, Mapping = %p", t_inma_opsp, a_opsp);
...
void inm_free_bio_info(dm_bio_info_t *bio_info)
{
    ...
    if (bio_info->dm_bio_flags & BINFO_ALLOCED_FROM_POOL) {
        ...
        inm_list_add_tail(&bio_info->entry, &driver_ctx->dc_bioinfo_list);
...
void inm_free_bioinfo_pool(void)
{
    ...
    while (!inm_list_empty(&driver_ctx->dc_bioinfo_list)) {
        info = inm_list_entry(driver_ctx->dc_bioinfo_list.next, dm_bio_info_t, entry);
...
void inm_alloc_bioinfo_pool(void)
{
    ...
    if (INM_ATOMIC_READ(&driver_ctx->dc_nr_bioinfo_alloced) < NR_MAX_ALLOCATIONS) {
        info = INM_KMALLOC(sizeof(dm_bio_info_t), GFP_NOIO, INM_KERNEL_HEAP);
    if (!info)
            return;
...
void inm_free_chdnodes_pool(void)
{
    ...
    while (!inm_list_empty(&driver_ctx->dc_chdnodes_list)) {
        chg_node = inm_list_entry(driver_ctx->dc_chdnodes_list.next, change_node_t, next);
...
void inm_alloc_chdnodes_pool(void)
{
    ...
    if (INM_ATOMIC_READ(&driver_ctx->dc_nr_chdnodes_alloced) < NR_MAX_ALLOCATIONS) {
        chg_node = INM_KMALLOC(sizeof(change_node_t), GFP_NOIO, INM_KERNEL_HEAP);
        if (!chg_node)
            return;

        INM_SPIN_LOCK_IRQSAVE(&driver_ctx->page_pool_lock, lock_flag);
        INM_ATOMIC_INC(&driver_ctx->dc_nr_chdnodes_alloced);
        inm_list_add_tail(&chg_node->next, &driver_ctx->dc_chdnodes_list);
    INM_SPIN_UNLOCK_IRQRESTORE(&driver_ctx->page_pool_lock, lock_flag);
    }
}
...
void inm_alloc_pools(void)
{
    ...
    if(INM_ATOMIC_READ(&driver_ctx->dc_nr_bioinfo_alloced) < NR_MAX_ALLOCATIONS)
        nr_bioinfos = NR_MAX_ALLOCATIONS - INM_ATOMIC_READ(&driver_ctx->dc_nr_bioinfo_alloced);

    if (INM_ATOMIC_READ(&driver_ctx->dc_nr_chdnodes_alloced) < NR_MAX_ALLOCATIONS)
        nr_chgnodes = NR_MAX_ALLOCATIONS - INM_ATOMIC_READ(&driver_ctx->dc_nr_chdnodes_alloced);

    if (INM_ATOMIC_READ(&driver_ctx->dc_nr_metapages_alloced) < NR_MAX_ALLOCATIONS)
        nr_metapages = NR_MAX_ALLOCATIONS - INM_ATOMIC_READ(&driver_ctx->dc_nr_metapages_alloced);
    INM_SPIN_UNLOCK_IRQRESTORE(&driver_ctx->page_pool_lock, lock_flag);
...
void inm_balance_pools(void)
{
    ...
    INM_SPIN_LOCK_IRQSAVE(&driver_ctx->page_pool_lock, lock_flag);
    if (INM_ATOMIC_READ(&driver_ctx->dc_nr_bioinfo_alloced) > NR_MAX_ALLOCATIONS)
        nr_free = INM_ATOMIC_READ(&driver_ctx->dc_nr_bioinfo_alloced) - NR_MAX_ALLOCATIONS;
    ...
    nr_free = 0;
    if (INM_ATOMIC_READ(&driver_ctx->dc_nr_chdnodes_alloced) > NR_MAX_ALLOCATIONS)
        nr_free = INM_ATOMIC_READ(&driver_ctx->dc_nr_chdnodes_alloced) - NR_MAX_ALLOCATIONS;
    ...
    while (nr_free) {
     ...
     nr_free = 0;
    if (INM_ATOMIC_READ(&driver_ctx->dc_nr_metapages_alloced) > NR_MAX_ALLOCATIONS)
        nr_free = INM_ATOMIC_READ(&driver_ctx->dc_nr_metapages_alloced) - NR_MAX_ALLOCATIONS;
...
process_bio:
    /* Handling reads */
    if (!inm_bio_is_write(bio) || !INM_BUF_COUNT(bio) || INM_IS_TEST_MIRROR(bio))
        goto next_bio;

    /* Trim requests */
    if (inm_bio_is_discard(bio) && !INM_DISCARD_ZEROES_DATA(q_info->q)) {
        dbg("Trim request: start sector = %llu, len = %d, vcnt = %d",
            (inm_u64_t)INM_BUF_SECTOR(bio), INM_BUF_COUNT(bio), bio->bi_vcnt);
        goto next_bio;
    }
...
flt_make_request_fn(struct request_queue *q, struct bio *bio)
{
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,14,0) || defined SLES12SP4 || defined SLES12SP5 || defined SLES15
    ...
    if (ctx) {
        ...
        if(is_target_filtering_disabled(ctx) || 
           (ctx->tc_dev_type == FILTER_DEV_MIRROR_SETUP && is_target_mirror_paused(ctx))) {
            INM_UP_READ(&driver_ctx->tgt_list_sem);
            dbg("mirror paused for scsi_id %s",ctx->tc_pname);
            goto map_and_exit;
        }
        if(is_target_read_only(ctx)) {
            INM_UP_READ(&driver_ctx->tgt_list_sem);
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 4, 0)
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 13, 0) || defined SLES12SP4 || defined SLES12SP5 || defined SLES15
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG_MIRROR_IO))) {
                ...
                atbio_q->make_request_fn(atbio_q, &(atbuf_wrap->imb_atbuf_buf));
                put_tgt_ctxt(ctx);
            }
            break;
...
flt_disk_removal(struct kobject *kobj)
{
    ...
    set_tag_drain_notify_status(ctx, TAG_STATUS_DROPPED, DEVICE_STATUS_REMOVED);
    volume_unlock(ctx);
     ...
     * set VolumeFilteringDisabled to FALSE - a hack
     */
    if (save_guid) {
...
void flt_disk_obj_rel(struct kobject *kobj)
{
    ...
    dbg("%s: queue = %p, kobj = %p, qkobj = %p", disk->disk_name, disk->queue,
        kobj, qkobj);
...
#else
static inm_s32_t completion_check_endio(struct bio *bio, inm_u32_t done, inm_s32_t error)
#endif
{
    ...
    dbg("Completion called in Soft Interrupt context for %s", ctx->tc_guid);
...
flt_revalidate_disk(struct gendisk *disk)
{
     ...
     *
     * TODO: There still is a window between when this
     * function is called and by the time we take the
     * volume lock in which unregister_disk_notification()
     * would lead disks original revalidate_disk() not
     * not being called.
     */
    volume_lock(tgt_ctx);
    if (hdc_dev->hdc_fops &&                
    ...
    if (nr_sect != (hdcp->hdc_volume_size >> 9)) {
        err("%s capacity change ... marking for resync", tgt_ctx->tc_guid);
        queue_worker_routine_for_set_volume_out_of_sync(tgt_ctx, 
                                                       ERROR_TO_REG_INVALID_IO,
                                                       -EINVAL);
...
unregister_disk_change_notification(target_context_t *ctx, host_dev_t *hdc_dev) 
{
    ...
    if (disk->fops->revalidate_disk != flt_revalidate_disk) {
        INM_BUG_ON(disk->fops->revalidate_disk != flt_revalidate_disk);
        return;
    }
#endif
...
stack_host_dev(target_context_t *ctx, inm_dev_extinfo_t *dinfo)
{
    ...
    hdcp->hdc_bio_info_pool =
        INM_MEMPOOL_CREATE(BIO_INFO_MPOOL_SIZE, INM_MEMPOOL_ALLOC_SLAB,
...
retry:
    __inm_list_for_each(ptr1, &driver_ctx->tgt_list) {
       ...
       if (tgt_ctx->tc_dev_type == FILTER_DEV_HOST_VOLUME || 
           ...
           host_dev_ctx_t *hdcp_ptr = (host_dev_ctx_t *) tgt_ctx->tc_priv;
    ...
    switch (dinfo->d_type) {
        case FILTER_DEV_HOST_VOLUME:
            ...
            hdc_dev = NULL;
            __inm_list_for_each(ptr2, &hdcp->hdc_dev_list_head) {
                hdc_dev = inm_list_entry(ptr2, host_dev_t, hdc_dev_list);
            ...
            if (hdc_dev) {
                ...
                init_tc_kobj(q_info, bdev, &hdc_dev->hdc_disk_kobj_ptr);
            }
            else
                ret = -EINVAL;
        ...
        case FILTER_DEV_MIRROR_SETUP:
            __inm_list_for_each(ptr1, dinfo->src_list) {
                vol_entry = inm_list_entry(ptr1, mirror_vol_entry_t, next);

                __inm_list_for_each(ptr2, &hdcp->hdc_dev_list_head) {
                    hdc_dev = inm_list_entry(ptr2, host_dev_t, hdc_dev_list);
                    if (hdc_dev->hdc_dev == vol_entry->mirror_dev->bd_inode->i_rdev)
                        break;
                    hdc_dev = NULL;
                }
                if (hdc_dev) {
                ...
                init_tc_kobj(q_info, vol_entry->mirror_dev, &hdc_dev->hdc_disk_kobj_ptr);
            }
            break;
        case FILTER_DEV_FABRIC_LUN:
            INM_BUG_ON(1);
    ...
    if ((ctx->tc_flags & VCF_FILTERING_STOPPED) || 
        ...
        set_unsignedlonglong_vol_attr(ctx, VolumeNblks, hdcp->hdc_nblocks);
    }
...
/* LCW, UMOUNT NOTIFY CODE : START */
//stores root device's inm_dev_t value
inm_s32_t get_root_info(void)
{
    ...
    return 0;
} /*  LCW, UMOUNT NOTIFY CODE : END  */

/*
 * TODO: Merge with get_root_info
 */
    ...
    do {
        ...
        if (hdcp) {
            __inm_list_for_each(hptr, &hdcp->hdc_dev_list_head) {
                ...
                dbg("Disk ptr = %p = %s", 
                    hdc_dev->hdc_disk_ptr, hdc_dev->hdc_disk_ptr->disk_name);
...
#endif
{
    ...
    case IOCTL_INMAGE_VOLUME_STACKING:
        error = process_volume_stacking_ioctl(filp, (void __user *)arg);
        break;    

    case IOCTL_INMAGE_MIRROR_VOLUME_STACKING:
        error = process_mirror_volume_stacking_ioctl(filp, (void __user *)arg);
        break;    
    ...
    case IOCTL_INMAGE_SERVICE_SHUTDOWN_NOTIFY:
        error = process_shutdown_notify_ioctl(filp, (void __user *) arg);
        break;
    ...
    case IOCTL_INMAGE_START_FILTERING_DEVICE_V2:
        error = process_start_filtering_ioctl(filp, (void __user *)arg);
        break;
    ...
    case IOCTL_INMAGE_TAG_COMMIT_V2:
         error = process_commit_revert_tag_ioctl(filp, (void __user *)arg);
         break;
    
    case IOCTL_INMAGE_CREATE_BARRIER_ALL:
         error = process_create_iobarrier_ioctl(filp, (void __user *)arg);
         break;

    case IOCTL_INMAGE_REMOVE_BARRIER_ALL:
         error = process_remove_iobarrier_ioctl(filp, (void __user *)arg);
         break;

    case IOCTL_INMAGE_IOBARRIER_TAG_VOLUME:
         error = process_iobarrier_tag_volume_ioctl(filp, (void __user *)arg);
         break;

    case IOCTL_INMAGE_START_MIRRORING_DEVICE:
        error = process_start_mirroring_ioctl(filp, (void __user *)arg);
        break;
    ...
    case IOCTL_INMAGE_GET_VOLUME_FLAGS:
        error = process_get_volume_flags_ioctl(filp, (void __user *)arg);
        break;
            
    case IOCTL_INMAGE_SET_VOLUME_FLAGS:
        error = process_set_volume_flags_ioctl(filp, (void __user *)arg);
        break;
    ...
    case IOCTL_INMAGE_SYS_SHUTDOWN:
        error = process_sys_shutdown_notify_ioctl(filp, (void __user *)arg);
        break;

    case IOCTL_INMAGE_SYS_PRE_SHUTDOWN:
        error = process_sys_pre_shutdown_notify_ioctl(filp, (void __user *)arg);
        break;
    ...
    case IOCTL_INMAGE_WAKEUP_ALL_THREADS:
        error = process_wake_all_threads_ioctl(filp,(void __user *)arg);
        break;
    ...
    case IOCTL_INMAGE_GET_DRIVER_VERSION:
        error = process_get_driver_version_ioctl(filp,(void __user *)arg);
        break;
    ...
    case IOCTL_INMAGE_AT_LUN_LAST_WRITE_VI:
        error = process_at_lun_last_write_vi(filp, (void __user *) arg);
        break;

    case IOCTL_INMAGE_AT_LUN_LAST_HOST_IO_TIMESTAMP:
        error = process_at_lun_last_host_io_timestamp(filp, (void __user *) arg);
        break;
    ...
    case IOCTL_INMAGE_SET_INVOLFLT_VERBOSITY:
        error = process_set_involflt_verbosity(filp, (void __user *)arg);
        dbg("IOCTL_INMAGE_SET_INVOLFLT_VERBOSITY ioctl err = %d\n", error);
        break;

    case IOCTL_INMAGE_MIRROR_TEST_HEARTBEAT:
        error = process_mirror_test_heartbeat(filp, (void __user *)arg);
    ...
    case IOCTL_INMAGE_GET_BLK_MQ_STATUS:
        error = process_get_blk_mq_status_ioctl(filp, (void __user*)arg);
        dbg("IOCTL_INMAGE_GET_BLK_MQ_STATUS ioctl err = %d", error);
        break;

    case IOCTL_INMAGE_REPLICATION_STATE:
        error = process_replication_state_ioctl(filp, (void __user*)arg);
    ...
    case IOCTL_INMAGE_COMMITDB_FAIL_TRANS:
        error = process_commitdb_fail_trans_ioctl(filp, (void __user*)arg);
        dbg("IOCTL_INMAGE_COMMITDB_FAIL_TRANS err = %d\n", error);
        break;

    case IOCTL_INMAGE_GET_CXSTATS_NOTIFY:
        error = process_get_cxstatus_notify_ioctl(filp, (void __user*)arg);
...
#endif
{
    /* Need to distinguish between noraml close and S2 shutdown in which
     * we need to perform the cleanup.
     */

    /* Perform cleanup due to S2 shutdown. private_data will be
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,10)) ||        \
    (LINUX_VERSION_CODE < KERNEL_VERSION(2,6,10) && !defined(CONFIG_HIGHMEM))
    struct inm_list_head *ptr, *hd;
    data_page_t *page;
...
inm_s32_t __init involflt_init(void)
{
    ...
    if((driver_state & DRV_LOADED_FULLY) && is_root_filesystem_is_rootfs()){
...
block_sd_open()
{
    ...
    for(i = 0; i < 26; i++) {
        snprintf(guid->volume_guid, GUID_SIZE_IN_CHARS-1, "/dev/sd%c", 'a' + i);
...
inm_s32_t
inm_prepare_atbuf(inm_mirror_atbuf *atbuf_wrap, inm_buf_t *bp, mirror_vol_entry_t *vol_entry, inm_u32_t count)
{
    inm_u32_t more = 0;
...
#if ((LINUX_VERSION_CODE < KERNEL_VERSION(5,12,0) && LINUX_VERSION_CODE >= KERNEL_VERSION(4,14,0)) || defined SLES12SP4 || defined SLES12SP5 || defined SLES15) && (!defined SLES15SP4)
    ...
    atbuf_wrap->imb_atbuf_iosz = INM_BUF_COUNT(&(atbuf_wrap->imb_atbuf_buf));
