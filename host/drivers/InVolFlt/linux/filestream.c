 ...
 * NOTES
 * 
 * return value :    fstream_t ptr    - for success
...
fstream_t *fstream_ctr(void *ctx)
{
    ...
    fs = (fstream_t *)INM_KMALLOC(sizeof(*fs), INM_KM_SLEEP, INM_KERNEL_HEAP);
...
if(IS_DBG_ENABLED(inm_verbosity, (INM_IDEBUG | INM_IDEBUG_BMAP))){
    info("leaving with status = %d , num of bytes written into bmap = %d",
	 (nr == size), (int)nr);
}
