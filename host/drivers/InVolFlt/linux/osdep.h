...
typedef struct _req_q_info
{
    ...
    struct kobj_type mod_bdev_ktype;
    struct kobj_type *orig_bdev_ktype;
#endif
} req_queue_info_t;
...
#define warn(format, arg...) \
    printk(KERN_WARN "%s[%s:%d (WARN)]: " format "\n" , DRIVER_NAME, __FUNCTION__, \
        __LINE__, ## arg)
/*
#define bio_for_each_segment_in_range(bvl, bio, start, end) \
        for(bvl = bio_iovec_idx((bio), start); \
            start <= end; bvl++, start++)
*/
...
#endif
/* BITMAP_EFFORT TODO replace with actual INM_EROFS readonly filesystem */
