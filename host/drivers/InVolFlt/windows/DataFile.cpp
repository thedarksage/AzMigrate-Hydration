...
NTSTATUS
CDataFile::Open(PUNICODE_STRING InputFileName, ULONGLONG ullAllocationSize)
{
    NTSTATUS        Status;
...
CDataFile::DeleteFile(PUNICODE_STRING InputFileName)
{
    ...
    do {
        ...
        Status = File.Open(FileName, 
                           ...
                           0);
        if (!NT_SUCCESS(Status)) {
            LogFileOpenFailure(Status, FileName, SourceLocationCDataFileDelete);
            break;
        }

        Status = File.DeleteOnClose();
        if (!NT_SUCCESS(Status)) {
            LogDeleteOnCloseFailure(Status, FileName, SourceLocationCDataFileDelete);
        }
