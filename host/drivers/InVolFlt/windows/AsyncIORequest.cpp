...
PVOID mem; 
    ...
    mem = (PVOID) ExAllocateFromNPagedLookasideList(&AsyncIORequest::lookasideList);
    if (mem) 
...
extern "C" NTSTATUS AsyncIORequestReadWriteCompletion(
...
TRC
    ...
    // Free MDL allocated by IoBuildAsynchronousFsdRequest
    ...
    //Mandatory to send the following status
...
NTSTATUS AsyncIORequest::ReadWrite(PVOID buffer, ULONG32 size, ULONG64 offset, ULONG32 IrpMJ, ULONG ulFlags)
{
TRC
...
NTSTATUS AsyncIORequest::Write(PVOID buffer, ULONG32 size, ULONG64 offset, ULONG ulFlags)
{
TRC
    return ReadWrite(buffer, size, offset, IRP_MJ_WRITE, ulFlags);
}

NTSTATUS AsyncIORequest::Read(PVOID buffer, ULONG32 size, ULONG64 offset, ULONG ulFlags)
{
TRC
    return ReadWrite(buffer, size, offset, IRP_MJ_READ, ulFlags);
}
