        ...
        while (bitsToProcess > 0) {
            ...
            if (NT_SUCCESS(status)) {
                ...
                if (nbrBytesChanged > 0) {
                    segmentMapper->UnlockAndMarkDirty(byteOffset + (firstByteChanged - bitBuffer));
                } else {
...
void * __cdecl SegmentedBitmap::operator new(size_t size) {
	return (PVOID)malloc((ULONG32)size, (ULONG32)TAG_SEGMENTEDBITMAP_OBJECT, NonPagedPool);
}
