...
// These are flags used in the bitmap file header
#define BITMAP_FILE_VERSION                         (0x00010004)
// This version to represent addition of new persistent values to tLogHeader
#define BITMAP_FILE_VERSION_V2                      (0x00010005)
#define BITMAP_FILE_VERSION_HEADER_SIZE_INCREASE    (8 * sizeof(UINT32) + 7 * sizeof(UINT64))
#define BITMAP_FILE_ENDIAN_FLAG                     (0x000000FF)

// Indicates the newly added fields to support OOD solution
#define BITMAP_HEADER_OOD_SUPPORT                   (0x00000001)

// TODO-SanKumar-1804: Pull these flags into an Enum.
#ifdef VOLUME_CLUSTER_SUPPORT
...
#endif
// Previously BITMAP_FLAGS_LEARN_PHYSICAL_OFFSET
#define BITMAP_FLAGS_DEPRECATED_1                   (0x00000004)

#define MAX_WRITE_GROUPS_IN_BITMAP_HEADER_V1        (31)
#define MAX_WRITE_GROUPS_IN_BITMAP_HEADER_V2        (511)
#define MAX_WRITE_GROUPS_IN_BITMAP_HEADER_CURRENT   MAX_WRITE_GROUPS_IN_BITMAP_HEADER_V2

#define MAX_CHANGES_IN_WRITE_GROUP                  (64)
#define DISK_SECTOR_SIZE                            (512)
#define HEADER_CHECKSUM_SIZE                        (16)
#define HEADER_CHECKSUM_DATA_SIZE(MaxWriteGroups)   (LOG_HEADER_SIZE(MaxWriteGroups) - HEADER_CHECKSUM_SIZE)
#define LOG_HEADER_SIZE(MaxWriteGroups)             ((MaxWriteGroups * DISK_SECTOR_SIZE) + DISK_SECTOR_SIZE)

// set pInMageStatus to message id of the event for user mode app to format the error message
// and log the event with disk ID, number and given parameters.
#define SET_INMAGE_BITMAP_API_STATUS(pInMageStatus, descriptor, ...) { \
    if(NULL != pInMageStatus) \
        *pInMageStatus = MSG_##descriptor##_Message; \
    \
    InDskFltWriteEventWithDevCtxt(descriptor, (&m_DevLogContext), __VA_ARGS__); \
}

typedef struct {
    // Store GUID/Signature in form of String
    WCHAR   wDevID[DEVID_LENGTH];
    // Store disk number as information
    ULONG   ulDevNum;
} tDevLogContext;
...
enum tagLogRecoveryState {
    ...
    pendingChanges = 3,         // Pending changes during closing bitmap
    resyncMarked = 4            // Pending resync marked
};
...
typedef struct {
    ...
    UINT32 headerSize;                              // 0x14: This is size of core metaheader
    ...
    LONGLONG fltDevSize;                            // 0x38: Volume/Disk size depending on flt driver
    ...
    UINT64 LastTimeStamp;                           // 0x50: added to persist Global Time Stamp per device (V2)
    UINT64 LastSequenceNumber;                      // 0x58: added to persist Global sequence No per device (V2)
...
typedef struct {
    ...
    tLastChanceChanges changeGroup[1];
} tBitmapHeader;
...
public:
    BitmapAPI(const tDevLogContext *pDevLogContext);
    virtual ~BitmapAPI();
...
#endif
        bool bDelete = false,
        NTSTATUS *pInMageCloseStatus = NULL,
        ULONG ResyncErrorCode = 0, 
        ULONGLONG ResyncTimeStamp = 0);
    ...
    PDEVICE_OBJECT GetBitmapDevObj();
    ...
    NTSTATUS ChangeBitmapModeToRawIO(LONG *ErrorStatus);
#ifdef VOLUME_CLUSTER_SUPPORT
    void     ResetPhysicalDO();
    NTSTATUS CommitBitmap(tagLogRecoveryState recoveryState, BitmapPersistentValues &BitmapPersistentValue, NTSTATUS *pInMageStatus = NULL);

    NTSTATUS CommitBitmap(tagLogRecoveryState recoveryState, NTSTATUS *pInMageStatus = NULL);
#endif

    NTSTATUS IsDevInSync(BOOLEAN *pDevInSync, NTSTATUS *pOutOfSyncErrorCode);
...
protected:
    ...
    CHAR  m_chDevNum[DEVICE_NUM_LENGTH];               //Converted to CHAR as it is used in dbgprints. 
                                                       //It is documented to use %ws at IRQL >= PASSIVE_LEVEL. 
                                                       //It seems restriction is only in conversion in dbgprint from WCHAR to CHAR but not specified in document
#endif
    // Bitmap parameters that decide the granularity and size of bitmap.
    LONGLONG    m_llDevSize;     // Size of the device represented by this bitmap
    ULONG       m_ulBitmapGranularity;
    ULONG       m_ulBitmapOffset;
    ULONG       m_ulNumberOfBitsInBitmap;   // number of bits in bitmap file representing device data
    ...
    tBitmapHeader  * m_BitmapHeader; // stored at offset 0 of section for this device
    ...
    NTSTATUS SetWriteSizeNotToExceedDevSize(LONGLONG &llOffset, ULONG &ulLength);
    //Fix For Bug 27337
#ifdef VOLUME_FLT
    ...
    tDevLogContext m_DevLogContext;
    ULONG   m_MaxBitmapHdrWriteGroups;
    ULONG   m_BitMapHeaderEndOffset;

private:
    // These routines are called with mutex already held. User can not call this routines.
#ifdef VOLUME_CLUSTER_SUPPORT
    NTSTATUS CommitBitmapInternal(tagLogRecoveryState recoveryState, NTSTATUS *pInMageStatus, ULONG ResyncErrorCode = 0, ULONGLONG ResyncTimeStamp = 0,BitmapPersistentValues &BitmapPersistentValue);
    NTSTATUS InitBitmapFile(NTSTATUS *pInMageStatus, BitmapPersistentValues &BitmapPersistentValue);
    NTSTATUS LoadBitmapHeaderFromFileStream(NTSTATUS *pInMageStatus, tagLogRecoveryState* pBitmapRecoveryState, bool bClearOnOpen, BitmapPersistentValues &BitmapPersistentValue);
#else
    NTSTATUS CommitBitmapInternal(tagLogRecoveryState recoveryState, NTSTATUS *pInMageStatus, ULONG ResyncErrorCode = 0, ULONGLONG ResyncTimeStamp = 0);
    NTSTATUS InitBitmapFile(NTSTATUS *pInMageStatus);
    NTSTATUS LoadBitmapHeaderFromFileStream(NTSTATUS *pInMageStatus,  tagLogRecoveryState* pBitmapRecoveryState, bool bClearOnOpen);
#endif
    NTSTATUS CommitHeader(BOOLEAN bVerifyExistingHeaderInRawIO, NTSTATUS *pInMageStatus);
    NTSTATUS MoveRawIOChangesToBitmap(NTSTATUS  *pInMageStatus);
    NTSTATUS ReadAndVerifyBitmapHeader(NTSTATUS *pInMageStatus);

    _Must_inspect_result_
    _IRQL_requires_max_(PASSIVE_LEVEL)
    NTSTATUS ValidateLearntHeaderPhysicalOffsets(bool retainLogHeader);
};