...
SVERROR getLinuxReleaseValue( std::string & linuxetcReleaseValue, LINUX_TYPE& linuxFlavourType )
{
	...
	else if(( getFileContent( "/etc/os-release", release_Value ) == SVS_OK ) && ( release_Value.find("Debian") != std::string::npos || release_Value.find("SLES") != std::string::npos ||  release_Value.find("openSUSE Leap") != std::string::npos))
	{
		...
		else if( release_Value.find("SLES") != std::string::npos || release_Value.find("openSUSE Leap") != std::string::npos )
		{
			linuxFlavourType = SUSE_RELEASE;
		}
	}
