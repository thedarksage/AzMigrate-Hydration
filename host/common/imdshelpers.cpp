...
std::string GetImdsMetadata(const std::string& pathStr, const std::string& apiVersion, const std::string& format)
{
    std::string imdsUrl;
    if (!pathStr.empty() && !apiVersion.empty()) {
        imdsUrl = IMDS_ENDPOINT + pathStr + std::string("?") + apiVersion;
    }
