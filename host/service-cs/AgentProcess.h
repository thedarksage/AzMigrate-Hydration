...
public:
    ...
    AgentProcess(std::string deviceName, int group, std::string processName)
        : m_Id(deviceName, group), m_ProcessId(ACE_INVALID_PID), m_quitEvent(NULL), isAlive(false), m_processName(processName)
    {  
    }

    AgentProcess(char* deviceName, int group, char* processName)
        : m_Id(deviceName, group), m_ProcessId(ACE_INVALID_PID), m_quitEvent(NULL), isAlive(false), m_processName(processName)
    {  
    }
	...
	* 1 TO N sync: Added 2 constructors that initialize the end point device names
	*/
    AgentProcess(std::string deviceName, std::string endpointDeviceName, int group, std::string processName)
        : m_Id(deviceName, endpointDeviceName, group), m_ProcessId(ACE_INVALID_PID), m_quitEvent(NULL), isAlive(false), m_processName(processName)
    {  
    }

    AgentProcess(char* deviceName, char *endpointDeviceName, int group, char* processName)
        : m_Id(deviceName, endpointDeviceName, group), m_ProcessId(ACE_INVALID_PID), m_quitEvent(NULL), isAlive(false), m_processName(processName)
    {  
    }
