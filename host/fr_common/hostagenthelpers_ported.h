...
//
#ifndef HOSTAGENTHELPERS_PORTED__H
#define HOSTAGENTHELPERS_PORTED__H
...
template <typename T> inline void SAFE_ARRAY_DELETE( T*& p ) { delete [] p; p = NULL; }
#define SAFE_FCLOSE( p ) do { if( p ) { fclose( p ); p = NULL; } } while( 0 )
#define SAFE_STRCPY( dest, source ) do { strncpy( dest, source, sizeof( dest ) - 1 ); dest[ sizeof( dest ) - 1 ] = 0; } while( 0 )
...
#define UNKNOWNOS_CX_CODE  "0"
#define WINDOWS_CX_CODE    "1"
#define LINUX_CX_CODE      "2"
#define SUNOS_CX_CODE      "3"
#define HPUX_CX_CODE       "4"
#define AIX_CX_CODE        "5"
...
public:
    ...
    SVERROR append( const char* psz, size_t length )
    {
        ...
        {
            char* pszOld = m_psz;
            size_t old = m_CapacityNext >  2*(length + m_cChars + 1) ? m_CapacityNext : 2*(length + m_cChars + 1);
...
struct SSH_Configuration {
	...
	char ssh_homedir_path[SV_MAX_PATH];
};
...
struct SV_HOST_AGENT_PARAMS
{
	...
	SV_ULONG cbMaxRunsPerInvocation;
	SV_ULONG cbSequenceCount;
	SV_ULONG cbSequenceCountInMsecs;
	bool enforceStrictConsistencyGroups;
	SV_ULONG cbMaxTimeInSecsToRunPerInvocation; //unused
	SV_ULONG cbMaxGetThreadsPerReplication; //unused
	SV_ULONG cbMaxMemoryUsagePerReplication;
	SV_ULONG cbMaxInMemoryCompressedFileSize;
	SV_ULONG cbMaxInMemoryUnCompressedFileSize;
	SV_ULONG cbCompressionBufSize;
	SV_ULONG cbCompressionChunkSize;
    ...
    SV_ULONG SVFTPPort;
	SV_ULONG SVFTPConnectTimeOut;
	SV_ULONG SVFTPResponseTimeOut;
    ...
    bool runFastSourceInParallel;
    bool runFastTargetInParallel;
    //SSH info
...
//HRESULT SettingStandByCxValuesInRegistry(const char *pszServerName,int value);//vidyadhar
...
void GetOperatingSystemInfo(char* const psInfo, int nLength);
...
void FormatVolumeName(std::string& sName);
void UnformatVolumeName(std::string& sVolName);
...
enum CHANGEFSTAGOP { HIDE, UNHIDE_READONLY, UNHIDE_READWRITE, UNKNOWNOP };
...
void PostFailOverJobStatusToCX(char* cxip, SV_ULONG httpPort, SV_ULONG processId, SV_ULONG jobId,bool);
SVERROR getSocketOptions(char* psz, int cch) ;
