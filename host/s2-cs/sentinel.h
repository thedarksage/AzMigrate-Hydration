...
//typedef std::map<long int,Event> VOLUME_GROUP_EVENT;
...
public:
	...
	void OnChangeHostVolumeGroup( HOST_VOLUME_GROUP_SETTINGS );
	void OnChangePrismSettings( PRISM_SETTINGS );
    ...
    void CollectSourcesToReplicate(VolumeOps::VolToFilterDrvVol_t &voltodrvvol,
...
private:
	...
	long int m_liMonitoringGroupId;
	...
	boost::shared_ptr<StatsNMetricsManager> m_pStatsNMetricsManager;
	
public:
    /* TODO: m_HostVolumeGroupSettings and the m_PrismSettings should 
     * be kept as public since it is passed by reference to 
     * constructors of protectedgroupvolumes and prismvolumes */
    std::string m_HostId;
...
private:
    bool HasSettingsChanged(const HOST_VOLUME_GROUP_SETTINGS&);
    bool HasPrismSettingsChanged(const PRISM_SETTINGS&);
    void StartNewProtectedGroupVolumes(VolToVgsIter_t &voltovgsiter);
    void StartNewPrismVolume(const PRISM_VOLUME_INFO& prismVolumeInfo);
	...
	void StartStatsAndMetricsCollThread(const MASTER_PROTECTED_VOLUME_GROUP & masterProtectedGroup);
};
//}
