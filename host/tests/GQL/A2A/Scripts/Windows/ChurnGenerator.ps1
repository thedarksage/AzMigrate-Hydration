...
Function GetChurnVolumes()
{
    try
    {
        ...
        if ($osName -eq "2008")
        {
	        ...
	        foreach ($vol in $volInfo)
	        {
                ...
                ($vol.VolumeName -ne "Temporary Storage"))
        ...
        else
        {
            ...
            foreach ($vol in $volInfo)
            {
                ...
                if (($vol.FileSystemLabel -ne "System Reserved") -and ($vol.DriveType -eq "Fixed") -and 
                    ($vol.DriveLetter.GetHashCode() -ne 0) -and ($vol.FileSystemLabel -ne 'Temporary Storage'))
    ...
    {
        Logger "ERROR : Caught Exception : $_.Exception.Message"
        Logger "ERROR : @line : $_.InvocationInfo.ScriptLineNumber"
        exit 1
    }
}
