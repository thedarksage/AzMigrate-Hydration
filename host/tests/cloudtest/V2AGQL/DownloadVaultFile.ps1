...
try
{
	...
	if ($TPrint -eq $null) {
		...
		Throw "Failed to fetch the thumbprint"
    }
	...
	if (!$vault) {
		...
		if (!$vault) {
			...
			Throw "Unable to fetch the vault information"
		}
	}
	...
	if ((!$file) -or (!$file.FilePath) -or (!(Test-Path $file.FilePath))) {
		...
		Throw "Failed to download the vault credentials file: Error = $Error"
	}
	...
	{
		Throw "Failed to download the vault file"
	}
}
catch
{
