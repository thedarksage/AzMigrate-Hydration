...
Function GetSecretFromKeyVault
{
    ...
    try
    {
        ...
        $secretText = (Get-AzKeyVaultSecret -vaultName $global:SecretsKeyVaultName -name $SecretName).SecretValueText
		LogMessage -Message ("Password : {0}" -f $secretText) -LogType ([LogType]::Info)
...
Function LoginToAzure {
	...
	LogMessage -Message ("ThumbPrint: {0}, ApplicationId: {1} and TenantId: {2}" -f $TPrint, $global:DrDatapPlaneAppClientId, $global:TenantId) -LogType ([LogType]::Info)
	Login-AzAccount -ServicePrincipal -CertificateThumbprint $TPrint -ApplicationId $global:DrDatapPlaneAppClientId -Tenant $global:TenantId
	if (!$?) {
		LogMessage -Message ("Unable to login to Azure account using Thumbprint: {0}, ApplicationId: {1} and TenantId: {2}" -f $TPrint, $global:DrDatapPlaneAppClientId, $global:TenantId) -LogType ([LogType]::Error)
		return $false
	}
