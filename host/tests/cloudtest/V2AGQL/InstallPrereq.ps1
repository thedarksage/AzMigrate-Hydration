# This script installs Az Powershell 2.5.0, if it is not installed
...
Function Install-Prereq
{
    try
    {
        ...
        if (($version.Version.Major -le 5) -and ($version.Version.Major -ne 5 -or $version.Version.Minor -lt 1))
        {
            ...
            Throw "The minimum required Powershell Version is '5.1'. Current Powershell Version is $version"
        }
        ...
        Write-Host "Verifying if Az Powershell 2.5.0 is installed or not"
        $AzModule = Get-InstalledModule -Name "Az" -RequiredVersion 2.5.0 -ErrorAction silentlycontinue
        if (!$AzModule)
        {
            Set-PSRepository -Name PSGallery -InstallationPolicy Trusted
            Write-Host "Installing Az Powershell 2.5.0"
            Install-Module -Name Az -RequiredVersion 2.5.0 -AllowClobber -Scope AllUsers -Force
            Get-Module -ListAvailable Az
            Get-InstalledModule -Name Az
        }
        else
        {
            Write-Host "Az Powershell 2.5.0 is already installed. Skipping!!"
        }
