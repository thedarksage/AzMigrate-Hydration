...
class DevMetrics;
//For Churn Statistics and DR Metrics Collection
typedef std::map<ProtectedGroupVolumes*, Thread*> MAP_PROTECTED_GROUPS;
typedef std::map<ProtectedGroupVolumes*, boost::shared_ptr<DevMetrics> > MAP_PROTGRPS_DEVMETRICS;
class DevMetrics
{
private:
    ...
    // \hostid=<hostid>,deviceId=<dev id>,time stamp=<ts>,Tracked Bytes=<bytesTracked>,\
        Tracked Bytes Rate=<bytes tracked per second>
    std::string m_ChurnRecord;

    std::vector<std::string>m_vChurnRecords;
    std::vector<std::string>m_vThroughputRecords;
    SV_ULONGLONG m_prevCumulativeTrackedBytes;

    // \Cumulative data transferred by this volume in bytes till\
       previous DRMetrics iteration to collect this value.
    SV_ULONGLONG m_prevCumulativeSentBytes;

    // \Cumulative data transferred by this volume in bytes till\
        current DRMetrics iteration to collect this value.
    SV_ULONGLONG m_TrackedBytes;

    // \Data in bytes sent by this volume between previous and current\
        DRMetrics iteration.
    SV_ULONGLONG m_SentBytes;
    float m_TrackedBytesRate;
    float m_SentBytesRate;//Bytes per second.
    SV_ULONGLONG m_prevDroppdTagsCount;
    SV_ULONGLONG m_cumulativeDroppedTagsCount;
    SV_ULONGLONG m_diskReadLatency;
    SV_ULONGLONG m_networkTransmitLatency;
    SV_ULONGLONG m_collectionInterval;//in seconds
    FilterDrvVol_t m_fltDrvVolName;
    bool m_bIsProtected;
    int m_devType;
    std::string m_ChurnStatsAsString;
    std::string m_ThroughputStatsAsString;
    bool IsThisDeviceBeingProtected();
    FilterDrvVol_t GetDriverDevName();
    std::string GetTimeStampAsString();
public:
    DevMetrics(){}
    DevMetrics( std::string hostId,
                std::string devId,
                std::string hostName,
                std::string biosId,
                SV_ULONGLONG prevCumTrackedBytes,
                SV_ULONGLONG cumTagsDropped,
                SV_ULONGLONG prevCumSentBytes);
    void AddChurnRecord();
    void AddThroughputRecord();
    std::string GetChurnStats();
    std::string GetThroughputStats();
    SV_ULONGLONG GetCumulativeTrackedBytes();
    void SetCumulativeTrackedBytes(SV_ULONGLONG cumTrackedBytes);
    void SetCumulativeSentBytes(SV_ULONGLONG cumSentBytes);
    SV_ULONGLONG GetCumulativeDroppedTagsCount();
    void SetDroppedTagsCount(SV_ULONGLONG droppedTagsCount);
    void SetFilterDrvVolName(FilterDrvVol_t drvVol);
    FilterDrvVol_t GetFilterDrvVolName();
    void SetDeviceType(int devType);
    void SetDeviceId(std::string strDevName);
    std::string GetDeviceId();
    SV_ULONGLONG GetPrevCumulativeTrackedBytesCount();
    SV_ULONGLONG GetPrevCumulativeSentBytesCount();
    SV_ULONGLONG GetPrevCumulativeDroppedTagsCount();
    void SetTrackedBytes(SV_ULONGLONG trackedBytes, ACE_Time_Value elapsedTime);
    void SetSentBytes(SV_ULONGLONG sentBytes, ACE_Time_Value elapsedTime);
    SV_ULONGLONG GetTrackedBytes();
    float GetTrackedBytesRate();
    void ClearChurnRecords();
    void ClearThroughputRecords();
    std::string GetHostId();
	std::string GetDiskName();
    void SetProtectionState(bool bProtState);
};
...
public:
    std::string Host;
    std::map<std::string, std::string> DrpdTagsSmry;
    std::map<std::string, std::string> CumBytsTrkdSmry;
    std::vector<PrBuckets> DRSmry;

public:
    void serialize(JSON::Adapter& adapter)
    {
        JSON::Class root(adapter, "DRSummary", false);
        JSON_E(adapter, Host);
        JSON_E(adapter, DrpdTagsSmry);
        JSON_E(adapter, CumBytsTrkdSmry);
        JSON_T(adapter, DRSmry);
    }
};

class StatsNMetricsManager :public Entity, public Runnable
{
private:
    MAP_PROTGRPS_DEVMETRICS m_mapProtGrpDevMetrics;

    bool m_bIsForMetricsCollection;
    int m_nChurnNThroughputStatsCollInterval;
    int m_nChurnNThroughputStatsSendingIntervalToPs;
    int m_nWaitforLogCollectionAndDSMetrics;
    int m_nDrMetricsCollInterval;
    std::string m_nChurnStatsRecord;
    std::vector<std::string>m_ChurnStatsRecords;
    MAP_PROTECTED_GROUPS *m_pMapProtectedPairs;
    std::map<std::string, boost::shared_ptr<DevMetrics> >m_mapDeviceToDevMetrics;
    std::string m_strHostId;
    volatile bool m_bStopMonitoring;
    Event m_QuitEvent;
    ACE_Time_Value m_earlierChurnCollTime;
    ACE_Time_Value m_elapsedChurnCollTime;
    ACE_Time_Value m_earlierChurnSendTime;
    ACE_Time_Value m_earlierThroughputCollTime;
    ACE_Time_Value m_elapsedThroughputCollTime;
    ACE_Time_Value m_earlierThroughputSendTime;
    ACE_Time_Value m_earlierDRTime;
    std::map<std::string, std::string> m_drpdTagsSmry;
    std::map<std::string, std::string> m_cumTrkdBytesSmry;
    void PrintDroppedTags(ProtectedGroupVolumes* pg, DevMetrics* dm);
    void PrintCumulativeTrackedBytesCount(ProtectedGroupVolumes *pg, DevMetrics* dm);

public :

    StatsNMetricsManager();
    StatsNMetricsManager(const MAP_PROTECTED_GROUPS *pMapProtectedVolumeGroups);
    int Init();
    /*From all devices collect the churn records and keep it in memory*/
    void CollectChurnStats();
    /*Collect throughput records from source for all devices and keep in memory*/
    void CollectThroughputStats();
    /*after successful sending, clear the in-memory churn records*/
    int SendChurnStatsToPS();
    /*after successful sending, clear the in-memory throughput records*/
	void SendHeartbeatToPS();
    int SendThroughputStatsToPS();
    void CollectDRMetrics();
    int ManageLogCollectionForStatsAndMetrics();
    virtual THREAD_FUNC_RETURN Run();
    virtual THREAD_FUNC_RETURN Stop(const long int PARAM = 0);
    int StopDedicatedStatsNMetricsCollThread(const long int liParam);
    std::string GetHostId();
    void DRProfilingSummary() const;
    bool IsStopRequested();
    void StopMonitoring();
    WAIT_STATE WaitOnQuitEvent(long int seconds,long int milli_seconds);
};

//:end: For Churn Statistics and DR Metrics Collection
...
{
public:
    /// \brief constructor: called when pushing diffs on PS
    ProtectedGroupVolumes(const VOLUME_SETTINGS& VolumeSettings,             ///< volume settings
                          CDP_DIRECTION direction,                           ///< protection direction (forward/reverse)
                          CDP_STATUS status,                                 ///< protected or not
                          TRANSPORT_CONNECTION_SETTINGS transportSettings,   ///< CXPS transport settings
    ...
    /// \brief constructor: called when applying diffs on target directly
    ProtectedGroupVolumes(const VOLUME_SETTINGS& VolumeSettings,                       ///< volume settings
                          CDP_DIRECTION direction,                                     ///< protection direction (forward/reverse)
                          CDP_STATUS status,                                           ///< protected or not        
                          TRANSPORT_CONNECTION_SETTINGS transportSettings,             ///< CXPS transport settings
                          const DeviceFilterFeatures *pDeviceFilterFeatures,           ///< device filter features (like per io time stamp support, diff ordering support etc,.)
                          const char *pHostId,                                         ///< host id 
                          HOST_VOLUME_GROUP_SETTINGS::volumeGroups_t endPointVolGrps,  ///< target settings
                          const DataCacher::VolumesCache *pVolumesCache,               ///< volumeinfocollector cache
                          const size_t *pMetadataReadBufLen                           ///< read buffer length in metadata
                          );
    ...
    int ReturnHome();
    const VOLUME_SETTINGS& GetVolumeSettings() const;
    void SetSourceVolume(const Volume&);
    void SetEndPoints(VgsPtrs_t &vgsptrs);
    void SetEndPoints(const ENDPOINTS& EndPoints);
    void SetEndPoints(HOST_VOLUME_GROUP_SETTINGS::volumeGroups_t &endPointVolGrps);
    ...
    int TransferMonData(const char*, const long int, const std::string&, bool, bool bCreateMissingDirs = false);
...
protected:
    ...
    int OpenMonitoringTransportStream();
...
private:
    ...
    int RenameRemoteStatsFile(std::string const & preName, std::string const & postName);
    ...
    int IsATLUNStatsReqProcessed(void);
    ...
    bool LogNetworkFailures(int iStatus);
...
private:
    ...
    boost::shared_ptr<ENDPOINTS> m_pEndPoints;            //stores a copy of the target volume settings.
    boost::shared_ptr<VOLUME_SETTINGS> m_pVolumeSettings; //stores a copy of the volume settings
    ...
    SV_ULONGLONG m_transferredBytesToPS; //Bytes
    ...
    //TransportStream* m_pTransportStream;//points to the transport stream
    boost::shared_ptr<BaseTransportStream> m_pTransportStream;
    boost::shared_ptr<BaseTransportStream> m_pMonitorTransportStream; //adding a transport stream for monitoring data sent to PS
    ...
    long long m_lastReadErrLogTime;
    unsigned int m_readFailCnt;
    long long m_lastDiskNotFoundErrLogTime;
    unsigned int m_diskNotFoundCnt;
    long long m_lastNetworkErrLogTime;
    unsigned int m_networkFailCnt;

    unsigned int m_networkFailureCode;

    // indicates if the data transfer is throttled
    bool    m_bThrottleDataTransfer;
    SV_ULONGLONG m_prevDatapathThrottleLogTime;
    SV_UINT m_datapathThrottleCnt;
...
public:
    ...
    bool m_bForceDirectTrnasfer;
    bool m_bHasToCheckTS;
    long long m_lastChurnStatsSendFailTime;
    long long m_lastThroughputStatsSendFailTime;
    unsigned int m_sendChurnStatFailCnt;
    unsigned int m_sendThroughputStatFailCnt;
...
public:
    //:For collecting Churn Statistics and DR Metrics
    ProtectedGroupVolumes(const ProtectedGroupVolumes&);
    const std::string GetHostId();
    const DeviceFilterFeatures* GetDeviceFilterFeatures();
    DeviceStream* GetDeviceStream() const;
    int GetDeviceType();
    const std::string& GetDriverName();
    FilterDrvVol_t GetSourceNameForDriverInput();
    SV_ULONGLONG GetTransferredBytesToPS();
};
