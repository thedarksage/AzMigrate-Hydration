...
int ProtectedGroupVolumes::StartFiltering(void)
{
    ...
    else
    {
        ...
        bool bfilleddevinfo = drvifmajor.FillInmDevInfo(m_pDeviceFilterFeatures, &m_pVolumesCache->m_Vs, *m_pVolumeSettings, devInfo, vr);
        if (bfilleddevinfo)
        {
            // persistent name for /dev/sda is _dev_sda
            std::string pname = GetSourceDiskId().empty() ? GetSourceVolumeName() : GetSourceDiskId();
    ...
    if (SV_SUCCESS == iStatus)
    {
		...
		iStatus = drvifminor.PassMaxTransferSizeIfNeeded(m_pDeviceFilterFeatures, m_SourceNameForDriverInput, *m_pDeviceStream, vr);
