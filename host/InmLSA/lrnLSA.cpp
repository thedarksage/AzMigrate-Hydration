...
int __cdecl main( int argc, char *argv[] )
{
    ...
    PSID pSid;
    NTSTATUS Status;
    DWORD dwRetVal=INM_ERROR;          // assume error from main

    if(argc == 1) {
        fprintf(stderr,"LogOn As Service Account Usage:\n\n\t%s <Logon Account> [TargetServer]\n\tIf no TargetServer is mentioned, then it applies to local machine.\n", argv[0]);
        return INM_USAGE;
    }

    //
    // Get the account name on argv[1].
    // Assumes source is ANSI. Resultant string is ANSI or Unicode
    //
    _snwprintf_s(AccountName, 256, 255, TEXT("%hS"), argv[1]);

    //
    // Get the machine name on argv[2], if appropriate
    ...
    if(GetAccountSid(
        ...
        DisplayWinError("GetAccountSid", dwRetVal);
    }
