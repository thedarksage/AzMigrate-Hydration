//---------------------------------------------------------------
//  <copyright file="InstallMobilityServiceOperation.cs" company="Microsoft">
//      Copyright (c) Microsoft Corporation. All rights reserved.
//  </copyright>
//
//  <summary>
//  Implements push installation of mobility agent to VM via VMware Vim SDK.
//  </summary>
//
...
{
    class InstallMobilityServiceOperation : IOperation
    {
        /// <summary>
        ...
        /// <summary>
        /// Files to be pulled from VM.
        /// </summary>
        public string[] filesToPull = new string[] { };

        /// <summary>
        /// The folder in local machine to pull the files.
        /// </summary>
        public string pullFileDestination = String.Empty;
        ...
        /// <summary>
        /// Initializes new mobility service installation operation.
        /// </summary>
        /// <param name="inputs">Input command line arguments.</param>
        public InstallMobilityServiceOperation(Dictionary<string, string> inputs)
        {
            ...
            foreach (var key in inputs.Keys)
            {
                switch (key)
                {
                    ...
                    case "--outputfilepath":
                        ...
                        {
                            scriptOutputFile.Replace("/", "\\");
                        }
                        break;
                    case "--filestofetch":
                        filesToPull = inputs[key].Split(';');
                        break;
                    case "--localfoldertodownload":
                        pullFileDestination = inputs[key];
                        if (pullFileDestination != null)
                        {
                            pullFileDestination.Replace("/", "\\");
                        }
                        if (!pullFileDestination.EndsWith("\\"))
                        {
                            pullFileDestination = String.Concat(pullFileDestination, "\\");
                        }
                        break;
                    case "--stacktype":
        ...
        public void ValidateOperationInputs()
        {
            ...
            if (filesToPull == null ||
                filesToPull.Length == 0)
            {
                isValidationSuccessful = false;
                errorMessage = "Files to fetch are missing.";
            }

            if (String.IsNullOrEmpty(pullFileDestination))
            {
                isValidationSuccessful = false;
                errorMessage = 
                    "Location on local machine to place the fetched files from vm is missing.";
            }
        ...
        public void Execute()
        {
            ...
            while (!done)
            {
                try
                {
                    Logger.LogVerbose("Copying files to remote machine..\n");
                    ...
                    {
                        Logger.LogVerbose(String.Format("Script Output file is {0}.\n", scriptOutputFile));
            ...
            Logger.LogVerbose("Pulling files from the remote machine.\n");

            foreach (string fileToPull in filesToPull)
            {
                string fileName = String.Empty;
                ...
                int index = fileToPull.LastIndexOf(pathSeparator);
                fileName = fileToPull.Substring(index + 1);

                try
                {
                    client.FetchFileFromVm(
                        fileToPull,
                        pullFileDestination,
                        fileName);
                }
	            ...
	            {
                    if (vmex.errorCode == VMwarePushInstallErrorCode.FileToBeFetchedNotAvailableInVm)
                    {
                        Logger.LogVerbose(
                            String.Format(
                                "File {0} is not available on machine {1}. Ignoring this.\n",
                                fileToPull,
                                vm.Guest.HostName));
                    }
                    else
            ...
            Logger.LogVerbose("Pulled the files successfully.\n");
        }
        ...
        public override string ToString()
        {
            ...
            sb.AppendLine("Files to be fetched : " + this.filesToPull.Concat());
            sb.AppendLine("Folder to place fetched files : " + this.pullFileDestination);
            return sb.ToString();
        }
    }
}
