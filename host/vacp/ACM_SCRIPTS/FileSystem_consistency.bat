...
REM Set Volume Name and Tag name 
set VOLUME_NAME="All"
set FILESYSTEM_BOOKMARK_MSG="BM_FileServer%random%%random%"
...
REM Report error and exit
 ...
 ECHO "Filesystem Consistecy is not supported on this platform"
 ECHO "It is supported on Windows 2003 and Windows XP only"
 GOTO FAIL_EXIT

REM Create Filesystem Tags on Windows XP
:WINXP_CMD
...
:GENERATE_TAGS
 %VACP_CMD% -v %VOLUME_NAME% -t %FILESYSTEM_BOOKMARK_MSG%
