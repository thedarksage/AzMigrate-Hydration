...
set APPLICATION_NAME="ExchangeIS"
set APPLICATION_BOOKMARK_MSG="BM_ExchangeIS%random%%random%"
...
REM Report error and exit
 ...
 ECHO "It is supported on Windows 2003 only"
 GOTO FAIL_EXIT
...
:GENERATE_TAGS
 ...
 %VACP_CMD% -w %APPLICATION_NAME% -t %APPLICATION_BOOKMARK_MSG% > consistency_tag.txt
