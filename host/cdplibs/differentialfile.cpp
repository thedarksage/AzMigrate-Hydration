...
DifferentialFile::DifferentialFile(const string & path)
:File(path),
m_validate(true),
m_volsize(0)
{
    ...
    std::string basename = basename_r(path.c_str());
...
void DifferentialFile::set_max_io_size(SV_UINT& size)
{
	m_max_io_size = size;
}


SV_INT DifferentialFile::Open(BasicIo::BioOpenMode mode)
{
    DebugPrintf(SV_LOG_DEBUG,"ENTERED %s\n",FUNCTION_NAME);

    SV_INT rv = File::Open(mode);
    if (SV_SUCCESS == rv)
    {
        if(!m_start)
            m_start = 0;

        File::Seek(0, BasicIo::BioEnd);
        if(!m_end)
            m_end = File::Tell();
    }
...
void DifferentialFile::ANNOUNCE_FILE()
{
...
void DifferentialFile::ANNOUNCE_TAG(const std::string & tagname)
{
    stringstream l_stdout;
...
bool DifferentialFile::ParseDiff(DiffPtr & diffptr)
{
    DebugPrintf(SV_LOG_DEBUG,"ENTERED %s\n",FUNCTION_NAME);
    bool rv = false;
    bool done = false;
    ...
    File::Seek(m_start, BasicIo::BioBeg);

    SV_OFFSET_TYPE offset = File::Tell();
    ...
    */
    ...
    do
    {
        ...
        readin = File::Read( ( char*) &dataFormatFlags, sizeof( SV_UINT ) ) ;
        ...
        while ( offset < m_end )
        {
            ...
            *   (result = 0)                                                        */
            ...
            readin = File::Read((char*)&prefix, SVD_PREFIX_SIZE);
            ...
            if (SVD_PREFIX_SIZE != readin)
            {
                ...
                l_stdfatal << "Error detected  " << "in Function:" << FUNCTION_NAME 
                    ...
                    << "Prefix could not be be read at offset " << File::Tell() << "\n"
            ...
            else
            {
                ...
                default:
                    ...
                    DebugPrintf(SV_LOG_ERROR, "%s: %s", FUNCTION_NAME, l_stdfatal.str().c_str());
                    rv = false;
                    break;
                }
            }
        ...
        if ( rv )
        {
            ...
            {
                File::Seek(diffptr -> EndOffset(), BasicIo::BioBeg);
                break;
            }
        }         
...
bool DifferentialFile::VerifyDataBoundaries(DiffPtr & diffptr)
{
    bool rv = true;
    DebugPrintf(SV_LOG_DEBUG,"ENTERED %s\n",FUNCTION_NAME);
    do
    {
        ...
        for( ; drtds_iter != drtds_end ; ++drtds_iter)
        {
            ...
            if(drtd.get_length() == 0)
            {
                ...
                ChangeLenZeroAlert a(File::GetName());
            ...
            {                   
                ...
                ChangeBeyondSourceSizeAlert a(File::GetName());
...
bool DifferentialFile::VerifyIoOrdering(DiffPtr & diffptr)
{
    bool rv = true;
    DebugPrintf(SV_LOG_DEBUG,"ENTERED %s\n",FUNCTION_NAME);
    ...
    do
    {
        ...
        if( diffEndTime < diffStartTime )
        {
            ...
            l_stdalert << "Differential file : " << File::GetName() 
...
bool DifferentialFile::ParseandVerifyFileName(DiffPtr & diffptr)
{
    bool rv = true;
    ...
    do
    {
        try
        {
            ...
            if (tslc != diffptr -> endtime())
            {
                ...
                l_stdfatal << "Error detected  " << "in Function:" << FUNCTION_NAME 
                    << " @ Line:" << LINE_NO << " FILE:" << FILE_NAME << "\n\n"
                    << "\n" << File::GetName()   << "is not a valid differential file name.\n"
...
bool DifferentialFile::ReadSVD1(const SVD_PREFIX & prefix, DiffPtr & diffptr)
{
    DebugPrintf(SV_LOG_DEBUG,"ENTERED %s\n",FUNCTION_NAME);
	...
	SVD_HEADER1 hdr;
    readin = File::FullRead((char*)(&hdr), SVD_HEADER1_SIZE); 
...
bool DifferentialFile::ReadSVD2(const SVD_PREFIX & prefix, DiffPtr & diffptr)
{
    DebugPrintf(SV_LOG_DEBUG,"ENTERED %s\n",FUNCTION_NAME);
    ...
    // Read the header chunk
	SVD_HEADER_V2 hdr;
    readin = File::FullRead((char*)(&hdr),SVD_HEADER2_SIZE); 
...
bool DifferentialFile::ReadTSFC(const SVD_PREFIX & prefix, DiffPtr & diffptr)
{
    DebugPrintf(SV_LOG_DEBUG,"ENTERED %s\n",FUNCTION_NAME);
    assert(prefix.count == 1);
    ...
    SVD_TIME_STAMP tsfc;
    readin = File::FullRead((char*)&(tsfc),SVD_TIMESTAMP_SIZE);

    if (SVD_TIMESTAMP_SIZE == readin)
    {
        /* NEEDTOSWAP : convert to appropriate endian  START */
        m_svdConvertor->convertTimeStamp( tsfc ) ;
        /* NEEDTOSWAP : convert to appropriate endian  END */
...
bool DifferentialFile::ReadTFV2(const SVD_PREFIX & prefix, DiffPtr & diffptr)
{
    DebugPrintf(SV_LOG_DEBUG,"ENTERED %s\n",FUNCTION_NAME);
    assert(prefix.count == 1);
    ...
    readin = File::FullRead((char*)&(tsfc),SVD_TIMESTAMP_V2_SIZE);
...
bool DifferentialFile::ReadTSLC(const SVD_PREFIX & prefix, DiffPtr & diffptr)
{
    DebugPrintf(SV_LOG_DEBUG,"ENTERED %s\n",FUNCTION_NAME);
    assert(prefix.count == 1);
    ...
    // Read the timestamp chunk
    SVD_TIME_STAMP tslc;
    readin = File::FullRead((char*)&(tslc),SVD_TIMESTAMP_SIZE);
...
bool DifferentialFile::ReadTLV2(const SVD_PREFIX & prefix, DiffPtr & diffptr)
{
    DebugPrintf(SV_LOG_DEBUG,"ENTERED %s\n",FUNCTION_NAME);
    assert(prefix.count == 1);
    ...
    // Read the timestamp chunk
    readin = File::FullRead((char*)&(tslc),SVD_TIMESTAMP_V2_SIZE);
...
bool DifferentialFile::ReadUSER(const SVD_PREFIX & prefix, DiffPtr & diffptr, SV_UINT& dataFormatFlags)
{
    DebugPrintf(SV_LOG_DEBUG,"ENTERED %s\n",FUNCTION_NAME);
    assert(prefix.count == 1);
    ...
    do 
    {
        ...
        // Read the user chunk
        readin = File::FullRead(buffer.get(), prefix.Flags);
...
bool DifferentialFile::ReadLODC(const SVD_PREFIX & prefix, DiffPtr & diffptr)
{
    DebugPrintf(SV_LOG_DEBUG,"ENTERED %s\n",FUNCTION_NAME);
    assert(prefix.count == 1);
    ...
    do {
        ...
        readin = File::FullRead((char*)(&lodc), SVD_LODC_SIZE);

        /* NEEDTOSWAP : swaps lodc : START */
        ...
        File::Seek((SV_OFFSET_TYPE)lodc.QuadPart, BasicIo::BioCur);
        if (!File::Good())
        {
            stringstream l_stdfatal;
            l_stdfatal << "Error detected  " << "in Function:" << FUNCTION_NAME 
        ...
        readin = File::FullRead((char*)&pfx,SVD_PREFIX_SIZE);
        /* NEEDTOSWAP : Start */
        ...
        //Restore the offset.
        File::Seek(offset, BasicIo::BioBeg);
        rv = File::Good();
...
bool DifferentialFile::ReadDATA(const SVD_PREFIX & prefix, DiffPtr & diffptr)
{
    DebugPrintf(SV_LOG_DEBUG,"ENTERED %s\n",FUNCTION_NAME);
    assert(prefix.count == 1);
    ...
    do {
        ...
        // Read the timestamp chunk
        readin = File::FullRead((char*)&(dataInfo), SVD_DATA_INFO_SIZE);
        ...
        File::Seek(dataInfo.drtdStartOffset, BasicIo::BioBeg);
...
bool DifferentialFile::SkipBytes(const SVD_PREFIX & prefix, DiffPtr & diffptr)
{
    bool rv = true;
    ...
    File::Seek(prefix.count, BasicIo::BioCur);
...
bool DifferentialFile::ReadDRTD(const SVD_PREFIX & prefix, DiffPtr & diffptr)
{
	DebugPrintf(SV_LOG_DEBUG,"ENTERED %s\n",FUNCTION_NAME);
	...
	{	
		...
		readin = File::Read((char*)&header, SVD_DIRTY_BLOCK_SIZE);
		/* NEEDTOSWAP : START */
		...
		File::Seek((SV_OFFSET_TYPE)header.Length, BasicIo::BioCur);
		if (!File::Good())
		{
			stringstream l_stdfatal;
			l_stdfatal << "Error detected  " << "in Function:" << FUNCTION_NAME 
				<< " @ Line:" << LINE_NO << " FILE:" << FILE_NAME << "\n\n"
				<< "\n" << File::GetName()    << "is corrupt.\n"
				<< "Encountered an error on seek to end of chunk :" << header.Length 
...
bool DifferentialFile::ReadDRTDV2(const SVD_PREFIX & prefix, DiffPtr & diffptr)
{
    DebugPrintf(SV_LOG_DEBUG,"ENTERED %s\n",FUNCTION_NAME);
    ...
    {	
        ...
        // Read the record header
        readin = File::Read((char*)&header, SVD_DIRTY_BLOCK_V2_SIZE);
        /* NEEDTOSWAP : START */
...
bool DifferentialFile::ReadDRTDV3(const SVD_PREFIX & prefix, DiffPtr & diffptr)
{
    DebugPrintf(SV_LOG_DEBUG,"ENTERED %s\n",FUNCTION_NAME);
    ...
    if(rv && m_validate)
    {
        ...
        do
        {
            ...
            if(offset != dataStartOffset )
            {
                ...
                l_stdfatal << "Error detected  " << "in Function:" << FUNCTION_NAME 
                    << " @ Line:" << LINE_NO << " FILE:" << FILE_NAME
                    << "\n" << File::GetName() << " internal parser error.\n"
            ...
            // Try to skip past the actual data.
            File::Seek((SV_OFFSET_TYPE)dataLength, BasicIo::BioCur);
            if (!File::Good())
            {
                stringstream l_stdfatal;
                l_stdfatal << "Error detected  " << "in Function:" << FUNCTION_NAME 
                    << " @ Line:" << LINE_NO << " FILE:" << FILE_NAME << "\n\n"
                    << "\n" << File::GetName()    << "is corrupt.\n"
                    << "Encountered an error on seek to end of chunk :" << dataLength
                    << " bytes from offset " << dataStartOffset << "\n" 
                    << " Error Code: " << File::LastError() << "\n"
                    << " Error Message: " << Error::Msg(File::LastError()) << "\n";
