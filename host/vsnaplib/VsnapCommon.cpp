...
void InsertInList(PSINGLE_LIST_ENTRY Head, ULONGLONG Offset, SV_UINT Length,
					SV_UINT FileId, ULONGLONG FileOffset, SV_UINT TrackingId)
...
void VsnapCallBack(void *Key1, void *Key2, void *param, bool OverwriteFlag)
{
	...
	if((NewKey->Offset >= NodeKey->Offset) && (NewKeyEnd <= NodeKeyEnd))
	{
		...
		else
		{
			if(NodeKeyEnd > NewKeyEnd)
				InsertInList(Head, NewKeyEnd+1, (SV_UINT)(NodeKeyEnd-NewKeyEnd), 
					NodeKey->FileId, NodeKey->FileOffset + NewKeyEnd - NodeKey->Offset + 1, NodeKey->TrackingId);	

			if(NewKey->Offset > NodeKey->Offset)
				InsertInList(Head, NodeKey->Offset, (SV_UINT)(NewKey->Offset - NodeKey->Offset),
	...
	else if((NewKey->Offset < NodeKey->Offset) && (NewKeyEnd >= NodeKey->Offset) && (NewKeyEnd <= NodeKeyEnd))
	{
		if(!OverwriteFlag)
		{
			InsertInList(Head, NewKey->Offset, (SV_UINT)(NodeKey->Offset - NewKey->Offset), NewKey->FileId, 
		...
		{
			if(NodeKeyEnd > NewKeyEnd)
				InsertInList(Head, NewKeyEnd+1, (SV_UINT)(NodeKeyEnd-NewKeyEnd), NodeKey->FileId, 
			...
			if(NewKeyEnd > NodeKey->Offset)
			{
				...
				NodeKey->Length = (SV_UINT)(NewKeyEnd - NodeKey->Offset + 1);
				NodeKey->FileId = NewKey->FileId;
	...
	else if((NewKey->Offset >= NodeKey->Offset) && (NewKey->Offset <= NodeKeyEnd) && (NewKeyEnd > NodeKeyEnd))
	{
		if(!OverwriteFlag)
		{
			InsertInList(Head, NodeKeyEnd+1, (SV_UINT)(NewKeyEnd-NodeKeyEnd), NewKey->FileId, 
		...
		{
			if(NewKey->Offset > NodeKey->Offset)
				InsertInList(Head, NodeKey->Offset, (SV_UINT)(NewKey->Offset - NodeKey->Offset), NodeKey->FileId, 
			...
			if(NewKey->Offset < NodeKeyEnd)
			{
				...
				NodeKey->Offset = NewKey->Offset;
				NodeKey->Length = (SV_UINT)(NodeKeyEnd - NewKey->Offset + 1);
				NodeKey->FileId = NewKey->FileId;
				NodeKey->FileOffset = NewKey->FileOffset;
			}
			else
			{
				InsertInList(Head, NewKey->Offset + 1, (SV_UINT)(NewKey->Length - 1), NewKey->FileId, 
