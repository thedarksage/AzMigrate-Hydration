...
bjects are blessed into Serialization::Object::$serialized_class. (which has no methods)
...
Objects are blessed into Serialization::Object::$serialized_class. (which has no methods)
...
sub _encode {
    ...
    if ( $type eq 'null' ) {
        ...
        if ( ref($val) eq 'ARRAY' ) {
            ...
            for(my $i=0; $i <= $#{$val}; $i++)
			{
				my $process_buf = $val->[$i];
                $buffer .= $self->encode($i);
                $buffer .= $self->encode($process_buf);
            }
            $buffer .= '}';
        }
        else {
