    ...
    foreach $file (@files_cpp, @files_inl, @files_h, @files_mpc, @files_bor,
        ...
        if (open (FILE, $file)) {
            ...
            while (<FILE>) {
                ...
                if (/\$Id: fuzz.pl,v 1.1.2.3 2018/04/14 15:38:35 nichougu Exp $/) {
                    print_error ("$file:$.: Incorrect \$Id: fuzz.pl,v 1.1.2.3 2018/04/14 15:38:35 nichougu Exp $ found (remove colon)");
                }
                if (/\$Id\$/ || /\$Id: /) {
                    $found = 1;
                }
