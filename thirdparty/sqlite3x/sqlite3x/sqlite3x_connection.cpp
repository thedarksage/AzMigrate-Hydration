...
const char * sqlite3_connection::errmsg()
{
...
void sqlite3_connection::setbusyhandler(int(*f)(void*,int),void * arg) {
    if(!this->db) throw database_error("database is not open");

    if(sqlite3_busy_handler(this->db,f,arg) != SQLITE_OK)
        throw database_error(*this);
}

int sqlite3_connection::rowsmodified()
{
	if(!this->db) throw database_error("database is not open");
