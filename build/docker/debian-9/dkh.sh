    ...
    for i in $new_versions; do
        ...
        then
            ...
            if [ $debian_version = "9" ] && grep -q 'bpo' <<< $i; then
                execute_cmd "sudo apt-get install -y --allow-unauthenticated -t stretch-backports linux-headers-$i" "ignorefail"
            else
                execute_cmd "sudo apt-get install -y --allow-unauthenticated linux-headers-$i" "ignorefail"
            fi
