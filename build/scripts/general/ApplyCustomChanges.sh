...
# Make sure OS is RedHat/CentOS 6.6(64bit)
DO 'echo -n "Checking for Ubuntu 16.04(64bit) or Ubuntu 20.04(64bit) ... "'
if egrep -q 'DISTRIB_RELEASE=16.04|DISTRIB_RELEASE=20.04' /etc/lsb-release ; then
...
if [ $OSCheck -ne 0 ]; then
   DO 'echo "FAILED"' && exit 1
else
   DO 'echo "OK"'
fi
...
DO 'echo -n "Checking for required software packages ... "'
REQ_PKG_LIST="apt multipath-tools lsscsi kpartx python-pyasn1 lvm2"
if grep -q 'DISTRIB_RELEASE=16.04' /etc/lsb-release ; then
    REQ_PKG_LIST="$REQ_PKG_LIST linux-image-generic-hwe-16.04"
fi

rm -f /var/log/missing-packages.log /dev/null 2>&1
for pkgName in `echo $REQ_PKG_LIST`
do
    if dpkg -l |grep $pkgName > /dev/null 2>&1 ; then
       PKG_MISSING=0
    else
        PKG_MISSING=1
...
if [ -s /var/log/missing-packages.log ]; then
	...
	DO 'echo "Command to install package: apt-get install package-name"'
...
sed -i -e '/ulimit -c unlimited/d' /etc/bash.bashrc
