...
try
{
	...
	if(empty($post_data) || !$post_data['HOST_GUID'] || !$post_data['IP'] || !$post_data['PORT'] || !$post_data['PROTOCOL'])	throw new Exception ("Parameters not set");
	...
	$requestDetails_arr = get_request_xml($post_data['HOST_GUID'], $cs_ip, $cs_port, $http);
	...
	if(curl_errno($curl_obj))	throw new Exception ("Curl error: " . curl_error($curl_obj));
	curl_close($curl_obj);
	
	$response_array = parse_xml($response_xml);
	if ($response_array)	echo serialize($response_array);
	exit(0);
}
...
/// Functions ///
function parse_xml($response_xml)
{   
    ...
    $finalOutput = array();
	if(is_array($data['Body']['Function']['FunctionResponse']['ParameterGroup']))
	{
		$root = convertIntoIndexArr($data['Body']['Function']['FunctionResponse']['ParameterGroup']);

		foreach($root as $v) {
			if(($v['@attributes']['Id'] == 'tbs_data') || ($v['@attributes']['Id'] == 'sys_health')) {
				$tag = $v['@attributes']['Id'];
				$params = convertIntoIndexArr($v['Parameter']);
				foreach($params as $v1) {
					$attr = $v1['@attributes'];
					$finalOutput[$tag][$attr['Name']] = $attr['Value'];
				}
			}else {
				$tag = $v['@attributes']['Id'];
				$group_data = convertIntoIndexArr($v['ParameterGroup']);
				foreach($group_data as $v1) {
					$params = convertIntoIndexArr($v1['Parameter']);
					foreach($params as $v2) {
						$attr = $v2['@attributes'];
						$finalOutput[$tag][$v1['@attributes']['Id']][$attr['Name']] = $attr['Value'];
					}
				}
			}
		}
...
function get_request_xml($HOST_GUID, $cs_ip, $cs_port, $http)
{
	...
	$request_xml = '<Request Id="'.$request_id.'" Version="'.$version.'">
                    ...
                    <Body>
                            <FunctionRequest Name="'.$function_name.'">
