...
/**
  ...
  *
  * exportedMountPoints Data Access Object[DAO]
  * This class contains all database handling that is needed to 
  * permanently store and retrieve exportedMountPoints object instances. 
  * Generated Date:	Tue, 17 Jun 2008 14:28:36 +0530	
  * Pattern Version:1.0
  */
class ExportedMountPointsDAO
{
...
function createValueObject() {
	return new ExportedMountPointsVO();
 }
 ...
 */
function getObject($conn,$exportedDeviceName,$hostId,$mountPoint) {
	$valueObject = $this->createValueObject();
	$valueObject->set_exported_device_name($exportedDeviceName);
	$valueObject->set_host_id($hostId);
	$valueObject->set_mount_point($mountPoint);
...
function findByPrimaryKey($conn, $valueObject) {
	...
	$sql = "SELECT * FROM exportedMountPoints WHERE (exportedDeviceName = '".$valueObject->get_exported_device_name()."'  AND hostId = '".$valueObject->get_host_id()."'  AND mountPoint = '".$valueObject->get_mount_point()."') "; 
...
function findAll($conn) {
	...
	$sql = "SELECT * FROM exportedMountPoints ORDER BY exportedDeviceName ASC ,hostId ASC ,mountPoint ASC ";
...
function create($conn, $valueObject) {
	 $count = 0;
     $sql1 = "INSERT INTO exportedMountPoints(";
     $sql3 = ") VALUES ("; 
...
function update($conn, $valueObject) {
	$count = 0;
	$sql = "UPDATE exportedMountPoints SET ";
	if ($valueObject->isset_host_id())
 	{
		if($count>0)
		{
	...
	$sql = $sql." WHERE (exportedDeviceName = '".$valueObject->get_exported_device_name()."'  AND hostId = '".$valueObject->get_host_id()."'  AND mountPoint = '".$valueObject->get_mount_point()."')"; 
	$result = $this->databaseUpdate($conn, $sql);
    if ($result != 1) {
...
function delete($conn, $valueObject) {
	...
	$sql = "DELETE FROM exportedMountPoints WHERE (exportedDeviceName = '".$valueObject->get_exported_device_name()."'  AND hostId = '".$valueObject->get_host_id()."'  AND mountPoint = '".$valueObject->get_mount_point()."')"; 
	$result = $this->databaseUpdate($conn, $sql);
    if ($result != 1) {
         //print "PrimaryKey Error when updating DB!";
          return false;
...
function deleteAll($conn) {
	$sql = "DELETE FROM exportedMountPoints";
	$result = $this->databaseUpdate($conn, $sql);
	return true;
}
...
function countAll($conn) {
	$sql = "SELECT count(*) FROM exportedMountPoints";
	$allRows = 0;
...
function searchMatching($conn, $valueObject) {
	$first = true;
    $sql = "SELECT * FROM exportedMountPoints WHERE 1=1 ";
