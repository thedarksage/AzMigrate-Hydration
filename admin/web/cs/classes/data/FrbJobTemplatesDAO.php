...
/**
  ...
  *
  * frbJobTemplates Data Access Object[DAO]
  * This class contains all database handling that is needed to 
  * permanently store and retrieve frbJobTemplates object instances. 
  * Generated Date:	Tue, 27 Nov 2007 14:01:25 +0530	
  * Pattern Version:1.0
  */
class FrbJobTemplatesDAO
{
...
function findByPrimaryKey($conn, $valueObject) {
	...
	$sql = "SELECT * FROM frbJobTemplates WHERE (id = '".$valueObject->get_id()."') "; 
...
function findAll($conn) {
	...
	$sql = "SELECT * FROM frbJobTemplates ORDER BY id ASC ";
...
function create($conn, $valueObject) {
	 $count = 0;
     $sql1 = "INSERT INTO frbJobTemplates(";
     $sql3 = ") VALUES ("; 
	 ...
	 if ($valueObject->isset_template_name())
 	 {
 	  	if($count>0)
		{
			$sql2 = $sql2 . ",";
 	    ...
 	    $sql2 = $sql2."templateName";
 	    $sql4 = $sql4."'".$valueObject->get_template_name()."'"; 	
 		$count++;
 	 }
	 
	 if ($valueObject->isset_template_string())
 	 {
 	  	if($count>0)
		{
			$sql2 = $sql2 . ",";
 	    ...
 	    $sql2 = $sql2."templateString";
 	    $sql4 = $sql4."'".$valueObject->get_template_string()."'"; 	
 		$count++;
 	 }
	 
	 if ($valueObject->isset_template_type())
 	 {
 	  	if($count>0)
		{
			$sql2 = $sql2 . ",";
 	    ...
 	    $sql2 = $sql2."templateType";
 	    $sql4 = $sql4."'".$valueObject->get_template_type()."'"; 	
 		$count++;
 	 }
...
function update($conn, $valueObject) {
	$count = 0;
	$sql = "UPDATE frbJobTemplates SET ";
	if ($valueObject->isset_id())
 	{
		if($count>0)
		{
	...
	if ($valueObject->isset_template_name())
 	{
		...
		$sql = $sql."templateName = '".$valueObject->get_template_name()."'";
		$count++;
	}
	...
	if ($valueObject->isset_template_string())
 	{
		...
		$sql = $sql."templateString = '".$valueObject->get_template_string()."'";
		$count++;
	}
	...
	if ($valueObject->isset_template_type())
 	{
		...
		$sql = $sql."templateType = '".$valueObject->get_template_type()."'";
		$count++;
	}
...
function delete($conn, $valueObject) {
	...
	$sql = "DELETE FROM frbJobTemplates WHERE (id = '".$valueObject->get_id()."')"; 
	$result = $this->databaseUpdate($conn, $sql);
    if ($result != 1) {
         //print "PrimaryKey Error when updating DB!";
          return false;
...
function deleteAll($conn) {
	$sql = "DELETE FROM frbJobTemplates";
	$result = $this->databaseUpdate($conn, $sql);
	return true;
}
...
function countAll($conn) {
	$sql = "SELECT count(*) FROM frbJobTemplates";
	$allRows = 0;
...
function searchMatching($conn, $valueObject) {
	$first = true;
    $sql = "SELECT * FROM frbJobTemplates WHERE 1=1 ";
	...
	if ($valueObject->get_template_name() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND templateName LIKE '".$valueObject->get_template_name()."%' ";
    }
	
	if ($valueObject->get_template_string() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND templateString LIKE '".$valueObject->get_template_string()."%' ";
    }
	
	if ($valueObject->get_template_type() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND templateType LIKE '".$valueObject->get_template_type()."%' ";
    }
