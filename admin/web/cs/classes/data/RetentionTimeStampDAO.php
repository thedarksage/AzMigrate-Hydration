...
/**
  ...
  *
  * retentionTimeStamp Data Access Object[DAO]
  * This class contains all database handling that is needed to 
  * permanently store and retrieve retentionTimeStamp object instances. 
  * Generated Date:	Tue, 27 Nov 2007 14:01:28 +0530	
  * Pattern Version:1.0
  */
class RetentionTimeStampDAO
{
...
function createValueObject() {
	return new RetentionTimeStampVO();
 }
...
function findByPrimaryKey($conn, $valueObject) {
	...
	$sql = "SELECT * FROM retentionTimeStamp WHERE (id = '".$valueObject->get_id()."') "; 
...
function findAll($conn) {
	...
	$sql = "SELECT * FROM retentionTimeStamp ORDER BY id ASC ";
...
function create($conn, $valueObject) {
	 $count = 0;
     $sql1 = "INSERT INTO retentionTimeStamp(";
     $sql3 = ") VALUES ("; 
	 ...
	 if ($valueObject->isset_start_time_u_t_c())
 	 {
 	  	if($count>0)
		{
			$sql2 = $sql2 . ",";
 	    ...
 	    $sql2 = $sql2."StartTimeUTC";
 	    $sql4 = $sql4."'".$valueObject->get_start_time_u_t_c()."'"; 	
 		$count++;
 	 }
	 
	 if ($valueObject->isset_end_time_u_t_c())
 	 {
 	  	if($count>0)
		{
			$sql2 = $sql2 . ",";
 	    ...
 	    $sql2 = $sql2."EndTimeUTC";
 	    $sql4 = $sql4."'".$valueObject->get_end_time_u_t_c()."'"; 	
 		$count++;
 	 }
	 ...
	 if ($valueObject->isset_accuracy())
 	 {
 	  	if($count>0)
		{
 	    ...
 	    $sql2 = $sql2."accuracy";
 	    $sql4 = $sql4."'".$valueObject->get_accuracy()."'"; 	
 		$count++;
 	 }
...
function update($conn, $valueObject) {
	$count = 0;
	$sql = "UPDATE retentionTimeStamp SET ";
	if ($valueObject->isset_id())
 	{
		if($count>0)
		{
	...
	if ($valueObject->isset_start_time_u_t_c())
 	{
		...
		$sql = $sql."StartTimeUTC = '".$valueObject->get_start_time_u_t_c()."'";
		$count++;
	}
	...
	if ($valueObject->isset_end_time_u_t_c())
 	{
		...
		$sql = $sql."EndTimeUTC = '".$valueObject->get_end_time_u_t_c()."'";
		$count++;
	}
	...
	if ($valueObject->isset_accuracy())
 	{
		...
		$sql = $sql."accuracy = '".$valueObject->get_accuracy()."'";
		$count++;
	}
...
function delete($conn, $valueObject) {
	...
	$sql = "DELETE FROM retentionTimeStamp WHERE (id = '".$valueObject->get_id()."')"; 
	$result = $this->databaseUpdate($conn, $sql);
    if ($result != 1) {
         //print "PrimaryKey Error when updating DB!";
          return false;
...
function deleteAll($conn) {
	$sql = "DELETE FROM retentionTimeStamp";
	$result = $this->databaseUpdate($conn, $sql);
	return true;
}
...
function countAll($conn) {
	$sql = "SELECT count(*) FROM retentionTimeStamp";
	$allRows = 0;
...
function searchMatching($conn, $valueObject) {
	$first = true;
    $sql = "SELECT * FROM retentionTimeStamp WHERE 1=1 ";
	...
	if ($valueObject->get_start_time_u_t_c() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND StartTimeUTC LIKE '".$valueObject->get_start_time_u_t_c()."%' ";
    }
	
	if ($valueObject->get_end_time_u_t_c() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND EndTimeUTC LIKE '".$valueObject->get_end_time_u_t_c()."%' ";
    }
	...
	if ($valueObject->get_accuracy() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND accuracy LIKE '".$valueObject->get_accuracy()."%' ";
    }
