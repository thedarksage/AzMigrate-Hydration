...
/**
  ...
  *
  * frbJobs Data Access Object[DAO]
  * This class contains all database handling that is needed to 
  * permanently store and retrieve frbJobs object instances. 
...
function findByPrimaryKey($conn, $valueObject) {
	...
	$sql = "SELECT * FROM frbJobs WHERE (id = '".$valueObject->get_id()."') "; 
...
function findAll($conn) {
	...
	$sql = "SELECT * FROM frbJobs ORDER BY id ASC ";
...
function create($conn, $valueObject) {
	 ...
	 if ($valueObject->isset_assigned_state())
 	 {
 	  	if($count>0)
		{
			$sql2 = $sql2 . ",";
 	    ...
 	    $sql2 = $sql2."assignedState";
 	    $sql4 = $sql4."'".$valueObject->get_assigned_state()."'"; 	
 		$count++;
 	 }
	 
	 if ($valueObject->isset_current_state())
 	 {
 	  	if($count>0)
		{
			$sql2 = $sql2 . ",";
 	    ...
 	    $sql2 = $sql2."currentState";
 	    $sql4 = $sql4."'".$valueObject->get_current_state()."'"; 	
 		$count++;
 	 }
	 
	 if ($valueObject->isset_daemon_state())
 	 {
 	  	if($count>0)
		{
			$sql2 = $sql2 . ",";
 	    ...
 	    $sql2 = $sql2."daemonState";
 	    $sql4 = $sql4."'".$valueObject->get_daemon_state()."'"; 	
 		$count++;
 	 }
	...
	if ($valueObject->isset_assigned_state())
 	{
		...
		$sql = $sql."assignedState = '".$valueObject->get_assigned_state()."'";
		$count++;
	}
	...
	if ($valueObject->isset_current_state())
 	{
		...
		$sql = $sql."currentState = '".$valueObject->get_current_state()."'";
		$count++;
	}
	...
	if ($valueObject->isset_daemon_state())
 	{
		...
		$sql = $sql."daemonState = '".$valueObject->get_daemon_state()."'";
		$count++;
	}
...
function delete($conn, $valueObject) {
	...
	$sql = "DELETE FROM frbJobs WHERE (id = '".$valueObject->get_id()."')"; 
	$result = $this->databaseUpdate($conn, $sql);
    if ($result != 1) {
         //print "PrimaryKey Error when updating DB!";
          return false;
...
function searchMatching($conn, $valueObject) {
	$first = true;
    $sql = "SELECT * FROM frbJobs WHERE 1=1 ";
	...
	if ($valueObject->get_assigned_state() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND assignedState LIKE '".$valueObject->get_assigned_state()."%' ";
    }
	
	if ($valueObject->get_current_state() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND currentState LIKE '".$valueObject->get_current_state()."%' ";
    }
	
	if ($valueObject->get_daemon_state() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND daemonState LIKE '".$valueObject->get_daemon_state()."%' ";
    }
