...
/**
  ...
  *
  * sanVt_vi_info Data Access Object[DAO]
  * This class contains all database handling that is needed to 
  * permanently store and retrieve sanVt_vi_info object instances. 
  * Generated Date:	Tue, 27 Nov 2007 14:01:29 +0530	
  * Pattern Version:1.0
  */
class SanVtViInfoDAO
{
...
function findByPrimaryKey($conn, $valueObject) {
	...
	$sql = "SELECT * FROM sanVt_vi_info WHERE (id = '".$valueObject->get_id()."') "; 
...
function findAll($conn) {
	...
	$sql = "SELECT * FROM sanVt_vi_info ORDER BY id ASC ";
...
function create($conn, $valueObject) {
	 $count = 0;
     $sql1 = "INSERT INTO sanVt_vi_info(";
     $sql3 = ") VALUES ("; 
	 ...
	 if ($valueObject->isset_portwwn())
 	 {
 	  	if($count>0)
		{
 	    ...
 	    $sql2 = $sql2."portwwn";
 	    $sql4 = $sql4."'".$valueObject->get_portwwn()."'"; 	
 		$count++;
 	 }
	 
	 if ($valueObject->isset_nodewwn())
 	 {
 	  	if($count>0)
		{
			$sql2 = $sql2 . ",";
 	    ...
 	    $sql2 = $sql2."nodewwn";
 	    $sql4 = $sql4."'".$valueObject->get_nodewwn()."'"; 	
 		$count++;
 	 }
	 ...
	 if ($valueObject->isset_v_portwwn())
 	 {
 	  	if($count>0)
		{
			$sql2 = $sql2 . ",";
 	    ...
 	    $sql2 = $sql2."v_portwwn";
 	    $sql4 = $sql4."'".$valueObject->get_v_portwwn()."'"; 	
 		$count++;
 	 }
	 
	 if ($valueObject->isset_v_nodewwn())
 	 {
 	  	if($count>0)
		{
			$sql2 = $sql2 . ",";
 	    ...
 	    $sql2 = $sql2."v_nodewwn";
 	    $sql4 = $sql4."'".$valueObject->get_v_nodewwn()."'"; 	
 		$count++;
 	 }
	...
	if ($valueObject->isset_portwwn())
 	{
		...
		$sql = $sql."portwwn = '".$valueObject->get_portwwn()."'";
		$count++;
	}
	...
	if ($valueObject->isset_nodewwn())
 	{
		...
		$sql = $sql."nodewwn = '".$valueObject->get_nodewwn()."'";
		$count++;
	}
	...
	if ($valueObject->isset_v_portwwn())
 	{
		...
		$sql = $sql."v_portwwn = '".$valueObject->get_v_portwwn()."'";
		$count++;
	}
	...
	if ($valueObject->isset_v_nodewwn())
 	{
		...
		$sql = $sql."v_nodewwn = '".$valueObject->get_v_nodewwn()."'";
		$count++;
	}
...
function delete($conn, $valueObject) {
	...
	$sql = "DELETE FROM sanVt_vi_info WHERE (id = '".$valueObject->get_id()."')"; 
	$result = $this->databaseUpdate($conn, $sql);
    if ($result != 1) {
         //print "PrimaryKey Error when updating DB!";
          return false;
...
function countAll($conn) {
	$sql = "SELECT count(*) FROM sanVt_vi_info";
	$allRows = 0;
...
function searchMatching($conn, $valueObject) {
	$first = true;
    $sql = "SELECT * FROM sanVt_vi_info WHERE 1=1 ";
	...
	if ($valueObject->get_portwwn() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND portwwn LIKE '".$valueObject->get_portwwn()."%' ";
    }
	
	if ($valueObject->get_nodewwn() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND nodewwn LIKE '".$valueObject->get_nodewwn()."%' ";
    }
	...
	if ($valueObject->get_v_portwwn() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND v_portwwn LIKE '".$valueObject->get_v_portwwn()."%' ";
    }
	
	if ($valueObject->get_v_nodewwn() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND v_nodewwn LIKE '".$valueObject->get_v_nodewwn()."%' ";
    }
