...
/**
  ...
  *
  * sanInitiators Data Access Object[DAO]
  * This class contains all database handling that is needed to 
  * permanently store and retrieve sanInitiators object instances. 
  * Generated Date:	Thu, 17 Apr 2008 12:27:34 +0530	
  * Pattern Version:1.0
  */
class SanInitiatorsDAO
{
 ...
 */
function getObject($conn,$fabricId,$sanInitiatorPortWwn) {
...
function findByPrimaryKey($conn, $valueObject) {
	...
	$sql = "SELECT * FROM sanInitiators WHERE (fabricId = '".$valueObject->get_fabric_id()."'  AND sanInitiatorPortWwn = '".$valueObject->get_san_initiator_port_wwn()."') "; 
...
function find_by_fabric_id($conn, $params = array()) {
	...
	$sql = "SELECT * FROM sanInitiators WHERE (" . $sql . ")";
...
function find_by_port_wwn($conn, $params = array()) {
	...
	$sql = "fabricId='fabric_id' and sanInitiatorPortWwn='port_wwn'";
	$sql = str_replace(array_keys($params), array_values($params), $sql);
	$sql = "SELECT * FROM sanInitiators WHERE (" . $sql . ")";
	$searchResults = $this->listQuery($conn, $sql);
	return $searchResults;
...
function findAll($conn) {
	...
	$sql = "SELECT * FROM sanInitiators ORDER BY fabricId ASC ,sanInitiatorPortWwn ASC ";
	$searchResults = $this->listQuery($conn, $sql);
	return $searchResults;
}
...
function create($conn, $valueObject) {
	 $count = 0;
     $sql1 = "INSERT INTO sanInitiators(";
     $sql3 = ") VALUES ("; 
...
function update($conn, $valueObject) {
	...
	$sql = $sql." WHERE (fabricId = '".$valueObject->get_fabric_id()."'  AND sanInitiatorPortWwn = '".$valueObject->get_san_initiator_port_wwn()."')"; 
	$result = $this->databaseUpdate($conn, $sql);
    if ($result != 1) {
...
function delete($conn, $valueObject) {
	...
	$sql = "DELETE FROM sanInitiators WHERE (fabricId = '".$valueObject->get_fabric_id()."'  AND sanInitiatorPortWwn = '".$valueObject->get_san_initiator_port_wwn()."')"; 
	$result = $this->databaseUpdate($conn, $sql);
    if ($result != 1) {
         //print "PrimaryKey Error when updating DB!";
          return false;
...
function countAll($conn) {
	$sql = "SELECT count(*) FROM sanInitiators";
	$allRows = 0;
...
function searchMatching($conn, $valueObject) {
	$first = true;
    $sql = "SELECT * FROM sanInitiators WHERE 1=1 ";
