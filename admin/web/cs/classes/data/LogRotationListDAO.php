...
/**
  ...
  *
  * logRotationList Data Access Object[DAO]
  * This class contains all database handling that is needed to 
  * permanently store and retrieve logRotationList object instances. 
  * Generated Date:	Tue, 27 Nov 2007 14:01:26 +0530	
  * Pattern Version:1.0
  */
class LogRotationListDAO
{
...
function findByPrimaryKey($conn, $valueObject) {
	...
	$sql = "SELECT * FROM logRotationList WHERE (id = '".$valueObject->get_id()."') "; 
...
function findAll($conn) {
	...
	$sql = "SELECT * FROM logRotationList ORDER BY id ASC ";
...
function create($conn, $valueObject) {
	 $count = 0;
     $sql1 = "INSERT INTO logRotationList(";
     $sql3 = ") VALUES ("; 
	 ...
	 if ($valueObject->isset_log_name())
 	 {
 	  	if($count>0)
		{
 	    ...
 	    $sql2 = $sql2."logName";
 	    $sql4 = $sql4."'".$valueObject->get_log_name()."'"; 	
 		$count++;
 	 }
	 
	 if ($valueObject->isset_log_path_linux())
 	 {
 	  	if($count>0)
		{
			$sql2 = $sql2 . ",";
 	    ...
 	    $sql2 = $sql2."logPathLinux";
 	    $sql4 = $sql4."'".$valueObject->get_log_path_linux()."'"; 	
 		$count++;
 	 }
	 
	 if ($valueObject->isset_log_path_windows())
 	 {
 	  	if($count>0)
		{
			$sql2 = $sql2 . ",";
 	    ...
 	    $sql2 = $sql2."logPathWindows";
 	    $sql4 = $sql4."'".$valueObject->get_log_path_windows()."'"; 	
 		$count++;
 	 }
	 
	 if ($valueObject->isset_log_type())
 	 {
 	  	if($count>0)
		{
			$sql2 = $sql2 . ",";
 	    ...
 	    $sql2 = $sql2."logType";
 	    $sql4 = $sql4."'".$valueObject->get_log_type()."'"; 	
 		$count++;
 	 }
...
function update($conn, $valueObject) {
	$count = 0;
	$sql = "UPDATE logRotationList SET ";
	if ($valueObject->isset_id())
 	{
		if($count>0)
		{
	...
	if ($valueObject->isset_log_name())
 	{
		...
		$sql = $sql."logName = '".$valueObject->get_log_name()."'";
		$count++;
	}
	...
	if ($valueObject->isset_log_path_linux())
 	{
		...
		$sql = $sql."logPathLinux = '".$valueObject->get_log_path_linux()."'";
		$count++;
	}
	...
	if ($valueObject->isset_log_path_windows())
 	{
		...
		$sql = $sql."logPathWindows = '".$valueObject->get_log_path_windows()."'";
		$count++;
	}
	...
	if ($valueObject->isset_log_type())
 	{
		...
		$sql = $sql."logType = '".$valueObject->get_log_type()."'";
		$count++;
	}
...
function delete($conn, $valueObject) {
	...
	$sql = "DELETE FROM logRotationList WHERE (id = '".$valueObject->get_id()."')"; 
	$result = $this->databaseUpdate($conn, $sql);
    if ($result != 1) {
         //print "PrimaryKey Error when updating DB!";
          return false;
...
function deleteAll($conn) {
	$sql = "DELETE FROM logRotationList";
	$result = $this->databaseUpdate($conn, $sql);
	return true;
}
...
function countAll($conn) {
	$sql = "SELECT count(*) FROM logRotationList";
	$allRows = 0;
...
function searchMatching($conn, $valueObject) {
	$first = true;
    $sql = "SELECT * FROM logRotationList WHERE 1=1 ";
	...
	if ($valueObject->get_log_name() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND logName LIKE '".$valueObject->get_log_name()."%' ";
    }
	
	if ($valueObject->get_log_path_linux() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND logPathLinux LIKE '".$valueObject->get_log_path_linux()."%' ";
    }
	
	if ($valueObject->get_log_path_windows() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND logPathWindows LIKE '".$valueObject->get_log_path_windows()."%' ";
    }
	
	if ($valueObject->get_log_type() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND logType LIKE '".$valueObject->get_log_type()."%' ";
    }
