...
/**
  ...
  *
  * frbGlobalSettings Data Access Object[DAO]
  * This class contains all database handling that is needed to 
  * permanently store and retrieve frbGlobalSettings object instances. 
  * Generated Date:	Tue, 27 Nov 2007 14:01:25 +0530	
  * Pattern Version:1.0
  */
class FrbGlobalSettingsDAO
{
...
function createValueObject() {
	return new FrbGlobalSettingsVO();
 }
...
function findAll($conn) {
	...
	$sql = "SELECT * FROM frbGlobalSettings";
...
function create($conn, $valueObject) {
	 $count = 0;
     $sql1 = "INSERT INTO frbGlobalSettings(";
     $sql3 = ") VALUES ("; 
	 
	 if ($valueObject->isset_auto_clear_log_year())
 	 {
 	  	if($count>0)
		{
			$sql2 = $sql2 . ",";
 	    ...
 	    $sql2 = $sql2."autoClearLogYear";
 	    $sql4 = $sql4."'".$valueObject->get_auto_clear_log_year()."'"; 	
 		$count++;
 	 }
	 
	 if ($valueObject->isset_auto_clear_log_month())
 	 {
 	  	if($count>0)
		{
			$sql2 = $sql2 . ",";
 	    ...
 	    $sql2 = $sql2."autoClearLogMonth";
 	    $sql4 = $sql4."'".$valueObject->get_auto_clear_log_month()."'"; 	
 		$count++;
 	 }
	 
	 if ($valueObject->isset_auto_clear_log_week())
 	 {
 	  	if($count>0)
		{
			$sql2 = $sql2 . ",";
 	    ...
 	    $sql2 = $sql2."autoClearLogWeek";
 	    $sql4 = $sql4."'".$valueObject->get_auto_clear_log_week()."'"; 	
 		$count++;
 	 }
	 
	 if ($valueObject->isset_auto_clear_log_day())
 	 {
 	  	if($count>0)
		{
			$sql2 = $sql2 . ",";
 	    ...
 	    $sql2 = $sql2."autoClearLogDay";
 	    $sql4 = $sql4."'".$valueObject->get_auto_clear_log_day()."'"; 	
 		$count++;
 	 }
...
function deleteAll($conn) {
	$sql = "DELETE FROM frbGlobalSettings";
	$result = $this->databaseUpdate($conn, $sql);
	return true;
}
...
function countAll($conn) {
	$sql = "SELECT count(*) FROM frbGlobalSettings";
	$allRows = 0;
...
function searchMatching($conn, $valueObject) {
	$first = true;
    $sql = "SELECT * FROM frbGlobalSettings WHERE 1=1 ";
	
	if ($valueObject->get_auto_clear_log_year() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND autoClearLogYear LIKE '".$valueObject->get_auto_clear_log_year()."%' ";
    }
	
	if ($valueObject->get_auto_clear_log_month() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND autoClearLogMonth LIKE '".$valueObject->get_auto_clear_log_month()."%' ";
    }
	
	if ($valueObject->get_auto_clear_log_week() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND autoClearLogWeek LIKE '".$valueObject->get_auto_clear_log_week()."%' ";
    }
	
	if ($valueObject->get_auto_clear_log_day() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND autoClearLogDay LIKE '".$valueObject->get_auto_clear_log_day()."%' ";
    }
