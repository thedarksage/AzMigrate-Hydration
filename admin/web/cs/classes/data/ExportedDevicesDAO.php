...
/**
  ...
  *
  * exportedDevices Data Access Object[DAO]
  * This class contains all database handling that is needed to 
  * permanently store and retrieve exportedDevices object instances. 
  * Generated Date:	Tue, 17 Jun 2008 14:28:35 +0530	
  * Pattern Version:1.0
  */
class ExportedDevicesDAO
{
 ...
 */
function getObject($conn,$exportedDeviceName,$hostId) {
...
function findByPrimaryKey($conn, $valueObject) {
	...
	$sql = "SELECT * FROM exportedDevices WHERE (exportedDeviceName = '".$valueObject->get_exported_device_name()."'  AND hostId = '".$valueObject->get_host_id()."') "; 
...
function findAll($conn) {
	...
	$sql = "SELECT * FROM exportedDevices ORDER BY exportedDeviceName ASC ,hostId ASC ";
...
function create($conn, $valueObject) {
	 $count = 0;
     $sql1 = "INSERT INTO exportedDevices(";
     $sql3 = ") VALUES ("; 
	 ...
	 if ($valueObject->isset_creation_time())
 	 {
 	  	if($count>0)
		{
			$sql2 = $sql2 . ",";
 	    ...
 	    $sql2 = $sql2."creationTime";
 	    $sql4 = $sql4."'".$valueObject->get_creation_time()."'"; 	
 		$count++;
 	 }
...
function update($conn, $valueObject) {
	$count = 0;
	$sql = "UPDATE exportedDevices SET ";
	if ($valueObject->isset_host_id())
 	{
		if($count>0)
		{
	...
	if ($valueObject->isset_creation_time())
 	{
		...
		$sql = $sql."creationTime = '".$valueObject->get_creation_time()."'";
		$count++;
	}
	
	$sql = $sql." WHERE (exportedDeviceName = '".$valueObject->get_exported_device_name()."'  AND hostId = '".$valueObject->get_host_id()."')"; 
	$result = $this->databaseUpdate($conn, $sql);
    if ($result != 1) {
...
function delete($conn, $valueObject) {
	...
	$sql = "DELETE FROM exportedDevices WHERE (exportedDeviceName = '".$valueObject->get_exported_device_name()."'  AND hostId = '".$valueObject->get_host_id()."')"; 
	$result = $this->databaseUpdate($conn, $sql);
    if ($result != 1) {
         //print "PrimaryKey Error when updating DB!";
          return false;
...
function deleteAll($conn) {
	$sql = "DELETE FROM exportedDevices";
	$result = $this->databaseUpdate($conn, $sql);
	return true;
}
...
function countAll($conn) {
	$sql = "SELECT count(*) FROM exportedDevices";
	$allRows = 0;
...
function searchMatching($conn, $valueObject) {
	$first = true;
    $sql = "SELECT * FROM exportedDevices WHERE 1=1 ";
	...
	if ($valueObject->get_creation_time() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND creationTime LIKE '".$valueObject->get_creation_time()."%' ";
    }
