...
/**
  ...
  *
  * trapListeners Data Access Object[DAO]
  * This class contains all database handling that is needed to 
  * permanently store and retrieve trapListeners object instances. 
  * Generated Date:	Tue, 27 Nov 2007 14:01:30 +0530	
  * Pattern Version:1.0
  */
class TrapListenersDAO
{
 ...
 */
function getObject($conn,$trapListenerId) {
	$valueObject = $this->createValueObject();
	$valueObject->set_trap_listener_id($trapListenerId);
...
function findByPrimaryKey($conn, $valueObject) {
	...
	if (!$valueObject->get_trap_listener_id()) {
    	//print "Can not select without Primary-Key!";
		return false;
	}

	$sql = "SELECT * FROM trapListeners WHERE (trapListenerId = '".$valueObject->get_trap_listener_id()."') "; 
...
function findAll($conn) {
	...
	$sql = "SELECT * FROM trapListeners ORDER BY trapListenerId ASC ";
...
function create($conn, $valueObject) {
	 $count = 0;
     $sql1 = "INSERT INTO trapListeners(";
     $sql3 = ") VALUES ("; 
	 
	 if ($valueObject->isset_trap_listener_id())
 	 {
 	  	if($count>0)
		{
			$sql2 = $sql2 . ",";
 	    ...
 	    $sql2 = $sql2."trapListenerId";
 	    $sql4 = $sql4."'".$valueObject->get_trap_listener_id()."'"; 	
 		$count++;
 	 }
	 
	 if ($valueObject->isset_trap_listener_ipv4())
 	 {
 	  	if($count>0)
		{
			$sql2 = $sql2 . ",";
 	    ...
 	    $sql2 = $sql2."trapListenerIPv4";
 	    $sql4 = $sql4."'".$valueObject->get_trap_listener_ipv4()."'"; 	
 		$count++;
 	 }
	 
	 if ($valueObject->isset_trap_listener_port())
 	 {
 	  	if($count>0)
		{
			$sql2 = $sql2 . ",";
 	    ...
 	    $sql2 = $sql2."trapListenerPort";
 	    $sql4 = $sql4."'".$valueObject->get_trap_listener_port()."'"; 	
 		$count++;
 	 }
	...
	if ($valueObject->isset_trap_listener_id())
 	{
		...
		$sql = $sql."trapListenerId = '".$valueObject->get_trap_listener_id()."'";
		$count++;
	}
	...
	if ($valueObject->isset_trap_listener_ipv4())
 	{
		...
		$sql = $sql."trapListenerIPv4 = '".$valueObject->get_trap_listener_ipv4()."'";
		$count++;
	}
	...
	if ($valueObject->isset_trap_listener_port())
 	{
		...
		$sql = $sql."trapListenerPort = '".$valueObject->get_trap_listener_port()."'";
		$count++;
	}
	
	$sql = $sql." WHERE (trapListenerId = '".$valueObject->get_trap_listener_id()."')"; 
	$result = $this->databaseUpdate($conn, $sql);
    if ($result != 1) {
...
function delete($conn, $valueObject) {
	...
	$sql = "DELETE FROM trapListeners WHERE (trapListenerId = '".$valueObject->get_trap_listener_id()."')"; 
	$result = $this->databaseUpdate($conn, $sql);
    if ($result != 1) {
         //print "PrimaryKey Error when updating DB!";
          return false;
...
function countAll($conn) {
	$sql = "SELECT count(*) FROM trapListeners";
	$allRows = 0;
...
function searchMatching($conn, $valueObject) {
	$first = true;
    $sql = "SELECT * FROM trapListeners WHERE 1=1 ";
	
	if ($valueObject->get_trap_listener_id() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND trapListenerId LIKE '".$valueObject->get_trap_listener_id()."%' ";
    }
	
	if ($valueObject->get_trap_listener_ipv4() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND trapListenerIPv4 LIKE '".$valueObject->get_trap_listener_ipv4()."%' ";
    }
	
	if ($valueObject->get_trap_listener_port() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND trapListenerPort LIKE '".$valueObject->get_trap_listener_port()."%' ";
    }
	
	$sql = $sql." ORDER BY ";
	$sql = $sql."trapListenerId ASC ";
