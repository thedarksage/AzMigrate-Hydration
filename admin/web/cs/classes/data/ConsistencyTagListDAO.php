...
/**
  ...
  *
  * consistencyTagList Data Access Object[DAO]
  * This class contains all database handling that is needed to 
  * permanently store and retrieve consistencyTagList object instances. 
  * Generated Date:	Tue, 27 Nov 2007 14:01:23 +0530	
  * Pattern Version:1.0
  */
class ConsistencyTagListDAO
{
...
function createValueObject() {
	return new ConsistencyTagListVO();
 }
...
function findByPrimaryKey($conn, $valueObject) {
	...
	$sql = "SELECT * FROM consistencyTagList WHERE (id = '".$valueObject->get_id()."') "; 
...
function findAll($conn) {
	...
	$sql = "SELECT * FROM consistencyTagList ORDER BY id ASC ";
...
function create($conn, $valueObject) {
	 $count = 0;
     $sql1 = "INSERT INTO consistencyTagList(";
     $sql3 = ") VALUES ("; 
	 ...
	 if ($valueObject->isset_value_name())
 	 {
 	  	if($count>0)
		{
			$sql2 = $sql2 . ",";
 	    ...
 	    $sql2 = $sql2."ValueName";
 	    $sql4 = $sql4."'".$valueObject->get_value_name()."'"; 	
 		$count++;
 	 }
	 
	 if ($valueObject->isset_value_data())
 	 {
 	  	if($count>0)
		{
			$sql2 = $sql2 . ",";
 	    ...
 	    $sql2 = $sql2."ValueData";
 	    $sql4 = $sql4."'".$valueObject->get_value_data()."'"; 	
 		$count++;
 	 }
	 
	 if ($valueObject->isset_value_type())
 	 {
 	  	if($count>0)
		{
			$sql2 = $sql2 . ",";
 	    ...
 	    $sql2 = $sql2."ValueType";
 	    $sql4 = $sql4."'".$valueObject->get_value_type()."'"; 	
 		$count++;
 	 }
...
function update($conn, $valueObject) {
	$count = 0;
	$sql = "UPDATE consistencyTagList SET ";
	if ($valueObject->isset_id())
 	{
		if($count>0)
		{
	...
	if ($valueObject->isset_value_name())
 	{
		...
		$sql = $sql."ValueName = '".$valueObject->get_value_name()."'";
		$count++;
	}
	...
	if ($valueObject->isset_value_data())
 	{
		...
		$sql = $sql."ValueData = '".$valueObject->get_value_data()."'";
		$count++;
	}
	...
	if ($valueObject->isset_value_type())
 	{
		...
		$sql = $sql."ValueType = '".$valueObject->get_value_type()."'";
		$count++;
	}
...
function delete($conn, $valueObject) {
	...
	$sql = "DELETE FROM consistencyTagList WHERE (id = '".$valueObject->get_id()."')"; 
	$result = $this->databaseUpdate($conn, $sql);
    if ($result != 1) {
         //print "PrimaryKey Error when updating DB!";
          return false;
...
function deleteAll($conn) {
	$sql = "DELETE FROM consistencyTagList";
	$result = $this->databaseUpdate($conn, $sql);
	return true;
}
...
function countAll($conn) {
	$sql = "SELECT count(*) FROM consistencyTagList";
	$allRows = 0;
...
function searchMatching($conn, $valueObject) {
	$first = true;
    $sql = "SELECT * FROM consistencyTagList WHERE 1=1 ";
	...
	if ($valueObject->get_value_name() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND ValueName LIKE '".$valueObject->get_value_name()."%' ";
    }
	
	if ($valueObject->get_value_data() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND ValueData LIKE '".$valueObject->get_value_data()."%' ";
    }
	
	if ($valueObject->get_value_type() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND ValueType LIKE '".$valueObject->get_value_type()."%' ";
    }
