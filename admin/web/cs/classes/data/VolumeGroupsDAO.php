...
/**
  ...
  *
  * volumeGroups Data Access Object[DAO]
  * This class contains all database handling that is needed to 
  * permanently store and retrieve volumeGroups object instances. 
  * Generated Date:	Tue, 27 Nov 2007 14:01:30 +0530	
  * Pattern Version:1.0
  */
class VolumeGroupsDAO
{
...
function findByPrimaryKey($conn, $valueObject) {
	...
	$sql = "SELECT * FROM volumeGroups WHERE (id = '".$valueObject->get_id()."') "; 
...
function findAll($conn) {
	...
	$sql = "SELECT * FROM volumeGroups ORDER BY id ASC ";
...
function create($conn, $valueObject) {
	 ...
	 if ($valueObject->isset_display_name())
 	 {
 	  	if($count>0)
		{
			$sql2 = $sql2 . ",";
 	    ...
 	    $sql2 = $sql2."displayName";
 	    $sql4 = $sql4."'".$valueObject->get_display_name()."'"; 	
 		$count++;
 	 }
	...
	if ($valueObject->isset_display_name())
 	{
		...
		$sql = $sql."displayName = '".$valueObject->get_display_name()."'";
		$count++;
	}
...
function delete($conn, $valueObject) {
	...
	$sql = "DELETE FROM volumeGroups WHERE (id = '".$valueObject->get_id()."')"; 
	$result = $this->databaseUpdate($conn, $sql);
    if ($result != 1) {
         //print "PrimaryKey Error when updating DB!";
          return false;
...
function countAll($conn) {
	$sql = "SELECT count(*) FROM volumeGroups";
	$allRows = 0;
...
function searchMatching($conn, $valueObject) {
	$first = true;
    $sql = "SELECT * FROM volumeGroups WHERE 1=1 ";
	...
	if ($valueObject->get_display_name() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND displayName LIKE '".$valueObject->get_display_name()."%' ";
    }
