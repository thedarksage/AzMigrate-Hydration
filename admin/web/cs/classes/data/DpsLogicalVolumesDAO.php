...
/**
  ...
  *
  * dpsLogicalVolumes Data Access Object[DAO]
  * This class contains all database handling that is needed to 
  * permanently store and retrieve dpsLogicalVolumes object instances. 
  * Generated Date:	Tue, 27 Nov 2007 14:01:24 +0530	
  * Pattern Version:1.0
  */
class DpsLogicalVolumesDAO
{
...
function createValueObject() {
	return new DpsLogicalVolumesVO();
 }
 ...
 */
function getObject($conn,$deviceName,$id) {
	$valueObject = $this->createValueObject();
	$valueObject->set_device_name($deviceName);
...
function findByPrimaryKey($conn, $valueObject) {
	...
	$sql = "SELECT * FROM dpsLogicalVolumes WHERE (deviceName = '".$valueObject->get_device_name()."'  AND id = '".$valueObject->get_id()."') "; 
...
function findAll($conn) {
	...
	$sql = "SELECT * FROM dpsLogicalVolumes ORDER BY deviceName ASC ,id ASC ";
...
function create($conn, $valueObject) {
	 $count = 0;
     $sql1 = "INSERT INTO dpsLogicalVolumes(";
     $sql3 = ") VALUES ("; 
...
function update($conn, $valueObject) {
	$count = 0;
	$sql = "UPDATE dpsLogicalVolumes SET ";
	if ($valueObject->isset_device_name())
 	{
		if($count>0)
		{
	...
	$sql = $sql." WHERE (deviceName = '".$valueObject->get_device_name()."'  AND id = '".$valueObject->get_id()."')"; 
	$result = $this->databaseUpdate($conn, $sql);
    if ($result != 1) {
...
function delete($conn, $valueObject) {
	...
	$sql = "DELETE FROM dpsLogicalVolumes WHERE (deviceName = '".$valueObject->get_device_name()."'  AND id = '".$valueObject->get_id()."')"; 
	$result = $this->databaseUpdate($conn, $sql);
    if ($result != 1) {
         //print "PrimaryKey Error when updating DB!";
          return false;
...
function deleteAll($conn) {
	$sql = "DELETE FROM dpsLogicalVolumes";
	$result = $this->databaseUpdate($conn, $sql);
	return true;
}
...
function countAll($conn) {
	$sql = "SELECT count(*) FROM dpsLogicalVolumes";
	$allRows = 0;
...
function searchMatching($conn, $valueObject) {
	$first = true;
    $sql = "SELECT * FROM dpsLogicalVolumes WHERE 1=1 ";
