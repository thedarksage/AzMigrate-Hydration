...
/**
  ...
  *
  * replicationHistory Data Access Object[DAO]
  * This class contains all database handling that is needed to 
  * permanently store and retrieve replicationHistory object instances. 
...
function createValueObject() {
	return new ReplicationHistoryVO();
 }
...
function findByPrimaryKey($conn, $valueObject) {
	...
	$sql = "SELECT * FROM replicationHistory WHERE (id = '".$valueObject->get_id()."') "; 
...
function findAll($conn) {
	...
	$sql = "SELECT * FROM replicationHistory ORDER BY id ASC ";
...
function create($conn, $valueObject) {
	 $count = 0;
     $sql1 = "INSERT INTO replicationHistory(";
     $sql3 = ") VALUES ("; 
...
function update($conn, $valueObject) {
	$count = 0;
	$sql = "UPDATE replicationHistory SET ";
	if ($valueObject->isset_id())
 	{
		if($count>0)
		{
...
function delete($conn, $valueObject) {
	...
	$sql = "DELETE FROM replicationHistory WHERE (id = '".$valueObject->get_id()."')"; 
	$result = $this->databaseUpdate($conn, $sql);
    if ($result != 1) {
         //print "PrimaryKey Error when updating DB!";
          return false;
...
function deleteAll($conn) {
	$sql = "DELETE FROM replicationHistory";
	$result = $this->databaseUpdate($conn, $sql);
	return true;
}
...
function countAll($conn) {
	$sql = "SELECT count(*) FROM replicationHistory";
	$allRows = 0;
...
function searchMatching($conn, $valueObject) {
	$first = true;
    $sql = "SELECT * FROM replicationHistory WHERE 1=1 ";
