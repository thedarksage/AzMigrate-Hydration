...
/**
  ...
  *
  * sanFabrics Data Access Object[DAO]
  * This class contains all database handling that is needed to 
  * permanently store and retrieve sanFabrics object instances. 
 ...
 */
function getObject($conn,$fabricId) {
...
function find_by_fabricid($conn, $params = array()) {
	...
	$sql = "SELECT * FROM sanFabrics WHERE (" . $sql . ")";
...
function findByPrimaryKey($conn, $valueObject) {
	...
	$sql = "SELECT * FROM sanFabrics WHERE (fabricId = '".$valueObject->get_fabric_id()."') "; 
...
/**
 * findbyvalid_fabric-method. A user defined or custom findbyMethod  
 * This method select by fabric_id
 *
 * @param conn         This method requires working database connection.
 * @param params       associate array: sql will be searched and replaced by the matching values
 *					   associated with the token
 */
function find_by_valid_fabric($conn, $params = array()) {

	$sql = "fabricId != CONVERT( _utf8 '0'USING latin1 ) AND fabricId !=1";
	$sql = str_replace(array_keys($params), array_values($params), $sql);
	$sql = "SELECT * FROM sanFabrics WHERE (" . $sql . ")";
	$searchResults = $this->listQuery($conn, $sql);
	return $searchResults;
...
/**
 * findbyvalid_prism-method. A user defined or custom findbyMethod  
 * This method select by fabric_id
 *
 * @param conn         This method requires working database connection.
 * @param params       associate array: sql will be searched and replaced by the matching values
 *					   associated with the token
 */
function find_by_valid_prism($conn, $params = array()) {
 ...
 *					   associated with the token
 */
function find_by_fabric_name($conn, $params = array()) {

	$sql = "select name from sanFabrics where name='fabric_name'";
	$sql = str_replace(array_keys($params), array_values($params), $sql);
	$searchResults = $this->listQuery($conn, $sql);
	return $searchResults;
}		
...
function findAll($conn) {
	...
	$sql = "SELECT * FROM sanFabrics ORDER BY fabricId ASC ";
	$searchResults = $this->listQuery($conn, $sql);
	return $searchResults;
}
...
function create($conn, $valueObject) {
	 ...
	 if ($valueObject->isset_zone_set_name())
 	 {
 	  	if($count>0)
		{
			$sql2 = $sql2 . ",";
 	    ...
 	    $sql2 = $sql2."zoneSetName";
 	    $sql4 = $sql4."'".$valueObject->get_zone_set_name()."'"; 	
 		$count++;
 	 }
...
function update($conn, $valueObject) {
	...
	if ($valueObject->isset_zone_set_name())
 	{
		...
		$sql = $sql."zoneSetName = '".$valueObject->get_zone_set_name()."'";
		$count++;
	}
	...
	$sql = $sql." WHERE (fabricId = '".$valueObject->get_fabric_id()."')"; 
	$result = $this->databaseUpdate($conn, $sql);
    if ($result != 1) {
...
function delete($conn, $valueObject) {
	...
	$sql = "DELETE FROM sanFabrics WHERE (fabricId = '".$valueObject->get_fabric_id()."')"; 
	$result = $this->databaseUpdate($conn, $sql);
    if ($result != 1) {
         //print "PrimaryKey Error when updating DB!";
          return false;
...
function countAll($conn) {
	$sql = "SELECT count(*) FROM sanFabrics";
	$allRows = 0;
...
function searchMatching($conn, $valueObject) {
	$first = true;
    $sql = "SELECT * FROM sanFabrics WHERE 1=1 ";
	...
	if ($valueObject->get_zone_set_name() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND zoneSetName LIKE '".$valueObject->get_zone_set_name()."%' ";
    }
