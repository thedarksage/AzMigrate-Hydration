...
/**
  ...
  *
  * frbJobGroups Data Access Object[DAO]
  * This class contains all database handling that is needed to 
  * permanently store and retrieve frbJobGroups object instances. 
  * Generated Date:	Tue, 27 Nov 2007 14:01:25 +0530	
  * Pattern Version:1.0
  */
class FrbJobGroupsDAO
{
...
function findByPrimaryKey($conn, $valueObject) {
	...
	$sql = "SELECT * FROM frbJobGroups WHERE (id = '".$valueObject->get_id()."') "; 
...
function findAll($conn) {
	...
	$sql = "SELECT * FROM frbJobGroups ORDER BY id ASC ";
...
function create($conn, $valueObject) {
	 ...
	 if ($valueObject->isset_execution_type())
 	 {
 	  	if($count>0)
		{
			$sql2 = $sql2 . ",";
 	    ...
 	    $sql2 = $sql2."executionType";
 	    $sql4 = $sql4."'".$valueObject->get_execution_type()."'"; 	
 		$count++;
 	 }
	 
	 if ($valueObject->isset_schedule_mode())
 	 {
 	  	if($count>0)
		{
			$sql2 = $sql2 . ",";
 	    ...
 	    $sql2 = $sql2."scheduleMode";
 	    $sql4 = $sql4."'".$valueObject->get_schedule_mode()."'"; 	
 		$count++;
 	 }
	...
	if ($valueObject->isset_execution_type())
 	{
		...
		$sql = $sql."executionType = '".$valueObject->get_execution_type()."'";
		$count++;
	}
	...
	if ($valueObject->isset_schedule_mode())
 	{
		...
		$sql = $sql."scheduleMode = '".$valueObject->get_schedule_mode()."'";
		$count++;
	}
...
function delete($conn, $valueObject) {
	...
	$sql = "DELETE FROM frbJobGroups WHERE (id = '".$valueObject->get_id()."')"; 
	$result = $this->databaseUpdate($conn, $sql);
    if ($result != 1) {
         //print "PrimaryKey Error when updating DB!";
          return false;
...
function countAll($conn) {
	$sql = "SELECT count(*) FROM frbJobGroups";
	$allRows = 0;
...
function searchMatching($conn, $valueObject) {
	$first = true;
    $sql = "SELECT * FROM frbJobGroups WHERE 1=1 ";
	...
	if ($valueObject->get_execution_type() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND executionType LIKE '".$valueObject->get_execution_type()."%' ";
    }
	
	if ($valueObject->get_schedule_mode() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND scheduleMode LIKE '".$valueObject->get_schedule_mode()."%' ";
    }
