...
/**
  ...
  *
  * cxClusterLog Data Access Object[DAO]
  * This class contains all database handling that is needed to 
  * permanently store and retrieve cxClusterLog object instances. 
  * Generated Date:	Tue, 27 Nov 2007 14:01:23 +0530	
  * Pattern Version:1.0
  */
class CxClusterLogDAO
{
...
function findByPrimaryKey($conn, $valueObject) {
	...
	$sql = "SELECT * FROM cxClusterLog WHERE (id = '".$valueObject->get_id()."') "; 
...
function findAll($conn) {
	...
	$sql = "SELECT * FROM cxClusterLog ORDER BY id ASC ";
...
function create($conn, $valueObject) {
	 ...
	 if ($valueObject->isset_error_string())
 	 {
 	  	if($count>0)
		{
			$sql2 = $sql2 . ",";
 	    ...
 	    $sql2 = $sql2."errorString";
 	    $sql4 = $sql4."'".$valueObject->get_error_string()."'"; 	
 		$count++;
 	 }
	 
	 if ($valueObject->isset_time_string())
 	 {
 	  	if($count>0)
		{
			$sql2 = $sql2 . ",";
 	    ...
 	    $sql2 = $sql2."timeString";
 	    $sql4 = $sql4."'".$valueObject->get_time_string()."'"; 	
 		$count++;
 	 }
	 
	 if ($valueObject->isset_alert_sent())
 	 {
 	  	if($count>0)
		{
			$sql2 = $sql2 . ",";
 	    ...
 	    $sql2 = $sql2."alertSent";
 	    $sql4 = $sql4."'".$valueObject->get_alert_sent()."'"; 	
 		$count++;
 	 }
	...
	if ($valueObject->isset_error_string())
 	{
		...
		$sql = $sql."errorString = '".$valueObject->get_error_string()."'";
		$count++;
	}
	...
	if ($valueObject->isset_time_string())
 	{
		...
		$sql = $sql."timeString = '".$valueObject->get_time_string()."'";
		$count++;
	}
	...
	if ($valueObject->isset_alert_sent())
 	{
		...
		$sql = $sql."alertSent = '".$valueObject->get_alert_sent()."'";
		$count++;
	}
...
function delete($conn, $valueObject) {
	...
	$sql = "DELETE FROM cxClusterLog WHERE (id = '".$valueObject->get_id()."')"; 
	$result = $this->databaseUpdate($conn, $sql);
    if ($result != 1) {
         //print "PrimaryKey Error when updating DB!";
          return false;
...
function countAll($conn) {
	$sql = "SELECT count(*) FROM cxClusterLog";
	$allRows = 0;
...
function searchMatching($conn, $valueObject) {
	$first = true;
    $sql = "SELECT * FROM cxClusterLog WHERE 1=1 ";
	...
	if ($valueObject->get_error_string() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND errorString LIKE '".$valueObject->get_error_string()."%' ";
    }
	
	if ($valueObject->get_time_string() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND timeString LIKE '".$valueObject->get_time_string()."%' ";
    }
	
	if ($valueObject->get_alert_sent() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND alertSent LIKE '".$valueObject->get_alert_sent()."%' ";
    }
