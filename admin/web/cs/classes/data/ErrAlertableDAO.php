...
/**
  ...
  *
  * errAlertable Data Access Object[DAO]
  * This class contains all database handling that is needed to 
  * permanently store and retrieve errAlertable object instances. 
  * Generated Date:	Tue, 27 Nov 2007 14:01:24 +0530	
  * Pattern Version:1.0
  */
class ErrAlertableDAO
{
...
function findByPrimaryKey($conn, $valueObject) {
	...
	$sql = "SELECT * FROM errAlertable WHERE (id = '".$valueObject->get_id()."') "; 
...
function findAll($conn) {
	...
	$sql = "SELECT * FROM errAlertable ORDER BY id ASC ";
	 ...
	 if ($valueObject->isset_hostid())
 	 {
 	  	if($count>0)
		{
 	    ...
 	    $sql2 = $sql2."hostid";
 	    $sql4 = $sql4."'".$valueObject->get_hostid()."'"; 	
 		$count++;
 	 }
	 
	 if ($valueObject->isset_errlvl())
 	 {
 	  	if($count>0)
		{
			$sql2 = $sql2 . ",";
 	    ...
 	    $sql2 = $sql2."errlvl";
 	    $sql4 = $sql4."'".$valueObject->get_errlvl()."'"; 	
 		$count++;
 	 }
	 ...
	 if ($valueObject->isset_errtime())
 	 {
 	  	if($count>0)
		{
 	    ...
 	    $sql2 = $sql2."errtime";
 	    $sql4 = $sql4."'".$valueObject->get_errtime()."'"; 	
 		$count++;
 	 }
	 
	 if ($valueObject->isset_module())
 	 {
 	  	if($count>0)
		{
			$sql2 = $sql2 . ",";
 	    ...
 	    $sql2 = $sql2."module";
 	    $sql4 = $sql4."'".$valueObject->get_module()."'"; 	
 		$count++;
 	 }
	 
	 if ($valueObject->isset_alert_type())
 	 {
 	  	if($count>0)
		{
			$sql2 = $sql2 . ",";
 	    ...
 	    $sql2 = $sql2."alert_type";
 	    $sql4 = $sql4."'".$valueObject->get_alert_type()."'"; 	
 		$count++;
 	 }
	...
	if ($valueObject->isset_hostid())
 	{
		...
		$sql = $sql."hostid = '".$valueObject->get_hostid()."'";
		$count++;
	}
	...
	if ($valueObject->isset_errlvl())
 	{
		...
		$sql = $sql."errlvl = '".$valueObject->get_errlvl()."'";
		$count++;
	}
	...
	if ($valueObject->isset_errtime())
 	{
		...
		$sql = $sql."errtime = '".$valueObject->get_errtime()."'";
		$count++;
	}
	...
	if ($valueObject->isset_module())
 	{
		...
		$sql = $sql."module = '".$valueObject->get_module()."'";
		$count++;
	}
	...
	if ($valueObject->isset_alert_type())
 	{
		...
		$sql = $sql."alert_type = '".$valueObject->get_alert_type()."'";
		$count++;
	}
...
function delete($conn, $valueObject) {
	...
	$sql = "DELETE FROM errAlertable WHERE (id = '".$valueObject->get_id()."')"; 
	$result = $this->databaseUpdate($conn, $sql);
    if ($result != 1) {
         //print "PrimaryKey Error when updating DB!";
          return false;
...
function countAll($conn) {
	$sql = "SELECT count(*) FROM errAlertable";
	$allRows = 0;
...
function searchMatching($conn, $valueObject) {
	$first = true;
    $sql = "SELECT * FROM errAlertable WHERE 1=1 ";
	...
	if ($valueObject->get_hostid() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND hostid LIKE '".$valueObject->get_hostid()."%' ";
    }
	
	if ($valueObject->get_errlvl() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND errlvl LIKE '".$valueObject->get_errlvl()."%' ";
    }
	...
	if ($valueObject->get_errtime() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND errtime LIKE '".$valueObject->get_errtime()."%' ";
    }
	
	if ($valueObject->get_module() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND module LIKE '".$valueObject->get_module()."%' ";
    }
	
	if ($valueObject->get_alert_type() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND alert_type LIKE '".$valueObject->get_alert_type()."%' ";
    }
