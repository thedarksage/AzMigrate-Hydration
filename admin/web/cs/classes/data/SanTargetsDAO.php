...
/**
  ...
  *
  * sanTargets Data Access Object[DAO]
  * This class contains all database handling that is needed to 
  * permanently store and retrieve sanTargets object instances. 
 ...
 */
function getObject($conn,$fabricId,$sanTargetPortWwn) {
...
function findByPrimaryKey($conn, $valueObject) {
	...
	$sql = "SELECT * FROM sanTargets WHERE (fabricId = '".$valueObject->get_fabric_id()."'  AND sanTargetPortWwn = '".$valueObject->get_san_target_port_wwn()."') "; 
...
function find_by_fabric_id($conn, $params = array()) {
	...
	$sql = "SELECT * FROM sanTargets WHERE (" . $sql . ")";
...
function find_by_port_wwn($conn, $params = array()) {
	...
	$sql = "fabricId='fabric_id' and sanTargetPortWwn='port_wwn'";
	$sql = str_replace(array_keys($params), array_values($params), $sql);
	$sql = "SELECT * FROM sanTargets WHERE (" . $sql . ")";
	$searchResults = $this->listQuery($conn, $sql);
	return $searchResults;
...
function findAll($conn) {
	...
	$sql = "SELECT * FROM sanTargets ORDER BY fabricId ASC ,sanTargetPortWwn ASC ";
	$searchResults = $this->listQuery($conn, $sql);
	return $searchResults;
}
...
function update($conn, $valueObject) {
	...
	$sql = $sql." WHERE (fabricId = '".$valueObject->get_fabric_id()."'  AND sanTargetPortWwn = '".$valueObject->get_san_target_port_wwn()."')"; 
	$result = $this->databaseUpdate($conn, $sql);
    if ($result != 1) {
...
function delete($conn, $valueObject) {
	...
	$sql = "DELETE FROM sanTargets WHERE (fabricId = '".$valueObject->get_fabric_id()."'  AND sanTargetPortWwn = '".$valueObject->get_san_target_port_wwn()."')"; 
	$result = $this->databaseUpdate($conn, $sql);
    if ($result != 1) {
         //print "PrimaryKey Error when updating DB!";
          return false;
...
function countAll($conn) {
	$sql = "SELECT count(*) FROM sanTargets";
	$allRows = 0;
...
function searchMatching($conn, $valueObject) {
	$first = true;
    $sql = "SELECT * FROM sanTargets WHERE 1=1 ";
