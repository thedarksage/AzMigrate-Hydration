...
/**
  ...
  *
  * Applications Data Access Object[DAO]
  * This class contains all database handling that is needed to 
  * permanently store and retrieve Applications object instances. 
...
function findByPrimaryKey($conn, $valueObject) {
	...
	$sql = "SELECT * FROM Applications WHERE (id = '".$valueObject->get_id()."') "; 
	 ...
	 if ($valueObject->isset_alias())
 	 {
 	  	if($count>0)
		{
 	    ...
 	    $sql2 = $sql2."Alias";
 	    $sql4 = $sql4."'".$valueObject->get_alias()."'"; 	
 		$count++;
 	 }
	...
	if ($valueObject->isset_alias())
 	{
		...
		$sql = $sql."Alias = '".$valueObject->get_alias()."'";
		$count++;
	}
...
function delete($conn, $valueObject) {
	...
	$sql = "DELETE FROM Applications WHERE (id = '".$valueObject->get_id()."')"; 
	$result = $this->databaseUpdate($conn, $sql);
    if ($result != 1) {
         //print "PrimaryKey Error when updating DB!";
          return false;
...
function searchMatching($conn, $valueObject) {
	...
	if ($valueObject->get_alias() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND Alias LIKE '".$valueObject->get_alias()."%' ";
    }
