...
/**
  ...
  *
  * BPMAllocations Data Access Object[DAO]
  * This class contains all database handling that is needed to 
  * permanently store and retrieve BPMAllocations object instances. 
  * Generated Date:	Tue, 27 Nov 2007 14:01:23 +0530	
  * Pattern Version:1.0
  */
class BPMAllocationsDAO
{
...
function findByPrimaryKey($conn, $valueObject) {
	...
	$sql = "SELECT * FROM BPMAllocations WHERE (Id = '".$valueObject->get_id()."') "; 
...
function findAll($conn) {
	...
	$sql = "SELECT * FROM BPMAllocations ORDER BY Id ASC ";
...
function create($conn, $valueObject) {
	 $count = 0;
     $sql1 = "INSERT INTO BPMAllocations(";
     $sql3 = ") VALUES ("; 
	 ...
	 if ($valueObject->isset_policyid())
 	 {
 	  	if($count>0)
		{
 	    ...
 	    $sql2 = $sql2."Policyid";
 	    $sql4 = $sql4."'".$valueObject->get_policyid()."'"; 	
 		$count++;
 	 }
	 ...
	 if ($valueObject->isset_target_ip())
 	 {
 	  	if($count>0)
		{
			$sql2 = $sql2 . ",";
 	    ...
 	    $sql2 = $sql2."TargetIP";
 	    $sql4 = $sql4."'".$valueObject->get_target_ip()."'"; 	
 		$count++;
 	 }
	 
	 if ($valueObject->isset_allocated_bandwidth())
 	 {
 	  	if($count>0)
		{
			$sql2 = $sql2 . ",";
 	    ...
 	    $sql2 = $sql2."AllocatedBandwidth";
 	    $sql4 = $sql4."'".$valueObject->get_allocated_bandwidth()."'"; 	
 		$count++;
 	 }
	...
	if ($valueObject->isset_policyid())
 	{
		...
		$sql = $sql."Policyid = '".$valueObject->get_policyid()."'";
		$count++;
	}
	...
	if ($valueObject->isset_target_ip())
 	{
		...
		$sql = $sql."TargetIP = '".$valueObject->get_target_ip()."'";
		$count++;
	}
	...
	if ($valueObject->isset_allocated_bandwidth())
 	{
		...
		$sql = $sql."AllocatedBandwidth = '".$valueObject->get_allocated_bandwidth()."'";
		$count++;
	}
...
function delete($conn, $valueObject) {
	...
	$sql = "DELETE FROM BPMAllocations WHERE (Id = '".$valueObject->get_id()."')"; 
	$result = $this->databaseUpdate($conn, $sql);
    if ($result != 1) {
         //print "PrimaryKey Error when updating DB!";
          return false;
...
function deleteAll($conn) {
	$sql = "DELETE FROM BPMAllocations";
	$result = $this->databaseUpdate($conn, $sql);
	return true;
}
...
function countAll($conn) {
	$sql = "SELECT count(*) FROM BPMAllocations";
	$allRows = 0;
...
function searchMatching($conn, $valueObject) {
	$first = true;
    $sql = "SELECT * FROM BPMAllocations WHERE 1=1 ";
	...
	if ($valueObject->get_policyid() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND Policyid LIKE '".$valueObject->get_policyid()."%' ";
    }
	...
	if ($valueObject->get_target_ip() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND TargetIP LIKE '".$valueObject->get_target_ip()."%' ";
    }
	
	if ($valueObject->get_allocated_bandwidth() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND AllocatedBandwidth LIKE '".$valueObject->get_allocated_bandwidth()."%' ";
    }
