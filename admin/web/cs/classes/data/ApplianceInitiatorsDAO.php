...
/**
  ...
  *
  * applianceInitiators Data Access Object[DAO]
  * This class contains all database handling that is needed to 
  * permanently store and retrieve applianceInitiators object instances. 
  * Generated Date:	Fri, 22 Aug 2008 15:51:34 +0530	
  * Pattern Version:1.0
  */
class ApplianceInitiatorsDAO
{
...
function createValueObject() {
	return new ApplianceInitiatorsVO();
 }
 ...
 */
function getObject($conn,$applianceInitiatorPortWwn,$hostId) {
...
function findByPrimaryKey($conn, $valueObject) {
	...
	$sql = "SELECT * FROM applianceInitiators WHERE (applianceInitiatorPortWwn = '".$valueObject->get_appliance_initiator_port_wwn()."'  AND hostId = '".$valueObject->get_host_id()."') "; 
...
function find_by_fabricid($conn, $params = array()) {
	...
	$sql = "SELECT * FROM applianceFrBindingInitiators WHERE (" . $sql . ")";
...
function findAll($conn) {
	...
	$sql = "SELECT * FROM applianceInitiators ORDER BY applianceInitiatorPortWwn ASC ,hostId ASC ";
	$searchResults = $this->listQuery($conn, $sql);
	return $searchResults;
}
...
function create($conn, $valueObject) {
	 $count = 0;
     $sql1 = "INSERT INTO applianceInitiators(";
     $sql3 = ") VALUES ("; 
...
function update($conn, $valueObject) {
	$count = 0;
	$sql = "UPDATE applianceInitiators SET ";
	if ($valueObject->isset_appliance_initiator_port_wwn())
 	{
		if($count>0)
		{
	...
	$sql = $sql." WHERE (applianceInitiatorPortWwn = '".$valueObject->get_appliance_initiator_port_wwn()."'  AND hostId = '".$valueObject->get_host_id()."')"; 
	$result = $this->databaseUpdate($conn, $sql);
    if ($result != 1) {
...
function delete($conn, $valueObject) {
	...
	$sql = "DELETE FROM applianceInitiators WHERE (applianceInitiatorPortWwn = '".$valueObject->get_appliance_initiator_port_wwn()."'  AND hostId = '".$valueObject->get_host_id()."')"; 
	$result = $this->databaseUpdate($conn, $sql);
    if ($result != 1) {
         //print "PrimaryKey Error when updating DB!";
          return false;
...
function deleteAll($conn) {
	$sql = "DELETE FROM applianceInitiators";
	$result = $this->databaseUpdate($conn, $sql);
	return true;
}
...
function countAll($conn) {
	$sql = "SELECT count(*) FROM applianceInitiators";
	$allRows = 0;
...
function searchMatching($conn, $valueObject) {
	$first = true;
    $sql = "SELECT * FROM applianceInitiators WHERE 1=1 ";
...
function find_by_portwwn_hostid($conn, $params = array()) {
	...
	$sql = "applianceInitiatorPortWwn='port_wwn' and hostId='host_id'";
	$sql = str_replace(array_keys($params), array_values($params), $sql);
	$sql = "SELECT * FROM applianceInitiators WHERE (" . $sql . ")";
	$searchResults = $this->listQuery($conn, $sql);
	return $searchResults;
 ...
 * This method update portwwn_hostid
 *
 * @param conn         This method requires working database connection.
 * @param params       associate array: sql will be searched and replaced by the matching values
 *					   associated with the token
...
function update_by_portwwn_hostid($conn, $valueObject,$params = array()) {
	$wsql = "applianceInitiatorPortWwn='port_wwn' and hostId='host_id'";
