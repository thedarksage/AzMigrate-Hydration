...
/**
  ...
  *
  * frbHeartbeat Data Access Object[DAO]
  * This class contains all database handling that is needed to 
  * permanently store and retrieve frbHeartbeat object instances. 
  * Generated Date:	Tue, 27 Nov 2007 14:01:25 +0530	
  * Pattern Version:1.0
  */
class FrbHeartbeatDAO
{
...
function findByPrimaryKey($conn, $valueObject) {
	...
	$sql = "SELECT * FROM frbHeartbeat WHERE (id = '".$valueObject->get_id()."') "; 
...
function findAll($conn) {
	...
	$sql = "SELECT * FROM frbHeartbeat ORDER BY id ASC ";
...
function create($conn, $valueObject) {
	 ...
	 if ($valueObject->isset_heartbeat())
 	 {
 	  	if($count>0)
		{
			$sql2 = $sql2 . ",";
 	    ...
 	    $sql2 = $sql2."heartbeat";
 	    $sql4 = $sql4."'".$valueObject->get_heartbeat()."'"; 	
 		$count++;
 	 }
	...
	if ($valueObject->isset_heartbeat())
 	{
		...
		$sql = $sql."heartbeat = '".$valueObject->get_heartbeat()."'";
		$count++;
	}
...
function delete($conn, $valueObject) {
	...
	$sql = "DELETE FROM frbHeartbeat WHERE (id = '".$valueObject->get_id()."')"; 
	$result = $this->databaseUpdate($conn, $sql);
    if ($result != 1) {
         //print "PrimaryKey Error when updating DB!";
          return false;
...
function countAll($conn) {
	$sql = "SELECT count(*) FROM frbHeartbeat";
	$allRows = 0;
...
function searchMatching($conn, $valueObject) {
	$first = true;
    $sql = "SELECT * FROM frbHeartbeat WHERE 1=1 ";
	...
	if ($valueObject->get_heartbeat() != "") {
    	if ($first) { $first = false; }
        $sql = $sql."AND heartbeat LIKE '".$valueObject->get_heartbeat()."%' ";
    }
