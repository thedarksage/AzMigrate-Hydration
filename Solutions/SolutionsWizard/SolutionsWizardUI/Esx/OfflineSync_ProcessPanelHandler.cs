...
{
    class OfflineSync_ProcessPanelHandler : OfflineSyncPanelHandler
    {
        public static long TIME_OUT_MILLISECONDS = 360000;
        bool _targetESXPreChekCompleted = false;
        
        public OfflineSync_ProcessPanelHandler()
        {
           ...
           // importOfflineSyncForm.progressBar.Visible = true;
            ...
            if (_targetESXPreChekCompleted == false)
            {
                ...
                else
                {
                    ...
                    importOfflineSyncForm.generalLogTextBox.AppendText("Pre-req checks completed on secondary ESX... " + Environment.NewLine);
                }

            }
            if (_targetESXPreChekCompleted == true)
            {
                if (esx.TargetScriptForOffliceSyncImport(importOfflineSyncForm._esxIP,importOfflineSyncForm._esxUserName , importOfflineSyncForm._esxPassword , "\"" + "[" + importOfflineSyncForm.targetDataStore + "]" + "\"", "\"" + "[" + importOfflineSyncForm.vmdkDataStore + "]" + "\"") == 0)
                {
                    ...
                    document.Save(Directory.GetCurrentDirectory() + "\\ESX.xml");
                    config.AddOrReplaceMasterXmlFile(importOfflineSyncForm._selectedSourceList, importOfflineSyncForm.masterHost, importOfflineSyncForm._cxIP, "ESX_Master_" + importOfflineSyncForm._esxIP + ".xml", "yes");
                    esx.UploadFile(importOfflineSyncForm._esxIP,  importOfflineSyncForm._esxUserName , importOfflineSyncForm._esxPassword , "ESX_Master_" + importOfflineSyncForm._esxIP + ".xml");
                  
                    //esx.UploadOfflineSyncXmlFile(importOfflineSyncForm._esxIP, "\"" +importOfflineSyncForm._esxUserName + "\"", "\"" + importOfflineSyncForm._esxPassword + "\"", "ESX_Master.xml", "\"" + "[" + importOfflineSyncForm.targetDataStore + "]" + "\"");
        ...
        public bool TargetReadynessCheck(ImportOfflineSyncForm importOfflineSyncForm)
        {
            ...
            for (int j = 0; j < importOfflineSyncForm._finalMasterList._hostList.Count; j++)
            {
                ...
                p = esx.PreReqCheckForOfflineSync(importOfflineSyncForm._esxIP, importOfflineSyncForm._esxUserName, importOfflineSyncForm._esxPassword);
