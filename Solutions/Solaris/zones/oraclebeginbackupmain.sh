...
EOF"
            grep -w ARCHIVELOG $TEMP_FILE >>/dev/null
            if [ $? -ne 0 ]
            then
                echo "Archive log is not enabled for $ORACLE_SID"
                echo "Continuing with other databases..."
                continue
            fi
...
EOF"
            grep "System altered" $TEMP_FILE >>/dev/null
            if [ $? -ne 0 ]
            then
                echo "Archive log generation failed for $ORACLE_SID"
                echo "Continuing with other databases..."
                continue
            fi

            echo "Running begin backup command for $ORACLE_SID"
