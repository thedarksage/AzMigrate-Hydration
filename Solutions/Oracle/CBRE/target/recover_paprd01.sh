...
LD_LIBRARY_PATH=/usr/local/InMage/Fx/lib_alert:$HOME/lib
LOGDIR="$HOME/loginfo"
...
FieldNo=`echo $fileformat|awk -F"_"  '{ x=NF; for(i=1;i<=NF;i++) if ( $i == "%s.dbf") print i; }'`
echo $FieldNo
...
MaxArchiveNo=0
cd $Archivelogdir
for i in `ls -lrt|grep -v "^d"|awk '{ print $9 }'`
do
	temp=`echo $i|cut -f $FieldNo -d "_"|cut -f 1 -d "."`
